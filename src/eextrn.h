/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com eextrn.h - External
 * function definitions. Copyright (C) 1988-2009 JASSPA (www.jasspa.com)
...
extern int      meWindowBackwardChar(register meWindow * wp,
				     register int n);
extern int      meWindowForwardChar(register meWindow * wp,
				    register int n);
...
extern void     meBufferUpdateLocation(meBuffer * bp, meUInt noLines,
				       meUShort newOff);
...
extern int      bindkey(meUByte * prom, int f, int n,
			meUShort * lclNoBinds, meBind ** lclBinds);
 ...
 * buffer.c externals 
 */
extern int      getBufferName(meUByte * prompt, int opt, int defH,
			      meUByte * buf);
#if MEOPT_FILEHOOK
...
extern void     pokeScreen(int flags, int row, int col, meUByte * scheme,
			   meUByte * str);
#if MEOPT_EXTENDED
...
extern meUByte *getUsrLclCmdVar(meUByte * vname,
...
extern meVariable *SetUsrLclCmdVar(meUByte * vname, meUByte * vvalue,
				   register meVarList * varList);
extern int      setVar(meUByte * vname, meUByte * vvalue,
		       meRegister * regs);
...
/*
 * Note tok (the destination token string) must be meTOKENBUF_SIZE_MAX in
 * size, returning a string no bigger than meBUF_SIZE_MAX with the \0 
 */
...
extern int      getFileStats(meUByte * file, int flag, meStat * stats,
			     meUByte * lname);
...
extern int      fileLookup(meUByte * fname, meUByte * ext, meUByte flags,
			   meUByte * outName);
...
extern int      meBufferInsertFile(meBuffer * bp, meUByte * fname,
				   meUInt flags, meUInt hoffest,
				   meUInt loffest, meInt length);
...
extern int      writeOut(register meBuffer * bp, meUInt flags,
			 meUByte * fn);
...
#ifdef _WIN32
extern void     fileNameCorrect(meUByte * oldName, meUByte * newName,
				meUByte ** baseName);
#else
...
extern int      ffReadFile(meUByte * fname, meUInt flags, meBuffer * bp,
			   meLine * hlp, meUInt uoffset, meUInt loffset,
			   meInt length);
#define meBACKUP_CREATE_PATH 0x0001
extern int      createBackupName(meUByte * filename, meUByte * fn,
				 meUByte backl, int flag);


extern int      ffWriteFileOpen(meUByte * fname, meUInt flags,
				meBuffer * bp);
extern int      ffWriteFileWrite(register int len, register meUByte * buff,
				 int eolFlag);
extern int      ffWriteFileClose(meUByte * fname, meUInt flags,
				 meBuffer * bp);
...
extern int      mlCharReply(meUByte * prompt, int mask,
			    meUByte * validList, meUByte * helpStr);
extern int      mlyesno(meUByte * prompt);
extern void     mlDisp(meUByte * prompt, meUByte * buf, meUByte * cont,
		       int cpos);
...
/*
 * setup a #define with the correct flags for getting a filename, this
 * must consider where the platform file system only has one case and
 * where it is case insensitive 
 */
#ifdef _SINGLE_CASE
/*
 * Typically dos 
...
#endif
/*
 * Following must be setup when using MLUSER, mlgsStrList must be an array 
 * of completion strings, and mlgsStrListSize must be a count of the
 * number of completions. Not the completion array is sorted! 
 */
extern meUByte **mlgsStrList;
extern int      mlgsStrListSize;
extern int      meGetStringFromUser(meUByte * prompt, int option,
				    int defnum, meUByte * buf, int nbuf);
...
extern int      setupHistory(int option, meUByte ** numPtr,
			     meUByte *** list);
...
extern int      insert_key(register meUShort code, meUShort index,
			   meUInt arg);
 ...
 */
#if MEOPT_NARROW
extern meNarrow *meBufferCreateNarrow(meBuffer * bp, meLine * slp,
				      meLine * elp, meInt sln, meInt eln,
				      meInt name, meScheme scheme,
				      meUByte * markupLine,
				      meInt markupCmd, meInt undoCall);
extern void
...
meBufferRemoveNarrow(meBuffer * bp, register meNarrow * nrrw,
		     meUByte * firstLine, meInt undoCall);
extern void     meBufferExpandNarrowAll(meBuffer * bp);
extern void     meBufferCollapseNarrowAll(meBuffer * bp);
extern meInt    meBufferRegionExpandNarrow(meBuffer * bp,
					   meLine ** startLine,
					   meUShort soffset,
					   meLine * endLine,
					   meUShort eoffset, meInt remove);
...
extern int      meStrnicmp(const meUByte * str1, const meUByte * str2,
			   size_t nn);
extern int      meStridif(const meUByte * str1, const meUByte * str2);
extern void     sortStrings(int noStr, meUByte ** strs, int offset,
			    meIFuncSS cmpFunc);
...
extern meRegNode *regSet(meRegNode * root, meUByte * subkey,
			 meUByte * value);
...
extern int      doPipeCommand(meUByte * comStr, meUByte * path,
			      meUByte * bufName, int ipipe, int silent);
...
#if MEOPT_NARROW
extern void     meUndoAddUnnarrow(meInt sln, meInt eln, meInt name,
				  meScheme scheme, meInt markupCmd,
				  meLine * markupLine);
extern void     meUndoAddNarrow(meInt sln, meInt name, meInt markupCmd,
				meLine * firstLine);
#endif
...
#endif
/*
 * uses the bfind flags for finding buffer "name" if not null. Also if
 * WPOP_MKCURR set then makes the buffer current. 
 */
#define WPOP_MKCURR 0x100
#define WPOP_USESTR 0x200
#define WPOP_EXIST  0x400
extern meWindow *meWindowPopup(meUByte * name, int flags,
			       meBuffer ** bufferReplaced);
...
/*
 * gettimeofday - Retreives the time of day to millisecond resolution This 
 * is a 4.2 BSD function. IRIX 5.x and Solaris prior to 2.5 provide a
 * single argument version, for backward compatibility the 2nd argument is 
 * ignored. HP-UX 10.x and versions of Solaris above 2.5 provide a 2
 * argument version.  Win32 does not provide this function (of course).
...
struct meTimezone {
    int             tz_minuteswest;	/* Offset (+/-ve) from UTC in
					 * minutes */
    int             tz_dsttime;	/* Flag indicating the type of DST
				 * correction */
};
 ...
 * to the more familier invocation. Why the hell we need these functions
 * and the ANSI 'C' functions are not valid is beyond belief - typical
 * bloody Microsoft !! 
 */
...
/*
 * Differentiate between different styles for waiting for a process to
 * finish 
 */
#ifdef _BSD
/*
...
#if MEOPT_EXTENDED
/*
 * with extended option enable set-char-mask can be used to configure the
 * char sets 
 */
