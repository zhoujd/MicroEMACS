/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com termio.c - Generic
 * terminal support routines. Copyright (C) 1993-2001 Steven Phillips
...
/*
 * executable extension list, in reverse order of dos priority included
 * 4dos's btm files for completeness 
 */
meUByte        *execExtensions[noExecExtensions] =
    { ".btm", ".bat", ".exe", ".com" };
#endif
...
meUByte         ttSpeChars[TTSPECCHARS] = {
    ...
    '+',			/* 11/0x0b - Line Drawing / Bottom right
				 * _| */
    ...
    '-',			/* 18/0x12 - Line Drawing / Horizontal
				 * line - */
    ' ',			/* 19/0x13 - Undefined */
    ' ',			/* 20/0x14 - Undefined */
...
TTdoBell(int n)
{
	...
	pokeScreen(POKE_NOMARK + 0x10, frameCur->depth,
		   frameCur->width - 6, &scheme, (meUByte *) "[BELL]");
...
meUByte         meTimerState[NUM_TIMERS] = { 0 };	/* State of the
							 * timers, set or
							 * expired */
meInt           meTimerTime[NUM_TIMERS] = { 0 };	/* Absolute time
							 * of the timers */
...
{
    /*
     * the id is the timer id stored in meTimeId not the timer number,
     * convert 
     */
    int             ii = NUM_TIMERS;
	...
	if (meTimerId[ii] == id) {
	    meTimerId[ii] = 0;
	    meTimerState[ii] =
		(meTimerState[ii] & ~TIMER_SET) | TIMER_EXPIRED;
	}
}
#endif
#endif
...
timerCheck(meInt tim)
{
	...
	if (((meUInt) tim + TIMER_MIN) >= tbp->abstime) {
	     ...
	     */
	    meTimerState[tbp->id] =
		(meTimerState[tbp->id] & ~TIMER_SET) | TIMER_EXPIRED;
	    /*
...
timerSet(int id, meInt tim, meInt offset)
{
...
#ifdef _UNIX
    /*
     * On unix, must hold back the signals else we can get in a right
     * state 
     */
    meSigHold();
#endif
...
#ifdef _MULTI_NOID_TIMER
	/*
	 * each timer does not keep the id, so kill the current timer if
	 * set 
	 */
#ifdef _WIN32
#ifdef _ME_WINDOW
	...
	if (tim == 0) {
	    ...
	    gettimeofday(&tp, NULL);
	    tim =
		((tp.tv_sec - startTime) * 1000) + (tp.tv_usec / 1000) +
		offset;
	}
    }
#endif
...
_timerKill(int id)
{
...
#if (defined _CONST_TIMER) || (defined _SINGLE_TIMER)
	{
		...
		if (timers == NULL) {
		    ...
		    /*
		     * Must change the timer to the next timer which is a
		     * different time 
		     */
		    timerCheck(0);
#endif
    ...
    return (meTimerState[id] =
	    meTimerState[id] & ~(TIMER_SET | TIMER_EXPIRED));
}
...
handleTimerExpired(void)
{
    ...
    /*
     * if the user has changed the window focus using the OS and this is
     * the top level then change frames 
     */
...
#if MEOPT_SOCKET
    if (isTimerExpired(SOCKET_TIMER_ID))	/* socket connection
						 * time-out */
 ...
 */
#define ME_DELETE_KEY 0xffff	/* an invalid key number used to flag the
				 * key is deleted */
...
void
translateKeyAdd(meTRANSKEY * tcapKeys, int count, int time, meUShort * key,
		meUShort map)
{
    if (--count < 0) {
	tcapKeys->map = map;
	...
	if (ii == tcapKeys->count) {
	    tcapKeys->child =
		meRealloc(tcapKeys->child, (ii + 1) * sizeof(meTRANSKEY));
	    (tcapKeys->count)++;
...
#if MEOPT_EXTENDED
static void
translateKeyShow(meTRANSKEY * tcapKeys, meBuffer * bp, meUByte * key,
		 int depth)
{
    int             ii,
                    nd;
	...
	key[depth++] = ' ';
    for (ii = 0; ii < tcapKeys->count; ii++) {
	nd = depth + meGetStringFromChar(tcapKeys->child[ii].key,
					 key + depth);
...
TTgetc(void)
{
	...
	if (TTtransKey.count) {
		...
		if (keyNo == TTnoKeys) {
		    /*
		     * Wait for a quarter a second to see if any more keys 
		     * are available 
		     */
		    /*
		     * Only gocha here is that preferably you want an
		     * interuptable wait therefore we must set the
		     * TTnoKeys to 0 so we only interrupt on a receipt of
		     * a new key. Must remember to reset it as well. 
		     */
		    TTnoKeys = 0;
...
#if MEOPT_MOUSE
    /*
     * Reset the mouse context if mouse interaction is outstanding. This
     * should only be timer keys Kill off the timer 
     */
...
#if MEOPT_MOUSE
/*
 * when TTallKeys is set to 1 all mouse move events are added to the key
 * stack even if the key is not bound. Due to the speed at which windows
 * generates mouse move events, after exiting an OSD dialog mouse move
 * keys can still be in the buffer and this will generate problems as the
 * keys are not bound (e.g. breaks the main menus repeat undo function) -
 * remove the extras. 
 */
void
TTallKeysFlush(void)
{
	...
	if ((cc < (ME_SPECIAL | SKEY_mouse_move))
	    || (cc > (ME_SPECIAL | SKEY_mouse_move_5)))
	    break;
	TTnextKeyIdx = nidx;
	TTnoKeys--;
    }
...
#endif
{
	...
	if ((cc = *str++) == '\0')
		...
		if (stackDep > 1) {
		    stackDep--;
		    stack[stackDep - 1] =
			stack[stackDep - 1] + stack[stackDep];
		}
		break;
	    case '-':
		if (stackDep > 1) {
		...
		/*
		 * codes which pop a value from the stackand add it to the 
		 * output 
		 */
	    case 'c':
		if (stackDep > 0) {
		...
		*str = '\0';
		if (!stackDep) {
		     ...
		     * - occurs with cygwin color terminal. assume the
