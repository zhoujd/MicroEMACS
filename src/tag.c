/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com tag.c - Find tag
 * positions. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
/*
 * findTagInFile Search for a tag in the give tag filename returns 0 -
 * Found tag 1 - Found tag file but not tag 2 - Found nothing 
 */
static int
...
findTagSearch(meUByte * key, meUByte * file, meUByte * patt)
{
    ...
    /*
     * Process the "%tag-option" currently only the following options are
     * supported: 'r' == recursive ascent.  'c' == continue (multi-tag
     * processing) 
     */
    tagOption = 0;
    ...
    /*
     * Get the current directory location of our file and use this to
     * locate * the tag file. 
     */
	 ...
	 */
	if ((ii =
	     findTagInFile(file, baseName, tagTemp, tagOption, key,
			   patt)) == 0) {
	    if (tagOption & 4) {
		if (tagLastFile == NULL)
	...
	if (ii == 1) {
		...
		return mlwrite(MWABORT | MWCLEXEC,
			       (meUByte *)
			       "[Can't find tag [%s] in tag file : %s]",
			       key, file);
	    }
	} else if (!(tagOption & 1))
	    /*
    ...
    tagLastPos = -1;
    return mlwrite(MWABORT | MWCLEXEC,
		   (meUByte *) "[Failed to find tag : %s]", key);
}
...
findTagExec(int nn, meUByte tag[])
{
	...
	/*
	 * if the first char is a '/' then search forwards, '?' for
	 * backwards 
	 */
	ee = *ss++;
	if (ee == '?') {
	    flags =
		ISCANNER_BACKW | ISCANNER_PTBEG | ISCANNER_MAGIC |
		ISCANNER_EXACT;
	...
	if (ee != '\0') {
	    /*
	     * look for the end '/' or '?' - start at the end and look
	     * backwards 
	     */
	    dd = ss + meStrlen(ss);
	    while (dd != ss)
...
findTag(int f, int n)
{
	...
	/*---	Get user word. */
	if ((meGetString
	     ((meUByte *) "Enter Tag", MLNOSPACE, 0, tag,
	      meBUF_SIZE_MAX) <= 0) || (tag[0] == '\0'))
