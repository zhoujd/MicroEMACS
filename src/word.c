/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com word.c - Word processing
...
backWord(int f, int n)
{
	...
	while (inWord() != meFALSE)
	    if (meWindowBackwardChar(frameCur->windowCur, 1) == meFALSE)
		/*
		 * We can't move back any more cos we're at the start, BUT 
		 * as we have moved and we are in the buffers first word,
		 * we should succeed 
		 */
		return meTRUE;
    }
...
forwDelWord(int f, int n)
{
    ...
    size = 0;
    /*
     * if no argument was given then only kill the white space before the
     * word 
     */
    if (!f && !inWord()) {
	while (!inWord()
	       && (meWindowForwardChar(frameCur->windowCur, 1) > 0))
	    size++;
	if (size)
	    n = 0;
    } else {
...
backDelWord(int f, int n)
{
	...
	do
	    size++;
	while ((meWindowBackwardChar(frameCur->windowCur, 1) > 0)
	       && !inWord());
	if (inWord())
	    meWindowForwardChar(frameCur->windowCur, 1);
	n = 0;
	...
	do {
	    while (!inWord()
		   && (meWindowBackwardChar(frameCur->windowCur, 1) > 0))
		size++;
	    if (!inWord()) {
		size++;
		break;
 ...
 * word-break, eat it, and jump back to the end of the word. Returns
 * meTRUE on success, meFALSE on errors. 
 */

int
wrapWord(int f, int n)
{
    register meUShort cnt,
                    off;	/* size of word wrapped to next line */
    register meUByte c,
                    last = 2;	/* charector temporary */
	...
	if (((c = meLineGetChar(frameCur->windowCur->dotLine, off)) != ' ')
	    && (c != meCHAR_TAB))
	    return meTRUE;
    }
    ...
    frameCur->windowCur->dotOffset =
	meLineGetLength(frameCur->windowCur->dotLine) - off;
    return cnt;
}
...
getBestGap(void)
{
    ...
    int             score = 0,
	bestscore = 0,
	currscore = 0;		/* Number of gaps encountered */
    meUShort        bestoff,
                    curroff,
                    off;
    ...
    for (;;) {
	if (((c =
	      meLineGetChar(frameCur->windowCur->dotLine, off)) == '\0')
	    || (c == ' ')) {
	    if (last != ' ') {
		if (!isWord(last))
		    score -= 10;
 ...
 * text is at doto, and the scaller specifies the length of the text to be 
 * justified, as well as the new required length. This routine assumes the 
 * caller has already removed tabs as justification is performed on spaces 
 * only. Additionally the caller has already formatted the line to the
 * correct length. Justification is performed on the right margin only.
 * (The left margin is assumed to be done). leftMargin is a instruction
 * from fillPara() to inform justify where it's left margin is. justify()
 * determines it's own left margin and tabMode if passed a -ve values.
 * This is required for the bulleted "* " or "a) " and "ii - " type
 * indentation where a leading space must not be altered. Note that on
 * first inspection justify appears to be doing too much work in cleaning
 * up the line. This is true for a fillPara() invocation but it is not
 * true for any other invocation. 
 */
int
justify(int leftMargin, int leftDoto, meUByte jmode)
{
	...
	len--;
	if (((cc =
	      meLineGetChar(frameCur->windowCur->dotLine, len)) != ' ')
	    && (cc != meCHAR_TAB)) {
	    len++;
	    break;
	}
	...
	if (cc == meCHAR_TAB) {
	     ...
	     */
	    int             jj =
		next_tab_pos(getccol(), frameCur->bufferCur->tabWidth);
	    /*
    ...
    len =
	meLineGetLength(frameCur->windowCur->dotLine) - doto + leftMargin;

    /*
     * If there is leading white space and we are justifying centre or
     * right then delete the white space. 
     */
    if (jmode != 'b') {
	/*
	...
	if (len > frameCur->bufferCur->fillcol) {
	     ...
	     * paragraph to the right and the first line is not the
	...
	if (gaps == 0) {	/* No gaps ?? */
	    ...
	    do {
		frameCur->windowCur->dotOffset = doto;	/* Save doto
							 * position */
		getBestGap();
    ...
    /*
     * Set the left margin for center and right modes, * and change the
     * left margin back to tabs 
     */
    if (col != leftMargin)
...
lookahead(meInt fillState)
{
    ...
    if (fillbullet[0] == '\0')
	return fillState;
    limit = frameCur->windowCur->dotOffset + fillbulletlen;	/* Define
								 * the
								 * limit
								 * of
								 * search */
    ...
    /*
     * Scan the line for a paragraph starting point. Start from the
     * beginning of the line 
     */
    for (ii = 0, last_c = (char) 1; ii < limit; ii++) {
	c = meLineGetChar(frameCur->windowCur->dotLine, ii);
	if ((meStrchr(fillbullet, last_c) != NULL)
	    && (c == ' ' || c == meCHAR_TAB)) {
	    /*
		...
		if (frameCur->windowCur->dotLine != frameCur->bufferCur->baseLine) {	/* EOF 
			...
			((jj == jlen)
			 && (meLineGetLength(temp_dotp) <=
			     frameCur->bufferCur->fillcol))) {
			frameCur->windowCur->dotOffset = ii;	/* Assume
								 * position 
								 * 'i' */
	    ...
	    /*
	     * If auto detect is disabled or failed then manually prompt
	     * the user * for the indented position. 
	     */
	    if (status) {
		short           temp_doto;	/* Temporary character
						 * position */
		...
		if (fillState & FILL_INDALL)
		    ...
		    if ((status =
			 mlCharReply((meUByte *)
				     "Indent to <<<<<<<<<< (?/y/n/a/o) ? ",
				     mlCR_QUIT_ON_USER | mlCR_LOWER_CASE,
				     (meUByte *) "ynao", NULL)) == -2) {
			meUByte         scheme =
			...
			/*
			 * Force an update of the screen to to ensure that 
			 * the user can see the information in the correct 
			 * location 
			 */
			update(meTRUE);
			pokeScreen(0x10, frameCur->mainRow,
				   frameCur->mainColumn, &scheme,
				   (meUByte *) "<<<<<<<<<<");
			status =
			    mlCharReply((meUByte *)
					"Indent to <<<<<<<<<< (?/y/n/a/o) ? ",
					...
					(meUByte *) "ynao",
					(meUByte *)
					"(Y)es, (N)o, Yes to (a)ll, N(o) to all, (C-g)Abort ? ");
			mlerase(0);
		    }
		    if (status == -1) {
...
fillAutoDetect(char mode)
{
    ...
    len = meLineGetLength(frameCur->windowCur->dotLine);	/* Length
								 * of the
								 * line */
	 ...
	 * line to be approximatly balanced. Allow a margin of error of 6
	 * characters 
	 */
	sdiff =
	    (frameCur->bufferCur->fillcol >> 1) - (doto + (textLen >> 1));
	if (sdiff < 0)		/* Quick abs !! */
	    sdiff = -sdiff;
	ediff =
	    (frameCur->bufferCur->fillcol >> 1) - (len - (textLen >> 1));
	...
	if ((sdiff == 0) ||	/* We are certain !! */
	    ((sdiff < 2)
	     && (textLen < (frameCur->bufferCur->fillcol >> 1))))
     ...
     */
    if ((len >= frameCur->bufferCur->fillcol)
	&& (doto > (frameCur->bufferCur->fillcol >> 1)))
...
fillPara(int f, int n)
{
    ...
    int             c,
                    lastc;	/* current char durring scan */
    ...
    /*
     * A -ve cont indicates that we do not want to be prompted for
     * indentation We also do not prompt if we are right or centre
     * justifying 
     */
    if (n < 0) {
    ...
    /*
     * Sort out the justification mode that we are running in. An upper
     * case justification mode implies an automatic mode where we detect
     * the current state of the line and apply the appropriate
     * formatting. If the One Line mode (o) is operational then fill
     * compleate paragraphs to a single line. Indentation is disabled 
     */
	...
	if (jmode == 'o')
	    fillState =
		(fillState | FILL_LINE | FILL_MARGIN) & ~FILL_INDENT;
    ...
    if (jmode == 'n') {
	/*
	 * If there are no arguments then do nothing, otherwise advance
	 * the paragraph. 
	 */
	if (!f)
	    return meTRUE;
     ...
     * * knock off indent if disabled. We do not want to be prompting the
     * user. 
     */
    ...
    /*
     * If fill paragraph is called with no arguments then we must retain
     * the position of dot so that we can preserve the users position
     * after we have filled the paragraph, but only if we are not at the
    ...
    while (--n >= 0) {
	/*
	 * go to the beginning of the line and use forward-paragraph to go 
	 * to the end of the current or next paragraph, if this fails we
	 * are at the end of buffer 
	 */
	frameCur->windowCur->dotOffset = 0;	/* Got start of current
						 * line */
	if (windowForwardParagraph(meFALSE, 1) <= 0)
	    break;

	/*
	 * record the pointer to the line just past the EOP and then back 
	 * top the beginning of the paragraph. doto is at the beginning of 
	 * the first word 
	 */
	...
	if ((fillignore[0] != '\0') &&
	    (meStrchr
	     (fillignore,
	      meLineGetChar(frameCur->windowCur->dotLine,
			    frameCur->windowCur->dotOffset)) != NULL)) {
	    frameCur->windowCur->dotLine = eopline;	/* Goto the end of 
							 * the paragraph */
...
#if MEOPT_EXTENDED
	{
		...
		if (meLineGetFlag(lp) & meLINE_PROTECT)
		    return mlwrite(MWABORT,
				   (meUByte *)
				   "[Protected line in paragraph!]");
#if MEOPT_NARROW
		if ((lp != frameCur->windowCur->dotLine) &&
		    (meLineGetFlag(lp) & meLINE_ANCHOR_NARROW))
		    return mlwrite(MWABORT,
				   (meUByte *) "[Narrow in paragraph!]");
#endif
	    } while ((lp = meLineGetNext(lp)) != eopline);
	}
#endif
	/*
	 * Quick auto test to determine what mode the current paragraph
	 * is. Set up the modes in the fill status mask 
	 */
	if (fillState & FILL_AUTO)
	 ...
	 */
	if (((fillState & (FILL_INDENT | FILL_BOTH | FILL_LEFT)) >
	     FILL_INDENT) && ((fillState = lookahead(fillState)) == -1))
	    return meABORT;

	/*
	 * if lookahead has found a new left indent position, this
	 * position must be passed to justify so justify ignores the text
	 * to the left of this indent point 
	 */
...
#endif
	/*
	 * Get the indentation column, this is the real left indentation
	 * position 
	 */
	icol = getccol();

	/*
	 * Reset to initial settings. Modes are:- ~EOP - Not at end of
	 * paragraph ~FORCE - Not a forced paragraph line ~DOT - Not a
	 * period present FIRST - This is the first word 
	 */
	ccol = icol;
	...
	while ((fillState & FILL_EOP) == 0) {
		...
		if (wordlen) {
		    /*
		     * at a word break with a word waiting - reset offset
		     * to start of word 
		     */
			...
			newcol += filleoslen - 1;
		    if ((newcol <= frameCur->bufferCur->fillcol)
			|| (fillState & FILL_LINE)) {
			/*
			...
			if ((fillState & FILL_FIRST) == 0) {
			    int             nosp =
				(fillState & FILL_DOT) ? filleoslen : 1;
			    ccol += nosp;
			    if (fillState & FILL_SPACE) {
				frameCur->windowCur->dotOffset++;
				nosp--;
			...
			if (fillState & FILL_JUSTIFY) {
			    ...
			    /*
			     * reset the indent offset as following lines
			     * will not have the bullet text to the left
			     * of the indent column 
			     */
			    fdoto = -1;
			}
#if MEOPT_UNDO
		 ...
		 */
		if ((c == ' ')
		    && ((fillState & (FILL_FIRST | FILL_SPACE)) == 0)) {
		    frameCur->windowCur->dotOffset++;
		    fillState |= FILL_SPACE;
		} else
		    ldelete(1L, 0);
	 ...
	 */
	if ((f == 2)
	    && (meAnchorGet(frameCur->bufferCur, meANCHOR_FILL_DOT) > 0)) {
...
killPara(int f, int n)
{
    ...
    /*
     * See if this is a paragraph separator line. If it is then the kill
    ...
    /*
     * If we commenced on a separator line then we do not need to protect
	...
	if (n < 0) {
	    /*
	     * If killing backwards advance to the first character of the
	     * line in the paragraph so that we do not delete the
	     * paragraph space. 
	     */
	    while (!inPWord())
		if (meWindowForwardChar(frameCur->windowCur, 1) <= 0)
	    ...
	    /*
	     * If killing forwards goto the previous character so that we
	     * do not destroy the line separating the paragraph. 
	     */
...
countWords(int f, int n)
{
	 ...
	 */
	if ((ch = meLineGetChar(lp, offset)) == '\0') {	/* end of line */
	    ch = meCHAR_NL;
	    lp = meLineGetNext(lp);
	    offset = 0;
	    nlines++;
