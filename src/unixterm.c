/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com unixterm.c - Unix X-Term
 * and Termcap support routines. Copyright (C) 1993-2001 Steven Phillips
 * Copyright (C) 2002-2009 JASSPA (www.jasspa.com) This program is free
...
/*
 * TCAPkey; The termcap code table for the information that we extract
 * from termcap 
 */
struct TCAPkey {
...
/*
 * Set up the default ANSI colors. Many Termcap entries omit these when
 * they should not - we provide our own in the hope that they may be used. 
 */

/*
 * exit_attribute_mode/srg0/me/Turn off all attributes 
 */
static char     RCOLSTR[] = { 27, 91, 48, 109, 0 };	/* me - ESC[0m */
/*
 * set_a_foreground/setaf/AF/Set foreground color using ANSI escape 
 */
static char     FCOLSTR[] = { 27, 91, 51, 65, 109, 0 };	/* AF - ESC[3Am */
/*
 * set_a_background/setab/AB/Set background color using ANSI escape 
 */
static char     BCOLSTR[] = { 27, 91, 52, 65, 109, 0 };	/* AB - ESC[4Am */
#endif				/* MEOPT_COLOR */
...
meSetupPathsAndUser(char *progname)
{
     ...
     * The installation root path of the installation macros searched in
     * the order specified. Once a directory has been located then it is
...
#define _SEARCH_PATH _DEFAULT_SEARCH_PATH
    ...
    /*
     * If PWD is accurate, use it instead of calling gwd. This solves
     * problems with bad ~/ as the shell will store as /user/.... etc. 
     */
	...
	*dd = 0;
	 ...
	 * directory and pass through the argv[0] parameter with no
	 * pathname component. Attempt to fix this condition. Note that we 
	 * cannot add this to executableLookup() since we may not search
	 * the current directory unless it is specifically on the $PATH
	 * hence the additional test here. 
	 */
	...
	if (!gotUserPath && (homedir != NULL)) {
	    /*
	     * We have not found a user path so set ~/ as the user-path as 
	     * this is the best place for macros to write to etc. 
	     */
	    meStrcpy(buff, homedir);
	    if (ll) {
		ii = meStrlen(buff);
...
#endif
    /*
     * Note that we use the first timers expire time here and not the
     * actual time. sometimes the itimers fire a little bit early i.e. +-5 
     * msec, to ensure that we do not hang then we force the timer to
     * expire. There is no other way for the timer to expire (that I am
     * aware of) 
     */
    if (timers != NULL)
...
waitForEvent(int mode)
{
	...
	if (select(pfd, &select_set, NULL, NULL, NULL) >= 0) {
		...
		/*
		 * this is a check to see if we've lost stdin, if we have
		 * there's nothing we can do other than die. This is
		 * common when the X-window is destroyed using the window
		 * manager. 
		 */
#ifdef _USEPOLL
		...
		/*
		 * As there is only one event we can rely on the return
		 * status to tell us what has happened. 
		 */
		ii = poll(pfds, 1, 0);
		if ((ii < 0) ||
		    ((ii == 1)
		...
		if (!ii) {
		     ...
		     * which is a real bummer as me now dies! as a fix,
		     * and I know its a bit of a bodge, only die is this
		     * happens 3 times on the trot 
		     */
		    if (lost++ == 2) {
			/*
			 * If there's nothing to read, yet select flags
			 * meStdin as triggered then we've lost the input
			 * - Die... Unfortunately we must be very careful
			 * on what we do, we cannot write out to the
			 * x-window cause its gone, but meDie takes care
			 * of this 
			 */
			meDie();
		    }
...
TCAPgetWinSize(void)
{
	...
	((TTnewWid = size.ws_col) <= 0) || ((TTnewHig = size.ws_row) <= 0))
#else
#ifdef TIOCGSIZE
    /*
     * SunOS - style.  
     ...
     * * cannot easily correct. If this is the case then reduce the number 
     * of * lines by 1. 
     */
    if ((tcaptab[TCAPam].code.value != 0) &&
	(tcaptab[TCAPxenl].code.value == 0) && (TTaMarginsDisabled == 0))
	TTnewHig--;
}
...
__XTERMfontGetId(meUByte font)
{
    ...
    /*
     * remove the underline as this is not a different font, the underline 
     * is drawn on after 
     */
...
meFrameXTermSetScheme(meFrame * frame, meScheme scheme)
{
	...
	if (meFrameGetXGCFont(frame) != cc) {
	    ...
	    if (meFrameGetXGCValues(frame).font !=
		meFrameGetXGCFontId(frame)) {
		meFrameGetXGCValues(frame).font =
		    meFrameGetXGCFontId(frame);
		valueMask |= GCFont;
	    }
	}
    ...
    if (valueMask)
	XChangeGC(mecm.xdisplay, meFrameGetXGC(frame), valueMask,
		  &meFrameGetXGCValues(frame));
}

/*
 * meFrameXTermDrawSpecialChar; Draw a special character to the screen. x
 * is the lefthand edge of the character, y is the top of the character.
 * xgc is used for the colors. 
 */
void
meFrameXTermDrawSpecialChar(meFrame * frame, int x, int y, meUByte cc)
{
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 1,
		  y + mecm.fhdepth - mecm.fhwidth, x + mecm.fwidth - 2,
		  y + mecm.fhdepth - mecm.fhwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 2,
		  y + mecm.fhdepth - mecm.fhwidth, x + mecm.fwidth - 2,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 2,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth,
		  x + mecm.fwidth - 1,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x, y + mecm.fhdepth - mecm.fhwidth,
		  x + mecm.fwidth - 1, y + mecm.fhdepth - mecm.fhwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth,
		  x + mecm.fwidth - 1,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth);
	break;
	...
	XFillPolygon(mecm.xdisplay, meFrameGetXWindow(frame),
		     meFrameGetXGC(frame), points, 4, Convex,
		     CoordModeOrigin);
	break;
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + 1,
		  y + mecm.fhdepth - mecm.fhwidth, x + 1,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + 1,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth, x,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth);
	break;

    case 0x07:			/* Line space '.' */
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fhwidth, y + mecm.fhdepth,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 2,
		  y + mecm.fhdepth, x + mecm.fhwidth, y + mecm.fhdepth);
	...
	XFillPolygon(mecm.xdisplay, meFrameGetXWindow(frame),
		     meFrameGetXGC(frame), points, 3, Convex,
		     CoordModeOrigin);
	break;
	...
	ii = (mecm.fhdepth + 1) >> 1;
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + 1, y + mecm.fhdepth,
		  x + mecm.fhwidth - 1, y + mecm.fhdepth);
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 2,
		  y + mecm.fhdepth, x + mecm.fwidth - 2, y + ii - 1);
	points[0].x = x + mecm.fhwidth;
	points[0].y = y + ii;
	points[1].x = x + mecm.fhwidth;
	points[1].y = y + mecm.fdepth - ii - 1;
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x, y + mecm.fhdepth,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fhwidth, y,
		  x + mecm.fhwidth, y + mecm.fhdepth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fhwidth, y + mecm.fhdepth,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x,
		  y + mecm.fhdepth - mecm.fhwidth + 1, x + mecm.fwidth - 1,
		  y + mecm.fhdepth - mecm.fhwidth + 1);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth,
		  x + mecm.fwidth - 1,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x,
		  y + mecm.fhdepth - mecm.fhwidth + 1, x,
		  y + mecm.fhdepth + mecm.fwidth - mecm.fhwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fwidth - 1,
		  y + mecm.fhdepth - mecm.fhwidth + 1, x + mecm.fwidth - 1,
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  ...
		  x + mecm.fwidth - 1, y + mecm.fhdepth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + mecm.fhwidth,
		  y + mecm.fdepth - 1, x + mecm.fhwidth, y + mecm.fhdepth);
	break;
	...
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x, y + mecm.fdepth - 1,
		  x + mecm.fwidth - 1, y + mecm.fdepth - mecm.fwidth);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + 2, y + mecm.fdepth - 1,
		  x + mecm.fwidth - 1, y + mecm.fdepth - mecm.fwidth + 2);
	XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		  meFrameGetXGC(frame), x + 4, y + mecm.fdepth - 1,
	...
	for (ii = (y + 1) & ~1; ii < y + mecm.fdepth; ii += 2)
	    XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame),
		      meFrameGetXGC(frame), x, ii, x + mecm.fwidth - 1,
		      ii);
	break;
...
meFrameXTermDraw(meFrame * frame, int srow, int scol, int erow, int ecol)
{
	...
	for (flp = frame->store + srow; srow < erow; srow++, flp++) {
		...
		if (*fssp++ != schm) {	/* Change in colour ?? */
		    ...
		    meFrameXTermDrawString(frame, colToClient(tcol), row,
					   buff, length);
		    while (--spFlag >= 0) {
			while (((cc = sfstp[tcol]) & 0xe0) != 0)
			    tcol++;
			meFrameXTermDrawSpecialChar(frame,
						    tcol * mecm.fwidth,
						    row - mecm.ascent, cc);
			tcol++;
		    }
		...
		buff[length++] = cc;	/* Set & Increment the string
					 * length */
		col++;		/* Next colunm */
	    }
	    ...
	    if (length > 0) {
		meFrameXTermSetScheme(frame, schm);
		meFrameXTermDrawString(frame, colToClient(tcol), row, buff,
				       length);
		while (--spFlag >= 0) {
		    while (((cc = sfstp[tcol]) & 0xe0) != 0)
			tcol++;
...
meXEventHandler(void)
{
	...
	if ((frame = meXEventGetFrame(&event)) != NULL) {
	    ...
	    /*
	     * Only handle the Font mapping state, when flagged with a
	     * font change then the size hints of the unmapped window
	     * should be changed and the window immediately re-mapped. 
	     */
	    if (state == meXMAP_FONT) {
		/*
		 * Change the size hints of the window and re-map the
		 * window, note that we do not check the error return
		 * because there is not actually anything that we can do
		 * if the hints cannot be set. 
		 */
		XSetWMNormalHints(mecm.xdisplay, meFrameGetXWindow(frame),
				  &sizeHints);
		/*
		 * Now that the hints have been changed then the window
		 * may be re-mapped to display it again. 
		 */
	...
	if ((frame = meXEventGetFrame(&event)) != NULL) {
	    /*
	     * Get the width and heigth back and setup the frame->depthMax 
	     * etc 
	     */
	    int             ww,
	                    hh,
	     ...
	     */
	    /*
	     * Disable the window resize until BOTH width and height
	     * changes have been established. Both LINUX and SUN suffer
	     * from severe beating between the client and server when the
	     * resize requests are split. Allow the new sizes to be
	     * computed and then inform the server in one invocation. 
	     */
	     ...
	     * established. Only perform the resize if there is something
	     * to change 
	     */
	...
	if ((frame = meXEventGetFrame(&event)) != NULL) {
	    ...
	    /*
	     * Compute the start and end column widths. For the end column 
	     * then a rounding operation is performed to ensure that the
	     * whole character is covered 
	     */
	    scol = clientToCol(event.xexpose.x);
	    ecol =
		clientToCol(event.xexpose.x + event.xexpose.width +
			    mecm.fwidth - 1);
	    if (ecol > frame->width)
		ecol = frame->width;
	    srow = clientToRow(event.xexpose.y);
	    erow =
		clientToRow(event.xexpose.y + event.xexpose.height +
			    mecm.fdepth - 1);
		...
		if (event.xexpose.count == 0)	/* Last of the expose
						 * events ?? */
		...
		/*
		 * Update the historical bounding box infomation by using
		 * the maximum and minimum values of the existing and
		 * current Exposure boxes. 
		 */
		if (scol < sscol)
		    sscol = scol;
		if (ecol > secol)
	...
	if (meMouseCfg & meMOUSE_ENBLE) {
	    /*
	     * Collect the position of the mouse. Require the row/column
	     * mouse position. Also require the fractional bits incase we
	     * are scrolling Really we should re-evaluate the buttons
	     * here. However X-Windows does not seemed to have aquired an
	     * incorrect state (i.e. button state mis-represented). I will 
	     * leave it for the time being (JDG) 
	     */
	    ...
	    mouse_dX =
		((event.xmotion.x -
		  (mouse_X * mecm.fwidth)) << 8) / mecm.fwidth;
	    mouse_dY =
		((event.xmotion.y -
		  (mouse_Y * mecm.fdepth)) << 8) / mecm.fdepth;
	    /*
    ...
    case EnterNotify:
	/*
	 * Some window managers do not send the FocusIn and FocusOut
	 * events correctly To fix this we have to track down Window Enter 
	 * and Leave events and check for a focus change ourselves 
	 */
	if (((frame = meXEventGetFrame(&event)) != NULL) &&
	    (frame->flags & meFRAME_NOT_FOCUS)) {
	    Window          fwin;
	...
	if ((meMouseCfg & meMOUSE_ENBLE) &&
	    ...
	    if (!(frame->flags & meFRAME_NOT_FOCUS)) {
		/*
		 * if we haven't currently got the input focus, grab it
		 * now 
		 */
		XSetInputFocus(mecm.xdisplay, meFrameGetXWindow(frame),
	    ...
	    ss = (ME_SPECIAL | (SKEY_mouse_pick_1 + mouseKeys[bb] - 1) |
		  mouseKeyState);
	    addKeyToBuffer(ss);
	}
	break;
	...
	if (meMouseCfg & meMOUSE_ENBLE) {
	    ...
	    ss = event.xbutton.state;
	    mouseKeyState = ((ss & 0x01) << 8) | ((ss & 0x0C) << 7);
	    ss = (ME_SPECIAL | (SKEY_mouse_drop_1 + mouseKeys[bb] - 1) |
		  mouseKeyState);
	    addKeyToBuffer(ss);
	}
	break;
    ...
    case KeyPress:
	{
		...
		case XK_Break:
		    ...
		    goto special_key;
		    /*
		     * Mode switch - used on foreign keyboards, always
		     * ignore. 
		     */
...
#endif
		    /*
		     * Keypad Functions, keypad numbers cleverly chosen to 
		     * map to ascii 
		     */
		case XK_KP_Space:
		    ii = SKEY_space;
		...
		case XK_dead_diaeresis:
		    ...
		    /*
		     * Auxilliary Functions; note the duplicate
		     * definitions for left and right function keys; Sun
		     * keyboards and a few other manufactures have such
		     * function key groups on the left and/or right sides
		     * of the keyboard. We've not found a keyboard with
		     * more than 35 function keys total. 
		     */
		...
		case XK_Shift_Lock:
		    ...
		    /*
		     * ignore these as ME has its own S-pick/S-drop key
		     * event generators 
		     */
		case XK_Shift_L:
		case XK_Shift_R:
		...
		default:
	      ...
	      special_key:
		ii |= ME_SPECIAL;	/* bindKeyLookUp[ss &
					 * ME_BIND_MASK]; */
    ...
    case SelectionRequest:
	{
		...
		if ((event.xselectionrequest.target == XA_STRING)
			...
			*ss = '\0';
			...
			XChangeProperty(mecm.xdisplay, reply.requestor,
					reply.property, reply.target, 8,
					PropModeReplace, data, len);
		    }
		} else if (event.xselectionrequest.target ==
			   meAtoms[meATOM_TARGETS]) {
		    reply.property = event.xselectionrequest.property;
		    XChangeProperty(mecm.xdisplay, reply.requestor,
				    reply.property, reply.target, 32,
				    PropModeReplace,
				    (unsigned char *) (meAtoms +
						       meATOM_TARGETS), 2);
		}
	    }
	    XSendEvent(mecm.xdisplay, reply.requestor, False, 0,
		       (XEvent *) & reply);
	    break;
	}
	...
	if ((frame = meXEventGetFrame(&event)) != NULL) {
		...
		(XGetWindowProperty
		 (mecm.xdisplay, meFrameGetXWindow(frame),
		  meAtoms[meATOM_COPY_TEXT], 0, 0x1fffffffL, False,
		  AnyPropertyType, &type, &fmt, &nitems, &left,
		  &buff) == Success)) {
		if (type == meAtoms[meATOM_MULTIPLE])
...
#endif
			;
		else if ((type == meAtoms[meATOM_INCR]) && (fmt == 32)
			 && (nitems > 0)) {
		    nitems = ((long *) buff)[0];
		    XFree(buff);
		    /*
			 ...
			 */
			XDeleteProperty(mecm.xdisplay,
					meFrameGetXWindow(frame),
					meAtoms[meATOM_COPY_TEXT]);
			XFlush(mecm.xdisplay);
			/*
			 * don't flag that we've got the clipboard yet cos 
			 * we haven't 
			 */
			break;
		    }
			...
			/*
			 * always killSave, don't want to glue 'em
			 * together 
			 */
			killSave();
			if ((dd = killAddNode(nitems)) != NULL) {
	...
	if (((frame = meXEventGetFrame(&event)) != NULL) &&
	    ...
	    ret =
		XGetWindowProperty(mecm.xdisplay, meFrameGetXWindow(frame),
				   meAtoms[meATOM_COPY_TEXT], 0L,
				   0x1fffffffL, True, AnyPropertyType,
	...
	if (((frame = meXEventGetFrame(&event)) != NULL) &&
		...
		meAtoms[meATOM_WM_DELETE_WINDOW]) {
#if MEOPT_MWFRAME
		if (meFrameDelete(frame, 6) <= 0)
#endif
		{
		    /*
		     * Use the normal command to save buffers and exit if
		     * it doesn't exit then carry on as normal Must ensure 
		     * we ask the user, not a macro 
		     */
		    int             savcle;
		    savcle = clexec;
		    clexec = meFALSE;
		    ...
		    clexec = savcle;
		       ...
		       meAtoms[meATOM_WM_SAVE_YOURSELF]) {
		/*
...
#ifdef _USG
/*
 * Get the terminal attributes structure. There are cases when the call
 * will fail we have been launched off the desktop via an open action.
 * Return an integer status of -1 if the call failed and we filled in the
 * values. 
 */
static int
TCAPgetattr(meTermio p, int isX)
{
	...
	/*
	 * We are running X-Windows. We have been launched from the
	 * command line as we have obtained the terminal environment. Set
	 * this up for an interactive shell session. 
	 */
#ifdef _HPUX
	/*
	 * HP-UX seems to be susceptible to "su" problems when running
	 * "me&". Fix any base modes in-case we have been started in the
	 * background. At the very minimum we need to echo characters and
	 * process concanical input. 
	 */
	p->c_iflag |= ICRNL;
    ...
    /*
     * Logging for tracing the output of the termio statement at start-up. 
     * We * cannot dump onto stdio because it may not exist. 
     */
#if 0
    {
	FILE           *fp;
...
{
    char           *p;
    char            tcbuf[4096];	/* Jon 2001/12/15 increase for
					 * cygwin terminal */
    char           *tv_stype;
    char            err_str[72];
    int             ii;
    ...
    /*
     * Make sure that there was sufficient buffer space to process the
     * strings 
     */
    ...
    /*
     * Determine if there is a mechanism to enable and disable the
     * automatic margins. If there is then disable them now 
     */
	...
	sprintf(err_str, "%s termcap lacks %s or %s entry",
		tv_stype, tcaptab[TCAPclear].capKey,
		tcaptab[TCAPcup].capKey);
	puts(err_str);
	meExit(1);
    }
	...
	for (ii = 0; ii < SKEY_MAX; ii++) {
		...
		translateKeyAdd(&TTtransKey,
				charListToShorts(sl, tcapSpecKeyDefs[ii]),
				TTtransKey.time, sl, ME_SPECIAL | ii);
	}
	for (ii = 0; ii < SKEY_MAX; ii++) {
	    p = buf;
	    if ((tcapSpecKeyStrs[ii] != NULL)
		&& ((p = tgetstr(tcapSpecKeyStrs[ii], &p)) != NULL))
		translateKeyAdd(&TTtransKey,
				charListToShorts(sl, (meUByte *) p),
	 ...
	 */
	if ((tcapSpecKeyStrs[SKEY_page_up] == NULL)
	    && ((p = tgetstr("kP", &p)) != NULL)) {
	    tcapSpecKeyStrs[SKEY_page_up] = "kP";
	    translateKeyAdd(&TTtransKey,
			    charListToShorts(sl, (meUByte *) p),
			    TTtransKey.time, sl,
			    (SKEY_page_up | ME_SPECIAL));
	}
	/*
	 * KEY_NPAGE, sent by next-page key 
	 */
	if ((tcapSpecKeyStrs[SKEY_page_down] == NULL)
	    && ((p = tgetstr("kN", &p)) != NULL)) {
	    tcapSpecKeyStrs[SKEY_page_down] = "kN";
	    translateKeyAdd(&TTtransKey,
			    charListToShorts(sl, (meUByte *) p),
			    TTtransKey.time, sl,
			    (SKEY_page_down | ME_SPECIAL));
	}
    }
    /*
...
TCAPopen(void)
{
    ...
    /*
     * Set up the type ahead buffer. There are some inconsistancies here
     * on various systems so it is better simply to define a default
     * output buffer size. 
     */
...
TCAPshowCur(void)
{
    if ((frameCur->cursorRow <= frameCur->depth)
	&& (frameCur->cursorColumn < frameCur->width)) {
	/*
...
TCAPaddColor(meUByte index, meUByte r, meUByte g, meUByte b)
{
    ...
    if (noColors <= index) {
	colTable =
	    (meUInt *) meRealloc(colTable, (index + 1) * sizeof(meUInt));
...
TCAPschemeSet(meScheme scheme)
{
	...
	if (oschemeFont != font) {
		 ...
		 * state of some attributes so it is best to turn all off
		 * and start again, 
		 */
		if ((oschemeFont & meFONT_ITALIC)
		    && (tcaptab[TCAPritm].code.str != NULL))
		    putpad(tcaptab[TCAPritm].code.str);
		if ((oschemeFont & meFONT_UNDERLINE)
		    && (tcaptab[TCAPrmul].code.str != NULL))
		    putpad(tcaptab[TCAPrmul].code.str);
		if ((oschemeFont &
		     (meFONT_BOLD | meFONT_REVERSE | meFONT_LIGHT))
		 ...
		 */
		if ((font & meFONT_BOLD)
		    && (tcaptab[TCAPbold].code.str != NULL))
		    putpad(tcaptab[TCAPbold].code.str);
		if ((font & meFONT_ITALIC)
		    && (tcaptab[TCAPsitm].code.str != NULL))
		    putpad(tcaptab[TCAPsitm].code.str);
		if ((font & meFONT_LIGHT)
		    && (tcaptab[TCAPdim].code.str != NULL))
		    putpad(tcaptab[TCAPdim].code.str);
		if ((font & meFONT_REVERSE)
		    && (tcaptab[TCAPrev].code.str != NULL))
		    putpad(tcaptab[TCAPrev].code.str);
		if ((font & meFONT_UNDERLINE)
		    && (tcaptab[TCAPsmul].code.str != NULL))
...
TCAPschemeReset(void)
{
    ...
    if (oschemeFntr) {
	/*
	 * Remove the old font attributes. We cannot guarantee the state
	 * of some attributes so it is best to turn all off and start
	 * again, 
	 */
	if ((oschemeFont & meFONT_ITALIC)
	    && (tcaptab[TCAPritm].code.str != NULL))
...
#if MEOPT_COLOR
    if (meSystemCfg & meSYSTEM_ANSICOLOR) {
	if ((oschemeFcol != meCOLOR_INVALID)
	    || (oschemeBcol != meCOLOR_INVALID)) {
	    /*
...
TCAPmove(int row, int col)
{
     ...
     * movement to a function from a macro. In addition we should really
     * be checking if we can move with attributes enabled, if this is not
     * the case then we need to reset out modes, move and then re-apply.
     * The API would be better served with a TCAPmoveScheme() where we
     * could set the next scheme at the same time. Keep this for a rainy
     * day !! 
     */
...
TTinitMouse(void)
{
	...
	static Cursor   meCursors[meCURSOR_COUNT] =
	    { 0, 0, 0, 0, 0, 0, 0 };
	static meUByte  meCursorChar[meCURSOR_COUNT] = {
	    0, XC_arrow, XC_xterm, XC_crosshair, XC_hand2, XC_watch,
		XC_pirate
	};
	meUByte         cc;
	int             b1,
	...
	if (cc != meCurCursor) {
	    if (meCurCursor) {
		XUndefineCursor(mecm.xdisplay,
				meFrameGetXWindow(frameCur));
		meCurCursor = 0;
	    }
	    if (cc && ((meCursors[cc] != 0) ||
		       ((meCursors[cc] =
			 XCreateFontCursor(mecm.xdisplay,
					   meCursorChar[cc])) != 0))) {
		XDefineCursor(mecm.xdisplay, meFrameGetXWindow(frameCur),
			      meCursors[cc]);
		meCurCursor = cc;
	    }
	}
...
/*
 * XTERMsetFont; Determine the font that we shall use. Use the base string 
 * provided by the user (or system) and determine if there are any
 * derivatives of the font 
 */
static int
XTERMsetFont(char *fontName)
{
	...
	/*
	 * Break up the font. This must be the full X-Window font
	 * definition, we do not understand the X-Windows wildcards and
	 * aliases. Lifes too short to sit down and address these now !! 
	 */
	for (ii = 0; (ii < 14); ii++) {
	    if (*p != '-')
		break;
...
XTERMcreateWindow(meUShort width, meUShort depth)
{
    ...
    frameData->xwindow =
	XCreateSimpleWindow(mecm.xdisplay,
			    RootWindow(mecm.xdisplay, xscreen),
			    sizeHints.x, sizeHints.y, sizeHints.width,
			    sizeHints.height, 0, WhitePixel(mecm.xdisplay,
							    xscreen),
			    BlackPixel(mecm.xdisplay, xscreen));
    ...
    frameData->xgc =
	XCreateGC(mecm.xdisplay, frameData->xwindow, GCFont,
		  &frameData->xgcv);
     ...
     */
    /*
     * olwm on sunos has a Focus bug, it does not sent the FocusIn and
     * FocusOut events properly, but as it does send EnterNotify and
     * LeaveNotify events. use this to check the Focus state as well - SWP 
     * 17/8/99 
     */
    XSelectInput(mecm.xdisplay, frameData->xwindow,
		 (ExposureMask | StructureNotifyMask | KeyPressMask |
		  ButtonPressMask | ButtonReleaseMask | PointerMotionMask |
		  FocusChangeMask | PropertyChangeMask | LeaveWindowMask |
		  EnterWindowMask));

    /*
     * Map the window a.s.a.p. cos officially we can't draw to the window
     * until we get an Expose event. As this is not checked for by mapping 
     * now there is less chance of this being a problem. Jon 2006-09-17.
     * The Window cannot be Mapped until the Input is selected otherwise
     * we do not receive any (Un)MapNotify events in Linux. If the above
     * comment is true then we can easily check the Mapping status before
     * processing any paint/expose events. 
     */
...
meFrameXTermInit(meFrame * frame, meFrame * sibling)
{
	 ...
	 */
	else if ((frame->termData =
		  XTERMcreateWindow(frame->width,
				    frame->depth + 1)) == NULL)
	    return meFALSE;
#else
	frame->termData = firstFrameData;
...
XTERMstart(void)
{
    ...
    /*
     * Copy the Terminal I/O. We may spawn a terminal in the window later
     * and the termio structure must be initialised. The structure may be
    ...
    if (XrmGetResource
	(rdb, "MicroEmacs.font", "MicroEmacs.Font", &retType, &retVal)
    ...
    /*
     * Set up the protocol defaults required. We must do this before we
     * map the window. 
     */
    {
	...
	for (ii = 0; ii < 7; ii++)
	    meAtoms[ii] =
		XInternAtom(mecm.xdisplay, meAtomNames[ii], meFALSE);
    ...
    if (XrmGetResource
	(rdb, "MicroEmacs.name", "MicroEmacs.Name", &retType, &retVal)
	&& !strcmp(retType, "String")
	&& ((ss = meStrdup((meUByte *) retVal.addr)) != NULL))
	meName = ss;

    if (XrmGetResource
	(rdb, "MicroEmacs.iconname", "MicroEmacs.IconName", &retType,
	 &retVal) && !strcmp(retType, "String")
	&& ((ss = meStrdup((meUByte *) retVal.addr)) != NULL))
	meIconName = ss;
    ...
    if ((firstFrameData =
	 XTERMcreateWindow(TTwidthDefault, TTdepthDefault)) == NULL)
	return meFALSE;

#ifdef _ME_CONSOLE
#ifdef _TCAP
    /*
     * When the xterm version of ME has to die using meDie (using killing
     * it etc) it cannot write to the X-Window as it may have gone.
     * Therefore the die messages "*** Emergency..." are written to the
     * terminal. This is done by setting the meSystem variable to a value
     * which make it use TCAP. But TCAP has not been initialise and
     * mlwrite will use TCAPmove so we must initialise the TCAPcup (cursor 
     * move string) to something to stop it core-dumping. 
     */
...
meFrameXTermHideCursor(meFrame * frame)
{
	...
	schm = flp->scheme[frame->cursorColumn];	/* Get scheme
							 * under cursor */
...
meFrameXTermShowCursor(meFrame * frame)
{
	...
	if (!(frame->flags & meFRAME_NOT_FOCUS)) {
		...
		meFrameGetXGCValues(frame).background =
	    ...
	    if ((meSystemCfg & meSYSTEM_FONTFIX) && !((*cc) & 0xe0)) {
		static char     ss[1] = { ' ' };
		meFrameXTermDrawString(frame,
				       colToClient(frame->cursorColumn),
				       rowToClient(frame->cursorRow), ss,
				       1);
		meFrameXTermDrawSpecialChar(frame,
					    colToClient(frame->
							cursorColumn),
					    rowToClientTop(frame->
							   cursorRow),
					    *cc);
	    } else
		meFrameXTermDrawString(frame,
				       colToClient(frame->cursorColumn),
				       rowToClient(frame->cursorRow), cc,
				       1);
	} else {
	    if (meFrameGetXGCFCol(frame) != cursorColor) {
		meFrameSetXGCFCol(frame, cursorColor);
		meFrameGetXGCValues(frame).foreground =
		    colTable[cursorColor];
		XChangeGC(mecm.xdisplay, meFrameGetXGC(frame),
			  GCForeground, &meFrameGetXGCValues(frame));
	    }
	    XDrawRectangle(mecm.xdisplay, meFrameGetXWindow(frame),
			   meFrameGetXGC(frame),
			   colToClient(frame->cursorColumn),
			   rowToClientTop(frame->cursorRow),
...
changeFont(int f, int n)
{
    ...
    /*
     * Get the name of the font. If it is specified as default then do not 
     * collect the remaining arguments 
     */
    if (meGetString((meUByte *) "Font Name", 0, 0, buff, meBUF_SIZE_MAX) ==
	meABORT)
	return meFALSE;
    ...
    /*
     * Set up the arguments for a resize operation. Because the font has
     * changed then we need to define the new window base size, minimum
     * size and increment size. 
     */
    meFrameLoopBegin();
    ...
    XChangeGC(mecm.xdisplay, meFrameGetXGC(loopFrame), GCFont,
	      &meFrameGetXGCValues(loopFrame));
     ...
     * window, the hints size is then changed when we receive the
     * MapNotify event and the X-Window may be re-mapped. Unmap the window 
     * ready to change the size hints for the font. Note that the hint
     * values are the same irrespective of the number of windows. 
     */
    if (meFrameGetXMapState(loopFrame) == meXMAP_MAP) {
	/*
	 * Tell the Unmap event that this is a font change mapping and
	 * then unmap the window. 
	 */
...
XTERMaddColor(meColor index, meUByte r, meUByte g, meUByte b)
{
	...
	if (noCells < 0) {
	    /*
	     * printf("Warning: Failed to allocate colors, looking up
	     * closest\n") ;
	     */
	    if (((noCells = DisplayCells(mecm.xdisplay, xscreen)) > 0) &&
		((cells =
		  meMalloc(noCells * 3 * sizeof(meUByte))) != NULL)) {
		ii = noCells;
		ss = cells;
    ...
    /*
     * the default colors are created before the first frame is so check
     * there is a frame 
     */
    if (frameCur != NULL) {
	meFrameLoopBegin();
...
XTERMsetBgcol(void)
{
     ...
     */
    XSetWindowBackground(mecm.xdisplay, meFrameGetXWindow(loopFrame),
			 colTable[meStyleGetBColor
				  (meSchemeGetStyle(globScheme))]);
...
meFrameSetWindowSize(meFrame * frame)
{
	...
	/*
	 * printf("meFrameSetWindowSize::" __FILE__ ":%d:
	 * XResizeWindow\n", __LINE__);
	 */
	XResizeWindow(mecm.xdisplay, meFrameGetXWindow(frame),
...
meFrameRepositionWindow(meFrame * frame, int resize)
{
	 ...
	 * disagreement on what an XMoveWindow to x,y should do, move the
	 * WM frame to x,y or the window to x,y (frame to x-fx,y-fy).
	 * Judging by the ICCCM the window should be moved to x,y (i.e.
	 * same as if there is no WM) so this assumes this behaviour. Use 
	 * the XGetWindowAttributes to retrive the window offset position,
	 * this gives us the information that we need for the z and y
	 * offset relative to the parent. Jon - 2009-09-26. This section
	 * of code used to break Linux because the the sizes were being
	 * made up and the wxa attributes returned are actually the inner
	 * window which meant that a spurious size is returned. We cannot
	 * find out the X-Window boarder and top bar so simply use the
	 * values that are supplied. 
	 */
	status = XGetWindowAttributes(mecm.xdisplay,
				      meFrameGetXWindow(frame), &xwa);
	/*
	 * if (status == 0)
	 */
	/*
	 ...
	 * xwa.width, xwa.height, xwa.width/mecm.fwidth,
	 * xwa.height/mecm.fdepth);
	 */
	/*
	...
	if (status == 0) {
	    ...
	    /*
	     * Assume some sensible defaults in the absence of anything
	     * else 
	     */
	    wbs = 3;
	    tbs = 3;
	 ...
	 */
	/*
	 * TODO: The sizeHints is horribly overloaded and needs to be
	 * moved into meFrame 
	 */
	xx = sizeHints.x;
	yy = sizeHints.y;
	...
	if (resize && (((xx + ww + wbs + wbs) > sizeHints.max_width) ||
	    ...
	    /*
	     * printf("meFrameRepositionWindow::" __FILE__ ":%d:
	     * XMoveResizeWindow\n", __LINE__);
	     */
...
/*
 * meFrameSetWindowTitle Put the name of the buffer into the window frame 
 */
void
...
TTsetClipboard(void)
{
	...
	!(clipState & (CLIP_OWNER | CLIP_RECEIVING | CLIP_DISABLED))
	&& (kbdmode != mePLAY)) {
	XSetSelectionOwner(mecm.xdisplay, XA_PRIMARY,
			   meFrameGetXWindow(frameCur), CurrentTime);
	if (XGetSelectionOwner(mecm.xdisplay, XA_PRIMARY) ==
	    meFrameGetXWindow(frameCur))
	    clipState |= CLIP_OWNER;
    }
}
...
TTgetClipboard(void)
{
    ...
    {
	/*
	 * Add the CLIP_RECEIVING flag. This is really important if
	 * increment copy texts are being used. If they are and this flag
	 * is set after receiving the initial size, the killSave then take 
	 * ownership of the block and so we never get the copy text. Take
	 ...
	 */
	XConvertSelection(mecm.xdisplay, XA_PRIMARY, XA_STRING,
			  meAtoms[meATOM_COPY_TEXT],
			  meFrameGetXWindow(frameCur), CurrentTime);
	/*
	...
	/*
	 * reset the increment clip size to zero (just incase there was an 
	 * interuption) 
	 */
	meClipSize = 0;
	TTsetClipboard();
...
#ifdef _ME_CONSOLE
/*
 * Special TTstart when both console and window are enabled, Its purpose
...
TTwaitForChar(void)
{
	...
	meUShort        mc;
	meUInt          arg;
	mc = ME_SPECIAL | mouseKeyState | (SKEY_mouse_time +
					   mouseKeys[mouseButtonGetPick
						     ()]);
	/*
	 * mouse-time bound ?? 
	 */
...
#endif				/* _ME_CONSOLE */
	{
	     ...
	     * effect, allowing the server to process all of our
	     * outstanding events (typically mecm.xdisplay requests). 
	     */
	...
	/*
	 * Must only do one at a time else if you get A key while
	 * autosaving the key can be proccessed by TTahead and then you
	 * sit and wait in TCAPwait 
	 */
	handleTimerExpired();
	if (TTahead())
	    break;
...
TTsleep(int msec, int intable, meVarList * waitVarList)
{
    ...
    if (msec >= 0)
	/*
	 * Don't actually need the abs time as this will remain the next
	 * alarm 
	 */
...
TTahead(void)
{
	...
	/*
	 * don't process the timers if we have a key waiting! This is
	 * because the timers can generate a lot of timer keys, filling up 
	 * the input buffer - these are not wanted. By not processing, we
	 * leave them there until we need them. 
	 */
	if (TTnoKeys)
	    return TTnoKeys;
	...
	if (isTimerExpired(MOUSE_TIMER_ID)) {
		...
		|| (TTallKeys & 0x2)) {
		/*
		 * Timer has expired and timer still bound. Report the
		 * key. 
		 */
		/*
		 ...
		 */
		/*
		 * printf("Setting mouse timer for repeat
		 * %d\n",repeatTime) ;
		 */
...
#endif
	/*
	 * Pasting in termcap results in lots of keys all at once rather
	 * than lose them, keep them there until there enough room in the
	 * input key buffer to store them. 
	 */
	while (TTnoKeys != KEYBUFSIZ) {
	    ...
	    if ((status != 1)
		|| ((pfds[0].revents & (POLLIN | POLLPRI)) == 0))
		 ...
		 * horrible character to translate so we do it here before
		 * we enter the system. There is not an easy way to add
		 * this translation. 
		 */
		if (cc == '\0')
	...
	if (alarmState & meALARM_WINSIZE) {
	    frameChangeWidth(meTRUE, TTnewWid - frameCur->width);	/* Change 
									 * width 
									 */
	    frameChangeDepth(meTRUE, TTnewHig - (frameCur->depth + 1));	/* Change 
									 * depth 
									 */
	    alarmState &= ~meALARM_WINSIZE;
	}
...
putenv(const char *string)
{
    ...
    /*
     * The variable value does not exist in the environment, make a new
     * entry. First re-size the environment. 
     */
...
xdndInitialize(Display * display)
{
    ...
    /*
     * dnd.XdndActionMove = XInternAtom (dnd.display, "XdndActionMove",
     * False);
     */
    /*
     * dnd.XdndActionLink = XInternAtom (dnd.display, "XdndActionLink",
     * False);
     */
    /*
     * dnd.XdndActionAsk = XInternAtom (dnd.display, "XdndActionAsk",
     * False);
     */
    /*
     ...
     */
    /*
     * dnd.XdndTypeList = XInternAtom (dnd.display, "XdndTypeList",
     * False);
     */
    /*
     * dnd.XdndActionList = XInternAtom (dnd.display, "XdndActionList",
     * False);
     */
    /*
    ...
    dnd.Xdnd_NON_PROTOCOL_ATOM =
	XInternAtom(dnd.display, "meSelectionProperty", False);
    dnd.Xdnd_text_uri_list =
	XInternAtom(dnd.display, "text/uri-list", False);
...
xdndClientMessage(XEvent * xevent, meFrame * frame)
{
	...
	if ((dnd.dragger_window == XDND_DROP_SOURCE_WIN(xevent))
	     ...
	     */
	    XConvertSelection(xevent->xany.display, dnd.XdndSelection, dnd.Xdnd_text_uri_list, dnd.Xdnd_NON_PROTOCOL_ATOM, xevent->xany.window,	/* xevent->xclient.data.l[2] */
			      CurrentTime);
...
xdndPushFile(meFrame * frame, int x, int y, char *filename)
{
     ...
     */
    if ((dadp =
	 (struct s_DragAndDrop *) meMalloc(sizeof(struct s_DragAndDrop)
					   + len)) == NULL)
	return;
	...
	char           *p,
	               *q;	/* Working loop pointers */

	/*
	 * This is a local file URL. Remove the special URL characters of
	 * %xx where x is a hexadecimal character. 
	 */
	p = &filename[7];
	q = (char *) &dadp->fname[0];
	while ((*q = *p++) != '\0') {
...
xdndSelectionNotify(XEvent * xevent, meFrame * frame)
{
	...
	if ((xevent->xselection.property == dnd.Xdnd_NON_PROTOCOL_ATOM) &&
		...
		/*
		 * Build the buffer by concatinating the data extracted
		 * from the call into a linear buffer space. 
		 */
		if ((count > 0) && (format == 8)) {
		    int             oread = read;
		    char           *p;
		...
		/*
		 * XDND provides us with an absolute screen coordinate,
		 * this needs to be changed relative to the window. 
		 */
		x = dnd.drop_x;
		y = dnd.drop_y;
...
meSetIconState(Display * display, Window window)
{
    ...
    XpmCreatePixmapFromData(display,
			    window,
			    iconBytes, &iconPixmap, &iconMask, NULL);
...
#define iconHeight 48
    ...
    iconPixmap = XCreatePixmapFromBitmapData(display, window,
					     (char *) iconBits, iconWidth,
					     iconHeight,
					     WhitePixel(mecm.xdisplay,
							xscreen),
					     BlackPixel(mecm.xdisplay,
							xscreen), 1);
    wmHints.flags = (InputHint | StateHint);
    wmHints.input = True;
    wmHints.initial_state = NormalState;
