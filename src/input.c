/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com input.c - Various input
 ...
 */
int
mlCharReply(meUByte * prompt, int mask, meUByte * validList,
	    meUByte * helpStr)
{
    int             inpType = 0,
	cc;
	...
	*lp = NULL;
	...
	if ((clexec == meFALSE) || inpType) {
		...
		if (frameCur->mlStatus & MLSTATUS_POSOSD) {
		     ...
		     */
		    frameCur->mlStatus = 0;
		    mlwrite(((mask & mlCR_CURSOR_IN_MAIN) ? 0 : MWCURSOR) |
			    MWSPEC, pp);
		    lp = pp;
		    pp = prompt;
		    /*
		     ...
		     */
		    frameCur->mlStatus =
			(mask & mlCR_CURSOR_IN_MAIN) ? MLSTATUS_KEEP
			: (MLSTATUS_KEEP | MLSTATUS_POSML);
		    inpType = 2;
		}
	    }
	    cc = meGetKeyFromUser(meFALSE, 0,
				  meGETKEY_SILENT | meGETKEY_SINGLE);
	    frameCur->mlStatus &=
		~(MLSTATUS_KEEP | MLSTATUS_RESTORE | MLSTATUS_POSML);
		...
		cc = resultStr[0];
		       ...
		       && (buff[2] == 'n')) {
		/*
		 * if @mna (get all input from user) then rewind the
		 * execstr 
		 */
		if (buff[3] == 'a')
		    execstr = ss;
...
mlyesno(meUByte * prompt)
{
    ...
    ret =
	mlCharReply(buf, mlCR_LOWER_CASE, (meUByte *) "yn",
		    (meUByte *) "(Y)es, (N)o, (C-g)Abort ? ");
...
tgetc(void)
{
	...
	do {
	     ...
	     */
	    cc = TTgetc();
	} while (((cc & (ME_SPECIAL | 0x00ff)) >=
		  (ME_SPECIAL | SKEY_mouse_drop_1))
		 && ((cc & (ME_SPECIAL | 0x00ff)) <=
		     (ME_SPECIAL | SKEY_mouse_time_3)));
#else
	cc = TTgetc();
#endif
	/*
	...
	if (kbdlen > meBUF_SIZE_MAX - 5) {
	    ...
	    meUByte         dd;
	    /*
	     * must store 0xaabb as ff,2,aa,bb also must store 0x00ff as
	     * ff,ff & 0x0000 as 0xff01 Also 0x??00 stored as
	     * ff,2,??,ff,01 
	     */
	    if (cc > 0xff) {
...
/*
 * meGetKeyFromUser Get a command from the keyboard. Process all
 * applicable prefix keys 
 */
meUShort
meGetKeyFromUser(int f, int n, int flag)
{
    ...
    /*
     * Get a key or mouse event, if meGetKeyFirst is not < 0 then we
     * already have one to process (i.e. tab out of an OSD entry,
     * pre-loaded a TAB to auto complete on @ml etc). Next check if we are 
     * playing a keyboard macro, otherwise wait for an event from the user 
     */
    if (meGetKeyFirst >= 0) {
...
static int
mlForwardDelete(meUByte * buf, int curPos, int curLen, int delLen,
		int flags)
{
    meUByte        *ss,
                   *dd;
...
mlDisp(meUByte * prompt, meUByte * buf, meUByte * cont, int cpos)
{
    ...
    col = -1;
    len =
	expandexp(-1, buf, meMLDISP_SIZE_MAX - 1, 0, (meUByte *) expbuf,
		  cpos, &col, 0);
    if (col < 0)
	col = len;
    if (cont != NULL) {
    ...
    if (start >= promsiz)
	mlwrite(MWUSEMLCOL | MWCURSOR, (meUByte *) "%s%s",
		(start) ? "$" : "", expbuf + start - promsiz);
    else
	mlwrite(MWUSEMLCOL | MWCURSOR, (meUByte *) "%s%s%s",
		(start) ? "$" : "", prompt + start, expbuf);
		...
		int *fstPos, int *lstPos)
{
	...
	*lstPos = ii - 1;
	...
	/*
	 * try to eat away at the number of possible completions until
	 * there is only one. If successful the return as just one. Can
	 * remove fileIngore type files which are usually . .. and backup 
	 * files (*~) etc. 
	 */
	register int    ff,
	                ll,
...
createVarList(meUByte *** listPtr)
{
    ...
    *listPtr = list;
    ...
    for (vptr = frameCur->bufferCur->varList.head; vptr != NULL;
	 vptr = vptr->next, ii++) {
	if ((list[ii] = meMalloc(meStrlen(vptr->name) + 2)) == NULL)
	    return 0;
	list[ii][0] = ':';
...
mlHandleMouse(meUByte * inpBuf, int inpBufSz, int compOff)
{
    ...
    if (((row = mouse_Y - frameCur->windowCur->frameRow) >= 0)
	&& (row < frameCur->windowCur->depth - 1)
	&& ((col = mouse_X - frameCur->windowCur->frameColumn) >= 0)
	&& (col < frameCur->windowCur->width)) {
	if (col >= frameCur->windowCur->textWidth) {
	    /*
	     * only do scroll bar if on pick and bars are enabled 
	     */
	    if ((inpBuf == NULL)
		&& (frameCur->windowCur->vertScrollBarMode & WMSCROL)) {
		int             ii;
		for (ii = 0; ii <= (WCVSBML - WCVSBSPLIT); ii++)
		    if (mouse_Y <
			(meShort) frameCur->windowCur->
			vertScrollBarPos[ii])
			break;
	    ...
	    while (--row >= 0)
		lp = meLineGetNext(lp);
	    if ((lp->flag & meLINE_NOEOL)
		&& (col >= frameCur->windowCur->markOffset))
		ii = frameCur->windowCur->markOffset;
	    else
		ii = 0;
	    if ((ii == 0) && (lp->flag & meLINE_NOEOL)) {
		jj = 1;
		while ((meLineGetChar(lp, jj) != ' ')
		       || (meLineGetChar(lp, jj + 1) != ' '))
		    jj++;
	    } else
		...
		if (inpBuf != NULL) {
		    ...
		    /*
		     * if we already have this, assume the user has double 
		     * clicked to 'select', so return 2 to exit 
		     */
		    if ((inpBuf[compOff + jj] == '\0') &&
			!meStrncmp(inpBuf + compOff,
				   meLineGetText(lp) + ii, jj))
			return 2;
		    meStrncpy(inpBuf + compOff, meLineGetText(lp) + ii,
			      jj);
		    inpBuf[compOff + jj] = '\0';
		}
		update(meTRUE);
 ...
 */
int
meGetStringFromUser(meUByte * prompt, int option, int defnum,
		    meUByte * buf, int nbuf)
{
#if MEOPT_LOCALBIND
    ...
    /*
     * Blank line to force update Don't do this if in an osd dialog as the 
     * osd cursor position is not calculated until the first call to
     * osdDisp, osdCol is likely to be -1 which will blow ME up 
     */
    if (!(frameCur->mlStatus & MLSTATUS_POSOSD))
	mlerase(0);
	...
	ii = expandexp(-1, defaultStr, meBUF_SIZE_MAX - 5, ii + 11, prom,
		       -1, NULL, 0);
	meStrcpy(prom + ii, "]): ");
    } else {
	defaultStr = NULL;
	...
	case CK_DELFOR:	/* ^D : Delete the character under the
				 * cursor */
	    if ((ipos < ilen) & ii) {
	...
	case CK_INSTAB:
	    ...
	    if (!
		(option &
		 (MLCOMMAND | MLFILE | MLBUFFER | MLVARBL | MLUSER)))
	  ...
	  input_expand:
		 ...
		 * return the path as xxx/ and base as yyy/ but for
		 * completion we want to list yyy/ so move the base for
		 * the case 
		 */
...
#if MEOPT_SOCKET
		if (isFtpLink(fname) &&
		    ((curDirList.path == NULL)
		     || meStrcmp(curDirList.path, fname))) {
		    changed ^= 1;
		    if (!changed) {
			contstr = compFtpComp;
			break;
		...
		if ((gotPos = getFirstLastPos
		     (noStrs, strList, buf + compOff, option, &fstPos,
		      &lstPos)) == 0) {
		    contstr = compNoMch;
		    TTbell();
		    break;
		...
		if (fstPos == lstPos) {
			 ...
			 * 'TAB' completion should be for inside the new
			 * directory, so effectively we have changed the
			 * input point 
			 */
			changed = 1;
			...
			for (ii = fstPos; ii < lstPos; ii++)
				...
				(((option & MLINSENSCASE) == 0) ||
				 ((meUByte)
				  toLower(strList[ii][ilen - compOff]) !=
				  (meUByte) toLower(ch))))
				break;
			if (ii != lstPos)
			    break;
		...
		for (ii = fstPos; ii <= lstPos; ii++)
			...
			goto input_addexpand;
		/*
		 * if a new window is created to display the completion
		 * list then store the information we need to delete it
		...
		if (mlgsOldCwp == NULL) {
		    ...
		    br = NULL;
		if (meWindowPopup
		    (BcompleteN, BFND_CREAT | BFND_CLEAR | WPOP_MKCURR,
		     br) == NULL) {
		...
		do {
...
#endif
			{
			    if (last >= 0) {
				if (((len =
				      meStrlen(strList[last])) < lwidth)
				    && ((int) meStrlen(strList[ii]) <
					lwidth)) {
				    meStrcpy(line, strList[last]);
				    memset(line + len, ' ', lwidth - len);
				    meStrcpy(line + lwidth, strList[ii]);
				    addLineToEob(frameCur->bufferCur,
						 line);
				    frameCur->bufferCur->baseLine->prev->
					flag |= meLINE_NOEOL;
				    last = -1;
				} else {
				    addLineToEob(frameCur->bufferCur,
			    ...
			    } else if ((int) meStrlen(strList[ii]) <
				       lwidth) {
				line[0] = '\0';
				last = ii;
			    } else
		...
		} while ((frameCur->bufferCur->lineCount == 0)
			 && (--jj >= 0));
...
#endif
	    /*
	     * no break - if not multi-line input then backward-line also
	     * cycles history 
	     */
#if MEOPT_WORDPRO
...
#endif
	    /*
	     * no break - if not multi-line input then forward-line also
	     * cycles history 
	     */
#if MEOPT_WORDPRO
	case CK_GOEOP:		/* M-N : Got to next in history list */
	     ...
	     * (numHist-1) the oldest. However if numHist then its ** the
	     * current one (wierd but easiest to implement 
	     */
	    if (!(option & MLNOHIST) && (numHist > 0)) {
		if (onHist > numHist)
		    /*
	...
	case CK_SCLNXT:	/* M-N : Got to next in completion list */
		...
		if (fstPos == lstPos)
		    ...
		    if (++curPos > lstPos)
			curPos = fstPos;
		    meStrncpy(buf + compOff, strList[curPos],
			      nbuf - compOff);
		    buf[nbuf - 1] = '\0';
		    ipos = ilen = meStrlen(buf);
		}
	...
	case CK_TRNCHR:
	    /*
	     * ^T : Transpose the character before the cursor with the one 
	     * under the cursor. 
	     */
	    if (ipos && ipos < ilen) {
		cc = buf[ipos - 1];
	...
	case CK_QUOTE:		/* ^Q - quote the next character */
	    ...
	    while (ii--)
		if (mlInsertChar((meUByte) cc, buf, &ipos, &ilen, nbuf) ==
		    meFALSE)
		    TTbell();
	    changed = 1;
	    break;
	...
	case CK_YANK:		/* ^Y : insert yank buffer */
		...
		while (killp != NULL) {
		    pp = killp->data;
		    while ((cy = *pp++))
			if (mlInsertChar(cy, buf, &ipos, &ilen, nbuf) ==
			    meFALSE) {
			    TTbell();
			    break;
			}
	...
	case CK_INSFLNM:	/* insert file name */
		...
		p = (ii <
		     0) ? frameCur->bufferCur->name : frameCur->bufferCur->
		    fileName;
		if (p != NULL)
		    while (((ch = *p++) != '\0')
			   && mlInsertChar(ch, buf, &ipos, &ilen, nbuf));
		break;
	    }
	...
	case CK_FORWRD:	/* M-F : Move forward to start of next
				 * word. */
	    while ((ipos != ilen) && ii--) {
	...
	case CK_CAPWRD:	/* M-C : Capitalise next word and move
				 * past it. */
	    while (ilen && (ipos != ilen) && ii--) {
		mlForwardToNextWord(buf, ipos, ilen);
		buf[ipos] = toUpper(buf[ipos]);
		for (ipos++; (ipos < ilen) && isAlphaNum(buf[ipos]);
		     ipos++)
	...
	case CK_LOWWRD:	/* M-L : Converts to lower case the whole
				 * of the next word and move past it. */
	    while (ii--) {
		mlForwardToNextWord(buf, ipos, ilen);
		for (; (ipos < ilen) && isAlphaNum(buf[ipos]); ipos++)
		    buf[ipos] = toLower(buf[ipos]);
	...
	case CK_CTOMOUSE:
	     ...
	     * events, needs to handle osd pick and normal completion
	     * lists 
	     */
	    /*
	     * is it a pick event 
...
#if MEOPT_MOUSE
		/*
		 * ignore mouse move events - these are sometimes
		 * generated when executing a command from osd 
		 */
    ...
    if (cont_flag & 0x02) {
	/*
	 * if no string has been given and there's a default then return
	 * this 
	 */
