/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com buffer.c - Buffer
...
{
    /*
     * Only setup the history to a specific buffer if we're told to and
     * we're not running a macro 
     */
    ...
    return meGetString(prompt, opt | MLBUFFERCASE, defH, buf,
		       meBUF_SIZE_MAX);
}
...
addFileHook(int f, int n)
{
	...
	if (((fileHookExt =
	      meRealloc(fileHookExt,
			(fileHookCount + 1) * sizeof(char *))) == NULL)
	    ||
	    ((fileHookFunc =
	      meRealloc(fileHookFunc,
			(fileHookCount + 1) * sizeof(char *))) == NULL)
	    ||
	    ((fileHookArg =
	      meRealloc(fileHookArg,
			(fileHookCount + 1) * sizeof(short))) == NULL)) {
	    fileHookCount = 0;
	    return meFALSE;
	}
	if ((meGetString((meUByte *) "File id", 0, 0, buff, meBUF_SIZE_MAX)
	     <= 0)
	    || ((fileHookExt[fileHookCount] = meStrdup(buff)) == NULL)
	    ||
	    (meGetString
	     ((meUByte *) "Hook func", 0, 0, buff, meBUF_SIZE_MAX) <= 0)
...
/*
 * Check extent. Check the extent string for the filename extent. 
 */
	    ...
	    meIFuncSSI cmpFunc)
{
	...
	while (*sptr == ' ')
	    sptr++;
	extLen = 0;
	while (((cc = sptr[extLen]) != ' ')
	       && ((cc = sptr[extLen]) != '\0'))
	    extLen++;
	if ((extLen <= len) && !cmpFunc(sptr, filename - extLen, extLen))
	    return 1;
	if (cc == '\0')
	    return 0;
	sptr += extLen + 1;
...
assignHooks(meBuffer * bp, meUByte * hooknm)
{
	...
	meUByte         fn[meBUF_SIZE_MAX],
	                buff[meBUF_SIZE_MAX];	/* Temporary buffer */

	buff[0] = 'h';
	buff[1] = 'k';
	meStrcpy(buff + 2, hooknm + 6);
	if (!fileLookup
	    (buff, (meUByte *) ".emf", meFL_CHECKDOT | meFL_USESRCHPATH,
	     fn)) {
		...
		mlwrite(MWABORT | MWWAIT,
			(meUByte *) "Failed to find file [%s]", buff);
	} else {
	    execFile(fn, 0, 1);
...
setBufferContext(meBuffer * bp)
{
...
#if MEOPT_COLOR
    /*
     * First setup the global scheme - this can be missed by buffers
     * loaded with -c 
     */
	...
	!meModeTest(bp->mode, MDRBIN)
	&& ((bp->fileFlag & meBFFLAG_DIR) == 0)
	&& ((ii = fileHookCount) > 0)) {
	meUByte        *pp,
	                cc;
	meLine         *lp,
	 ...
	 */
	for (lp = meLineGetNext(bp->baseLine); lp != bp->baseLine;
	     lp = meLineGetNext(lp)) {
		...
		if (!isSpace(cc)) {
			...
			if (((nn = fileHookArg[ii]) != 0) &&
			    (meRegexComp
			     (&meRegexStrCmp, fileHookExt[ii],
			      (nn < 0) ? meREGEX_ICASE : 0) ==
			     meREGEX_OKAY)) {
			    if (nn < 0)
				nn = -nn;
			    tlp = lp;
			    do {
				if (meRegexMatch
				    (&meRegexStrCmp, meLineGetText(tlp),
				     meLineGetLength(tlp), 0,
				     meLineGetLength(tlp), 0)) {
				    assignHooks(bp, fileHookFunc[ii]);
				    if (bp->fhook >= 0) {
					ii = meRegexStrCmp.group[0].end -
					    meRegexStrCmp.group[0].start;
					if (ii >= meBUF_SIZE_MAX)
					    ii = meBUF_SIZE_MAX - 1;
					meStrncpy(resultStr,
						  meLineGetText(tlp) +
						  meRegexStrCmp.group[0].
						  start, ii);
					resultStr[ii] = '\0';
					ii = 0;
					break;
    ...
    if (bp->fhook >= 0)
	execBufferFunc(bp, bp->fhook, meEBF_ARG_GIVEN,
		       (bp->intFlag & BIFFILE));
}
#endif

int
swbuffer(meWindow * wp, meBuffer * bp)
{				/* make buffer BP current */
	...
	if (bp->intFlag & BIFFILE) {
	    ...
	    /*
	     * if this is also the current buffer then we have an
	     * out-of-date buffer being reloaded. We must be careful of
	     * what hooks we execute and the state stored 
	     */
	    if ((reload = (wp->buffer == bp)) != 0) {
		/*
		 * the buffer was empty, it now probably isn't and the
		 * window is wrong (dotLine is the base, lineNo is 0).
		 * Must correct this before the call to setBufferContext 
		 */
	...
	if ((bp->intFlag & BIFNOHOOK) == 0) {
	    setBufferContext(bp);
	    /*
	     * the buffer's fhook could have done some really whacky
	     * things like delete the buffer. If it has avoid crashing by
	     * checking the buffer is still a buffer! 
	     */
	    tbp = bheadp;
	    while (tbp != bp)
     ...
     */
    if ((wp == frameCur->windowCur) && !reload
	&& (frameCur->bufferCur->ehook >= 0)) {
	execBufferFunc(frameCur->bufferCur, frameCur->bufferCur->ehook, 0,
		       1);
	if (wp->buffer != tbp)
	    return mlwrite(MWABORT | MWWAIT,
			   (meUByte *)
			   "[Error: Buffer ehook has altered windows]");
    }
#endif
	...
	/*
	 * don't do this on a reload as the bclear means the window state
	 * is rubbish! 
	 */
	...
	if (meModeTest(frameCur->bufferCur->mode, MDPIPE)) {
	    /*
	     * if its a piped command we can't rely on the bp markp being
	     * correct, reset 
	     */
	...
	/*
	 * on a reload reset all other windows displaying this buffer to
	 * the tob 
	 */
	...
	if (bufferOutOfDate(bp)) {
	    ...
	    if (((bp->fileFlag & meBFFLAG_DIR)
		 || (mlyesno((meUByte *) "File changed on disk, reload") >
		     0)) && (bclear(bp) > 0)) {
		/*
		 * achieve cheekily by calling itself as the bclear make
		 * it inactive, This ensures the re-evaluation of the
		 * hooks etc. 
		 */
		/*
		 * SWP 7/7/98 - If the file did not exist to start with,
		 * i.e. new file, and now it does then we need to set the
		 * BIFFILE flag 
		 */
		bp->intFlag |= BIFFILE;
...
#if MEOPT_FRAME
	meFrame        *fc = frameCur;
	for (frameCur = frameList; frameCur != NULL;
	     frameCur = frameCur->next) {
#endif
	    for (wp = frameCur->windowList; wp != (meWindow *) NULL;
		 wp = wp->next) {
		if (wp->buffer == bp) {
			...
			if ((bp1 = replacebuffer(bp)) == bp) {
			     ...
			     * ensure the buffer hooks are correctly setup 
			     * and executed 
			     */
#if MEOPT_FILEHOOK
			    setBufferContext(bp);
...
{				/* switch to the next buffer in the buffer 
				 * list */
    meBuffer       *bp,
                   *pbp;
...
replacebuffer(meBuffer * oldbuf)
{
	...
	*next = NULL;
	...
	if ((bp != oldbuf) && !meModeTest(bp->mode, MDHIDE)) {
		...
		if (wc) {
		    /*
		     * check that the buffer is visible on the current
		     * frame 
		     */
		    meWindow       *wp;
		    for (wc = 0, wp = frameCur->windowList; wp != NULL;
			 wp = wp->next) {
			if (wp->buffer == bp) {
			    wc = 1;
			    break;
 ...
 */
/*
 * Note for Dynamic file names bclear does not touch the file name. 
 */
int
bclear(register meBuffer * bp)
{
    meMode          bmode;
    register meAnchor *mk,
                   *nmk;
    register meLine *lp;
    register int    s;
	...
	/*
	 * while waiting for a Yes from the user, the process may have
	 * finished, so be careful 
	 */
	ipipe = ipipes;
	while (ipipe != NULL) {
	    if (ipipe->bp == bp) {
...
#endif
    if (bufferNeedSaving(bp) && !(bp->intFlag & BIFBLOW)) {	/* Something 
								 * changed 
								 */
     ...
     * buffer (frameCur->windowCur) then the end hook needs to be executed 
     * before the local variables are blown away. It would seem sensible
     * that it is done here. Execute the end hook and then kill off the
     * end hook to ensure that it is not executed again. On a re-read of a 
     * file the file hooks are re-assigned so this should not cause a
     * problem - I hope !!. Note that this problem has been introduced by 
     * local variables it is not a problem if global variables are used.
     * zotbuf() calls bclear; hence any operation to delete a buffer will
     * cause the end hook to be executed. 
     */
     ...
     */
    /*
     * The following modes to preserve are: MDBINARY, MDRBIN, MDCRYPT,
     * MDDEL 
     */
...
{
    register meBuffer *sb,
                   *lb;		/* buffer to insert after */
...
/*
 * Note for Dynamic file names zotbuf frees the file name so it must be
 * dynamic and unique to buffer or NULL. 
 */
int
zotbuf(register meBuffer * bp, int silent)
...
bufferDelete(int f, int n)
{
    ...
    if ((s =
	 getBufferName((meUByte *) "Delete buffer", MLNOSTORE, 1,
		       bufn)) <= 0)
	return (s);
	...
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[%s: no such buffer]", bufn);
    if (bp->intFlag & BIFNODEL)
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[%s cannot be deleted]", bufn);
...
deleteSomeBuffers(int f, int n)
{
	...
	if (meModeTest(bp->mode, MDNACT)) {
		...
		if ((s =
		     mlCharReply((meUByte *)
				 "Delete buffers not yet active (?/y/n/a) ? ",
				 mlCR_LOWER_CASE, (meUByte *) "yna",
				 (meUByte *)
				 "(Y)es, (N)o, Yes to (a)ll, (C-g)Abort ? "))
		    < 0)
...
int
changeBufName(int f, int n)
{				/* Rename the current buffer */
    meUByte         bufn[meBUF_SIZE_MAX],
                   *nn;		/* buffer to hold buffer name */
    meBuffer       *bp1,
                   *bp2;
    int             s;
     ...
     */
    if ((s =
	 getBufferName((meUByte *) "New buffer name", 0, 0, bufn)) <= 0)
	return s;
...
#endif
	if ((bp->name[0] != '*') && ((cp2 = bp->fileName) != NULL)) {
...
bfind(register meUByte * bname, int cflag)
{
	...
	while (bp != NULL) {
		...
		if (cflag & BFND_CLEAR) {
		    ...
		    bclear(bp);
		    /*
		     * The BFND_CLEAR functionality is only used for
		     * system buffers such as *help* *command* etc. i.e.
		     * not user loaded buffer. These buffers do not and
		     * must not be linked to a filename. This can happen
		     * if the user renames "./foo" to *compile* say and
		     * then runs another compile, it its still linked to
		     * foo then it gets loaded 
		     */
		    bp->intFlag &= ~BIFFILE;
		    if (bp->fileName != NULL) {
			meFree(bp->fileName);
...
int
adjustMode(meBuffer * bp, int nn)
{				/* change the editor mode status */
    meMode          mode;
    int             func;
    meUByte         prompt[50];	/* string to prompt user with */
    meUByte         cbuf[meSBUF_SIZE_MAX];	/* buffer to recieve mode
						 * name into */
	 ...
	 */
	if (meGetString(prompt, MLLOWER | MLUSER, 0, cbuf, meSBUF_SIZE_MAX)
	    == meABORT)
	    return meFALSE;
...
{				/* prompt and set an editor mode */
    ...
    if ((s =
	 getBufferName((meUByte *) "Buffer name", MLNOSTORE, 1,
		       bufn)) <= 0)
	return s;
