/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com undo.c - Undo & Redo
 * support routines. Copyright (C) 1996-2001 Steven Phillips Copyright
...
meUndoAddInsChar(void)
{
	...
	if (((nn = frameCur->bufferCur->undoHead) != NULL)
	    && (nn->type == type)
	    && (nn->udata.dotp == frameCur->windowCur->dotLineNo)
...
meUndoAddDelChar(void)
{
	...
	if ((cc =
	     frameCur->windowCur->dotLine->text[frameCur->windowCur->
						dotOffset]) == '\0') {
	    if ((frameCur->windowCur->dotLineNo ==
		 frameCur->bufferCur->lineCount - 1)
		&& (frameCur->windowCur->dotOffset))
		/*
		 * This is trying to just remove the end line when the
		 * line before is not empty, this will fail so don't store 
		 * it 
		 */
		return;
	    cc = meCHAR_NL;
	...
	if ((nn != NULL) && ((nn->type & ~meUNDO_FORWARD) == type) &&
	    (nn->udata.dotp == frameCur->windowCur->dotLineNo)) {
	    if (!(nn->type & meUNDO_FORWARD)
		&& (nn->doto == frameCur->windowCur->dotOffset));
	    else if (((nn->doto - 1) == frameCur->windowCur->dotOffset)
		     && (((nn->type & meUNDO_FORWARD) != 0)
			 || (nn->count == 1))) {
		nn->doto--;
		...
		((nn =
		  meRealloc(nn,
			    sizeof(meUndoNode) + nn->count + 18)) == NULL))
		return;
	    frameCur->bufferCur->undoHead = nn;
	    nn->str[nn->count++] = cc;
	    nn->str[nn->count] = '\0';
...
meUndoAddRepChar(void)
{
    if (meModeTest(frameCur->bufferCur->mode, MDUNDO)) {
	meUByte         type =
	    meUNDO_SINGLE | meUNDO_INSERT | meUNDO_DELETE | meUNDO_FORWARD;
	meUndoNode     *nn;
	meUByte         cc;
	...
	if ((nn != NULL) && (nn->type == type) &&
	    ...
	    nn->str[nn->count] = '\0';
	    frameCur->bufferCur->undoContFlag = undoContFlag;
	} else if ((nn = meUndoCreateNode(sizeof(meUndoNode) + 18)) !=
		   NULL) {
...
meUndoAddDelChars(meInt numChars)
{
	...
	int             len;
	meUByte        *dd = nn->str,
	    *ss = ll->text + frameCur->windowCur->dotOffset;

	nn->type |= meUNDO_DELETE;
	nn->count = 0;
	if ((len =
	     meLineGetLength(ll) - frameCur->windowCur->dotOffset) <
	    numChars) {
	    for (;;) {
		numChars -= len + 1;
		for (; len; len--)
		 ...
		 */
		if ((numChars == 0)
		    && (ll == frameCur->bufferCur->baseLine))
		    break;
		*dd++ = meCHAR_NL;
...
{
    if (meModeTest(frameCur->bufferCur->mode, MDUNDO)) {
	frameCur->bufferCur->undoHead->udata.dotp =
	    frameCur->windowCur->dotLineNo;
	frameCur->bufferCur->undoHead->doto =
	    frameCur->windowCur->dotOffset;
...
meUndoAddReplace(meUByte * dstr, meInt count)
{
	...
	if ((nn == NULL) || !meUndoIsReplace(nn) ||
	    ...
	    meUByte        *dd;
	    if ((nn =
		 meUndoCreateNode(sizeof(meUndoNode) + meStrlen(dstr))) ==
		NULL)
		return;
	...
	if (!(nn->doto & 0x0f) &&
	    ((nn->udata.pos =
	      meRealloc(nn->udata.pos,
			sizeof(meUndoCoord) * (nn->doto + 16))) == NULL))
	    return;
	co = nn->udata.pos;
...
#if MEOPT_NARROW
/*
 * NOTE: Narrow undos are only added if there is already an undo. This is
 * so the state of the buffer will be correct for the previous undo 
 */
void
meUndoAddNarrow(meInt sln, meInt name, meInt markupCmd, meLine * firstLine)
{
    meUndoNarrow   *nn;
    int             ll;
	...
	((nn =
	  (meUndoNarrow *) meUndoCreateNode(sizeof(meUndoNarrow) + ll)) !=
	 NULL)) {
...
meUndo(int f, int n)
{
	...
	if (n == -4) {
		...
		if (meUndoIsLineSort(nn)) {
		    fprintf(undoFp, "Undo 0x%02x %p %ld %ld:", nn->type,
			    nn->next, nn->udata.lineSort[0], nn->count);
		    ...
		    fprintf(undoFp,
			    "Undo 0x%02x %p Nrrw %x %ld %ld %d [%s]\n",
			    nun->type, nun->next, nun->name, nun->count,
			    nun->udata.dotp, nun->markupCmd, nun->str);
		}
#endif
		else {
		    fprintf(undoFp, "Undo 0x%02x %p %ld (%ld,%d) [%s]\n",
			    nn->type, nn->next, nn->count, nn->udata.dotp,
			    nn->doto, nn->str);
	...
	if ((lastflag != meCFUNDO)
	    && ((cun = frameCur->bufferCur->undoHead) != NULL)) {
	    cdoto = cun->doto;
	    ccount = cun->count;
	}
	for (;;) {
		...
		if (meUndoIsSetEdit(cun)) {
			...
			frameAddModeToWindows(WFMODE);	/* update ALL mode 
							 * lines */
		    }
		    ...
		    if ((list =
			 meMalloc(cun->count * sizeof(meLine *))) == NULL)
			return meABORT;
		    undoInfo = meUndoAddLineSort(cun->count);
		    eln = frameCur->windowCur->dotLine;
		    ln = meLineGetPrev(eln);
		    for (count = 0; count < cun->count;
			 eln = meLineGetNext(eln), count++) {
			list[*lineSort++] = eln;
			eln->prev = (meLine *) count;
		    }
		    for (count = 0; count < cun->count;
			 ln = meLineGetNext(ln), count++) {
			if (undoInfo != NULL) {
			...
			meBufferCreateNarrow(frameCur->bufferCur, slp,
					     frameCur->windowCur->dotLine,
					     nun->udata.dotp, ccount, name,
					     nun->scheme,
					     (nun->markupFlag) ? nun->
					     str : NULL, nun->markupCmd,
					     1);
		    }
		}
#endif
		...
		/*
		 * When dealing with long lines this loop becomes
		 * infinitly long because of the number of times that the
		 * line is re-allocated - pre-allocate the line length
		 * first. In order to reduce the processing overhead then
		 * we find the longest strings and then add them back in
		 * in one go, this ensures that we only ever re-allocate
		 * the line once. Jon - 99/12/12. 
		 */
		meUByte        *ss,
		                cc;
		ss = cun->str;
