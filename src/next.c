/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com next.c - Next file/line
 * goto routines. Copyright (C) 1994-2001 Steven Phillips Copyright (C)
...
flNextFind(meUByte * str, meUByte ** curFilePtr, meInt * curLine)
{
    ...
    if (iscanner
	(pat, 1, ISCANNER_PTBEG | ISCANNER_MAGIC | ISCANNER_EXACT,
	 NULL) <= 0)
	return 0;

    /*
     * Found it, is it an ignore line? if so return 2 else fill in the
     * slots and return 1. 
     */
    if (*str == '0')
	return 2;
    ...
    if (fileRpt >= 0) {
	len =
	    mereRegexGroupEnd(fileRpt + 1) - mereRegexGroupStart(fileRpt +
								 1);
	meNullFree(*curFilePtr);
	if ((*curFilePtr = meMalloc(len + 1)) == NULL)
	    return meFALSE;
	meStrncpy(*curFilePtr,
		  frameCur->windowCur->dotLine->text + soff +
		  mereRegexGroupStart(fileRpt + 1), len);
	(*curFilePtr)[len] = '\0';
    } else if (buffRpt >= 0) {
	len =
	    mereRegexGroupEnd(buffRpt + 1) - mereRegexGroupStart(buffRpt +
								 1);
	meNullFree(*curFilePtr);
	if ((*curFilePtr = meMalloc(len + 3)) == NULL)
	    return meFALSE;
	(*curFilePtr)[0] = '*';
	meStrncpy((*curFilePtr) + 1,
		  frameCur->windowCur->dotLine->text + soff +
		  mereRegexGroupStart(fileRpt + 1), len);
	(*curFilePtr)[len + 1] = '*';
	(*curFilePtr)[len + 2] = '\0';
    ...
    if (lineRpt >= 0)
	*curLine =
	    meAtoi(frameCur->windowCur->dotLine->text + soff +
		   mereRegexGroupStart(lineRpt + 1));
	...
	*tbp;
	...
	if ((tbp = bfind(nextName[ii], 0)) != NULL) {
		...
		((bp == NULL) || (bp->windowCount < tbp->windowCount) ||
		 ((bp->windowCount == tbp->windowCount)
		  && (bp->histNo < tbp->histNo)))) {
		bp = tbp;
		no = ii;
	    }
	}
	...
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[No next buffer found]");
    if ((noNextLines = nextLineCnt[no]) == 0)
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[No lines for next buffer %s]",
		       bp->name);
    ...
    while ((frameCur->windowCur->dotLineNo)++,
	   (frameCur->windowCur->dotLine =
	    meLineGetNext(frameCur->windowCur->dotLine)) != bp->baseLine) {
	ii = noNextLines;
	while (--ii >= 0) {
	    frameCur->windowCur->dotOffset = 0;
	    if ((rr =
		 flNextFind(nextLines[ii], &nextFile,
			    &curLine)) == meABORT) {
		frameCur->windowCur->dotOffset = 0;
		...
		if (nextFile != NULL) {
		    ...
		    var =
			SetUsrLclCmdVar((meUByte *) "next-file", value,
					&(bp->varList));
		    meFree(nextFile);
		    if (var == NULL)
			return meFALSE;
		    ...
		    if ((nextFile =
			 getUsrLclCmdVar((meUByte *) "next-file",
					 &(bp->varList))) == errorm)
		    return mlwrite(MWABORT, (meUByte *) "[No File name]");
		else
		    ii = meStrlen(nextFile);
		SetUsrLclCmdVar((meUByte *) "next-line", meItoa(curLine),
				&(bp->varList));
		...
		if ((nextFile[0] == '*') && (nextFile[ii - 1] == '*')) {
		    ...
		    if (findSwapFileList
			(nextFile, (BFND_CREAT | BFND_MKNAM), 0, 0) <= 0)
		    return meFALSE;
		if (curLine <= 0)
		    return mlwrite(MWABORT,
				   (meUByte *) "[No line number]");
		/*
		 * if for some strange reason the file wasn't found, but
		 * the directory was and its read only, the
		 * findSwapFileList will succeed (new buffer) BUT the file 
		 * name will be null! catch this and just use the buffer
		 * name 
		 */
		mlwrite(0, (meUByte *) "File %s, line %d",
			(frameCur->bufferCur->fileName ==
			 NULL) ? frameCur->bufferCur->name : frameCur->
			bufferCur->fileName, curLine);
...
addNextLine(int f, int n)
{
    ...
    if (meGetString((meUByte *) "next name", 0, 0, name, meBUF_SIZE_MAX) <=
	0)
	return meFALSE;
    ...
    line[0] = (n < 0) ? '0' : '1';
    if (meGetString
	((meUByte *) "next line", 0, 0, line + 1, meBUF_SIZE_MAX) <= 0)
	return meFALSE;
    if (no == noNextLine) {
	noNextLine++;
	if (((nextName =
	      meRealloc(nextName, noNextLine * sizeof(meUByte *))) == NULL)
	    || ((nextName[no] = meStrdup(name)) == NULL)
	    ||
	    ((nextLineCnt =
	      meRealloc(nextLineCnt,
			noNextLine * sizeof(meUByte))) == NULL)
	    ||
	    ((nextLineStr =
	      meRealloc(nextLineStr,
			noNextLine * sizeof(meUByte **))) == NULL)) {
	    noNextLine = 0;
	    return meFALSE;
	}
    ...
    if (((nextLineStr[no] =
	  meRealloc(nextLineStr[no], (cnt + 1) * sizeof(char *))) == NULL)
...
doRcsCommand(meUByte * fname, register meUByte * comStr)
{
	...
	if ((cc == '%') &&
		...
		if (meGetString
		    ((meUByte *) "Enter message", 0, 0, pat + ii,
		     meBUF_SIZE_MAX - ii) <= 0)
		    return meFALSE;
		ii = meStrlen(pat);
	    }
...
rcsCiCoFile(int f, int n)
{
    ...
    if (meModeTest(frameCur->bufferCur->mode, MDVIEW)) {	/* if
								 * read-only 
								 * then
								 * unlock */
	if (n < 0)
...
#endif
#ifdef _WIN32
	    frameCur->bufferCur->stats.stmode &= ~FILE_ATTRIBUTE_READONLY;	/* Write 
										 * permission 
										 * for 
										 * owner 
										 */
	...
	if ((str = rcsCoUStr) == NULL)
	    return mlwrite(MWABORT,
			   (meUByte *) "[rcs cou command not set]");
    } else {
	if (n < 0) {
	    /*
	     * unedit changes 
		...
		return mlwrite(MWABORT,
			       (meUByte *)
			       "[rcs file not found - cannot unedit]");
	    str = rcsUeStr;
	} else {
	    if (ss > 0)
		str = rcsCiStr;
	...
	if (str == NULL)
	    return mlwrite(MWABORT,
			   (meUByte *) "[rcs ci or cif command not set]");
    ...
    /*
     * must execute the command and then reload, reload taken from
     * swbuffer 
     */
    if ((doRcsCommand(frameCur->bufferCur->fileName, str) <= 0) ||
	(bclear(frameCur->bufferCur) <= 0) ||
	((frameCur->bufferCur->intFlag |=
	  BIFFILE), (frameCur->bufferCur->dotLineNo =
		     lineno + 1), (swbuffer(frameCur->windowCur,
