/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com registry.c - Internal
 * registry routines. Copyright (c) 1998-2001 Jon Green Copyright (C)
...
rnodeFind(meRegNode * np, meUByte * name)
{
	...
	meUByte         rc,
	               *r = name;	/* Temporary search string */
	meUByte         pc,
	               *p = np->name;	/* Temporary registry string */
	...
	/*
	 * If it's bigger than the currnent node we have gone too far 
	 */
	if (pc > rc)
	    return NULL;
    }
...
parseFile(meRegNode * rnp, meLine * hlp)
{
	...
	case '=':
		...
		mlwrite(MWWAIT,
			(meUByte *)
			"[Registry error: expected <name>=<value> %s:%d]",
			rnp->value, lineNo);
		return;
	    }
	    needValue = 1;
	...
	default:
		...
		mlwrite(MWWAIT,
			(meUByte *)
			"[Registry error: unexpected token \"%s\":%d]",
			buf, lineNo);
		return;
	    }
	    lnp->mode |= meAtoi(buf);
	...
	mlwrite(MWWAIT,
		(meUByte *)
		"[Registry error: mismatched '}' at EOF. %d levels open %s:%d]",
...
regSave(meRegNode * rnp, meUByte * fname, int mode)
{
	...
	if ((fname == NULL) || (fname[0] == '\0')) {
		...
		return mlwrite(MWABORT | MWPAUSE,
			       (meUByte *)
			       "Registry: No file name specified on save");
	}
	...
	buff[len++] = '"';
	len =
	    expandexp(-1, rr->name, 4096 - 11, len, buff, -1, NULL,
	...
	if (rr->mode & (meREGMODE_HIDDEN | meREGMODE_INTERNAL)) {
	    buff[len++] = ' ';
	    buff[len++] =
		'0' + (rr->mode & (meREGMODE_HIDDEN | meREGMODE_INTERNAL));
	}
	if (rr->value != NULL) {
	    buff[len++] = ' ';
	    buff[len++] = '=';
	    ...
	    buff[len++] = '"';
	    len =
		expandexp(-1, rr->value, 4096 - 4, len, buff, -1, NULL,
			  meEXPAND_BACKSLASH | meEXPAND_FFZERO |
			  meEXPAND_PRINTABLE);
	    buff[len++] = '"';
	}
	...
	for (;;) {
		...
		rr = rr->parent;
		/*
		 * as we are assending the tree, at least the first
		 * 'level' number of chars in buffer must be ' 's so just
		 * splat in the '}' 
		 */
		len = level;
		buff[len++] = '}';
		if ((mode & meREGMODE_FROOT) == 0) {
		    buff[len] = '\0';
		    if ((ss =
			 addLine(frameCur->windowCur->dotLine, buff)) <= 0)
			break;
		    lineCount += ss;
		    charCount += len + 1;
		} else if ((ss = ffWriteFileWrite(len, buff, 1)) <= 0)
	...
	if (ss <= 0)
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "[Failed to write registry %s]",
			   fname);
    }
    return meTRUE;
}
...
regSet(meRegNode * sroot, meUByte * subkey, meUByte * value)
{
    ...
    /*
     * Assign the new value !! Note that the root node hodes the filename 
     */
    if (sroot != &root)
...
regTestSave(meRegNode * sroot, int flags)
{
     ...
     * then nothing to do 
     */
    if (!(sroot->mode & meREGMODE_FROOT)
	|| !(sroot->mode & meREGMODE_CHANGE)
	...
	int             ret;
	meStrcpy(prompt,
		 (sroot->value != NULL) ? sroot->value : sroot->name);
...
regRead(meUByte * rname, meUByte * fname, int mode)
{
	...
	/*
	 * if not merging or reloading then we've can use the existing
	 * node 
	 */
	...
	if ((fname != NULL) && (fname[0] != '\0')) {
	    ...
	    if (fileLookup
		(fname, (meUByte *) ".erf",
		 meFL_CHECKDOT | meFL_USESRCHPATH, filename))
	 ...
	 */
	    ...
	    mlwrite(MWABORT | MWWAIT,
		    (meUByte *) "[No file name given to load]");
	    return NULL;
	}
	...
	if ((ffReadFile(fn, flags, NULL, &hlp, 0, 0, 0) == meABORT) &&
	    ...
	    mlwrite(MWABORT | MWWAIT,
		    (meUByte *) "[Cannot load registry file %s]", fname);
	    return NULL;
	}
	lp = &hlp;
    } else
...
regDecodeMode(int *modep, meUByte * modeStr)
{
	...
	for (ii = 0; (cc = modeStr[ii]) != '\0'; ii++) {
		...
		return mlwrite(MWABORT | MWPAUSE,
			       (meUByte *) "Illegal Registry mode [%c]",
			       cc);
...
readRegistry(int f, int n)
{
     ...
     */
    if ((meGetString((meUByte *) "Read registry root", 0, 0, rootbuf, 32)
	 == meABORT) || (((n & 4) == 0)
			 &&
			 (meGetString
			  ((meUByte *) "Registry file", MLFILECASE, 0,
			   filebuf, meBUF_SIZE_MAX) == meABORT))
	|| (meGetString((meUByte *) "File Mode", 0, 0, modebuf, 10) ==
	    meABORT))
	return meABORT;
...
markRegistry(int f, int n)
{
     ...
     */
    if ((meGetString
	 ((meUByte *) "Registry node", 0, 0, rootbuf,
	  meBUF_SIZE_MAX) == meABORT)
	|| (meGetString((meUByte *) "Mark modes", 0, 0, modebuf, 10) ==
	    meABORT))
	return meABORT;
	...
	return mlwrite(MWCLEXEC | MWABORT,
		       (meUByte *) "[Cannot find node %s]", rootbuf);
	...
	if ((rnp = tnp) == NULL)
	    return mlwrite(MWCLEXEC | MWABORT,
			   (meUByte *) "[Cannot locate node %d]", f);
    }
...
saveRegistry(int f, int n)
{
     ...
     * Get the input from the command line 
     */
    if (meGetString((meUByte *) "Save registry root", 0, 0, rootbuf, 128)
	== meABORT)
	return meFALSE;
	 ...
	 */
	if (meGetString
	    ((meUByte *) "Registry file", MLFILECASE | MLFILECASE, 0,
	     filebuf, meBUF_SIZE_MAX) <= 0)
	    return meFALSE;
	mode |= meREGMODE_FROOT;
    }
...
anyChangedRegistry(void)
{
    ...
    while (rnp != NULL) {
	if ((rnp->
	     mode & (meREGMODE_FROOT | meREGMODE_CHANGE |
		     meREGMODE_DISCARD)) ==
	    (meREGMODE_FROOT | meREGMODE_CHANGE))
	    return meTRUE;
	rnp = rnp->next;
    }
    return meFALSE;
...
deleteRegistry(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Registry Path", 0, 0, rootbuf,
	 meBUF_SIZE_MAX) == meABORT)
	return meABORT;
    if ((rnp = regFind(&root, rootbuf)) == NULL)
	return mlwrite(MWCLEXEC | MWABORT,
...
setRegistry(int f, int n)
{
     ...
     */
    if (meGetString
	((meUByte *) "Registry Path", 0, 0, buf1,
	 meBUF_SIZE_MAX) == meABORT)
	return meABORT;
    if (n & 0x02) {
	if (((rnp = regFind(&root, buf1)) == NULL) || (rnp == &root))
	    return mlwrite(MWCLEXEC | MWABORT,
			   (meUByte *) "[Cannot find node %s]", buf1);

	/*
	 * setting the name of the node, not the value 
	 */
	if (meGetString((meUByte *) "name", 0, 0, buf2, meBUF_SIZE_MAX) ==
	    meABORT)
	    return meABORT;
	...
	if (name != NULL) {
	    *name++ = '\0';
		...
		return mlwrite(MWCLEXEC | MWABORT,
			       (meUByte *) "[Cannot find node %s]", buf2);
	    nnp = pnp;
	    do {
		if (nnp == rnp)
		    return mlwrite(MWCLEXEC | MWABORT,
				   (meUByte *)
				   "[Cannot move node to itself or one of its children]");
	...
	if ((pnp != rnp->parent) || meStrcmp(name, rnp->name)) {
		...
		return mlwrite(MWCLEXEC | MWABORT,
			       (meUByte *) "[Cannot set registry node]");
	...
	if (meGetString((meUByte *) "Value", 0, 0, buf2, meBUF_SIZE_MAX) ==
	    meABORT)
	    return meABORT;
...
findRegistry(int f, int n)
{
	...
	resultStr[0] = '\0';
	return mlwrite(MWCLEXEC | MWABORT,
		       (meUByte *) "Cannot find node at index %d", f);
    }
...
listRegistry(int f, int n)
{
    ...
    meUByte        *bn,
                    buf[meBUF_SIZE_MAX * 2];	/* Working line buffer */
	...
	if ((rnp = regFind(rnp, buf)) == NULL)
	    return mlwrite(MWCLEXEC | MWABORT,
			   (meUByte *) "[Cannot find node %s]", buf);
    }
     ...
     */
    if ((wp =
	 meWindowPopup(bn, BFND_CREAT | BFND_CLEAR | WPOP_USESTR,
		       NULL)) == NULL)
	return meFALSE;
	...
	buf[len++] = '"';
	len =
	    expandexp(-1, rnp->name, (meBUF_SIZE_MAX * 2) - 7, len, buf,
		      -1, NULL, meEXPAND_BACKSLASH | meEXPAND_FFZERO);
	buf[len++] = '"';
	...
	if ((rnp->value != NULL) && !(rnp->mode & meREGMODE_INTERNAL)) {
	    ...
	    buf[len++] = '"';
	    len =
		expandexp(-1, rnp->value, (meBUF_SIZE_MAX * 2) - 2, len,
			  buf, -1, NULL,
			  meEXPAND_BACKSLASH | meEXPAND_FFZERO);
	    buf[len++] = '"';
	}
	 ...
	 */
	if ((cnp != NULL)
	    && !(rnp->mode & (meREGMODE_HIDDEN | meREGMODE_INTERNAL))) {
	    vstrbuf[level] = (level
			      && (nnp != NULL)) ? boxChars[BCNS] : ' ';
	    level++;
	    rnp = cnp;
	    continue;
	}
