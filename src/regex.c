/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com regex.c - regex compiler
 * and matcher. Copyright (C) 1999-2001 Steven Phillips Copyright (C)
...
typedef struct meRegex {
    ...
    int             flags;	/* compile + okay flag - set to 0 to force 
				 * recompile */
 ...
 * meREGEXITEM_BEGBUFF must be the first non-closure appending item (i.e.
 * \`* == \`\*) in the list with all items which can have closures before
 * it 
 */
enum {
    meREGEXITEM_NOOP = 0,
...
/*
 * during regex compiling, we know if the nth \( has been encountered but
 * we need to know if the \) has been encounted, this is cause "\1\(x\)"
 * == ERROR "\(x\1\)" == x1 "\(x\)\1" == xx Weird but true - so we use the 
 * matchItem array to store this using the macro below 
 */
...
static int
meRegexGroupMatch(meRegex * regex, unsigned char *string, int len,
		  int offset)
{
    meRegexItem    *ci;
    int             ii,
	...
	while ((ci->matchMax < 0) || (matchNo < ci->matchMax)) {
		...
		int            *iip;
		if ((itp =
		     (meRegexItem **) malloc((matchSz + 128 + 4) *
					     (sizeof(meRegexItem *) +
					      sizeof(meRegexItem *) +
					      sizeof(int) +
					      sizeof(int)))) == NULL)
	    ...
	    /*
	     * if the item is 0 length (\1 could be), only match it
	     * matchMin times 
	     */
	...
	if ((ci = ci->next) == NULL) {
		...
		if (offset > regex->group[0].end) {
		    /*
		     * this is the longest match so far, set the group
		     * offsets - note we make the group offsets relative
		     * to the match start 
		     */
		    int             ss = matchIOff[0];
			...
			ii = regex->groupNo + 1;
			while (--ii > 0)
			    regex->group[ii].start = regex->group[ii].end =
				-1;
			for (ii = 1; ii < matchLen; ii++) {
			    if ((matchItem[ii])->type == meREGEXITEM_GROUP) {
				int             jj =
				    (matchItem[ii])->data.group;
				if ((regex->group[jj].start == -1)
				    || (regex->group[jj].end != -1)) {
				    regex->group[jj].start =
					matchIOff[ii] - ss;
				    regex->group[jj].end = -1;
				} else
				    regex->group[jj].end =
...
/*
 * meRegexMatch returns 1 if matched at current location, else 0 
 */
int
meRegexMatch(meRegex * regex, unsigned char *string, int len,
	     int offsetS, int offsetE, int flags)
{
     ...
     */
    if (!(regex->flags & meREGEX_VALID) || (offsetS < 0)
	|| (offsetE > len))
	return 0;
	...
	do
	    if (((regex->flags & meREGEX_MAYBEEMPTY)
		 || meRegexClassTest(regex->start, string[offset]))
...
meRegexGroupComp(meRegex * regex, meRegexItem ** head,
		 unsigned char **regStr);

/*
 * NOTE - Then next these are not GNU compliant, but they make life
 * easier! 
 */
	       ...
	       unsigned char **regStr, meRegexItem ** retItem)
{
    ...
    *retItem = item;
	...
	case '(':
		...
		if ((ii =
		     meRegexGroupComp(regex, &(item->child),
				      &rs)) != meREGEX_ERROR_CGROUP) {
		    if (ii == meREGEX_OKAY)
	...
	case '9':
	    ...
	    /*
	     * if we're currently compiling this group, this is a literal
	     * char 
	     */
		 ...
		 unsigned char **regStr)
{
    ...
    *head = NULL;
	...
	if ((ii = meRegexItemGet(regex, ci, regStr, &ni)) != meREGEX_OKAY) {
	    /*
	     * can be \| or \) in which case we must create a NOOP is the 
	     * Alt is empty and continue if its a \| 
	     */
...
meRegexItemStartComp(meRegex * regex, meRegexItem * item, int first)
{
	...
	/*
	 * the first is past down to the group to handle, is flag as not
	 * the first 
	 */
	first = 0;
	break;
    ...
    case meREGEXITEM_NWORDCHR:
	/*
	 * the WORDCHR & SPACE classes are not burnt in so flag that we
	 * are using them so if the classes are flagged as changed the
	 * first will be recompiled. 
	 */
	{
	    int             ii = 255,
		jj,
	 ...
	 * they are not chars. meREGEXITEM_BACKREF returns 0 cos to have
	 * got here we must have parsed the Group and if we're still we
	 * should continue 
	 */
	gotStart = 0;
    }
    ...
    if (first && (item->type != meREGEXITEM_BEGBUFF)
	&& (item->type != meREGEXITEM_BEGLINE))
...
meRegexComp(meRegex * regex, unsigned char *regStr, int flags)
{
    ...
    if (matchSz == 0) {
	if ((matchItem =
	     (meRegexItem **) malloc((128 + 4) *
				     (sizeof(meRegexItem *) +
				      sizeof(meRegexItem *) + sizeof(int) +
				      sizeof(int)))) == NULL)
	    return meREGEX_ERROR_MALLOC;
	matchSz = 128;
	...
	|| (regex->regex == NULL)) {
	if ((regex->regex =
	     (unsigned char *) realloc(regex->regex, ii + 1)) == NULL) {
	    regex->regexSz = 0;
	    return meREGEX_ERROR_MALLOC;
	}
	regex->regexSz = ii;
    } else
	if (((regex->flags & (meREGEX_VALID | meREGEX_ICASE)) ==
	     (flags | meREGEX_VALID))
	    && !strcmp((char *) regStr, (char *) regex->regex)) {
	/*
	 * same as last time, check and maybe recompile the first, then
	 * return 
	 */
    ...
    if ((ii =
	 meRegexGroupComp(regex, &(regex->head), &regStr)) != meREGEX_OKAY)
	return ii;
	...
	if ((regex->group =
	     (meRegexGroup *) realloc(regex->group,
				      (regex->groupNo +
				       1) * sizeof(meRegexGroup))) ==
	    NULL) {
	    regex->groupSz = 0;
	    return meREGEX_ERROR_MALLOC;
	}
...
{
    int             ii;
    printf("%*c%s %d %2 d ", indent, ' ', meRegexItemNames[item->type],
...
main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
	printf
	    ("Usage: %s <regex-str> <match-str> [flags] [start-offset,end-offset]\n\n"
	...
	case 1:
	    rs = (unsigned char *)
		"\\(a.*\\|ab.*\\|abc.*\\|abc.*\\)\\(a.*\\|ab.*\\|abc.*\\|abc.*\\)\\1";
	    ss = (unsigned char *)
		"";
	    break;
	default:
	    rs = (unsigned char *)
		"\\(a.*bcde\\|a.*cde\\|a.*de\\|a.*e\\)\\(a.*b\\|a.*c\\|a.*d\\|a.*e\\)\\1";
	...
	printf("regex matched - %d %d\nGroups:",
	       mereRegex.group[0].start, mereRegex.group[0].end);
	for (ii = 0; ii <= mereRegex.groupNo; ii++)
	    printf(" (%d %d)", mereRegex.group[ii].start,
		   mereRegex.group[ii].end);
	printf("\n");
    } else
