/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com eterm.h - Terminal
 * interface definitions. Copyright (C) 1994-2009 JASSPA (www.jasspa.com)
 ...
 * regular signal() however is is easier to just assume all signals are
 * broken and use the safe ones !! We hope that the compiler will optimise
 * structure re-use when we have a series of these, but we are not too
 * worried either way !! 
 */
#ifdef _BSD_42
#define signal(x,y)        \
...
#if MEOPT_COLOR
extern int      TCAPaddColor(meColor index, meUByte r, meUByte g,
			     meUByte b);
...
typedef struct {
    ...
    Font            fontTbl[meFONT_MAX];	/* table of font X ids for 
						 * diff styles */
    meUByte        *fontPart[meFONT_MAX];	/* pointers to parts in
						 * fontname */
 ...
 */
#define colToClient(x)    (mecm.fwidth*(x))	/* Convert column char =>
						 * pixel */
#define rowToClient(y)    ((mecm.fdepth*(y))+mecm.ascent)	/* Convert 
								 * row
								 * char => 
								 * pixel
								 * (for
								 ...
								 * drawing) 
								 */
#define rowToClientTop(y) (mecm.fdepth*(y))	/* Convert row char =>
						 * pixel (top of row) */
#define clientToRow(y)    ((y)/mecm.fdepth)	/* Convert row pixel =>
						 * char */
#define clientToCol(x)    ((x)/mecm.fwidth)	/* Convert column pixel => 
						 * char */
...
extern void     meFrameXTermDraw(meFrame * frame, int srow, int scol,
				 int erow, int ecol);
extern void     meFrameXTermDrawSpecialChar(meFrame * frame, int x, int y,
					    meUByte cc);
...
extern int      XTERMaddColor(meColor index, meUByte r, meUByte g,
			      meUByte b);
 ...
 * so effectively when a timer pops you have to work out which one has if
 * any. Bummer is you can kill them either! Thank you Bill 
 */
#define	_MULTI_NOID_TIMER 1	/* Multi timer but no ids */
#endif
#define	TIMER_MIN    1		/* The minimum timer allowed */
/*
 * executable extension list, in reverse order of dos priority - Must have 
 * the end NULL included 4dos's btm files for completeness 
 */
...
extern void     ConsoleDrawString(meUByte * s, WORD wAttribute, int x,
				  int y, int len);
...
extern int      WinMouseMode(int buttonMask, int highlight,
			     int cursorShape);
extern int      WinLaunchProgram(meUByte * cmd, int flags,
				 meUByte * inFile, meUByte * outFile,
#if MEOPT_IPIPES
				 meIPipe * ipipe,
#endif
				 meInt * sysRet);
