/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com winterm.c - Win32
 * platform support. Copyright (C) 1996-2001 Jon Green Copyright (C)
 * 2002-2009 JASSPA (www.jasspa.com) This program is free software; you
...
/*
 * Shell objects for application directory locations. may not work in
 * versions earler than 6.x 
 */
#ifndef _WIN32s
...
/*
 * For the Win32s then we have to perform a thunking operation in order to 
 * get a synchronous spawn to operate correctly. In addition to the fact
 * we actually have to process using a BAT file because CreateProcess()
 * and WinExec() are both f**ked !! CreateProcess() does not allow
 * re-direction. WinExec() does not allow the parent environment to be
 * inherited, therefore we can never get the current directory correct !!. 
 * To find all of this out you need to look in the MSDM developer database 
 * - even then it is not spelt out. 
 */
#ifdef _WIN32s
#define SYNCHSPAWN 1		/* Our command to spawn */
#define W32SUT_32  1		/* Tell "w32sut.h" that we are 32-bit
				 * complier */
...
#ifdef _ME_WINDOW
/*
 * When we get a message that we can't completely handle we must dispatch
 * the message and let windows handle it. Used a define for easy searching 
 */
...
/*
 * CellMetrics This structure defines the character font items used to
 * address the screen. Thise details the size of the font in screen space
 * and also contains the windows definition of the font. The
 * cellRowPos/cellColPos is a look up table for screen to client
 * coordinates. This is the text position. The LUT's provide a faster and 
 * more convienient method of moving from screen coordinate space to
 * client coordinate space. The 2 pixel offset from the edge of the screen
 * makes testing for the edges a little cumbersome and is best done via the 
 * LUT since it is unconditional and therefore faster - at the expense of
 * 1/2K's worth of memory. 
 */
...
#ifdef _ME_CONSOLE
/*
 * Windows NT Console screen buffer handle.  All console I/O is done
 * through this handle. 
 */
static HANDLE   hInput,
                hOutput;	/* Handles to console I/O */
static char     chConsoleTitle[256];	/* Preserve the title of the
					 * console. */
static DWORD    ConsoleMode,
                OldConsoleMode;	/* Current and old console modes */
static SMALL_RECT consolePaintArea = { 0 };	/* Update area for console 
						 */
static int      ciScreenSize = 0;	/* Size of screen buffer memory */
static CHAR_INFO *ciScreenBuffer = NULL;	/* Copy of screen buffer
						 * memory */
static COORD    OldConsoleSize;
meUInt         *colTable = NULL;	/* Currently allocated colour
					 * table */
#define consoleNumColors 16	/* Number of colours in console window */
static meUByte  consoleColors[consoleNumColors * 3] = {
    0, 0, 0, 0, 0, 200, 0, 200, 0, 0, 200, 200, 200, 0, 0, 200, 0, 200,
	200, 200, 0, 200, 200, 200,
    75, 75, 75, 0, 0, 255, 0, 255, 0, 0, 255, 255, 255, 0, 0, 255, 0, 255,
 ...
 * Function prototypes 
 */
static          HANDLE WinKillToClipboard(void);
#endif
/***************************************************************************/
 ...
 */
/*
 * mouseState A integer interpreted as a bit mask that holds the current
 * state of the mouse interaction. 
 */
...
static HCURSOR  meCursors[meCURSOR_COUNT] =
...
/*
 * Convert the mouse coordinates to cell space. Compute the fractional
 * bits which are 1/128ths. Because we lock the mouse into the window then 
 * cater for the -ve case by ANDing with 0x8000. Added console support 
 */
...
#else				/* MEOPT_MOUSE */
/*
 * if the mouse is disabled we must still do some work or it will look
 * very neglected - do the bare minumum 
 */
...
SynchSpawn(LPCSTR lpszCmdLine, UINT nCmdShow)
{
     ...
     * problems with a win32s 32-bit DLL under Win 3.1 with win32s
     * installed. 
     */
  again:
...
TTopenClientServer(void)
{
	...
	/*
	 * Open the response file for read/write, if this fails we are not 
	 * the server, another ME is 
	 */
...
TTcheckClientServer(void)
{
	...
	return 0;
    /*
     * The handle exists. If the file is non-NULL then there is something
     * to do 
     */
...
TTkillClientServer(void)
{
	...
	/*
	 * if found (should be) delete buffer (this will close the file
	 * handles 
	 */
	if (ipipe != NULL) {
...
TTconnectClientServer(void)
{
	...
	if (meTestExist(fname) || DeleteFile(fname))
	    return 0;
	if ((connectHandle =
	     CreateFile(fname, GENERIC_WRITE, FILE_SHARE_READ, NULL,
	...
	/*
	 * Try opening the response file and get the base window handle
	 * value 
	 */
	mkTempName(fname, meUserName, ".rsp");
	if ((hndl =
	     CreateFile(fname, GENERIC_READ,
...
{
    CHAR_INFO      *pCI;	/* Pointer to current screen buffer
				 * location */
...
meGetConsoleMessage(MSG * msg, int mode)
{
	...
	/*
	 * if ReadConsoleInput fails we have lost the console input and as 
	 * the user is waiting on this bomb out 
	 */
	if (!mode)
	    meDie();
	...
	msg->lParam = 0;
	/*
	 * SWP - 8/5/99 another odd one from bill, the cursor keys on
	 * win9? seem to come through with an AsciiChar value of -32 or
	 * 224 instead of 0... Why? 
	 */
	if ((kr->uChar.AsciiChar == 0)
	    || (((meUByte) kr->uChar.AsciiChar) == 224)) {
	    /*
	     * WM_KEYDOWN of WM_KEYUP 
	     */
	    if (kr->bKeyDown) {
		/*
		 * on win98, for some reason that only bill can explain,
		 * when a cursor key is pressed a shift keydown is
		 * generated and when released, just before a shift keyup
		 * is generated. to stop this, only allow shift keydown
		 * events if the the shift bit is set 
		 */
		if ((kr->wVirtualKeyCode != VK_SHIFT)
		    || (kr->dwControlKeyState & SHIFT_PRESSED))
	...
	if (msg->message != 0) {
	    ...
	    ttmodif = 0;
	    if (kr->
		dwControlKeyState & (LEFT_ALT_PRESSED | RIGHT_ALT_PRESSED))
	    {
		msg->lParam |= 1 << 29;
		ttmodif |= ME_ALT;
	    }
	...
	if (dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED) {
	    ...
	    /*
	     * Haven't got NT5 so cant see it working and as usual the MS
	     * docs are crap so I don't know whether this was a wheel up
	     * or down event 
	     */
	...
	if (msg->message != 0) {
	     ...
	     */
	    msg->lParam =
		((mr->dwMousePosition.Y) << 16) | mr->dwMousePosition.X;

	    return meTRUE;
	}
    } else if (ir.EventType == WINDOW_BUFFER_SIZE_EVENT) {
	/*
	 * in true MS style the console resize stuff is a complete pain!
	 * The user can change the buffer size and we get the change, but
	 * this is no use to us as we want window size changes. Instead we 
	 * have to get the current window size and change the screen size
	 * to that. 
	 */
	...
	/*
	 * this should be the window size, not the buffer size as this
	 * needs the scroll-bar to use 
	 */
	...
	if (ir.Event.FocusEvent.bSetFocus) {
	    ...
	    /*
	     * Kick of the blinker - as default value for cursorBlink is 0 
	     * this will not happen until after the window is created 
	     */
	    ...
	    if (cursorState >= 0) {
		/*
		 * because the cursor is a part of the solid cursor we
		 * must remove the old one first and then redraw 
		 */
		if (blinkState)
...
/*
 * WinSpecialChar; Draw a special character to the screen. x is the
 * lefthand edge of the character, y is the top of the character. The
 * background is assumed to be filled. A foreground pen is expected to be
 * selected. 
 */
static void
WinSpecialChar(HDC hdc, CharMetrics * cm, int x, int y, meUByte cc,
	       COLORREF fcol)
{
	...
	LineTo(hdc, x + cm->sizeX - 1,
	       y + cm->midY + cm->sizeX - cm->midX);
    ...
    case 0x1a:			/* Line Drawing / Bottom right _| with
				 * resize */
	MoveToEx(hdc, x, y + cm->midY, NULL);
	LineTo(hdc, x + cm->midX, y + cm->midY);
	LineTo(hdc, x + cm->midX, y - 1);
...
/*
 * meFrameDrawCursor; Draw the cursor on the screen. We use the
 * information from the frame store to determine what character to render 
 */
void
meFrameDrawCursor(meFrame * frame, HDC hdc)
{
    ...
    style = meSchemeGetStyle(flp->scheme[frame->cursorColumn]);	/* Get
								 * style
								 * under
								 * cursor */

    /*
     * Check the current character set and select the special font or
     * convert the character for the current font. Bring the appropriate
	...
	pen =
	    CreatePen(PS_SOLID, 0,
		      eCellMetrics.pInfo.cPal[meStyleGetFColor(style)].
		      cpixel);
	...
	if (frame->flags & meFRAME_NOT_FOCUS) {
	    ...
	    pen =
		CreatePen(PS_SOLID, 0,
			  eCellMetrics.pInfo.cPal[cursorColor].cpixel);
	    ...
	    LineTo(hdc, rline.top + eCellMetrics.cell.sizeY + 1,
		   rline.left - 1);
	    LineTo(hdc, rline.top + eCellMetrics.cell.sizeY + 1,
		   rline.left + eCellMetrics.cell.sizeX + 1);
	    LineTo(hdc, rline.top - 1,
		   rline.left + eCellMetrics.cell.sizeX + 1);
     ...
     */
    SetTextColor(hdc,
		 eCellMetrics.pInfo.cPal[meStyleGetBColor(style)].cpixel);
     ...
     */
    ExtTextOut(hdc, clientCol,	/* Text start position */
	       clientRow, ETO_OPAQUE | ETO_CLIPPED,	/* Fill background 
							 */
	...
	SetTextColor(hdc,
		     ...
		     cpixel);
	SetBkColor(hdc,
		   eCellMetrics.pInfo.cPal[meStyleGetBColor(style)].
		   cpixel);
	...
	ExtTextOut(hdc, clientCol,	/* Text start position */
		   clientRow, ETO_CLIPPED,	/* Clip char to smaller
						 * rectangle */
		   &rline,	/* Background area */
		   &cc,		/* Text string */
		   1,		/* Length of string */
...
/*
 * WinLoadFont load the specified font variant, i.e. italic/bold etc 
 */
void
WinLoadFont(int font)
{
	...
	ttlogfont.lfUnderline =
	    (font & meFONT_UNDERLINE) ? meTRUE : meFALSE;

	/*
	 * Create the font - use the existing font if it exists 
	 */
	if ((eCellMetrics.fontdef[font] =
	     CreateFontIndirect(&ttlogfont)) == NULL)
...
#define DEBUG_BG 0
    ...
    /*
     * Convert the paint region from client coordinates to screen
     * coordinates. Note that the end row/column is rounded down at the
     * sub-pixel level. 
     */
     ...
     * character canvas, the only special case is the left hand edge when
     * we are running with an offset. 
     */
	...
	drawCursor = ((srow <= frame->cursorRow)
		      && (erow >= frame->cursorRow)
		      && (scol <= frame->cursorColumn)
		      && (ecol > frame->cursorColumn));
    else
	drawCursor = 0;
	...
	/*
	 * As we render right to left then we start with the end character 
	 * - 1 
	 */
	col--;

	ftext = flp->text;	/* Point to appropriate text block */
	fschm = flp->scheme + col;	/* Point to appropriate colour
					 * block */
	...
	for (;;) {
		...
		if (bcol != ff) {
		    bcol = ff;
		    SetBkColor(ps.hdc,
			       eCellMetrics.pInfo.cPal[bcol].cpixel);
		}
#if meFONT_MAX
		ff = (meUByte) meStyleGetFont(style);

		/*
		 * If there is a modification on the font then apply it
		 * now. Note that the following looks a little cumbersome
		 * and unecessary, however the compiler will reduce the
		 * first pair of expressions into a single test so we only 
		 * enter the conditional block when we need to. Both of
		 * the following are applied at the end of the line and
		 * occur infrequently. 
		 */
		if (meSchemeTestNoFont(schm)) {
		    ff &=
			~(meFONT_BOLD | meFONT_ITALIC | meFONT_UNDERLINE);
		}
	    ...
	    /*
	     * how many characters can we draw until we get a color change 
	     * or * reach the end 
	     */
	    tcol = col;
	    spFlag = 0;
	    ...
	    /*
	     * Output the current text item. Set up the current left
	     * margin and determine the length of text that we have to
	     * output. 
	     */
	    length = tcol - col;
	     ...
	     * Output regular text 
	     */
	    ExtTextOut(ps.hdc, eCellMetrics.cellColPos[col],	/* Text
								 * start
								 * position 
								 */
		       clientRow, ETO_OPAQUE,	/* Fill background */
		       &rline,	/* Background area */
		       tbp + col,	/* Text string */
		 ...
		 * because we expect less special characters. We do not
		 * want to create and delete new pens unless we really are 
		 * going to use them. 
		 */
		if (pfcol != fcol) {
		    HPEN            p;
		...
		if (pbcol != bcol) {
			...
			DeleteObject(bbrush);
		    bbrush =
			CreateSolidBrush(eCellMetrics.pInfo.cPal[bcol].
					 cpixel);
		}
		/*
...
WinKillToClipboard(void)
{
    ...
    /*
     * Determine the size of the data in the kill buffer. Make sure that
    ...
    if ((noEmpty =
	 ((meSystemCfg & meSYSTEM_NOEMPTYANK) && (killSize == 0))) != 0)
	killSize++;
	    ...
	    *bufp++ = ' ';
		...
		while ((cc = *dd++)) {
			...
			*bufp++ = '\r';
		     ...
		     */
		    else if ((meSystemCfg & meSYSTEM_FONTFIX)
			     && (cc < TTSPECCHARS))
			cc = ttSpeChars[cc];
		    /*
...
{
    /*
     * We aquire the clipboard and flush it under the following
     * conditions; "We do NOT own it" or "Clipboard is stale". The
     * clipboard becomes stale when we own it but another application has
     * aquired our clipboard data. In this case we need to reset the
     * clipboard so that the application may aquire our next data block
     * that has changed. 
     */
	...
	if (clipState & CLIP_OWNER)
	    /*
	     * if we are currently the owner of the clipboard, the call to 
	     * EmptyClipboard will generate a WM_DESTROYCLIPBOARD to this
	     * window, ignore it! 
	     */
	    clipState |= CLIP_IGNORE_DC;
	EmptyClipboard();
	SetClipboardData(((ttlogfont.lfCharSet ==
			   OEM_CHARSET) ? CF_OEMTEXT : CF_TEXT), NULL);
	CloseClipboard();
	clipState |= CLIP_OWNER;
...
TTgetClipboard(void)
{
    ...
    meUByte        *dd,
                   *tp;		/* Pointers to the data areas */

    /*
     * Check the standard clipboard status, if owner or it has been
     * disabled then there's nothing to do 
     */
     ...
     */
    if ((hmem =
	 GetClipboardData((ttlogfont.lfCharSet ==
			   OEM_CHARSET) ? CF_OEMTEXT : CF_TEXT)) != NULL) {
	int             len,
	                ll;
	meUByte        *tmpbuf;
	...
	/*
	 * Compute the length of the data and construct a stripped down
	 * copy of the string excluding the '\r' characters 
	 */
	...
	*tp = '\0';
	...
	/*
	 * Make sure that it is not the same as the current save buffer
	 * head 
	 */
...
#endif
{
	...
	/*
	 * wait for the main thread to read all available output and flag
	 * for us to start waiting again 
	 */
    } while ((WaitForSingleObject(ipipe->threadContinue, INFINITE) ==
	      WAIT_OBJECT_0) && !(ipipe->flag & meIPIPE_CHILD_EXIT));
 ...
 */
int
WinLaunchProgram(meUByte * cmd, int flags, meUByte * inFile,
		 meUByte * outFile,
#if MEOPT_IPIPES
		 meIPipe * ipipe,
#endif
		 meInt * sysRet)
{
    ...
    meUByte        *cmdLine = NULL,
	*cp;			/* Buffer for the command line */
    meUByte         dummyInFile[meBUF_SIZE_MAX];	/* Dummy input
							 * file */
    meUByte         pipeOutFile[meBUF_SIZE_MAX];	/* Pipe output
							 * file */
    int             status;
#ifdef _WIN32s
...
#ifdef _WIN32s
    /*
     * If the pipe to stderr is not defined then get the value. Note that
     * this is a once off get and we only look on the first run 
     */
    if (pipeStderr == 0)
	...
	if ((outFile == NULL)
	    && ((flags & (LAUNCH_SYSTEM | LAUNCH_FILTER | LAUNCH_IPIPE)) ==
		0)) {
	    /*
	...
	if ((flags & LAUNCH_SYSTEM) == 0) {
...
#else
		HANDLE          h;
		if ((meSuInfo.hStdInput =
		     CreateFile(inFile, GENERIC_READ, FILE_SHARE_READ,
				&sbuts, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
				NULL)) == INVALID_HANDLE_VALUE) {
		    meFree(cmdLine);
		    return meFALSE;
		}
		if ((meSuInfo.hStdOutput =
		     CreateFile(outFile, GENERIC_WRITE, FILE_SHARE_READ,
				&sbuts, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
				NULL)) == INVALID_HANDLE_VALUE) {
		 ...
		 */
		if (CreatePipe(&meSuInfo.hStdInput, &inHdlTmp, &sbuts, 0)
		    == 0) {
		    meFree(cmdLine);
		    return meFALSE;
		}
		if (CreatePipe(&outHdlTmp, &meSuInfo.hStdOutput, &sbuts, 0)
		    == 0) {
		    CloseHandle(meSuInfo.hStdInput);
		...
		// the
		// properties and, as a result, non-closeable handles to
		// the pipes
		// are created.
		...
		/*
		 * Duplicate stdout => stderr, don't really care if this
		 * fails 
		 */
		...
		/*
		 * the output file name will be set as either one was
		 * given or we have created one 
		 */
#ifdef _WIN32s
		if (pipeStderr > 0)
		    strcat(cp, " >& ");
...
#else
		meSuInfo.hStdOutput =
		    CreateFile(outFile, GENERIC_WRITE, FILE_SHARE_WRITE,
			       &sbuts, CREATE_ALWAYS,
		 ...
		 * Otherwise the grep command hangs on an input steam that
		 * does not exist. I added this before which sorted the
		 * problem - the code has since been re-arranged and
		 * omitted. Jon 17/11/97 SWP - 20/7/99 Found that some
		 * launched programs return before sub programs have
		 * finished, this leaves the "stdin.~~~" file locked. This
		 * is not a problem in this case because the file will be
		 * empty and we should still be able to open it, so don't
		 * fail if we fail to create the file, only fail if we fail 
		 * to open it. 
		 */
		/*
		 * Create a dummy input file to stop the process from
		 * locking up. Construct the dummy input file 
		 */
		mkTempName(dummyInFile, DUMMY_STDIN_FILE, NULL);

		if ((dumHdl =
		     CreateFile(dummyInFile, GENERIC_WRITE,
		 ...
		 */
		if ((dumHdl =
		     CreateFile(dummyInFile, GENERIC_READ, FILE_SHARE_READ,
				&sbuts, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
				NULL)) == INVALID_HANDLE_VALUE) {
	    ...
	    /*
	     * For some reason Win98 shell-command start-up path is
	     * incorrect unless a dummy input file is used, no idea why
	     * but doing the following (taken from above) works! 
	     */
	    mkTempName(dummyInFile, DUMMY_STDIN_FILE, NULL);

	    if ((dumHdl =
		 CreateFile(dummyInFile, GENERIC_WRITE, FILE_SHARE_WRITE,
    ...
    if (CreateProcess(NULL,
		      ...
		      NULL,
		      NULL,
		      ((flags & (LAUNCH_SHELL | LAUNCH_NOWAIT)) ? meFALSE :
		       meTRUE),
		      ((flags & LAUNCH_DETACHED) ? DETACHED_PROCESS :
		       CREATE_NEW_CONSOLE), NULL, NULL, &meSuInfo,
		      &mePInfo)) {
	status = meTRUE;
	...
	if ((flags & LAUNCH_IPIPE) == 0) {
	    /*
	     * For shells we must close the process handle but we dont
	     * wait for it 
	     */
		...
		for (;;) {
		    ...
		    procStatus =
			WaitForSingleObject(mePInfo.hProcess, 200);
		     ...
		     */
		    if (sysRet != NULL)
			GetExitCodeProcess(mePInfo.hProcess,
					   (LPDWORD) sysRet);
		    break;
		}
	    }
	...
	if (status == meFALSE) {
	    ...
	    /*
	     * attempt to create a new thread to wait for activity, this
	     * is because windows pipes are crap and doing a Wait on them
	     * fails. so us poor programmers have to jump through lots of
	     * hoops just to make Bils crap usable, and what really hurts
	     * is that after doing so every non-programmer thinks Bills
	     * stuff is wonderful! Sometimes the world sucks. Set the
	     * childActive event to manual so we can do the global MsgWait 
	     * and then after its Set do a SingleWait on each reset those
	     * that are set. 
	     */
	    ipipe->childActive = NULL;
	    ipipe->threadContinue = NULL;
		...
		if (((ipipe->childActive =
		      CreateEvent(NULL, meTRUE, meFALSE, NULL)) != 0)
		    &&
		    ((ipipe->threadContinue =
		      CreateEvent(NULL, meFALSE, meFALSE, NULL)) != 0)
		    &&
		    ((thread =
		      _beginthread(childActiveThread, 0, ipipe)) != -1))
		...
		((ipipe->threadContinue =
		  CreateEvent(NULL, meFALSE, meFALSE, NULL)) != 0))
		ipipe->thread =
		    CreateThread(NULL, 0, childActiveThread, ipipe, 0,
				 &(ipipe->threadId));
	    else
		ipipe->thread = NULL;
#endif
...
meFrameResizeWindow(meFrame * frame)
{
	 ...
	 */
	GetClientRect(meFrameGetWinHandle(frame), &meFrameGetWinCanvas(frame));	/* Get 
										 * the 
										 * new 
										 * canvas 
										 * size 
...
/*
 * WinShutdown; Shutdown the system. It is important that we relinquish
 * some of the resources that we may have aquired during the session. The
 * most important resource to relinquish is the font resource. 
 */
void
WinShutdown(void)
{
...
TTinitMouse(void)
{
	...
	{
	    meUByte         cc =
		(meUByte) ((meMouseCfg & meMOUSE_ICON) >> 16);
...
WinMouse(HWND hwnd, UINT message, UINT wParam, LONG lParam)
{
    ...
    case WM_MOUSEMOVE:
	/*
	 * If a button was pressed/release outside the window we don't see 
	 * it!!! The first chance we get to see it is when the user moves
	 * the mouse back into the window - so check the mouse button
	 * state is the same, if not treat this event as a button event 
	 */
	if (((wParam ^ mouseButs) & (MK_LBUTTON | MK_RBUTTON | MK_MBUTTON))
	    == 0) {
	     ...
	     */
	    /*
	     * Convert the mouse coordinates to cell space. Compute the
	     * fractional bits which are 1/128ths 
	     */
	    mousePosUpdate(lParam);
	    /*
	     * if the mouse is not in the client area and no button is
	     * currently pressed then we don't handle this event, the
	     * default handler should to pass on NC events 
	     */
	    if (!mouseInFrame && !mouseButs)
		return meFALSE;

	    /*
	     * Found that on NT ME continually gets MOUSEMOVE messages
	     * even though the mouse hasn't moved! I wonder if thats why
	     * it runs so slow? To stop the mouse cursor reappearing &
	     * generating continuous mouse_move 'keys' store the last
	     * position and only do something if the mouse really has
	     * moved. 
	     */
	    if (lParam != lastPos) {
		/*
		 ...
		 */
		cc = ME_SPECIAL | ttmodif | (SKEY_mouse_move +
					     mouseKeys[mouseState &
						       MOUSE_STATE_BUTTONS]);
		/*
		 * Are we after all movements or mouse-move bound ?? 
		 */
		if ((!TTallKeys && (decode_key(cc, &arg) != -1))
		    || (TTallKeys & 0x1))
		    addKeyToBufferOnce(cc);	/* Add key to keyboard
						 * buffer */
		mouseShow();
		lastPos = lParam;
	    }
	 ...
	 * was not in fact pressed. As a result of the above we now
	 * consider it dangerous to infer button presses from mouse
	 * movements. Hence we no longer drop through and process the
	 * button state. Simply ignore the button state and ignore the
	 * mouse move !! A click of the mouse is required to correct this
	 * state. I have not been able to re-create the above. The fix has
	 * not yet shown any adverse effects - although I would not expect
	 * it to. 
	 */
	return meFALSE;
    ...
    case WM_RBUTTONUP:
	{
	    ...
	    mousePosUpdate(lParam);
	    /*
	     * if the mouse is not in the client area and no other button
	     * is currently pressed then we don't handle this event, the
	     * default handler should to pass on NC events 
	     */
	    if (!mouseInFrame && !mouseButs)
		return meFALSE;
	    ...
	    /*
	     * fprintf(logfp,"Mouse button %x %08x %08x - %d
		...
		if (((mouseCode ^ mouseState) & ii) != 0) {
			...
			cc = SKEY_mouse_pick_1;	/* Get mouse pick key
						 * binding */
		    else
			cc = SKEY_mouse_drop_1;	/* Get mouse drop key
						 * binding */
		...
		if (mouseState & MOUSE_STATE_BUTTONS) {
			...
			SetCapture(meFrameGetWinHandle(frame));	/* Lock
								 * the
								 * mouse */
    ...
    case WM_MOUSEWHEEL:
	{
	     ...
	     */
	    /*
	     * unlike mouse move or button events the lParam mouse
	     * position is absolute, not the position within the window,
	     * we should therefore subtract the top left point of the
	     * window from the position to get an ME mouse position, but
	     * that can lead to negative numbers which is probably why
	     * windows is inconsistent in this area so let the other mouse 
	     * events set the position mousePosUpdate(lParam) ;
	     */
...
WinKeyboard(HWND hwnd, UINT message, UINT wParam, LONG lParam)
{
	...
	if ((fp = fopen("c:/me.dump", "a")) != NULL) {
	    ...
	    fprintf(fp,
		    "%s::%d(0x%08x). wParam = %d(%04x) lParam = %d(%08x) modif %x\n",
    ...
    case WM_SYSKEYDOWN:
	/*
	 * For some unknown reason, bill has decided that an F10 key press 
	 * will generate a SYSKEYDOWN, cope with this oddity 
	 */
	if (wParam != VK_F10) {
#ifdef _ME_WINDOW
#ifdef _ME_CONSOLE
...
#endif				/* _ME_CONSOLE */
	    {
		if ((lParam & (1 << 29)) == 0) {	/* Windows got no
							 * input focus ? */
		    SetFocus(meFrameGetWinHandle(frame));	/* No -
								 * aquire
								 * focus */
		    return meFALSE;
		}
	...
	case VK_NEXT:
	     ...
	     * Distinguish control-pad/number-pad keys 
	     */
	    cc = ((lParam & 0x01000000) ? SKEY_page_down :
		  SKEY_kp_page_down);
	    goto do_keydown;
	case VK_HOME:
	    /*
	...
	case VK_CLEAR:
	    /*
	     * This comes from the number pad 5 with numlock off. Mapped
	     * to kp-begin (UNIX derivation). 
	     */
	    cc = SKEY_kp_begin;
	    goto do_keydown;
	case VK_APPS:
	...
	case VK_PAUSE:
	    ...
	    /*
	     * the following lock keys must be bound to generate a key
	     * event 
	     */
	case VK_CAPITAL:
	    cc = SKEY_caps_lock;
	...
	case VK_SCROLL:
	  ...
	  test_do_keydown:
	    {
		meUInt          arg;
		if (decode_key((meUShort) (ME_SPECIAL | ttmodif | cc), &arg) != -1)	/* Key 
											 * bound 
											 * ?? 
											 */
		    goto do_keydown;
	...
	case VK_RWIN:
	     ...
	     * specially in that if the key is bound then we process it,
	     * if it is not bound then we pretend that we have not
	     * processed it, this will allow the default handler to
	     * process the key 
	     */
	    {
		meUInt          arg;	/* Decode key argument */

		cc = ME_SPECIAL | ttmodif | ((wParam == VK_LWIN) ?
					     SKEY_start_left :
					     SKEY_start_right);
		if (decode_key(cc, &arg) != -1)	/* Key bound ?? */
		    goto do_addbuf;	/* Yes - allow key to be
					 * pr0ocessed */
	...
	case VK_F12:
	  ...
	  do_keydown:
	    ...
	    /*
	     * Add the character to the typeahead buffer. Note that we do
	     * not process (lParam & 0xff) which is the auto-repeat count
	     * - this always appears to be 1. 
	     */
#ifdef _WIN_KEY_DEBUGGING
	    {
		FILE           *fp = NULL;
	    ...
	    addKeyToBuffer(cc);
	    /*
	     * hide the mouse cursor This must be done whether MEOPT_MOUSE 
	     * is enabled or not 
	     */
	    mouseHide();
	    break;
	...
	default:
		...
		return (meFALSE);	/* NOT PROCESSED - return a false
					 * state */
	    /*
	     * found that C-1, C-2 etc come down this route 
	     */
	    if ((ttmodif & ME_CONTROL) && (wParam >= '0')
		&& (wParam <= '9')) {
		static meUByte  shiftDigits[] = ")!\"#$%^&*(";

		cc = (ttmodif & (ME_CONTROL | ME_ALT)) |
		    ((ttmodif & ME_SHIFT) ? shiftDigits[wParam - '0'] :
		     wParam);
	    } else if ((lParam & (1 << 29)) == 0) {	/* Get out quickly 
							 * - this is
							 * fastest check */
		/*
		 * Jon 99/01/27: Another hole we are missing <Ctrl-@>, *
		 * <Ctrl-'>, etc. and they appear to be coming in here so
		 * plug * it up !! I'm not sure if there is any rational
		 * to the way * Microsoft have mapped their character set
		 * or are we doing * something terribly wrong ?? * * Fail
		 * on most keys 
		 */
		...
		/*
		 * The only keys we want to process here are those that do 
		 * not come back to use as WM_CHAR. Fail on all of the
		 * others - we will see them later as a different message
		 * type 
		 */
		if (wParam & 0x80) {
		    wParam &= 0x7f;
		...
		cc = ttmodif | SKEY_delete | ME_SPECIAL	/* |
							 * SKEY_kp_delete */ ;
	    else if (wParam == VK_INSERT)
		cc = ttmodif | ME_SPECIAL | SKEY_insert	/* |
							 * SKEY_kp_insert */ ;
...
#ifdef _WIN32s
		    /*
		     * Special case for Alt-Tab - this is not for us !!
		     * Windows does not intercept this key on win32s. 
		     */
		    if ((wParam == '\t')
			&& ((ttmodif & (ME_CONTROL | ME_SHIFT)) == 0))
	    ...
	    /*
	     * Add the character to the typeahead buffer. Note that we do
	     * no process (lParam & 0xff) which is the auto-repeat count.
	     * - this always appears to be 1 
	     */
#ifdef _WIN_KEY_DEBUGGING
	    {
	...
	case VK_RETURN:
	    ...
	    {
		/*
		 * For the console check the control flag. A control flag
		 * will indicate that the control C-m has been pressed as
		 * opposed to <RETURN> 
		 */
	     ...
	     * <RETURN> is 0x1c. If this is a C-m then simply drop
	     * through, wParam should be 0x1d which is C-m. 
	     */
...
#if 0
		/*
		 * Jon:991129; Moved to case above. More of the keys need
		 * to be migrated like this. 
		 */
		if (cc == 0x0d) {
		    /*
		     * Distinguish between the Number Pad and standard
		     * enter 
		     */
		    cc = ((lParam & 0x01000000) ? SKEY_kp_enter :
			  SKEY_return);
		    goto return_spec;
		}
#endif
	...
	/*
	 * Add the character to the typeahead buffer. * Note that we do no 
	 * process (lParam & 0xff) which is the * auto-repeat count. -
	 * this always appears to be 1 
	 */
#ifdef _WIN_KEY_DEBUGGING
	{
	...
	addKeyToBuffer(cc);
	/*
	 * hide the mouse cursor This must be done whether MEOPT_MOUSE is
	 * enabled or not 
	 */
	mouseHide();
	break;
...
TTaddColor(meColor index, meUByte r, meUByte g, meUByte b)
{
	...
	if (GetDeviceCaps(hDC, RASTERCAPS) & RC_PALETTE) {
	    int             maxPaletteSize;	/* Maximum size of the
						 * palette */
	     ...
	     */
	    if ((index < noColors)
		&& (eCellMetrics.pInfo.cPal[index].rgb != 0)) {
		closeIndex =
		    GetNearestPaletteIndex(eCellMetrics.pInfo.hPal,
					   eCellMetrics.pInfo.cPal[index].
					   rgb);

		/*
		 * Reference count has reached zero. Remove the palette
		 * entry 
		 */
		if (--eCellMetrics.pInfo.hPalRefCount[closeIndex] <= 0) {
			...
			if (closeIndex < eCellMetrics.pInfo.hPalSize) {
			    /*
			     * Reduce the size of the palette by
			     * re-shuffling 
			     */
			    ...
			    /*
			     * Move the reference count entry - do not
			     * bother to resize the reference count table 
			     */
			    eCellMetrics.pInfo.hPalRefCount[closeIndex] =
				eCellMetrics.pInfo.
				hPalRefCount[eCellMetrics.pInfo.hPalSize];
			}
			/*
			 * Resize the palette 
			 */
			ResizePalette(eCellMetrics.pInfo.hPal,
				      eCellMetrics.pInfo.hPalSize);
		    }
		}
	    }
	    ...
	    /*
	     * Create a new entry for the colour * Check the current
	     * palette for the colour. If we have it already we do * not
	     * need to create a new palette entry. 
	     */
	    if (eCellMetrics.pInfo.hPalSize > 0) {
		closeIndex =
		    GetNearestPaletteIndex(eCellMetrics.pInfo.hPal, cReq);
		if (closeIndex != CLR_INVALID) {
		    GetPaletteEntries(eCellMetrics.pInfo.hPal, closeIndex,
				      1, &closeEntry);
		    closeColor =
			RGB(closeEntry.peRed, closeEntry.peGreen,
			    closeEntry.peBlue);
		} else
		...
		if (cReq == closeColor) {
			 ...
			 */
			eCellMetrics.pInfo.hPalRefCount =
			    (int *) meRealloc(eCellMetrics.pInfo.
					      hPalRefCount,
					      (sizeof(int) *
					       eCellMetrics.pInfo.
					       hPalSize));
		...
		lPal->palVersion = 0x300;	/* Windows magic number !! 
						 */
		lPal->palNumEntries = 1;	/* 2 entries in the
						 * palette */
		...
		HeapFree(GetProcessHeap(), 0, lPal);	/* Dispose of the
							 * local palette */
	    ...
	    /*
	     * Determine what colour will actually be used on non-colour
	     * mapped systems 
	     */
	...
	if (index >= noColors) {
	    ...
	    eCellMetrics.pInfo.cPal =
		meRealloc((void *) eCellMetrics.pInfo.cPal,
	     ...
	     int fontHeight, int fontWidth)
{
	...
	if (fontType <= -2) {
	    ...
	    /*
	     * This stupid Microsoft system, why is the CHOOSEFONT
	     * structure size not sizeof (CHOOSEFONT). Typical Microsoft
	     * !! 
	     */
	    if (ChooseFont(&chooseFont) == 0)
		/*
		 * SWP - if the user cancelled just return false leaving
		 * the font alone 
		 */
		return meFALSE;
	    /*
	     ...
	     */
	    sprintf(resultStr, "%1d%4d%4d%4d%s",
		    (int) (logfont.lfWeight / 100), (int) logfont.lfWidth,
		    (int) logfont.lfHeight, logfont.lfCharSet,
		    logfont.lfFaceName);
	    if (fontType < -2)
		/*
		 * if a -ve argument was past to changeFont then don't set 
		 * the font 
		 */
		return meTRUE;
	    ...
	    strncpy(logfont.lfFaceName, fontName,
  ...
  defaultFont:
    if (newFont == NULL) {
	newFont =
	    GetStockObject((fontType ==
			    OEM_CHARSET) ? OEM_FIXED_FONT :
			   ANSI_FIXED_FONT);
	logfont.lfCharSet = fontType;
    }
...
#else
    eCellMetrics.cell.sizeX = ((textmetric.tmMaxCharWidth <= 0) ? 1 : textmetric.tmMaxCharWidth	/* + 
												 * textmetric.tmExternalLeading */ );
#endif
    eCellMetrics.cell.sizeY =
	(textmetric.tmHeight <= 0) ? 1 : textmetric.tmHeight;
    eCellMetrics.cell.midX = eCellMetrics.cell.sizeX / 2;
    eCellMetrics.cell.midY = eCellMetrics.cell.sizeY / 2;

    /*
     * Store logfont into the ttlogfont for font style and language char
     * set changes 
     */
...
/*
 * changeFont Change the size of the font. change-font <name> <charSet>
 * <weight> <width> <height> <font> - The name of the font. Max of 32
 * characters.  Select Fixed mono fonts only. Proportional fonts may be
 * specified but the cursor will not align with the characters on the
 * screen.  "default" may be specified and the sytem default OEM font. No 
 * other arguments are required when specified.  Note that "Courier New"
 * is not actually a fixed mono font as might be expected. <charSet> -
 * The type of character set required.  0 = OEM (or bitmapped) 1 = ANSI
 * (True Type etc). <weight> - The weight of the font. The values are
 * defined as:- 0 = Don't care (Automatically selected).  1 = Thin 2 =
 * Extra Light 3 = Light 4 = Normal 5 = Medium 6 = Semi-Bold 7 = Bold 8 =
 * Extra-Bold 9 = Heavy Note that you may request a weight and it is not
 * honoured.  Typically 4 and 7 are honoured by most font definitions.  4
 * is typically used. <width> - The width of the font.  Specifies the
 * average width, in logical units, of characters in the requested font.
 * If this value is zero, the font mapper chooses a "closest match" value. 
 * The "closest match" value is determined by comparing the absolute
 * values of the difference between the current device's aspect ratio and
 * the digitized aspect ratio of available fonts.  Note that if the width 
 * is specified as zero then the height should be specified and the width
 * will be automatically selected. <height> - The height of the font.
 * Specifies the desired height, in logical units, of the requested font's 
 * character cell or character. (The character height value is the
 * character cell height value minus the internal-leading value.) If this
 * value is greater than zero, the font mapper matches it against
 * available character cell height values; if this value is zero, the font 
 * mapper uses a default height value when it searches for a match; if
 * this value is less than zero, the font mapper matches it against
 * available character height values.  Note: as with the weight the width 
 * and height may not be honoured if the font cannot support the specified 
 * width/height in which case the closest matching height is automatically 
 * selected 
 */

#define FONTBUFSIZ 33		/* Size of buffer is 33 Max font name is
				 * 32 chars */
int
changeFont(int f, int n)
{
	...
	/*
	 * Get the name of the font. If it is specified as default then do 
	 * not collect the remaining arguments 
	 */
	if (meGetString
	    ("Font Name ['' for default]", 0, 0, fontName,
	     FONTBUFSIZ) == meABORT)
	    return (meFALSE);
	if (fontName[0] == '\0')
	    status = TTchangeFont(NULL, -1, 0, 0, 0);
	else if ((meGetString("Font Type [ANSI=0]", 0, 0, buff, FONTBUFSIZ)
		  > 0)
		 && ((fontType = meAtoi(buff)),
		     (meGetString
		      ("Font Weight [1-9; 0=don't care]", 0, 0, buff,
		       FONTBUFSIZ) > 0))
		 && ((fontWeight = meAtoi(buff)),
		     (meGetString("Font Width", 0, 0, buff, FONTBUFSIZ) >
		      0))
		 && ((fontWidth = meAtoi(buff)),
		     (meGetString("Font Height", 0, 0, buff, FONTBUFSIZ) >
		      0))) {
...
{
    if ((frame->cursorRow <= frame->depth)
	&& (frame->cursorColumn < frame->width)) {
#ifdef _ME_CONSOLE
#ifdef _ME_WINDOW
	if (meSystemCfg & meSYSTEM_CONSOLE)
#endif				/* _ME_WINDOW */
	{
	    ...
	    flp = frame->store + frame->cursorRow;
	    cc = flp->text[frame->cursorColumn];	/* Get char under
							 * cursor */
	    schm = flp->scheme[frame->cursorColumn];	/* Get colour
							 * under cursor */

	    dcol = (WORD) TTschemeSet(schm);
	    ConsoleDrawString(&cc, dcol, frame->cursorColumn,
			      frame->cursorRow, 1);
	}
#ifdef _ME_WINDOW
...
#if meFONT_MAX
	    /*
	     * If the cursor preceeeds an italic character then we need to 
	     * fix the previous character. This is done by ME drawing the
	     * previous char but not invalidating the area so that only
	     * the overhang into the next char is drawn. 
	     */
	    meFrameLine    *flp;	/* Frame store line pointer */

	    flp = frame->store + frame->cursorRow;
	    if (startCol > 0) {
		if ((meSchemeGetStyle(flp->scheme[startCol - 1]) &
		     meSTYLE_ITALIC) && (flp->text[startCol - 1] != ' ')) {
		    /*
...
#endif
	    if (meFrameGetWinPaintStartCol(frame)[frame->cursorRow] >
		startCol)
		meFrameGetWinPaintStartCol(frame)[frame->cursorRow] =
		    startCol;
	    if (meFrameGetWinPaintEndCol(frame)[frame->cursorRow] <=
		frame->cursorColumn)
		meFrameGetWinPaintEndCol(frame)[frame->cursorRow] =
		    frame->cursorColumn + 1;
	    /*
	     * Set up the area on the client window to be modified and
	     * signal that the client is about to be updated 
	     */
	    /*
	     * SWP 20040108 - XP ClearType smooth edge font rendering
	     * seems to spill the bg out onto the next character by one
	     * pixel so to avoid dropping vertical lines add 1 to the
	     * right edge 
	     */
	    rect.left = eCellMetrics.cellColPos[frame->cursorColumn];
	    rect.right =
		eCellMetrics.cellColPos[frame->cursorColumn + 1] + 1;
...
meFrameShowCursor(meFrame * frame)
{
...
#endif				/* _ME_WINDOW */
	{
		...
		(WORD)
		TTcolorSet(colTable
			   [meStyleGetBColor(meSchemeGetStyle(schm))],
...
#ifdef _ME_WINDOW
	{
	     ...
	     */
	    /*
	     * SWP 20040108 - XP ClearType smooth edge font fix - see
	     * first comment 
	     */
	    rect.left = eCellMetrics.cellColPos[frame->cursorColumn];
	    rect.right =
...
meGetMessage(MSG * msg, int mode)
{
	...
	if (ii > hTableSize) {
		...
		/*
		 * if this fails we really are in big trouble, time to get 
		 * out! 
		 */
		meDie();
	    }
	...
	for (;;) {
...
#endif
		{
			...
			if (ipipe->thread != NULL) {
			    ...
			    if (PeekNamedPipe
				(ipipe->rfd, (LPVOID) NULL, (DWORD) 0,
				 (LPDWORD) NULL, &doRead,
				 (LPDWORD) NULL) == meFALSE) {
			    /*
			     * If peek failed, wipe our hands of it. Close 
			     * the process 
			     */
			    GetExitCodeProcess(ipipe->process,
					       (LPDWORD) & (ipipe->
							    exitCode));
			...
			jj = 1;
			        ...
			        */
			       (!GetExitCodeProcess
				(ipipe->process, &doRead)
				|| (doRead != STILL_ACTIVE))) {
			/*
			 * Win95 fails to spot the exit state some times,
			 * this fixes it 
			 */
			GetExitCodeProcess(ipipe->process,
					   (LPDWORD) & ipipe->exitCode);
			CloseHandle(ipipe->process);
			ipipe->pid = -4;
			pp = ipipe;
		    }
	    ...
	    /*
	     * Now simply loop through creating a wait object list of all
	     * remaining processes & console. 
	     */
	    ii = 0;
#ifdef _ME_CONSOLE
#ifdef _ME_WINDOW
		...
		if (hInput != INVALID_HANDLE_VALUE)
		    ...
		    /*
		     * stdin has gone and we're only interested in user
		     * input - exit 
		     */
		    meDie();
	    }
	     ...
	     */
	    jj = MsgWaitForMultipleObjects(ii, hTable, meFALSE, INFINITE,
	    ...
	    {
		if (PeekMessage
		    (msg, meHWndNull, WM_TIMER, WM_TIMER,
		     PM_REMOVE) != meFALSE)
		    return meTRUE;
		if (((jj < 0) || (jj >= ii)
		     || ((jj == 0) && (hInput != INVALID_HANDLE_VALUE)))
...
TTwaitForChar(void)
{
    ...
    meUInt          arg;
    /*
     * If no keys left then if theres currently no mouse timer and theres
     * a button press (No mouse-time key) then check for a time-mouse-?
     * key, if found add a timer start a mouse checking 
     */
	 ...
	 * mouse-time bound ?? 
	 */
	if ((!TTallKeys && (decode_key(mc, &arg) != -1))
	    || (TTallKeys & 0x2)) {
	    /*
...
#if MEOPT_MWFRAME
	/*
	 * if the user has changed the window focus using the OS but ME
	 * can swap to this frame because there is an active frame then
	 * give a warning 
	 */
	 ...
	 */
		...
		meTimerState[msg.wParam] =
		    (meTimerState[msg.wParam] & ~TIMER_SET) |
		    TIMER_EXPIRED;
		continue;
	    }
#endif				/* _ME_WINDOW */
...
#ifdef _ME_WINDOW
	/*
	 * Only get here if we have not handled the message post to the
	 * dispatcher if not a console. 
	 */
#ifdef _ME_CONSOLE
	if (!(meSystemCfg & meSYSTEM_CONSOLE))
#endif
...
TTputs(int row, int col, int len)
{
	...
	/*
	 * Set up the area on the client window to be modified and signal
	 * that the client is about to be updated 
	 */
	/*
	 * SWP 20040108 - XP ClearType smooth edge font fix - see first
	 * comment 
	 */
...
TTapplyArea(void)
{
	...
	for (row = ttRect.top; row < ttRect.bottom; row++) {
	    if (meFrameGetWinPaintStartCol(frameCur)[row] >
		(meShort) ttRect.left)
		meFrameGetWinPaintStartCol(frameCur)[row] =
		    (meShort) ttRect.left;
	    if (meFrameGetWinPaintEndCol(frameCur)[row] <
		(meShort) ttRect.right)
		meFrameGetWinPaintEndCol(frameCur)[row] =
		    (meShort) ttRect.right;
	}
	/*
	 * SWP 20040108 - XP ClearType smooth edge font fix - see first
...
meFrameTermMakeCur(meFrame * frame)
{
	...
	if (IsIconic(meFrameGetWinHandle(frame)))
	    ShowWindow(meFrameGetWinHandle(frame), SW_SHOWNORMAL);
		  ...
		  GetForegroundWindow()) != meFrameGetWinHandle(frame)) {
	    /*
	     * To get the require permissions to set the foreground window 
	     * ME needs to attach to the current thread 
	     */
	    if ((cwHwnd != NULL) &&
		((ThreadID1 = GetWindowThreadProcessId(cwHwnd, NULL)),
		 (ThreadID2 =
		  GetWindowThreadProcessId(meFrameGetWinHandle(frame),
					   NULL)),
		 (ThreadID1 != ThreadID2))) {
...
meFrameTermInit(meFrame * frame, meFrame * sibling)
{
...
#ifdef _ME_WINDOW
	{
	    ...
	    frameData->hwnd = CreateWindow("MicroEmacsClass", ME_FULLNAME " " meVERSION, WS_OVERLAPPEDWINDOW,	/* No 
														 * scroll 
														 * bars 
														 */
					   TTdefaultPosX,
					   ...
					   CW_USEDEFAULT,
					   CW_USEDEFAULT,
					   NULL, NULL, ttInstance, NULL);
	    meFrameSetWindowSize(frame);
	    ShowWindow(frameData->hwnd, ttshowState);	/* Create the
							 * window */
	    UpdateWindow(frameData->hwnd);	/* Show it off - ready for 
						 * errors */

	    /*
	     * calc the boarder sizes by differencing the window and the
	     * client area 
	     */
	    GetWindowRect(frameData->hwnd, &wRect);
	    GetClientRect(frameData->hwnd, &cRect);
	    eCellMetrics.borderWidth =
		(wRect.right - wRect.left) - cRect.right;
	    eCellMetrics.borderDepth =
...
TTstart(void)
{
	...
	meSystemCfg =
	    (meSystemCfg & ~(meSYSTEM_FONTS | meSYSTEM_RGBCOLOR)) |
	    (meSYSTEM_ANSICOLOR | meSYSTEM_XANSICOLOR);

	/*
	 * This will allocate a console if started from the windows NT
	 * program manager. 
	 */
	AllocConsole();

	/*
	 * Save the titlebar of the window so we can restore it when we
	 * leave. 
	 */
	...
	if (GetConsoleScreenBufferInfo(hOutput, &Console)) {
	     ...
	     */
	    TTwidthDefault =
		Console.srWindow.Right - Console.srWindow.Left + 1;
	    TTdepthDefault =
		Console.srWindow.Bottom - Console.srWindow.Top + 1;
	}
#if MEOPT_EXTENDED
	else if (alarmState & meALARM_PIPED) {
	    /*
	     * can't get the console so can't get the size but as running
	     * in -p piped mode set to a default 80x50 
	     */
	...
	{
	    /*
	     * now fix the window buffer size to this window size to get
	     * rid of the horrid scroll bars! 
	     */
    ...
    TTcolour(meCOLOR_FDEFAULT, meCOLOR_BDEFAULT);	/* Default colours 
							 * - none created
							 * yet */

    /*
     * To be continued in meFrameTermInit after the display memory has
     * been initialised 
     */
    return meTRUE;
}
...
TTahead(void)
{
	...
	INPUT_RECORD    ir;
	/*
	 * If TTnoKeys == KEYBUFSIZ then one of 2 things has happend, 1)
	 * The user is running a big command (execute macro 3 billion
	 * times) and has carried on typing 128+ chars (without pressing
	 * the abort key) 2) Pasting in a dos window using the window
	 * paste results in lots of keys all at once, the clipboard must
	 * contain 128+ chars. (1) is unlikely compared to the (2) so
	 * rather than lose the extra chars, keep them there until there
	 * enough room in the input key buffer to store them. 
	 */
	while (TTnoKeys != KEYBUFSIZ) {
	    if ((hInput != INVALID_HANDLE_VALUE) &&
		(PeekConsoleInput(hInput, &ir, 1, &dwCount) != meFALSE)
		&& (dwCount > 0)) {
		if (meGetConsoleMessage(&msg, 0)) {
			...
			&& (msg.message <= WM_KEYLAST))
			WinKeyboard(msg.hwnd, msg.message, msg.wParam,
				    msg.lParam);
#if MEOPT_MOUSE
		    else if ((msg.message >= WM_MOUSEFIRST)
			     && (msg.message <= WM_MOUSELAST))
			WinMouse(msg.hwnd, msg.message, msg.wParam,
				 msg.lParam);
#endif
		}
	    } else
	...
	for (;;) {
	    if (PeekMessage
		(&msg, meHWndNull, WM_KEYFIRST, WM_KEYLAST,
		 PM_REMOVE) != meFALSE) {
		TranslateMessage(&msg);	/* Translate keyboard characters */
		if (!WinKeyboard
	     ...
	     */
		     ...
		     (&msg, meHWndNull, WM_MOUSEFIRST, WM_MOUSELAST,
		      PM_REMOVE) != meFALSE) {
		if (WinMouse(msg.hwnd, msg.message, msg.wParam, msg.lParam)
		    == meFALSE)
		    meMessageHandler(&msg);
    ...
    /*
     * don't process the timers if we have a key waiting! This is because
     * the timers can generate a lot of timer keys, filling up the input
     * buffer - these are not wanted. By not processing, we leave them
	...
	mc = ME_SPECIAL | ttmodif | (SKEY_mouse_time +
				     mouseKeys[mouseState &
					       MOUSE_STATE_BUTTONS]);
	/*
	 * mouse-move bound ?? 
...
TTaheadFlush(void)
{
	...
	for (;;) {
	    ...
	    } else if (PeekMessage(&msg, meHWndNull, 0, 0, PM_REMOVE) !=
		       meFALSE) {
	...
	for (;;) {
		...
		if ((msg.message >= WM_KEYFIRST)
		    && (msg.message <= WM_KEYLAST)) {
		    TranslateMessage(&msg);	/* Translate keyboard
						 * characters */
		    if (!WinKeyboard
			(msg.hwnd, msg.message, msg.wParam, msg.lParam))
			meMessageHandler(&msg);
...
TTsleep(int msec, int intable, meVarList * waitVarList)
{
    ...
    /*
     * Do not actually need the absolute time as this will remain the next 
     * alarm. Ensure that the value is sane 
     */
    if (msec >= 0) {
	if (msec < 10)
	    msec = 10;
	...
	if ((waitVarList != NULL) &&
	    (((ss =
	       getUsrLclCmdVar((meUByte *) "wait", waitVarList)) == errorm)
	     || !meAtoi(ss)))
	    break;
...
meFrameSetWindowSizeInternal(meFrame * frame)
{
    ...
    GetClientRect(meFrameGetWinHandle(frame), &meFrameGetWinCanvas(frame));	/* Get 
										 ...
										 * size 
										 */
    InvalidateRect(meFrameGetWinHandle(frame), &meFrameGetWinCanvas(frame),
		   meFALSE);
	...
	/*
	 * Construct the cell spacing. This is the width of the
	 * characters. 
	 */
	 ...
	 */
	eCellMetrics.cellColTmpPos =
	    meRealloc(eCellMetrics.cellColTmpPos, width + 1);
	eCellMetrics.cellWidthCount = width;
    }
    /*
     * Initialise the column cell LUT tables - this must always be done as 
     * only the font may have changed. 
     */
	...
	/*
	 * Set up the row cell LUT positions. Note allocate a single array 
	 * and split into 4 for re-use. 
	 */
	eCellMetrics.cellRowPos =
	    meRealloc(eCellMetrics.cellRowPos,
		      sizeof(meShort) * 2 * (depth + 1));
	eCellMetrics.cellDepthCount = depth;
    }
    /*
     * Initialise the row cell LUT tables - this must always be done as
     * only the font may have changed. 
     */
	...
	meFrameGetWinPaintStartCol(frame) =
	    meRealloc(meFrameGetWinPaintStartCol(frame),
		      sizeof(meShort) * 2 * (depth + 1));
	meFrameGetWinPaintEndCol(frame) =
	    &(meFrameGetWinPaintStartCol(frame)[depth + 1]);
...
meFrameSetWindowSize(meFrame * frame)
{
	...
	if ((width * depth) > ciScreenSize) {
		...
		free(ciScreenBuffer);
	    ciScreenBuffer =
		(CHAR_INFO *) meMalloc(sizeof(CHAR_INFO) * width * depth);
	    memset((void *) ciScreenBuffer, 0,
		   sizeof(CHAR_INFO) * width * depth);
	}
    }
#ifdef _ME_WINDOW
    else
	 ...
	 */
	/*
	 * Resize the x axis if requested. Compute the desired window
	 * width, no horizontal size restriction 
	 */
	nwidth = (width * eCellMetrics.cell.sizeX) + wBorder;

	/*
	 * Resize the y axis if requested. If the requested size is
	 * greater than the screen depth then refuse, maximum depth is the 
	 * screen depth 
	 */
	ndepth = (depth * eCellMetrics.cell.sizeY) + dBorder;
	if (ndepth > eCellMetrics.maxDepth) {
	    ndepth =
		(eCellMetrics.maxDepth -
		 dBorder) / eCellMetrics.cell.sizeY;
	...
	if ((nwidth != (wRect.right - wRect.left)) ||
	    ...
	    /*
	     * fprintf(logfp,"SetWindowPos - %d %d -> %d %d
	     * (%d)\n",width,depth,nwidth,ndepth,eCellMetrics.maxDepth) ;
	     */
	    /*
	     * fflush(logfp) ;
	     */
	    SetWindowPos(meFrameGetWinHandle(frame), NULL, 0, 0, nwidth,
			 ndepth,
...
meFrameRepositionWindow(meFrame * frame, int resize)
{
	...
	/*
	 * the monitor window sizes do not allow for the boarder which is
	 * not shown, we must allow for it 
	 */
...
#endif
	/*
	 * Always reposition so the top left is visible and as much of the 
	 * window 
	 */
	GetWindowRect(meFrameGetWinHandle(frame), &wRect);
	...
	if (resize
	    && (((left + width) > mRect.right)
		|| ((top + depth) > mRect.bottom))) {
	    if ((left + width) > mRect.right)
		width = mRect.right - left;
	    if ((top + depth) > mRect.bottom)
		depth = mRect.bottom - top;
	    SetWindowPos(meFrameGetWinHandle(frame), NULL, left, top,
			 width, depth, SWP_NOZORDER | SWP_NOACTIVATE);
...
/*
 * meSetupPathsAndUser On windows the user name has already been setup -
 * required for ini file reading The ini file may also have found an
 * mepath or meinstallpath setting 
 */
void
meSetupPathsAndUser(char *progname)
{
     ...
     */
    appDataBuff[0] = '\0';
    if (SUCCEEDED(SHGetSpecialFolderLocation(NULL, CSIDL_APPDATA, &idList))
	&& (idList != NULL)) {
	IMalloc        *im;
    ...
    if (searchPath != NULL) {
	/*
	 * explicit path set by the user, don't need to look at anything
	 * else 
	 */
	/*
	...
	if (meUserPath != NULL) {
	    /*
	     * check that the user path is first in the search path, if
	     * not add it 
	     */
		...
		/*
		 * meMalloc will exit if it fails as ME has not finished
		 * initialising 
		 */
	...
	if (appData != NULL) {
		...
		/*
		 * as this is the user's area, use this directory unless
		 * we find a .../<$user-name>/ directory 
		 */
		gotUserPath = -1;
	}

	/*
	 * Get the system path of the installed macros. Use $MEINSTPATH as 
	 * the * MicroEmacs standard macros 
	 */
	if (meInstallPath != NULL)
	    ll = mePathAddSearchPath(ll, evalResult, meInstallPath,
				     &gotUserPath);
	else if (((ss = meGetenv("MEINSTALLPATH")) != NULL)
		 && (ss[0] != '\0')) {
	    strcpy(buff, ss);
	 ...
	 */
	if ((meProgName != NULL)
	    && ((ss = meStrrchr(meProgName, DIR_CHAR)) != NULL)) {
	...
	if (!gotUserPath && (appData != NULL)) {
	    /*
	     * We have not found a user path so add the $APPDATA as the
	     * user-path as this is the best place for macros to write to
	     * etc. 
	     */
	    strcpy(buff, appData);
	    if (ll) {
	...
	if (meUserPath == NULL) {
	    /*
	     * no user path yet, take the first path from the search-path, 
	     * this should be a sensible directory to use 
	     */
...
meIniFileEntry(meUByte * label, meUByte * value)
{
	...
	/*
	 * mename=name user has chaged their MENAME then remember the new
	 * name. 
	 */
	stringp = &meUserName;
    else if (meStricmp(label, "mepath") == 0)
	/*
	 * mepath=path User has changed the MEPATH then remember the new
	 * name. 
	 */
	stringp = &searchPath;
    else if (meStricmp(label, "meinstallpath") == 0)
	/*
	 * meinstallpath=path User has changed the MEINSTALLPATH then
	 * remember the new name. 
	 */
	stringp = &meInstallPath;
    else if (meStricmp(label, "meuserpath") == 0)
	/*
	 * meuserpath=path userpath=path Set the user path, we use the new 
	 * definition 'meuserpath' and the existing definition 'userpath'
    ...
    {
	/*
	 * geometry=wxh+x+y Get the initial window size and position 
	 */
	if (meStricmp(label, "geometry") == 0) {
		...
		int             ww = TTwidthDefault,
		    dd = TTdepthDefault;
		sscanf((char *) (value), "%dx%d+%d+%d", &ww, &dd,
		       &TTdefaultPosX, &TTdefaultPosY);
		TTwidthDefault = ww;
		TTdepthDefault = dd;
	    }
	    return;
...
meIniFileRead(void)
{
	 ...
	 */
	for (lpTemp = lpSectionNames; *lpTemp;
	     lpTemp += lstrlen(lpTemp) + 1) {
	    GetPrivateProfileString(iniSections[ii], lpTemp, "", buf1,
				    meBUF_SIZE_MAX, ME_INI_FILE);
	...
	int nCmdShow)
{
     ...
     * is returned as UNI-Code characters. In addition we have to cope for 
     * spaces in the name, so we check for the ".exe" extension 
     */
    {
	...
	if ((pp = GetCommandLine()) == NULL) {
	    ...
	    /*
	     * ingore the first quote, if 2 quotes then the progname was
	     * in quotes so don't stop at a space 
	     */
	    if (*pp == '"') {
		ec = '"';
		pp++;
	...
	while (*lpbuf != '\0') {
	    ...
	    if ((endc = *lpbuf) != '"') {
		/*
		 * if the next argument does not start with a '-' (an
		 * Com-line option) and the rest of the line makes up the
		 * name of an existing file then add the rest of the line
		 * as the last arg and quit. Why? Windows stupidity of
		 * course! With Explorer extension associativity a user
		 * should specify to open .foo files with [me32 "%1"] so
		 * the file name is correctly quoted. But windows being
		 * windows allows [me32] so when the user double clicks on 
		 * c:\program files\fred.foo the command line is [me32
		 * c:\program files\fred.foo] - nuff said. 
		 */
    ...
    /*
     * Just incase the window has been resized during start up go and
     * check 
     */
	...
	if (TTbreakFlag) {
	    TTinflush();
	    if ((selhilight.flags & (SELHIL_ACTIVE | SELHIL_KEEP)) ==
		SELHIL_ACTIVE)
	 ...
	 * files now. Note we make the invocation here as we know we have
	 * returned to a base state; any macro's would have been aborted.
	 * However it is better that macro debugging is disabled so
	...
	if (dadHead != NULL) {
		...
		mouse_dX =
		    (meShort) (((dadp->mousePos.x -
				 colToClient(mouse_X)) << 8) /
			       eCellMetrics.cell.sizeX);
		mouse_dY =
		    (meShort) (((dadp->mousePos.y -
				 rowToClient(mouse_Y)) << 8) /
			       eCellMetrics.cell.sizeY);
		if (mouse_X > frameCur->width)
		    mouse_X = frameCur->width;
		if (mouse_Y > frameCur->depth)
...
#if MEOPT_EXTENDED
		/*
		 * if the current window is locked to a buffer find
		 * another 
		 */
...
meFrameGainFocus(meFrame * frame)
{
	...
	/*
	 * We have been swapped out. Therefore we potentially do not own
	 * the clipboard contents
	 */
	clipState &= ~CLIP_OWNER;

	/*
	 * Make sure the cursor is ok This must be done whether
	 * MEOPT_MOUSE is enabled or not 
	 */
...
MainWndProc(HWND hWnd, UINT message, UINT wParam, LONG lParam)
{
    ...
    case WM_SETCURSOR:
	/*
	 * If we have not got the focus or this is not the main window
	 * cursor then we dont handle it, use the default Proc handle *
	...
	if (eCellMetrics.pInfo.hPal != NULL) {
	    /*
	     * we have a palette simply return TRUE as the
	     * WM_PALETTECHANGED message will handle the actual ins and
	     * outs of palette swapping 
	     */
	    return meTRUE;
	}
	break;
	 ...
	 */
	/*
	 * we only want to do this once so only do it if this is to the
	 * baseHwnd. Note that it could be us changing the palette. 
	 */
	if ((eCellMetrics.pInfo.hPal != NULL) && (hWnd == baseHwnd)) {
		...
		InvalidateRect(meFrameGetWinHandle(loopFrame), NULL,
			       meFALSE);
	 ...
	 */
	meMouseCfg |=
	    GetSystemMetrics(SM_CMOUSEBUTTONS) & meMOUSE_NOBUTTONS;
	TTinitMouse();
#endif
	/*
	 * still must do some work if mouse is disabled 
	 */
	meCursors[meCURSOR_DEFAULT] = meCursors[meCURSOR_ARROW] =
	 ...
	 * not know what state the user has left the system in. Hive away
	 * the drag and drop files until we are ready to process them.
	 * Retain the cursor position so that we may show the appropriate
	 * file in a specified buffer. 
	 */
	{
	    POINT           pt;	/* Position in the window */
	    WORD            fcount;	/* Count of the number of files */
	    meUByte         dfname[meBUF_SIZE_MAX];	/* Dropped
							 * filename */
		...
		for (ii = 0; ii < fcount; ii++) {
			...
			meMalloc(sizeof(struct s_DragAndDrop) + len);
	...
	if (wParam >= NUM_TIMERS)
	    ...
	    return meTRUE;
	meTimerState[wParam] =
	    (meTimerState[wParam] & ~TIMER_SET) | TIMER_EXPIRED;
	break;
	...
	if ((wParam != SIZE_MINIMIZED)
	    && ((frame = meMessageGetFrame(hWnd)) != NULL)) {
	    RECT            cRect;
	    int             nrow,
	                    ncol,
	                    setSize = 0;
	    if (wParam == SIZE_MAXIMIZED)
		meFrameGetWinMaximized(frame) = 1;	/* Set the
							 * maximized state 
							 */
	    else if (wParam == SIZE_RESTORED)
		meFrameGetWinMaximized(frame) = 0;	/* Release the
							 * minimised state 
							 */
	    /*
	     * Inform Emacs window S/W that the window has changed size 
	    ...
	    /*
	     * fprintf(logfp,"WM_SIZE - %x %x %d %d -> %d
	...
	if (meMessageGetFrame(hWnd) != NULL) {
		...
		col =
		    (pos->cx -
		     eCellMetrics.borderWidth) / eCellMetrics.cell.sizeX;
		row =
		    (pos->cy -
		     eCellMetrics.borderDepth) / eCellMetrics.cell.sizeY;
		/*
		 * fprintf(logfp,"WM_WINDOWPOSCHANGING - %x %x - %d %d -
		 * %d %d -> %d
		 ...
		 */
		pos->cx =
		    (col * eCellMetrics.cell.sizeX) +
		    eCellMetrics.borderWidth;
		pos->cy =
		    (row * eCellMetrics.cell.sizeY) +
		    eCellMetrics.borderDepth;
	    }
	}
	goto unhandled_message;
    ...
    case WM_RENDERFORMAT:
	{
	    ...
	    hmem = WinKillToClipboard();
	    SetClipboardData((ttlogfont.lfCharSet ==
			      OEM_CHARSET) ? CF_OEMTEXT : CF_TEXT, hmem);
	    /*
	     * Force the stale state. If another application is pulling
	     * data from us while we are the clipboard owner we must force 
	     * the clipboard to be refreshed whenever the 'yank' buffer
	     * changes. If nobody is listening then we do not care. This
	     * simply allows us to process optimally and not to
	     * continually report a clipboard change whenever we change
	     * the yank data. 
	     */
	    clipState |= CLIP_STALE;
	}
	...
	if ((frame = meMessageGetFrame(hWnd)) != NULL) {
		...
		PostMessage(hWnd, WM_CLOSE, 0, 0);
		     ...
		     !(frame->flags & meFRAME_NOT_FOCUS)) {
		/*
		 * Use the normal command to save buffers and exit if we
		 * have focus. If it doesn't exit then carry on as normal
    ...
    case WM_DESTROY:
	/*
	 * if just closing down a frame the frame will already be unlinked 
	 * so we won't find the frame 
	 */
	if ((frame = meMessageGetFrame(hWnd)) != NULL) {
	    WinShutdown();
	    /*
	...
	if ((frame = meMessageGetFrame(hWnd)) != NULL) {
	    /*
	     * Move; Handle the position of the window changing. Need to
	     * force a re-paint of then window. 
	     */
	...
	if ((frame = meMessageGetFrame(hWnd)) != NULL) {
	    /*
	     * Indicate to the PAINT that we must honour the next draw
	     * region and not try to optimise the painting otherwise we
	     * will not re-paint the screen fully. Must invalidate the
	     * window incase we don't get a paint message in which case
	     * the window will stop updating 
	     */
	    InvalidateRect(hWnd, NULL, meFALSE);
	    meFrameGetWinPaintAll(frame) = 1;
	}
    ...
    case WM_NCMOUSEMOVE:
	/*
	 * The mouse is in the non-client area, therefore the cursor may
	 * have changed, reset the last lParam to force a reset of the
	 * cursor when it re-enters the client area 
	 */
	setCursorLastLParam = -1;
	goto unhandled_message;
...
#endif				/* _ME_CONSOLE */
#ifdef _ME_WINDOW
	SetWindowText(meFrameGetWinHandle(frame), buf);	/* Change the
							 * window text */
#endif				/* _ME_WINDOW */
}

/*
 * TTsetBgcol; Set the default fill of the background. Now in windows then 
 * we have to modify the class window in order to effect the default
 * background fill. 
 */
void
TTsetBgcol(void)
{
	...
	if (((newBrush =
	      CreateSolidBrush(eCellMetrics.pInfo.cPal[bcol].cpixel)) !=
	     NULL)
	    &&
	    (SetClassLong
	     (meFrameGetWinHandle(loopFrame), GCL_HBRBACKGROUND,
	      (LONG) (newBrush)) != (LONG) (NULL))) {
	    /*
	     * The new brush has been installed. Delete the old brush if
	     * we have defined it and remember the old context 
	     */
	    if (ttBrush != NULL)
		DeleteObject(ttBrush);
	    ttBrush = newBrush;
