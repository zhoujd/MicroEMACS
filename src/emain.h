/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com emain.h - Encapsulate all 
 * platform definitions. Copyright (C) 1997-2009 JASSPA (www.jasspa.com)
...
/*
 * the following 3 defines are used for debugging and memory leak
 * checking, _ME_WIN32_FULL_DEBUG - Enables Windows debugging
...
/*
 * These next define is platform specific, but as all supported platforms 
 * use these and all future ones should I've put them here for now. NOTE
 * APRAM has been removed, it became unused & redundant 
 */
#define _STDARG 1		/* Use <stdarg.h> in defintions */

/*
 * One may think that this should be platform dependant, BUT time and
 * experience has shown that it is best to always use a '/' in the body of 
 * MicroEmacs (simplifies everything) and convert to or from the system
 * DIR char if required. No system yet needs this but as some need to
 * check and convert to a '/' they define a CONVDIR_CHAR 
 */
...
/*
 * NeXT provides us with libc.h this includes all of the API definitions
 * for the standard 'C' library calls. Nice touch !! 
 */
#include <libc.h>
...
#endif
/*
 * The next option is commented out as the win32*.mak file define it when
 * required 
 */
/*
 * #define _SOCKET 1
