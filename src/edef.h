/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com edef.h - Global variable
 * definitions. Originally written by Dave G. Conroy for MicroEmacs
...
#endif
#ifdef _WIN32
extern meUInt   meSYSTEM_MASK;	/* ME system mask - dependant on win32
				 * flavour */
#endif
...
#define meMOUSE_ICON        0xf0000	/* Mask to set the mouse pointer
					 * icon */
 ...
 * window interaction. If the command is a macro the the following nasty
 * things can happen if this is not so 1) The 1st yank in a macro may
 * return a different value to the 2nd yank, if the user defines another
 * region in a different window. 2) If the macro is looping round cutting 
 * and pasting then the user will now be able to reliably cut 'n' paste in 
 * another window. 3) A macro could cut a region and the paste a
 * different one! Nasty. Try and ensure these don't happen. 
 */
...
extern meUByte  evalResult[meTOKENBUF_SIZE_MAX];	/* Result string
							 * from functions */
...
meStyle         defaultScheme[meSCHEME_STYLES * 2] =	/* Default colour
							 * scheme */
{
...
#endif
meScheme        mlScheme = meSCHEME_NDEFAULT;	/* Message line color
						 * scheme */
...
meUByte         boxChars[BCLEN + 1] =	/* Set of box characters */
    "|+++++++++-";
meUByte         windowChars[WCLEN + 1] =	/* Set of window
						 * characters */
{
...
meUByte         lastReplace = 0;	/* set to non-zero if last was a
					 * replace */
meUByte         modeLineFlags =	/* current modeline flags */
    (WFMODE | WFRESIZE | WFMOVEL);
meUInt          meSystemCfg =	/* ME system config variable */
#ifdef _DOS
    (meSYSTEM_CONSOLE | meSYSTEM_ANSICOLOR | meSYSTEM_XANSICOLOR |
     meSYSTEM_OSDCURSOR | meSYSTEM_MSSYSTEM | meSYSTEM_DRIVES |
     meSYSTEM_DOSFNAMES | meSYSTEM_TABINDANY | meSYSTEM_ALTMENU |
     meSYSTEM_ALTPRFX1);
#endif
#ifdef _WIN32
(meSYSTEM_RGBCOLOR | meSYSTEM_FONTS | meSYSTEM_OSDCURSOR |
 meSYSTEM_MSSYSTEM | meSYSTEM_DRIVES | meSYSTEM_DOSFNAMES |
 meSYSTEM_TABINDANY | meSYSTEM_ALTMENU | meSYSTEM_ALTPRFX1 |
 meSYSTEM_CTCHASPC
#if MEOPT_IPIPES
 | meSYSTEM_IPIPES
#endif
...
#ifdef _UNIX
(meSYSTEM_RGBCOLOR | meSYSTEM_FONTS | meSYSTEM_OSDCURSOR |
 meSYSTEM_UNIXSYSTEM | meSYSTEM_IPIPES | meSYSTEM_TABINDANY |
 meSYSTEM_ALTMENU | meSYSTEM_ALTPRFX1);
#endif
#ifdef _WIN32
meUInt          meSYSTEM_MASK =	/* ME system mask - dependant on win32
				 * flavour */
    (meSYSTEM_FONTS | meSYSTEM_OSDCURSOR | meSYSTEM_TABINDANY |
     meSYSTEM_ALTMENU | meSYSTEM_ALTPRFX1 | meSYSTEM_KEEPUNDO |
     meSYSTEM_FONTFIX | meSYSTEM_CTCHASPC | meSYSTEM_SHOWWHITE |
     meSYSTEM_HIDEBCKUP | meSYSTEM_TABINDFST | meSYSTEM_NOEMPTYANK |
     meSYSTEM_NOCLIPBRD
#if !defined (_WIN32s)
     | meSYSTEM_CLNTSRVR
#endif
...
#if MEOPT_MOUSE
meUInt          meMouseCfg = (3 | meMOUSE_ENBLE);	/* ME mouse config 
							 * variable */
meShort         mouse_dX = 0;	/* mouse delta X last event pos */
meShort         mouse_dY = 0;	/* mouse delta X last event pos */
#else
meUInt          meMouseCfg = 3;	/* ME mouse config variable - mouse not
				 * enabled */
#endif
...
gid_t          *meGidList = NULL;	/* list of group ids the user
					 * belongs to */
#endif				/* _UNIX */
...
#endif				/* _POSIX_SIGNALS */
#ifdef _BSD_SIGNALS
int             meSigHoldMask = 0;	/* signals held when spawning and
					 * reading */
#endif				/* _BSD_SIGNALS */
...
meUShort        prefixc[ME_PREFIX_NUM + 1] = { ME_INVALID_KEY,	/* unused
								 * 0th
								 * value */
...
meUByte         lkbdptr[meBUF_SIZE_MAX];	/* Holds last keybd macro
						 * data */
...
meUByte         resultStr[meBUF_SIZE_MAX];	/* Result string from
						 * commands */
