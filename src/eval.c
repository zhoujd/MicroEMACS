/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com eval.c - Expresion
 * evaluation functions. Copyright (C) 1988-2009 JASSPA (www.jasspa.com)
...
meUByte         evalResult[meTOKENBUF_SIZE_MAX];	/* resulting
							 * string */
 ...
 * define-macro l3 set-variable @1 @2 !emacro define-macro l2 l3 @1 @2
 * !emacro define-macro l1 l2 @1 @2 !emacro define-macro Test l1 #l0 @#
 * ml-write #l0 !emacro 500 Test The above example should write out "500".
 * its not good enough for @1 in l3 to return #l0, it must return Test's
 * #l0 and thats horrid, hence the variable. Similarly it should be set to
 * Test's @#, not l3's which will be 1. The hopefully final saga to this is 
 * that the process of setting a variable may not be instantaneous, e.g.
 * when doing "set-variable #l1 @ml "get value"" it waits for the user to
 * enter a value. During this time a callback macro may occur which could
 * (probably will) change the value of gmaLocalRegPtr so the value must be
 * cached as soon as the variable to be set is obtained. 
 */
...
/*
 * Quick and easy interface to regex compare return -1 on error, 0 on no
 * match 1 if matched 
 */
int
regexStrCmp(meUByte * str, meUByte * reg, int flags)
{
	...
	/*
	 * or in the STRCMP bit so that hunt-forw/backward can still
	 * determine whether the last main search was magic or not & @s1
	 * also work correctly 
	 */
...
/*
 * meItoa integer to ascii string.......... This is too inconsistant to
 * use the system's 
 */
...
meVariable     *
SetUsrLclCmdVar(meUByte * vname, meUByte * vvalue,
		register meVarList * varList)
{
    /*
     * set a user variable 
     */
    register meVariable *vptr,
                   *vp;
    register int    ii,
                    jj;
     ...
     */
    if ((vp =
	 (meVariable *) meMalloc(sizeof(meVariable) + meStrlen(vname))) !=
	NULL) {
...
setVar(meUByte * vname, meUByte * vvalue, meRegister * regs)
{
    ...
    case TKREG:
	{
		...
		return mlwrite(MWABORT,
			       (meUByte *) "[No such register %s]", vname);
	...
	if (*nn == 'w') {
		...
		if ((frameCur->windowCur->dotLineNo ==
		...
		if ((status =
		     ldelete(meLineGetLength(frameCur->windowCur->dotLine),
			     6)) > 0) {
		    ll = meStrlen(vvalue);
		...
		if (frameCur->windowCur->dotOffset >=
		    frameCur->windowCur->dotLine->length)
		    return mlwrite(MWABORT,
				   (meUByte *) "[Cannot set @wc here]");

		lineSetChanged(WFMAIN);
#if MEOPT_UNDO
		meUndoAddRepChar();
#endif
		meLineSetChar(frameCur->windowCur->dotLine,
			      frameCur->windowCur->dotOffset, vvalue[0]);
	    }
	} else if (*nn == 'c') {
	    int             ii;
	    if (nn[2] == 'k') {
	    ...
	    return mlwrite(MWABORT, (meUByte *) "[Cannot set variable %s]",
			   vname);
	break;
#endif
	...
	case EVREPEATTIME:
	     ...
	     * does not support the mouse but context menus need to be
	     * positioned sensibly 
	     */
	case EVMOUSE:
...
#ifdef _XTERM
		/*
		 * on unix, if using x-window then can't set ANSI || XANSI 
		 * bits 
		 */
...
#endif
		/*
		 * may changes may case a redraw so always redraw next
		 * time 
		 */
		sgarbf = meTRUE;
		break;
...
#endif
	case EVFRMDPTH:
	    return frameChangeDepth(meTRUE,
				    meAtoi(vvalue) - (frameCur->depth +
						      1));
#if MEOPT_EXTENDED
	case EVFRMID:
...
#endif
	case EVFRMWDTH:
	    return frameChangeWidth(meTRUE,
				    meAtoi(vvalue) - frameCur->width);
	case EVABSCOL:
	...
	case EVWMRKCOL:
		...
		return meFALSE;
		       ...
		       && ((frameCur->bufferCur->intFlag & BIFNACT) == 0)
		       && (frameCur->windowCur->markLine != NULL)
		       && (status >
			   meLineGetLength(frameCur->windowCur->
					   markLine))) {
	...
	case EVWYSCROLL:
	    if ((status = meAtoi(vvalue)) < 0)
		status = 0;
	    else if (frameCur->bufferCur->lineCount
		     && (status >= frameCur->bufferCur->lineCount))
		status = frameCur->bufferCur->lineCount - 1;
	    if (frameCur->windowCur->vertScroll != status) {
		frameCur->windowCur->vertScroll = status;
		frameCur->windowCur->updateFlags |=
		    WFMAIN | WFSBOX | WFLOOKBK;
	...
	case EVINDWIDTH:
	    ...
	    break;
	case EVBUFINDWIDTH:
	    if ((frameCur->bufferCur->indentWidth =
		 (meUByte) meAtoi(vvalue)) <= 0)
	...
	case EVTABWIDTH:
	    ...
	    break;
	case EVBUFTABWIDTH:
	    if ((frameCur->bufferCur->tabWidth =
		 (meUByte) meAtoi(vvalue)) <= 0)
	...
	case EVGLOBSCHM:
	    ...
	    frameCur->video.lineArray[frameCur->depth].endp =
		frameCur->width;
	    sgarbf = meTRUE;
	    TTsetBgcol();
	    break;
	case EVTRNCSCHM:
	    ...
	    break;
	case EVBUFSCHM:
	    frameCur->bufferCur->scheme =
		convertUserScheme(meAtoi(vvalue),
				  frameCur->bufferCur->scheme);
	...
	case EVBUFINP:
	    ...
	    break;
	case EVLINEFLAGS:
	    frameCur->windowCur->dotLine->flag =
		((meLineFlag) (meAtoi(vvalue) & meLINE_SET_MASK)) |
	...
	case EVLINESCHM:
		...
		if (ii >= 0) {
		    ...
		    frameCur->windowCur->dotLine->flag =
			(frameCur->windowCur->dotLine->
			 flag & ~meLINE_SCHEME_MASK) | (ii <<
							meLINE_SCHEME_SHIFT);
		} else if (meLineIsSchemeSet(frameCur->windowCur->dotLine))
		    frameCur->windowCur->dotLine->flag =
			(frameCur->windowCur->dotLine->
			 flag & ~meLINE_SCHEME_MASK);
		lineSetChanged(WFMAIN);
		break;
	    }
	case EVBUFHIL:
	    if (((frameCur->bufferCur->hilight =
		  (meUByte) meAtoi(vvalue)) >= noHilights)
	...
	case EVBUFIND:
	    if (((frameCur->bufferCur->indent =
		  (meUByte) meAtoi(vvalue)) >= noIndents)
	...
	case EVCNAMES:
		...
		sortStrings(commNames.size, commNames.list, 0,
			    (meIFuncSS) strcmp);
	    }
	    break;
	case EVFNAMES:
		...
		if (fileNames.mask != NULL) {
			...
			*mm = '\0';
			...
			pathNameCorrect(buff, PATHNAME_COMPLETE, path,
					NULL);
		    }
	...
	case -1:
		 ...
		 */
		meUByte         buff[meBUF_SIZE_MAX + meSBUF_SIZE_MAX],
		               *ss;
	...
	default:
	     ...
	     * * EVSYSRET EVUSEX, EVVERSION, EVWMDLINE, EVEOBLINE or
	     * system dependant vars where this isn't the system (e.g.
	     * use-x) which cant be set 
	     */
	    return meFALSE;
	}
	break;
...
static meUByte *
gtenv(meUByte * vname)
{				/* vname name of environment variable to
				 * retrieve */
    int             ii;
	 ...
	 */
    case EVAUTOTIME:
	return meItoa(autoTime);
#if MEOPT_MOUSE
    case EVDELAYTIME:
	return meItoa(delayTime);
    case EVREPEATTIME:
	return meItoa(repeatTime);
#endif
#if MEOPT_CALLBACK
    case EVIDLETIME:
	return meItoa(idleTime);
#endif
    case EVBOXCHRS:
	return boxChars;
    case EVMODELINE:
	...
	return meProgName;
    case EVCURSORX:
	return meItoa(frameCur->mainColumn);
    case EVCURSORY:
	return meItoa(frameCur->mainRow);
    case EVSYSTEM:
	return meItoa(meSystemCfg);
#if MEOPT_WORDPRO
    case EVBUFFILLCOL:
	return meItoa(frameCur->bufferCur->fillcol);
    case EVBUFFILLMODE:
	...
	return fillbullet;
    case EVFILLBULLETLEN:
	return meItoa(fillbulletlen);
    case EVFILLCOL:
	return meItoa(fillcol);
    case EVFILLEOS:
	return filleos;
    case EVFILLEOSLEN:
	return meItoa(filleoslen);
    case EVFILLIGNORE:
	return fillignore;
    case EVFILLMODE:
...
#if MEOPT_EXTENDED
    case EVPAUSETIME:
	return meItoa(pauseTime);
    case EVFSPROMPT:
	return meItoa(fileSizePrompt);
    case EVKEPTVERS:
	return meItoa(keptVersions);
    case EVMOUSE:
	return meItoa(meMouseCfg);
    case EVMOUSEPOS:
	return meItoa(mouse_pos);
    case EVMOUSEX:
	return meItoa(mouse_X);
    case EVMOUSEY:
	return meItoa(mouse_Y);
    case EVBNAMES:
	mv = &buffNames;
	...
	while (mv->curr < mv->size) {
		...
		(ss, mv->mask,
		 (mv->
		  exact) ? meRSTRCMP_WHOLE : meRSTRCMP_ICASE |
		 meRSTRCMP_WHOLE))
		return ss;
	}
	return emptym;
    ...
    case EVTIME:
	{
	    ...
	    /*
	     * Format the result. Format is:- YYYYCCCMMDDWhhmmssSSS
	     * Where: YYYY - Year i.e. 1997 CCC - Day of the week since
	     * January 1st (0-365).  MM - Month of year 1-12 DD - Day of
	     * the month 1-31 W - Weekday since sunday 0-6 (sunday == 0)
	     * hh - Hours 0-23 mm - Minutes 0-59 ss - Seconds 0-59 SSS -
	     * Milliseconds 0-999 
	     */
	    ...
	    /*
	     * SWP - Took out the 0 padding because of macro use.  ME
	     * interprets numbers of the form 0## as octagon based numbers 
	     * so in september the number is 09 which is illegal etc. 
	     */
	    sprintf((char *) evalResult, "%4d%3d%2d%2d%1d%2d%2d%2d%3d", time_ptr->tm_year + 1900,	/* Year 
													 * - 
													 * 2000 
													 * should 
													 * be 
...
#endif
    case EVFRMDPTH:
	return meItoa(frameCur->depth + 1);
#if MEOPT_EXTENDED
    case EVFRMID:
	return meItoa(frameCur->id);
#endif
    case EVFRMWDTH:
	return meItoa(frameCur->width);
    case EVABSCOL:
	return meItoa(getcwcol());
    case EVCURCOL:
	return meItoa(frameCur->windowCur->dotOffset);
    case EVCURLINE:
	return meItoa(frameCur->windowCur->dotLineNo + 1);
#if MEOPT_EXTENDED
    case EVABSLINE:
	return meItoa(windowGotoAbsLine(-1) + 1);
    case EVEOBLINE:
	return meItoa(frameCur->bufferCur->lineCount + 1);
    case EVWMRKCOL:
	return meItoa(frameCur->windowCur->markOffset);
    case EVWMRKLINE:
	return meItoa((frameCur->windowCur->markLine == NULL) ? 0 :
		      frameCur->windowCur->markLineNo + 1);
    case EVBMDLINE:
	if (frameCur->bufferCur->modeLineStr == NULL)
	    return modeLineStr;
	return frameCur->bufferCur->modeLineStr;
    case EVWXSCROLL:
	return meItoa(frameCur->windowCur->horzScrollRest);
    case EVWXCLSCROLL:
	return meItoa(frameCur->windowCur->horzScroll);
    case EVWYSCROLL:
	return meItoa(frameCur->windowCur->vertScroll);
    case EVWMDLINE:
	return meItoa(frameCur->windowCur->frameRow +
		      frameCur->windowCur->textDepth);
    case EVWSBAR:
	return meItoa(frameCur->windowCur->frameColumn +
		      frameCur->windowCur->textWidth);
    case EVWFLAGS:
	return meItoa(frameCur->windowCur->flags);
    case EVWID:
	return meItoa(frameCur->windowCur->id);
    case EVWDEPTH:
	return meItoa(frameCur->windowCur->textDepth);
    case EVWWIDTH:
	return meItoa(frameCur->windowCur->textWidth);
#endif
    case EVWINCHRS:
	return windowChars;
    case EVCBUFBACKUP:
	if ((frameCur->bufferCur->fileName == NULL)
	    || createBackupName(evalResult, frameCur->bufferCur->fileName,
				'~', 0))
	    return emptym;
#if MEOPT_EXTENDED
#ifndef _DOS
	...
	return evalResult;
    case EVCBUFNAME:
	return frameCur->bufferCur->name;
    case EVBUFFMOD:
	return meItoa((((meInt) frameCur->bufferCur->fileFlag) << 16) |
		      ((meInt) frameCur->bufferCur->stats.stmode));
    case EVGLOBFMOD:
	return meItoa(meUmask);
    case EVCFNAME:
	return mePtos(frameCur->bufferCur->fileName);
#if MEOPT_DEBUGM
    case EVDEBUG:
	return meItoa(macbug);
#endif
    case EVSTATUS:
	return meLtoa(cmdstatus);
#if MEOPT_TIMSTMP
    case EVTIMSTMP:
	return time_stamp;
#endif
    case EVINDWIDTH:
	return meItoa(indentWidth);
    case EVBUFINDWIDTH:
	return meItoa(frameCur->bufferCur->indentWidth);
    case EVTABWIDTH:
	return meItoa(tabWidth);
    case EVBUFTABWIDTH:
	return meItoa(frameCur->bufferCur->tabWidth);
    case EVSRCHPATH:
	return mePtos(searchPath);
    case EVHOMEDIR:
	return mePtos(homedir);
#if MEOPT_EXTENDED
    case EVMODECHRS:
	return modeCode;
    case EVSHWMDS:
    ...
    case EVSHWRGN:
	return meItoa(selhilight.uFlags);
#endif
    case EVCURSORBLK:
	return meItoa(cursorBlink);
#if MEOPT_COLOR
    case EVCURSORCOL:
	return meItoa(cursorColor);
    case EVMLSCHM:
	return meItoa(mlScheme / meSCHEME_STYLES);
    case EVMDLNSCHM:
	return meItoa(mdLnScheme / meSCHEME_STYLES);
    case EVSBARSCHM:
	return meItoa(sbarScheme / meSCHEME_STYLES);
    case EVGLOBSCHM:
	return meItoa(globScheme / meSCHEME_STYLES);
    case EVTRNCSCHM:
	return meItoa(trncScheme / meSCHEME_STYLES);
    case EVBUFSCHM:
	return meItoa(frameCur->bufferCur->scheme / meSCHEME_STYLES);
    case EVSBAR:
	return meItoa(gsbarmode);
#if MEOPT_OSD
    case EVOSDSCHM:
	return meItoa(osdScheme / meSCHEME_STYLES);
#endif
#endif
#if MEOPT_IPIPES
    case EVBUFIPIPE:
	...
	return evalResult;
    case EVLINEFLAGS:
	return meItoa(frameCur->windowCur->dotLine->flag);
    case EVBUFMASK:
	{
    ...
    case EVFILEIGNORE:
	return mePtos(fileIgnore);
#endif
#if MEOPT_HILIGHT
    case EVLINESCHM:
	return meItoa(meLineIsSchemeSet(frameCur->windowCur->dotLine) ?
		      (frameCur->bufferCur->
		       lscheme[meLineGetSchemeIndex
			       (frameCur->windowCur->dotLine)] /
		       meSCHEME_STYLES) : -1);
    case EVBUFHIL:
	return meItoa(frameCur->bufferCur->hilight);
    case EVBUFIND:
	return meItoa(frameCur->bufferCur->indent);
#endif
#if MEOPT_FILENEXT
    case EVFILETEMP:
	return mePtos(flNextFileTemp);
    case EVLINETEMP:
	return mePtos(flNextLineTemp);
#endif
#if MEOPT_RCS
    case EVRCSFILE:
	return mePtos(rcsFile);
    case EVRCSCOCOM:
	return mePtos(rcsCoStr);
    case EVRCSCOUCOM:
	return mePtos(rcsCoUStr);
    case EVRCSCICOM:
	return mePtos(rcsCiStr);
    case EVRCSCIFCOM:
	return mePtos(rcsCiFStr);
    case EVRCSUECOM:
	return mePtos(rcsUeStr);
#endif
    case EVUSERNAME:
	return mePtos(meUserName);
    case EVUSERPATH:
	return mePtos(meUserPath);
    case EVSCROLL:
	return meItoa(scrollFlag);
    case EVQUIET:
	return meItoa(quietMode);
    }
...
getMacroArg(int index)
{
    ...
    /*
     * move the register pointer to the parent as any # reference will be
     * w.r.t the parent 
     */
    crp = meRegCurr;
...
{				/* find the value of a token */
	...
	if (tkn[1] == 'w') {
		...
		*dd = '\0';
	    ...
	    /*
	     * Get and evaluate the next argument - this is the history
	     * number 
	     */
	    ...
	    *dd = '\0';
		 ...
		 */
		/*
		 * note: the result buffer cant be used directly as it is
		 * used in modeline which is is by update which can be
		 * used by meGetStringFromUser 
		 */
		...
		if (flag & 0x04)
		    /*
		     * set the first key to 'tab' to expand the input
		     * according to the completion list 
		     */
		...
		if (cc == 1) {
		    ...
		    if ((ret =
			 meGetStringFromUser(prompt, option, 0, buff,
					     meBUF_SIZE_MAX)) > 0)
			fileNameCorrect(buff, evalResult, NULL);
		} else {
		    /*
		     * Note that buffer result cannot be used directly as
		     * in the process of updating the screen the result
		     * buffer can be used by meItoa so use a temp buffer
		     * and copy across. note that inputFileName above
		     * doesn't suffer from this as the function uses a
		     * temp buffer 
		     */
		    ret =
			meGetStringFromUser(prompt, option, defH, buff,
					    meBUF_SIZE_MAX);
		...
		if (ret & 0x04) {
		     ...
		     * help string 
		     */
		    if (meGetString(NULL, 0, 0, helpBuff, meBUF_SIZE_MAX)
			<= 0)
			return abortm;
		    helpStr = helpBuff;
		} else
		...
		return
		    meItoa((frameCur->store[row].
			    scheme[col] & meSCHEME_STYLE) /
			   meSCHEME_STYLES);
	    else {
...
#endif
	{
	    mlwrite(MWABORT | MWWAIT, (meUByte *) "[Unknown argument %s]",
		    tkn);
	    return abortm;
	}
	return evalResult;
    ...
    /*
     * If here then it is either TKDIR, TKLBL or TKNUL. Doesn't matter
     * which cause something has gone wrong and we return abortm 
     */
    return abortm;
}
...
meUByte        *
gtfun(meUByte * fname)
{				/* evaluate a function given name of
				 * function */
#if MEOPT_EXTENDED
    ...
    meUByte         arg1[meBUF_SIZE_MAX];	/* value of first argument 
						 */
    meUByte         arg2[meBUF_SIZE_MAX];	/* value of second
						 * argument */
	...
	mlwrite(MWABORT | MWWAIT, (meUByte *) "[Unknown function &%s]",
		fname);
	return abortm;
    }
#if MEOPT_EXTENDED
	...
	if ((ii <= 0) ||
	    ...
	    mlwrite(MWABORT | MWWAIT,
		    (meUByte *)
		    "[Failed to get argument for function &%s]", fname);
	    return abortm;
	}
	...
	default:
	    /*
	     * If here then it is either TKARG, TKFUN, TKDIR, TKSTR,
	     * TKLIT, TKLBL or TKNUL. Doesn't matter which cause &exist
	     * doesn't work on these, return NO 
	     */
	    return meLtoa(0);
	}
    case UFKBIND:
...
#endif
    case UFABS:
	return meItoa(labs((meAtoi(arg1))));
    case UFADD:
    ...
    case UFXIREP:
	{
	    ...
	    if (meRegexComp
		(&meRegexStrCmp, arg2,
		 (fnum == UFXREP) ? 0 : meREGEX_ICASE) != meREGEX_OKAY)
		return abortm;

	    slen = meStrlen(arg1);
	    for (;;) {
		ii = meRegexMatch(&meRegexStrCmp, arg1, slen, soff, slen,
				  (meREGEX_BEGBUFF | meREGEX_ENDBUFF));
		if (ii) {
			...
			if ((cc == '\\') && (*rr != '\0')) {
				...
				|| ((cc >= '0') && (cc <= '9')
				    && ((((int) cc) - '0') <=
					meRegexStrCmp.groupNo))) {
				cc = (cc == '&') ? 0 : (cc - '0');
				/*
				 * if start offset is < 0, it was a ? auto 
				 * repeat which was not found, therefore
				 * replace str == "" 
				 */
				if ((soff =
				     meRegexStrCmp.group[cc].start) >= 0) {
				    mlen =
					meRegexStrCmp.group[cc].end - soff;
				    if (dlen + mlen >= meBUF_SIZE_MAX)
					break;
				    if (cc)
					soff +=
					    meRegexStrCmp.group[0].start;
				    if (changeCase) {
					while (--mlen >= 0) {
					    cc = arg1[soff++];
					    if (changeCase > 0) {
    ...
    case UFINWORD:
	return (meLtoa(isWord(arg1[0])));
#endif
    case UFBMODE:
	return queryMode(arg1, frameCur->bufferCur->mode);
    case UFGMODE:
	return queryMode(arg1, globMode);
    case UFBAND:
	return (meItoa(meAtoi(arg1) & meAtoi(arg2)));
    case UFBNOT:
	return (meItoa(~meAtoi(arg1)));
    case UFBOR:
	return (meItoa(meAtoi(arg1) | meAtoi(arg2)));
    case UFBXOR:
	return (meItoa(meAtoi(arg1) ^ meAtoi(arg2)));
    case UFNOT:
	return (meLtoa(meAtol(arg1) == meFALSE));
    case UFAND:
	return (meLtoa(meAtol(arg1) && meAtol(arg2)));
    case UFOR:
	return (meLtoa(meAtol(arg1) || meAtol(arg2)));
    case UFEQUAL:
	return (meLtoa(meAtoi(arg1) == meAtoi(arg2)));
    case UFLESS:
	return (meLtoa(meAtoi(arg1) < meAtoi(arg2)));
    case UFGREATER:
	return (meLtoa(meAtoi(arg1) > meAtoi(arg2)));
    case UFSEQUAL:
	return (meLtoa(meStrcmp(arg1, arg2) == 0));
    case UFSLESS:
	return (meLtoa(meStrcmp(arg1, arg2) < 0));
    case UFSGREAT:
	return (meLtoa(meStrcmp(arg1, arg2) > 0));
    case UFISEQUAL:
	return (meLtoa(meStricmp(arg1, arg2) == 0));
#if MEOPT_EXTENDED
    case UFXSEQ:
	return (meLtoa
		(regexStrCmp
		 (arg1, arg2, meRSTRCMP_WHOLE | meRSTRCMP_USEMAIN) == 1));
    case UFXISEQ:
	return (meLtoa
		(regexStrCmp
		 (arg1, arg2,
		  meRSTRCMP_ICASE | meRSTRCMP_WHOLE | meRSTRCMP_USEMAIN) ==
		 1));
    case UFLDEL:
	{
	...
	{
	    if (!fileLookup
		(arg1, arg2, meFL_CHECKDOT | meFL_USESRCHPATH, evalResult))
		return errorm;
	    return evalResult;
	}
    case UFWHICH:
		  ...
		  get_flag:
			...
			*s = '\0';
			if (index < meBUF_SIZE_MAX - 16)
			     ...
			     */
			    index +=
				sprintf((char *) arg3 + index, (char *) p,
					nn);
			*s = c;
			break;
		    default:
			if (isDigit(c)) {
			    ...
			    arg3[index++] = c;	/* Just a literal char -
						 * pass in */
		    }
		} else if (index >= meBUF_SIZE_MAX)
		    break;
    ...
    case UFSTAT:
	{
	    ...
	    case 'i':
		{
		     ...
		     * Note: - this version differs from 't' as links have a file type of 'L' and
		     * ftp files return the file type not 'F' - The fourth value is the file
		     * attributes (-1 == not available) - The size has an upper int and lower int
		     * value to handle large files - The file modification time will be the 7th
		     * value (n.y.i.) 
		     */
		    meUByte         v1,
		                    v2,
			...
			*dd,
			...
			v1 = 'F';
			if (ffFileOp
			    (arg2, NULL, meRWFLAG_STAT | meRWFLAG_SILENT,
			     -1) > 0) {
			    v2 = evalResult[0];
			    dd = evalResult + 1;
			    if (*dd != '|') {
			...
			if (FileTimeToLocalFileTime(&stats.stmtime, &ftmp)
			...
			if (evalResult[0] != '\0') {
			    meStat          lstats;
			    v2 = 'L';
			    v3 = typeRet[getFileStats
					 (evalResult, 0, &lstats, NULL)];
			} else
			    v2 = typeRet[ftype];
	    ...
	    case 'm':
		{
			...
			sprintf((char *) evalResult,
				"%4d%3d%2d%2d%1d%2d%2d%2d  -",
				tmp->tm_year + 1900, tmp->tm_yday,
			...
			&& FileTimeToSystemTime(&ftmp, &tmp)) {
			doy =
			    meGetDayOfYear(tmp.wYear, tmp.wMonth,
					   tmp.wDay);
			sprintf((char *) evalResult,
				"%4d%3d%2d%2d%1d%2d%2d%2d%3d", tmp.wYear,
				(int) doy, tmp.wMonth, tmp.wDay,
				tmp.wDayOfWeek, tmp.wHour, tmp.wMinute,
				tmp.wSecond, tmp.wMilliseconds);
		    } else
#endif
			sprintf((char *) evalResult,
				"   -  - - -- - - -  -");
		...
		if (ftype != meFILETYPE_REGULAR)
		    ftype = -1;
		else if ((stats.stsizeHigh > 0)
			 || (stats.stsizeLow & 0x80000000))
		    ftype = 0x7fffffff;
		else
		 ...
		 */
		if ((ftype == meFILETYPE_NASTY)
		    || (ftype == meFILETYPE_HTTP)
		    || (ftype == meFILETYPE_FTP))
		    return meLtoa(0);

		/*
		 * if it doesnt exist - yes 
		 */
		if (ftype == meFILETYPE_NOTEXIST) {
                    return meLtoa(1);
                }
...
#if (defined _DOS) || (defined _WIN32)
		/*
		 * If directory, simulate unix style execute flag and
		 * return yes 
		 */
		if (ftype == meFILETYPE_DIRECTORY)
		    return meLtoa(1);
		 ...
		 */
		return
		    meLtoa((((stats.stuid == meUid)
			     && (stats.stmode & S_IXUSR))
			    || ((stats.stgid == meGid)
				&& (stats.stmode & S_IXGRP))
 ...
 */
int
unsetVariable(int f, int n)
{				/* Delete a variable */
    register meVarList *varList;	/* User variable pointer */
    register meVariable *vptr,
                   *prev;	/* User variable pointer */
    register int    vnum;	/* ordinal number of var refrenced */
    meRegister     *regs;
    meUByte         var[meSBUF_SIZE_MAX];	/* name of variable to
						 * fetch */
    meUByte        *vv;
    ...
    gmaLocalRegPtr = meRegCurr;
    vnum =
	meGetString((meUByte *) "Unset variable", MLVARBL, 0, var,
		    meSBUF_SIZE_MAX);
    regs = gmaLocalRegPtr;
    alarmState &= ~meALARM_VARIABLE;
    if (vnum <= 0)
...
int
setVariable(int f, int n)
{				/* set a variable */
    register int    status;	/* status return */
    meUByte         var[meSBUF_SIZE_MAX];	/* name of variable to
						 * fetch */
    meUByte         value[meBUF_SIZE_MAX];	/* value to set variable
						 * to */
    meRegister     *regs;
    ...
    alarmState |= meALARM_VARIABLE;
    status =
	meGetString((meUByte *) "Set variable", MLVARBL, 0, var,
		    meSBUF_SIZE_MAX);
    alarmState &= ~meALARM_VARIABLE;
    regs = gmaLocalRegPtr;
    if (status <= 0)
	...
	meStrcpy(value, meItoa(n));
	      ...
	      meGetString((meUByte *) "Value", MLFFZERO, 0, value,
			  meBUF_SIZE_MAX)) <= 0)
	return status;
...
{				/* describe a variable */
     ...
     */
    if ((ii =
	 meGetString((meUByte *) "Show variable", MLVARBL, 0, var,
		     meSBUF_SIZE_MAX)) <= 0)
	return ii;
    if ((((ii = getMacroTypeS(var)) != TKREG) && (ii != TKVAR)
	 && (ii != TKENV) && (ii != TKLVR) && (ii != TKCVR))
	|| ((ss = getval(var)) == NULL))
 ...
 */
static void
showVariable(meBuffer * bp, meUByte prefix, meUByte * name,
	     meUByte * value)
{
    ...
    buf[len++] = '"';
    len =
	expandexp(-1, value, meBUF_SIZE_MAX + meSBUF_SIZE_MAX + 16 - 2,
		  len, buf, -1, NULL,
		  meEXPAND_BACKSLASH | meEXPAND_FFZERO);
    buf[len++] = '"';
    buf[len] = '\0';
    addLineToEob(bp, buf);
}
...
listVariables(int f, int n)
{
    ...
    if ((wp =
	 meWindowPopup(BvariablesN,
		       (BFND_CREAT | BFND_CLEAR | WPOP_USESTR),
		       NULL)) == NULL)
	return meFALSE;
    bp = wp->buffer;
