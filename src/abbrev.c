/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com abbrev.c - Abbrevation
 * expansion routines Copyright (c) 1995-2001 Steven Phillips Copyright
...
setAbbrev(int f, int n, meAbbrev ** abrevPtr)
{
    ...
    if ((status =
	 meGetString((meUByte *) "Abbrev file", MLFILECASE, 0, buf,
		     meBUF_SIZE_MAX)) <= 0)
	return status;
	...
	if ((abrev == NULL) &&
	    ((abrev =
	      meMalloc(sizeof(meAbbrev) + meStrlen((char *) buf))) !=
	     NULL)) {
...
doExpandAbbrev(meUByte * abName, int abLen, meAbbrev * abrev)
{
	...
	if (!fileLookup
	    (abrev->fname, (meUByte *) ".eaf",
	     meFL_CHECKDOT | meFL_USESRCHPATH, fname)
	    || (ffReadFile(fname, meRWFLAG_SILENT, NULL, hlp, 0, 0, 0) ==
		meABORT))
	    return mlwrite(MWABORT,
			   (meUByte *) "[Failed to abbrev file %s]",
			   abrev->fname);
	abrev->loaded = 1;
    }
...
expandAbbrev(int f, int n)
{
    ...
    if (((frameCur->bufferCur->abbrevFile != NULL)
	 || (globalAbbrevFile != NULL)) && (--ii >= 0)
	&& isWord(frameCur->windowCur->dotLine->text[ii])) {
	len = 1;
	while ((--ii >= 0)
	       && isWord(frameCur->windowCur->dotLine->text[ii]))
	    len++;
	strncpy((char *) buf,
		(char *) &(frameCur->windowCur->dotLine->text[++ii]), len);
	if (((frameCur->bufferCur->abbrevFile != NULL)
	     &&
	     ((ii =
	       doExpandAbbrev(buf, len,
			      frameCur->bufferCur->abbrevFile)) !=
	      meFALSE)) || ((globalAbbrevFile != NULL)
			    &&
			    ((ii =
			      doExpandAbbrev(buf, len,
					     globalAbbrevFile)) !=
    ...
    /*
     * We used to insert a space if the expansion was not defined there
     * seems to be no good reason to do this. Instead bitch if we are not
     * in a macro and return a false condition. The error is not serious
     * enough to abort 
     */
