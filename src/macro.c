/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com macro.c - Macro Handling
 * routines. Copyright (C) 1995-2009 JASSPA (www.jasspa.com) This
...
executeString(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Enter string", MLFFZERO, 0, sbuf,
	 meBUF_SIZE_MAX) <= 0)
	return 0;
...
createMacro(meUByte * name)
{
    ...
    if ((name == NULL) &&
	(meGetString
	 ((meUByte *) "Macro name", MLCOMMAND | MLEXECNOUSER, 0, buff,
	  meBUF_SIZE_MAX) > 0) && (buff[0] != '\0'))
	name = buff;
	...
	if (idx < CK_MAX) {
	    mlwrite(MWABORT | MWPAUSE,
		    (meUByte *) "Error! can't re-define a base function");
	    meFree(hlp);
	    return NULL;
	}
	mac = getMacro(idx);
	...
	if ((cmdTableSize & 0x1f) == 0) {
	    ...
	    meCommand     **nt;
	    if ((nt =
		 (meCommand **) meMalloc((cmdTableSize + 0x20) *
					 sizeof(meCommand *))) == NULL)
		return NULL;
...
/*
 * macroDefine: Set up a macro buffer and flag to store all executed
 * command lines there 
 */
...
/*
 * macroFileDefine: Set up a macro buffer and flag to store all executed
 * command lines there 
 */
int
macroFileDefine(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Enter file", MLCOMMAND, 0, fname,
	 meBUF_SIZE_MAX) <= 0)
	return meFALSE;
...
helpBufferLoad(meBuffer * hbp)
{
	...
	if (!fileLookup
	    (helpFileName, NULL, meFL_CHECKDOT | meFL_USESRCHPATH, fname))
	    return mlwrite(MWABORT,
			   (meUByte *) "[Help file \"%s\" is not on-line]",
			   helpFileName);
	/*
...
findHelpItem(meUByte * item, int silent)
{
  ...
  try_again:
	...
	if (!silent)
	    mlwrite(MWABORT, (meUByte *) "[Can't find help on %s%s]", item,
		    sect);
	return meABORT;
    }
    if ((wp =
	 meWindowPopup(BhelpN, BFND_CREAT | BFND_CLEAR | WPOP_USESTR,
		       NULL)) == NULL)
	return meFALSE;
	...
	*ss = '\0';
	...
	if (cc == '|') {
		...
		if (sect[1] == '2') {
			...
			meStrcpy(line, "    ");
			ss = line + 4;
			for (ktp = &keytab[0]; ktp->code != ME_INVALID_KEY;
			     ktp++) {
			    if (ktp->index == ii) {
				*ss++ = '"';
...
helpItem(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Help on item", 0, 0, buf, meBUF_SIZE_MAX - 10) <= 0)
	return meFALSE;
...
helpCommand(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Help on command", MLCOMMAND, 0, buf,
	 meBUF_SIZE_MAX - 10) <= 0)
	return meFALSE;
    ss = buf + meStrlen(buf);
    meStrcpy(ss, "(2)");
...
helpVariable(int f, int n)
{
    ...
    if (meGetString
	((meUByte *) "Help on variable", MLVARBL, 0, buf,
	 meBUF_SIZE_MAX - 10) <= 0)
	return meFALSE;
    meStrcat(buf, "(5)");
    return findHelpItem(buf, 0);
...
/*
 * macroHelpDefine: Set up a macro help definition 
 */
int
macroHelpDefine(int f, int n)
{
	...
	return meABORT;
    if (meGetString
	((meUByte *) "Enter name", MLCOMMAND, 0, name + 4,
	 meBUF_SIZE_MAX - 4) <= 0)
	return meFALSE;
    sect[0] = '\0';
    if (meGetString((meUByte *) "Enter section", 0, 0, sect, 20) ==
	meABORT)
	return meFALSE;
    /*
     * and set the macro store pointers to it 
...
nameKbdMacro(int f, int n)
{
	...
	n = expandexp(lkbdlen, lkbdptr, meBUF_SIZE_MAX - 2, 16, buf, -1,
		      NULL,
...
userGetMacro(meUByte * buf, int len)
{
    ...
    if (meGetString
	((meUByte *) "Enter macro name ", MLCOMMAND, 2, buf, len) > 0) {
