/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com isearch.c - incremental
 * search routines. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This 
...
static meUByte *statusStr[5] = {
    (meUByte *) " [LOST]", (meUByte *) " [FOUND]",
	(meUByte *) " [INCOMPLETE]", (meUByte *) " [REGEX ERROR]",
	(meUByte *) " [SEARCHING]"
};

static          meInt
isearchGotoEnd(meUByte * patrn, int flags, int histNo, meShort * histLen,
	       SCANNERPOS * histPos)
{
    if (histLen[histNo] == 0)
	return meTRUE;
	...
	meInt           tmplno;
	meByte          cc;
	tmpline = frameCur->windowCur->dotLine;	/* Store the current
						 * position */
	tmpoff = frameCur->windowCur->dotOffset;	/* incase its not
							 * found */
	...
	if (iscanner(patrn, 0, flags, histPos + histNo + 1) <= 0) {
	    ...
	    frameCur->windowCur->dotLine = tmpline;	/* Reset the
							 * position */
...
static          meInt
isearchGotoStart(meUByte * patrn, int flags, int histNo, meShort * histLen,
		 SCANNERPOS * histPos)
{
    if (histLen[histNo] == 0)
	return meTRUE;
	...
	patrn[histLen[histNo]] = '\0';
	if (iscanner
	    (patrn, 0, (flags ^ (ISCANNER_BACKW | ISCANNER_PTBEG)),
	     histPos + histNo + 1) <= 0) {
	    patrn[histLen[histNo]] = cc;
	    frameCur->windowCur->dotLine = tmpline;	/* Reset the
							 * position */
 ...
 */
static          meInt
scanmore(meUByte * patrn, int flags, int histNo, meShort * histLen,
	 SCANNERPOS * histPos)
{
    int             sts;	/* search status */

    /*
     * Must flag as seaching, even if current status is meABORT cos if the 
     * char is a ']' then the search may continue 
     */
    ...
    /*
     * iscanner can return: -2 - regex class error -1 - regex error 0 -
     * Failed to find 1 - found 
     */
...
isearch(int flags)
{
     ...
     */
    srchPat[0] = '\0';
    histPos[0].startline = histPos[0].endline = frameCur->windowCur->dotLine;	/* Save 
										 * the 
										 * current 
										 * line 
										 * pointer 
										 */
    histPos[0].startoff = histPos[0].endoffset = frameCur->windowCur->dotOffset;	/* Save 
											 * the 
											 * current 
											 * line 
											 * pointer 
											 */
    histPos[0].startline_no = histPos[0].endline_no = frameCur->windowCur->dotLineNo;	/* Save 
											 * the 
											 * current 
											 * line 
											 * pointer 
  ...
  get_another_key:
    ...
    case CK_GOBOF:
#endif
    case CK_YANK:		/* C-y - yank the contents of the kill
				 * buffer */
	meGetKeyFirst = c;
	if ((status =
	     meGetString(isHeadStr, MLSEARCH | MLISEARCH, 0, srchPat,
			 meBUF_SIZE_MAX)) <= 0) {
	    restoreUseMlBinds();
	    return status;
	}
	isHeadStr[9] = ':';
	if (meGetKeyFirst < 0) {
	    status = scanmore(srchPat, flags, 0, histLen, histPos);	/* do 
									 * 1 
									 * search 
									 */
	...
	case CK_ABTCMD:	/* ^G : Abort input and return */
		...
		frameCur->windowCur->updateFlags |= WFMOVEL;	/* Say
								 * that
								 * we've
								 * moved */
	    }
	...
	case CK_FORSRCH:	/* If forward search */
	  ...
	  find_next:
	    ...
	    tmpline = frameCur->windowCur->dotLine;	/* Store the
							 * current
							 * position */
	    tmpoff = frameCur->windowCur->dotOffset;	/* incase its not
							 * found */
		...
		if ((histPos[histNo].startoff == histPos[histNo].endoffset)
		    && (histPos[histNo].startline_no ==
			histPos[histNo].endline_no)) {
		/*
		 * the search string has matched a zero length string,
		 * e.g. "^" so move the cursor position one character 
		 */
		if (flags & ISCANNER_BACKW)	/* if backward */
		...
		c = meTRUE;
	    if ((c > 0) && ((c = scanmore(srchPat, flags, histNo, histLen, histPos)) != meTRUE)) {	/* Start 
													 * the 
													 * search 
													 * again 
													 */
	...
	case CK_KILREG:	/* ^W : Kill the whole line? */
		...
		if (flags & ISCANNER_BACKW)
		    isearchGotoEnd(srchPat, flags, histNo, histLen,
				   histPos);
		...
		while (((inWord() == 0) == tt) && (cpos < meBUF_SIZE_MAX)) {
		    if (meLineGetLength(frameCur->windowCur->dotLine) ==
			...
			srchPat[cpos++] = '\\';
		    srchPat[cpos++] = c;
		    if (meWindowForwardChar(frameCur->windowCur, 1) ==
			meFALSE)
			break;
		}
		srchPat[cpos] = 0;	/* null terminate the buffer */
		memcpy(histPos + histNo + 1, histPos + histNo,
		       sizeof(SCANNERPOS));
		histNo++;
		...
		if (flags & ISCANNER_BACKW)
		    isearchGotoStart(srchPat, flags, histNo, histLen,
				     histPos);
		goto is_redraw;
	    }
	    break;
	...
	case CK_DELBAK:	/* ^H : backwards delete.  */
...
#if MEOPT_IPIPES
	    /*
	     * due to the dynamic nature of an active ipipe buffer, things 
		...
		if (histLen[histNo] == histLen[histNo + 1]) {
		     ...
		     */
		    if ((histPos[histNo].endline ==
			 histPos[histNo + 1].startline)
			&& (histPos[histNo].endoffset ==
			    histPos[histNo + 1].startoff)) {
			flags ^= ISCANNER_BACKW | ISCANNER_PTBEG;
			isearchGotoEnd(srchPat, flags, histNo, histLen,
				       histPos);
		    } else {
			if (histStt[histNo + 1] == meTRUE) {
			    isearchGotoStart(srchPat, flags, histNo,
					     histLen, histPos + 1);
			...
			isearchGotoStart(srchPat, flags, histNo, histLen,
					 histPos + 1);
			srchPat[cpos] = 0;
			if (((frameCur->windowCur->dotLine !=
			      histPos[histNo].startline)
			     || (frameCur->windowCur->dotOffset !=
				 histPos[histNo].startoff))
			    && (histStt[histNo] == meTRUE))
			    iscanner(srchPat, 0,
				     (flags ^
				      (ISCANNER_BACKW | ISCANNER_PTBEG)),
				     &(histPos[histNo]));
			isearchGotoEnd(srchPat, flags, histNo, histLen,
				       histPos);
		    }
	...
	default:		/* All other chars */
		 ...
		 */
		if (((c & ~(ME_SHIFT | ME_CONTROL | ME_ALT)) ==
		     (ME_SPECIAL | SKEY_pick))
		    || ((c & ~(ME_SHIFT | ME_CONTROL | ME_ALT)) ==
			(ME_SPECIAL | SKEY_drop)))
	  ...
	  isAddChar:
	    ...
	    if (cpos >= meBUF_SIZE_MAX) {	/* too many chars in
						 * string? */
		/*
		 ...
		 */
		status =
		    scanmore(srchPat, flags | ISCANNER_SCANMR, histNo,
			     histLen, histPos);
		if (TTbreakFlag)
	...
	if (histNo == HISTBUFSIZ - 1) {
	    /*
	     * history is full to bust, best thing to do is ditch the
	     * first half and thus make it half full. 
	     */
	    memcpy(histPos, histPos + (HISTBUFSIZ - (HISTBUFSIZ / 2)),
		   (HISTBUFSIZ / 2) * sizeof(SCANNERPOS));
	    memcpy(histLen, histLen + (HISTBUFSIZ - (HISTBUFSIZ / 2)),
		   (HISTBUFSIZ / 2) * sizeof(meShort));
	    memcpy(histStt, histStt + (HISTBUFSIZ - (HISTBUFSIZ / 2)),
		   (HISTBUFSIZ / 2) * sizeof(meUByte));
...
#if MEOPT_MWFRAME
    /*
     * Check the focus has not changed before we execute the extra
     * command, if it has then change focus 
     */
    {
	extern int      commandDepth;

	/*
	 * if the user has changed the window focus using the OS and this
	 * is the top level then change frames 
	 */
