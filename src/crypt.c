/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com crypt.c - Encryption
 * Routines. Copyright (C) 1986 Dana L. Hoggatt Copyright (C) 1988-2009
 * JASSPA (www.jasspa.com) This program is free software; you can
...
setBufferCryptKey(meBuffer * bp, meUByte * key)
{
	...
	if (meGetString
	    (keybuf, MLNOHIST | MLHIDEVAL, 0, keybuf,
	     meSBUF_SIZE_MAX) <= 0)
	    return meFALSE;
	key = keybuf;
    ...
    bp->cryptKey = NULL;
    if ((key[0] == 0) || ((bp->cryptKey = meStrdup(key)) == NULL))
...
meCrypt(register meUByte * bptr, register meUInt len)
{
	...
	if ((cc >= ' ') && (cc <= '~')) {
...
#else
	    /*
	     * We feed the upper few bits of the key back into itself.
	     * This assures us that the starting key affects the entire
	     * message. We go through some effort here to prevent the key
	     * from exceeding 29 bits.  This is so the aritmetic
	     * caluclation in a later statement, which impliments our
	     * autokey, won't overflow, making the key go negative.
	     * Machine behavior in these cases does not tend to be
	     * portable. 
	     */
	    ...
	    /*
	     * Down-bias the character, perform a Beaufort encipherment,
	     * and up-bias the character again.  We want key to be
	     * positive so that the left shift here will be more portable 
	     * and the mod95() faster 
	     */

	    cc = mod95((int) (key % 95) - (cc - ' ')) + ' ';

	    /*
	     * The salt will spice up the key a little bit, helping to
	     * obscure any patterns in the clear text, particularly when
	     * all the characters (or long sequences of them) are the
	     * same.  We do not want the salt to go negative, or it will
	     * affect the key too radically.  It is always a good idea to 
	     * chop off cyclics to prime values. 
	     */
	     ...
	     * text, and salt. * * I had to allow the older broken key
	     * calculation to let us * decrypt old files.  I hope to take
	     * this out eventually. 
	     */
	 ...
	 * assures us that the starting key affects the entire message. We 
	 * go through some effort here to prevent the key from exceeding
	 * 29 bits.  This is so the aritmetic caluclation in a later
	 * statement, which impliments our autokey, won't overflow, making 
	 * the key go negative. Machine behavior in these cases does not
	 * tend to be portable. 
	 */
	...
	/*
	 * Down-bias the character, perform a Beaufort encipherment, and
	 * up-bias the character again.  We want key to be positive so
	 * that the left shift here will be more portable and the
	 * mod255() faster 
	 */
	...
	/*
	 * Our autokey (a special case of the running key) is being *
	 * generated by a weighted checksum of clear text, cipher * text, 
	 * and salt. * * I had to allow the older broken key calculation
	 * to let us * decrypt old files.  I hope to take this out
	 * eventually. 
	 */
