/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com frame.c - Multiple frame
 * support. Copyright (C) 2002-2009 JASSPA (www.jasspa.com) This program 
...
meFrameEnlargeVideo(meFrame * frame, int rows)
{
	 ...
	 */
	if ((vs =
	     (meVideoLine *) meMalloc(rows * sizeof(meVideoLine))) == NULL)
	    return (meFALSE);

	/*
	 * Allocation successful. Reset the contents to zero and swap for
	 * the existing one 
	 */
...
meFrameChangeWidth(meFrame * frame, int ww)
{
	...
	/*
	 * Must extend the length of loopFrame->mlLine,
	 * loopFrame->mlLineStore, and all window mode lines 
	 */
	meWindow       *wp;	/* Temporary window pointer */
	meFrameLine    *flp;	/* Frame line pointer */
	meFrameLine     fl;	/* Temporary frame line */
	int             ii,
	                jj;	/* Local loop counters */
	meLine         *ml;
	meUByte        *mls;
	...
	/*
	 * Fix up the frame store by growing the lines. Do a safe grow
	 * where by we can recover if a malloc fails. 
	 */
	for (flp = loopFrame->store, ii = 0; ii < loopFrame->depthMax;
	     ii++, flp++) {
	    if ((fl.scheme =
		 meMalloc(ww * (sizeof(meUByte) + sizeof(meStyle)))) ==
		NULL)
		return meFALSE;
	     ...
	     * information and pad endings with valid data. Strictly we do 
	     * not need to do this for all platforms, however if it is
	     * safer if we make sure the data is valid. Resize is an
	     * infrequent operation and time is not critical here 
	     */
	    memcpy(fl.text, flp->text,
		   sizeof(meUByte) * loopFrame->widthMax);
	    memcpy(fl.scheme, flp->scheme,
		   sizeof(meScheme) * loopFrame->widthMax);
	    jj = ww;
	...
	loopFrame->video.lineArray[loopFrame->depth].line =
	    (loopFrame->mlLine = ml);
...
meFrameChangeDepth(meFrame * frame, int dd)
{
    ...
    /*
     * Only process if the window size is different from the current
     * window size. If we got here that is true. Go and get some more
     * screen space 
     */
	...
	/*
	 * Grow the Frame store depthwise do this safely so that we do
	 * not cause a crash at the video end. Grow the frame store
	 * first. Reallocate and then copy across the old information. 
	 */
	if ((flp =
	     (meFrameLine *) meMalloc(sizeof(meFrameLine) * dd)) == NULL)
	    return meFALSE;

	memcpy(flp, loopFrame->store,
	       sizeof(meFrameLine) * loopFrame->depthMax);
	 ...
	 */
	for (flp += loopFrame->depthMax, ii = loopFrame->depthMax; ii < dd;
	     ii++, flp++) {
	    if ((flp->scheme =
		 meMalloc(loopFrame->widthMax *
			  (sizeof(meUByte) + sizeof(meScheme)))) == NULL)
		return meFALSE;
     ...
     */
    loopFrame->video.lineArray[loopFrame->depth].flag = 0;	/* Decouple 
								 * the old 
								 * one */
    loopFrame->video.lineArray[loopFrame->depth].line = NULL;
    loopFrame->video.lineArray[dd - 1].flag = VFMESSL;	/* Bind in new
							 * message line */
...
frameChangeWidth(int f, int n)
{
	...
	/*
	 * n is a delta, add this to the current width to get the new
	 * width 
	 */
	n = frameCur->width + n;

    if ((n < 8) || (n > 400))	/* In range ?? */
	return mlwrite(MWABORT, (meUByte *) "Screen width %d out of range",
		       n);
...
frameChangeDepth(int f, int n)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "[Screen depth %d out of range]", n);
...
/*
 * initialize a frame, mallocing required video and framestore space. The 
 * frame depthMax and widthMax must be set 
 */
meFrame        *
meFrameInit(meFrame * sibling)
{
    ...
    if (((frame->video.lineArray =
	  meMalloc(frame->depthMax * sizeof(meVideoLine))) == NULL)
     ...
     */
    memset(frame->video.lineArray, 0,
	   frame->depthMax * sizeof(meVideoLine));
    ...
    /*
     * Frame Store storage The frame store hold's 'n' lines of video
     * information. The frame->store is an array of points to meFrameLine
     * structures which hold the physical line contents. Allocate lines
     * in the frame store to hold the video information. 
     */
    if ((frame->store =
	 meMalloc(sizeof(meFrameLine) * frame->depthMax)) == NULL)
	return NULL;
    for (flp = frame->store, ii = 0; ii < frame->depthMax; ii++, flp++) {
	if ((flp->scheme =
	     meMalloc(frame->widthMax *
		      (sizeof(meUByte) + sizeof(meStyle)))) == NULL)
	    return NULL;
...
/*
 * meFrameInitWindow Creates the initial window for a frame, would be
 * part of meFrameInit except the order of events at startup make this
...
meFrameFree(meFrame * frame)
{
     ...
     */
    for (vvptr = frame->video.next; vvptr != NULL;
	 vvptr = frame->video.next) {
...
{
    /*
     * use next frame to get a replacement frame if required, always try
     * to get an internal (sibling) replacement frame as if one exists
     * then the window must not be deleted 
     */
...
meFrameMakeCur(meFrame * frame, int quiet)
{
	...
	if (frame->flags & meFRAME_HIDDEN) {
	    ...
	    frame->flags =
		(frame->
		 flags & ~(meFRAME_HIDDEN | meFRAME_NOT_FOCUS)) | (ff->
								   flags &
								   meFRAME_NOT_FOCUS);
	    ff->flags |= meFRAME_HIDDEN;
	}
	...
	if (frameOld->mlStatus & (MLSTATUS_RESTORE | MLSTATUS_KEEP)) {
	    ...
	    /*
	     * move the current mlstatus over to the new frame, this
	     * should usually be zero but in some cases (e.g. notes) the
	     * swapping of frames can occur during a ml prompt. There are
	     * potantial side effects to doing this (e.g. what if its an
	     * OSD dialog and the position is outside the the new frame),
	     * so this should be avoid if at all possible 
	     */
	    if (frameOld->mlStatus & MLSTATUS_RESTORE) {
		frame->mlColumn = frameOld->mlColumnStore;
		mlStatus =
		    (frameOld->
		     mlStatus & ~MLSTATUS_RESTORE) | MLSTATUS_KEEP;
	    ...
	    frame->mlStatus = 0;
	    mlwrite(((mlStatus & MLSTATUS_POSML) ? MWSPEC | MWUSEMLCOL |
		     MWCURSOR : MWSPEC), mlStr);
	...
	if (frame->mainId == frameOld->mainId)
	    ...
	    /*
	     * A quiet operation will not change the terminal to perform
	     * the swap 
	     */
...
frameCreate(int f, int n)
{
    ...
    /*
     * these functions cannot use meMalloc as they are used to init ME, so 
     * if they fail we must warn of the malloc failure 
     */
    if (((frame = meFrameInit(sf)) == NULL) ||
	(meFrameInitWindow(frame, frameCur->bufferCur) <= 0))
	return mlwrite(MWCURSOR | MWABORT | MWWAIT,
		       (meUByte *) "[Failed to create new frame]");
#if MEOPT_MWFRAME
    if (sf != NULL)
#endif
