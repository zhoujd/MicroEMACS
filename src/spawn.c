/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com spawn.c - Routines for
 * launching external process. Copyright (C) 1988-2009 JASSPA
 * (www.jasspa.com) This program is free software; you can redistribute
...
meShell(int f, int n)
{
    ...
    register char  *cp;
#endif
    meUByte         path[meBUF_SIZE_MAX];	/* pathfrom where to
						 * execute */
    int             cd,
                    ss;
	...
	case 0:
	    ...
	    mlwrite(MWABORT, (meUByte *) "exec failed, %s",
		    strerror(errno));
	    meExit(127);
	case -1:
	    ss = mlwrite(MWABORT, (meUByte *) "exec failed, %s",
			 strerror(errno));
	default:
	    ss = meTRUE;
	}
...
doShellCommand(meUByte * cmdstr, int flags)
{
     ...
     * our mode. We might check all states, but with hidden things like
     * flashing etc. its not worth the effort - sorry, you do it if you
     * want. 
     */
    TTopen();
#endif
...
{
    meUByte        *cmdstr,
                    cmdbuff[meBUF_SIZE_MAX + 20];
    meBuffer       *bp;

    /*
     * get the line wanted 
     */
    if ((meGetString((meUByte *) "System", 0, 0, cmdbuff, meBUF_SIZE_MAX))
	<= 0)
	return meABORT;
    if (n & LAUNCH_BUFCMDLINE) {
	if ((bp = bfind(cmdbuff, 0)) == NULL)
	    return mlwrite(MWABORT, (meUByte *) "[%s: no such buffer]",
			   cmdbuff);
...
{
    typedef         HANDLE(WINAPI * CREATETOOLHELP32SNAPSHOT) (DWORD,
							       DWORD);
    typedef         BOOL(WINAPI * PROCESS32FIRST) (HANDLE,
						   LPPROCESSENTRY32);
    typedef         BOOL(WINAPI * PROCESS32NEXT) (HANDLE,
						  LPPROCESSENTRY32);
	...
	if ((libHandle = LoadLibrary("kernel32")) != NULL) {
	    procCreateSnapshot =
		(CREATETOOLHELP32SNAPSHOT) GetProcAddress(libHandle,
							  "CreateToolhelp32Snapshot");
	    if (((procGetFirst =
		  (PROCESS32FIRST) GetProcAddress(libHandle,
						  "Process32First")) ==
		 NULL)
		||
		((procGetNext =
		  (PROCESS32NEXT) GetProcAddress(libHandle,
						 "Process32Next")) ==
		 NULL))
...
ipipeKillBuf(meIPipe * ipipe, int type)
{
	...
	if (type == 2) {
	    ...
	    {
		/*
		 * on windows surprise surprise writing C-c to stdin does
		 * not work very well, it does not have the same effect.
		 * There are a few extra things we can do and hte
		 * combination of doing them all does seem to have the
		 * right effect - but this may be very machine dependant.
		 * One thing we can do is make the child process window
		 * have the current input focus and then send the key
		 * strokes down to it. But to do this we must have a
		 * foreground window and we must attatch the thread to the
		 * current thread so we can steal the current window. 
		 */
		HWND            foreWnd,
		                chldWnd;
		DWORD           foreThread,
		...
		if (((foreWnd = GetForegroundWindow()) != NULL) &&
			...
			/*
			 * success, now we can start sending the string
			 * down 
			 */
			BYTE            scanCodeCtrl,
			                scanCodeC;
			...
			keybd_event(VK_CONTROL, scanCodeCtrl,
				    KEYEVENTF_KEYUP, 0);

			/*
			 * call halt here and let the other process get
			 * the keys - the length of the sleep is a fine
			 * balance, too small and the child process will
			 * not get the Ctrl-C, but if its to large the
			 * user could press a key, entering a char or
			 * changing the state of the Ctrl or Shift keys -
			 * these events should go to the original foreWnd
			 * so that app will get very confused! So the
			 * sleep should be as small as possible 
			 */
			Sleep(50);
		     ...
		     */
		    if (foreThread)
			AttachThreadInput(GetCurrentThreadId(), foreThread,
					  meFALSE);
		    if (chldThread)
			AttachThreadInput(GetCurrentThreadId(), chldThread,
					  meFALSE);
		}

		/*
		 * one other thing we can do is to send a Ctrl-C event if
		 * we are trying * to write a C-c (0x03), but we will
		 * write the string as well 
		 */
		GenerateConsoleCtrlEvent(CTRL_C_EVENT, ipipe->processId);

		/*
		 * if we've succeeded in sending the Ctrl-C keys and the
		 * event then quit now - if they don't work, nothing will 
		 */
		if (success)
		    return;
	    }
	...
	if (ipipe->pid > 0) {
		...
		/*
		 * cannot terminate the tree - we can only Terminate this
		 * one 
		 */
...
ipipeRemove(meIPipe * ipipe)
{
	...
	if (GetExitCodeThread(ipipe->thread, &exitCode)
...
readFromPipe(meIPipe * ipipe, int nbytes, meUByte * buff)
{
    ...
    if ((PeekNamedPipe(ipipe->rfd, (LPVOID) NULL, (DWORD) 0,
		       (LPDWORD) NULL, &bytesRead,
		       (LPDWORD) NULL) != meTRUE) || (bytesRead <= 0))
	return 0;
...
ipipeRead(meIPipe * ipipe)
{
	...
	ipipe->rfd =
	    accept(ipipe->rfd, (struct sockaddr *) &cssa, (void *) &ii);
    }
#endif
#endif
    meAnchorGet(bp, 'I');
	...
	/*
	 * Work out which windows are locked to the current cursor
	 * position 
	 */
	meWindow       *wp;
    ...
    /*
     * This is a quick sanity check which is needed if the buffer has been 
     * changed by something. If curRow becomes greater than dotLineNo the
     * vertScroll becomes negative and things go wrong. Discovered problem 
     * when using gdb mode as the gdb input handler kills ^Z^Z lines
     * making curRow > dotLineNo. 
     */
	...
	case meCHAR_NL:
		...
		&& (curRow + 1 < ipipe->noRows)) {
		/*
		 * if in over-write mode and not at the bottom, move
		 * instead 
		 */
		nb = curRow + 1;
		na = 0;
	...
	case 27:
		...
		if (cc == '[') {
		  get_another:
		    if (getNextCharFromPipe
			(ipipe, cc, rbuff, curROff, curRRead)) {
			if (isDigit(cc)) {
			    gotN |= 1;
			    na = na * 10 + (cc - '0');
			    goto get_another;
			...
			case 'H':
			  ...
			  move_cursor_pos:
			    ...
			    bp->dotLine = lp_old;
			    meStrcpy(buff, lp_old->text);
			    meBufferStoreLocation(lp_old, (meUShort) len,
						  bp->dotLineNo);
			    na -= meStrlen(buff);
			    p1 = buff + len;
			    if (na > 0) {
			...
			case 'm':
			    /*
			     * These are font changes, like bold etc. We
			     * won't do anything with them for now 
			     */
			    break;
			case 'n':
			    {
			...
			default:
			  cant_handle_this:
			    ...
			    if (nb < 0)
				printf
				    ("Don't cope with term code \\E[%s%d%c\n",
				     (gotQ) ? "?" : "", na, cc);
			    else
				printf
				    ("Don't cope with term code \\E[%s%d;%d%c\n",
				     (gotQ) ? "?" : "", nb, na, cc);
#endif
			    break;
			}
	...
	default:
		...
		p1[1] = '\0';
		     ...
		     && (get_tab_pos(curOff, bp->tabWidth) == 0)) {
		/*
		 * theres a strangeness with vt100 tab as it doesn't seem
		 * to erase the next character and seems to be used (at
		 * least by tcsh) to move the cursor one to the right. So
		 * catch this special case of one character move. NOTE the 
		 * previous else if checked there is another char. 
		 */
		p1++;
		curOff++;
		break;
...
#if MEOPT_CLIENTSERVER
    /*
     * the unix client server trashed the rfd at the top of this function
     * due to the way sockets are handled. But the read handle is the same 
     * as the write so its trivial to restore 
     */
    if (ipipe->pid == 0)
    ...
    /*
     * as soon as the BLOCK of sigchld is removed, if the process has
     * finished while reading then it will get registered now, this fact
     * has to be taken into acount and handled carefully. If not typically 
     * the [EXIT] line will be missed if signals still blocked during the
     * execution of the ipipe function which leads to side effects like
     * poor cursor blink etc. 
     */
    meSigRelease();
#endif
	 ...
	 */
	execBufferFunc(bp, bp->ipipeFunc,
		       (meEBF_ARG_GIVEN | meEBF_USE_B_DOT | meEBF_HIDDEN),
		       (ii >= 0));
...
ipipeWrite(int f, int n)
{
     ...
     */
    if ((ss =
	 meGetString((meUByte *) "String", 0, 0, buff,
		     meBUF_SIZE_MAX)) <= 0)
	return ss;
...
ipipeSetSize(meWindow * wp, meBuffer * bp)
{
    ...
    if (((ipipe->noRows != wp->textDepth) ||
	 (ipipe->noCols != wp->textWidth - 1)) && (ipipe->pid > 0)) {
	int             ii;
	...
	/*
	 * Check the window is displaying this buffer before we mess with
	 * the window settings 
	 */
...
#ifdef _UNIX
/*
 * allocatePty; Allocate a pty. We use the old BSD method of searching for 
 * a pty, once we have aquired one then we look for the tty. Return the
 * name of the tty to the caller so that it may be opened. 
 */
...
allocatePty(meUByte * ptyName)
{
     ...
     * the usual sequence - for example, on HP9000/S700 systems, there are 
     * no pseudoterminals with names ending in 'f'.  So we wait for three
     * failures in a row before deciding that we've reached the end of the 
     * ptys.  
     */
    int             failed_count = 0;
...
#endif
		    /*
		     * If we can read and write the tty then it is not in
		     * use. 
		     */
		    if (access((char *) ptyName, W_OK | R_OK) != 0) {
			/*
			 * tty in use, close down the pty and try the next 
			 * one 
			 */
			close(fd);
			continue;
 ...
 * process. We restore the settings from our initial save of the
 * environmet. 
 */
static void
childSetupTty(void)
...
static int
doIpipeCommand(meUByte * comStr, meUByte * path, meUByte * bufName,
	       int ipipeFunc, int flags)
{
    meIPipe        *ipipe;
    meBuffer       *bp;
     ...
     */
    if ((rr =
	 WinLaunchProgram(comStr, (LAUNCH_IPIPE | flags), NULL, NULL,
			  ipipe, NULL)) <= 0) {
	if (cd)
	    meChdir(curdir);
	free(ipipe);
	...
	/*
	 * On the BSD systems we open the tty prior to the fork. If this
	 * is a later POSIX platform then we will expect O_NOCTTY to exist 
	 * and we open the tty with O_NOCTTY. Do not let this terminal
	 * become our controlling tty. This prevents an application from
	 * unintentionally aquiring the controlling terminal as a side
	 * effect of the open. 
	 */
#if (defined O_NOCTTY)
     ...
     * apply these on the open, BSD systems then it is not possible to
     * apply these on the open. For ease of code maintainance then we
     * apply these afterwards usig fnctl. POSIX calls for O_NONBLOCK. BSD 
     * systems and earlier UNIX systems are O_NDELAY. Pick whichever is
     * defined - they are typically defined to the same thing if both
     * exist Note that these settings apply irrespective of whether we
     * are dealing with a PTY or PIPE 
     */
#ifdef O_NONBLOCK
    if (fds[0] > 0)
	...
	/*
	 * Under BSD then we allocate a dummy tty and then immediatly shut 
	 * it. This has the desired effect of dissassociating the terminal 
	 */
	if (ptyFp >= 0) {
	    /*
	     * Under BSD 4.2 then we have to break the tty off. We make a
	     * dummy call to open a tty and then immediately close it.
	     * This was fixed on BSD4.3, but the same technique works so
	     * continue to use it !! 
	     */
	    int             tempFp;
	...
	/*
	 * Put the process into parent group 0. Note that setsid() does
	 * the * same job under SVR4. 
	 */
	setpgrp(0, 0);		/* BSD */
#else
	/*
	 * Under POSIX.1 environments then simply use setsid() to
	 * dissassociate from the terminal. This will also sort out the
	 * group ID's groups. 
	 */
	 ...
	 * debatable if we actually need this because setsid() might do
	 * it, however no harm will come from re-assigning the parent
	 * group. 
	 */
#ifdef _SVID
	 ...
	 * disciplines for communication, especially if we are a pty
	 * otherwise we will get some problems with the shell. For simple
	 * pipes we do not need to bother. 
	 */
...
#endif				/* defined (NTTYDISC) && defined
				 * (TIOCSETD) */

	/*
	 * The child process has inherited the parent signals. Ensure that 
	 * all of the signals are reset to their correct default value 
	 */
	...
	/*
	 * On solaris (this is POSIX I believe) then push the line
	 * emulation modes 
	 */
#ifdef _SUNOS
	if (ptyFp >= 0) {
...
#else
	/*
	 * We need to push the environment variable, however in order to
	 * do this then we need to supply the absolute pathname of the
    ...
    if ((ipipe->bp = bp) == NULL) {
	ipipeRemove(ipipe);
	return mlwrite(MWABORT, (meUByte *) "[Failed to create %s buffer]",
		       bufName);
    }
    /*
     * setup the buffer 
	...
	if ((flags & LAUNCH_SILENT)
	    || ((wp = meWindowPopup(bp->name, 0, NULL)) == NULL))
	    wp = frameCur->windowCur;
	/*
	 * while executing the meWindowPopup function the ipipe could have 
	 * exited so check 
	 */
	if (ipipes == ipipe) {
	    ipipeSetSize(wp, bp);
		 ...
		 */
		execBufferFunc(bp, bp->ipipeFunc,
			       (meEBF_ARG_GIVEN | meEBF_USE_B_DOT |
				meEBF_HIDDEN), 1);
	}
    }
    /*
...
ipipeCommand(int f, int n)
{
    ...
    meUByte         lbuf[meBUF_SIZE_MAX],
                   *cl;		/* command line send to shell */
    meUByte         nbuf[meBUF_SIZE_MAX],
                   *bn;		/* buffer name */
     ...
     */
    if ((ss =
	 meGetString((meUByte *) "Ipipe", 0, 0, lbuf,
		     meBUF_SIZE_MAX)) <= 0)
	return ss;
    if (n & LAUNCH_BUFCMDLINE) {
    ...
    return doIpipeCommand(cl, pbuf, bn, ipipeFunc,
			  (n & LAUNCH_USER_FLAGS));
}
 ...
 */
int
doPipeCommand(meUByte * comStr, meUByte * path, meUByte * bufName,
	      int ipipeFunc, int flags)
{
	...
	*dd++ = '>';
    ...
    /*
     * Must flag to our sigchild handler that we are running a piped
     * command otherwise it will call waitpid with -1 and loose the exit
...
#ifdef _UNIX
    ret =
	meBufferInsertFile(bp, NULL, meRWFLAG_SILENT | meRWFLAG_PRESRVFMOD,
			   0, 0, 0);
    /*
...
#else
    ret =
	meBufferInsertFile(bp, filnam,
			   meRWFLAG_SILENT | meRWFLAG_PRESRVFMOD, 0, 0, 0);
    /*
...
pipeCommand(int f, int n)
{
     ...
     */
    if ((ss =
	 meGetString((meUByte *) "Pipe", 0, 0, lbuf, meBUF_SIZE_MAX)) <= 0)
	return ss;
    if (n & LAUNCH_BUFCMDLINE) {
	if ((bp = bfind(lbuf, 0)) == NULL)
	    return mlwrite(MWABORT, (meUByte *) "[%s: no such buffer]",
...
meFilter(int f, int n)
{
    ...
    register meBuffer *bp;	/* pointer to buffer to zot */
    meUByte         line[meBUF_SIZE_MAX];	/* command line send to
						 * shell */
     ...
     */
    if ((s =
	 meGetString((meUByte *) "Filter", 0, 0, line,
		     meBUF_SIZE_MAX)) <= 0)
	return s;
	...
	if (errno)
	    mlwrite(MWCURSOR | MWWAIT, (meUByte *) "exit status %d ",
		    exitstatus);
	else
	    mlwrite(MWCURSOR | MWWAIT,
		    (meUByte *) "exit status %d, errno %d ", exitstatus,
		    errno);
    }
    TTopen();
    sgarbf = meTRUE;
	...
	bp->fileName = filnam2;
	if ((bclear(bp) <= 0) ||
	    ((frameCur->bufferCur->intFlag |=
	      BIFFILE), (swbuffer(frameCur->windowCur,
				  frameCur->bufferCur) <= 0)))
	    s = meFALSE;
    }
    /*
     * reset file name 
