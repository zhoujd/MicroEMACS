/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com region.c - Region (dot to 
 * cursor) routines. Copyright (C) 1988-2009 JASSPA (www.jasspa.com)
...
getregion(register meRegion * rp)
{
	...
	if (frameCur->windowCur->dotOffset <
	    ...
	    rp->size =
		(long) (frameCur->windowCur->markOffset -
			frameCur->windowCur->dotOffset);
	} else {
	    rp->offset = frameCur->windowCur->markOffset;
	    rp->size =
		(long) (frameCur->windowCur->dotOffset -
			frameCur->windowCur->markOffset);
	}
	...
	size =
	    frameCur->windowCur->markOffset + meLineGetLength(lp) - size -
	...
	size =
	    frameCur->windowCur->dotOffset + meLineGetLength(lp) - size +
	    frameCur->windowCur->dotLineNo -
	    frameCur->windowCur->markLineNo;
    }
    rp->line = lp;
...
copyRegion(int f, int n)
{
	...
	left +=
	    meBufferRegionExpandNarrow(frameCur->bufferCur, &line,
				       region.offset, elp, eoffset, 0);
...
killRectangle(int f, int n)
{
	...
	while (ii < kk) {
		 ...
		 */
		if (meLineGetChar(frameCur->windowCur->dotLine, ii) ==
		    meCHAR_TAB)
		    off[ii] -= lspc;
		coff = soff;
		break;
	...
	if (coff < soff) {
		...
		if ((ll = off[jj]) != 0) {
		    ...
		    if ((coff + ll) > eoff) {
			/*
			 * as we can convert tabs to spaces, delete and
			 * convert 
			 */
			if (cc == meCHAR_TAB) {
			    lspc += coff + ll - eoff;
		    ...
		    if (cc == meCHAR_TAB) {
			/*
			 * convert tabs to spaces for better column
			 * support 
			 */
			do
			    *kstr++ = ' ';
	    ...
	    if (coff < eoff) {
		/*
		 * line too short to get to the end point, fill with
		 * spaces 
		 */
		coff = eoff - coff;
		memset(kstr, ' ', coff);
...
yankRectangleKill(struct meKill *pklist, int soff, int notLast)
{
	...
	while (*ss != '\0') {
		...
		if ((coff + off[jj]) > soff) {
		    /*
		     * if its a tab we can remove the tab and replace with 
		     * spaces 
		     */
		    if (meLineGetChar(frameCur->windowCur->dotLine, jj) ==
			meCHAR_TAB) {
			ldel = 1;
	...
	frameCur->windowCur->dotOffset =
	    ((size_t) dd -
	     (size_t) meLineGetText(frameCur->windowCur->dotLine));
    }
    return meTRUE;
}
