/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com display.c - Display
...
/*
 * Poke screen flags Poke screen maintains the rectangular extents of the
 * region of the screen that has been damaged. These screen areas are
 * fixed on the next invocation to update(). Note that we only initialise
 * the flag, the other fields should be automatically initialised since we 
 * start in a garbaged state. Note that all poke extents are 'inclusive'
 * values. 
 */
...
convertUserScheme(int n, int defaultStyle)
{
	...
	mlwrite(MWABORT | MWPAUSE,
		(meUByte *) "[Invalid color-scheme index %d]", n);
	return defaultStyle;
    }
...
shilightWindow(meWindow * wp)
{
    ...
    /*
     * Hilight selection has been modified. Work out the start and end
     * points 
     */
	...
	if (wp->updateFlags & WFSELDRAWN) {
	    ...
	    meUShort        sline,
	                    eline;	/* physical screen line to update */

	    /*
	     * Determine the video line position and determine the video
	     * block that is being used. 
	     */
	...
	/*
	 * Determine the video line position and determine the video block 
	 * that is being used. 
	 */
	vptr = wp->video->lineArray;	/* Video block */
	sline = wp->frameRow;	/* Start line */
	...
	while (sline < eline) {
		...
		if (lineNo == selhilight.sline) {
			...
			vptr[sline].flag =
			    (vflag & ~VFSHMSK) | (VFSHBEG | VFSHEND |
						  VFCHNGD);
		    else
			vptr[sline].flag =
			    (vflag & ~VFSHMSK) | (VFSHBEG | VFCHNGD);
		} else if (lineNo == selhilight.eline)
		    vptr[sline].flag =
			(vflag & ~VFSHMSK) | (VFSHEND | VFCHNGD);
		else if ((vflag & VFSHALL) == 0)
		    vptr[sline].flag =
			(vflag & ~VFSHMSK) | (VFSHALL | VFCHNGD);
	    }
	    /*
	     * on to the next one 
	     */
...
showRegion(int f, int n)
{
	...
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[No current region]");
    if (((absn == 2) || (n == 4))
	&& (selhilight.bp != frameCur->bufferCur))
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[Current region not in this buffer]");
    switch (n) {
    case -3:
	selhilight.bp = frameCur->bufferCur;	/* Select the current
						 * buffer */
	selhilight.flags = SELHIL_ACTIVE | SELHIL_CHANGED;
	selhilight.markOffset = frameCur->windowCur->dotOffset;	/* Current 
								 * mark
								 * offset */
	selhilight.markLineNo = frameCur->windowCur->dotLineNo;	/* Current 
								 * mark
								 * line
								 * number */
	selhilight.dotOffset = frameCur->windowCur->dotOffset;	/* Current 
								 * mark
								 * offset */
	selhilight.dotLineNo = frameCur->windowCur->dotLineNo;	/* Current 
								 * mark
								 * line
								 * number */
	break;
	...
	if (selhilight.bp == frameCur->bufferCur) {
	     ...
	     * check for m >= dot > d 
	     */
	    else if (((selhilight.markLineNo <
		       frameCur->windowCur->dotLineNo)
		      ||
		      ((selhilight.markLineNo ==
			frameCur->windowCur->dotLineNo)
		       && (selhilight.markOffset <=
			   frameCur->windowCur->dotOffset)))
		     &&
		     ((selhilight.dotLineNo >
		       frameCur->windowCur->dotLineNo)
		      ||
		      ((selhilight.dotLineNo ==
			frameCur->windowCur->dotLineNo)
...
updCursor(register meWindow * wp)
{
	...
	jj = 0;
	/*
	 * must reset the dotCharOffset if its changed cos the flag will
	 * be lost for next time! Nasty bug!! Must check the line is still 
	 * dotp if we are to keep it cos it could have been freed. 
	 */
	if ((wp->dotCharOffset->next != wp->dotLine)
	    || (wp->dotLine->flag & meLINE_CHANGED))
	    wp->dotCharOffset->next = NULL;
	...
	int             leftMargin;	/* Base left margin position
					 * (scrolled) */
	 ...
	 */
	     ...
	     */
	    ii = ((jj - leftMargin - wp->textWidth) % (wp->textWidth -
						       (wp->
							marginWidth << 1)))
		+ wp->marginWidth;
	    jj -= ii;		/* Screen offset */
	 ...
	 */
	    ...
	    /*
	     * Check to see if a separate line scroll is in effect and can 
	     * be knocked off because the text is on screen. 
	     */
	...
	/*
	 * must make sure the disLineBuff is at least as long as the
	 * current scroll otherwise if the horizontal scroll is set to 2
	 * and the first line is short updateline will scribble as it
	 * assumes the buffer is long enough 
	 */
	...
	if (wp->horzScrollRest) {
	     ...
	     * Reset scroll column 
	     */
	    wp->horzScrollRest = 0;	/* Set scroll column to base
					 * position */
	...
	if (wp->horzScrollRest != (int) jj) {
	     ...
	     * Reset scroll column 
	     */
	    wp->horzScrollRest = (meUShort) jj;	/* Set scroll column to
						 * base position */
	    wp->updateFlags |= WFREDRAW;	/* Force a screen update */
	}
	break;
...
updateline(register int row, register meVideoLine * vp1, meWindow * window)
{
	...
	if (meLineIsSchemeSet(vp1->line)) {
	    scheme =
		window->buffer->lscheme[meLineGetSchemeIndex(vp1->line)];
	    /*
	     * We have to assume this line is an exception and the hilno & 
	     * bracket for the next line should be what this line would
	     * have been 
	     */
	     ...
	     * isWordMask to find word boundaries. Context save the
	     * existing word mask and restore from the buffer. Do the
	     * hilighting and then restore the previous setting. 
	     */
...
#endif
	{
	  ...
	  hideLineJump:
		...
		if (flag & VFSHALL) {
		    if (lineLen > 0)
			blkp[0].column =
			    renderLine(s1, lineLen, 0, window->buffer);
		    else
			blkp[0].column = 0;
		    blkp[0].scheme = scheme + meSCHEME_SELECT;
		    noColChng = 1;
		} else {
		    register meUShort wid,
		                    len;
			...
			if (selhilight.eoff > len) {
			     ...
			     */
			    wid =
				renderLine(s1 + len, selhilight.eoff - len,
					   wid, window->buffer);
			    blkp[noColChng].scheme =
		    ...
		    if (lineLen > len)
			wid =
			    renderLine(s1 + len, lineLen - len, wid,
				       window->buffer);
		    blkp[noColChng].column = wid;
		    noColChng += 1;
		}
	...
	s1 = disLineBuff;
	{
		...
		scroll = window->horzScrollRest;	/* Hard window
							 * scroll */
	    if (scroll != 0) {
		register meUShort ii;

		/*
		 * Line is scrolled. The effect we want is if any text at
		 * all is on the line we place a dollar at the start of
		 * the line in the last highlighting colour. If the line
		 * is empty then we do not want the dollar. Only process
		 * the line if it is not empty, this ensures that we do
		 * not insert a dollar where not required. 
		 */
		if (blkp->column > 0) {
		    s1 += scroll;
		    ...
		    /*
		     * set the first char to the truncate '$' and set the
		     * scheme 
		     */
	...
	if (blkp[noColChng - 1].column >= ncol) {
	    while ((noColChng > 1)
		   && (blkp[noColChng - 2].column >= (ncol - 1)))
		noColChng--;
		...
		/*
		 * remove the fonts as these can effect the next char
		 * which will probably be the scroll bar 
		 */
	    ...
	    blkp[noColChng].scheme =
		scheme | (blkp[noColChng - 1].
			  scheme & (meSCHEME_CURRENT | meSCHEME_SELECT));
	    noColChng++;
	} else {
	    /*
	     * An extra space is added to the last column so that the
	     * cursor will be the right colour 
	     */
		...
		if (((noColChng == 1) && (blkp[0].column == 0)) ||
		    ((noColChng > 1)
			 ...
			 blkp[noColChng - 2].column))) {
		    blkp[noColChng - 1].column++;
		    blkp[noColChng - 1].scheme =
			meSchemeSetNoFont(blkp[noColChng - 1].scheme);
		} else {
		    blkp[noColChng].column =
			blkp[noColChng - 1].column + 1;
		    blkp[noColChng].scheme =
			meSchemeSetNoFont(blkp[noColChng - 1].scheme);
		    noColChng++;
		}
	...
	if (flag & VFMODEL)
	    blkp->scheme =
		mdLnScheme +
		((flag & VFCURRL) ? meSCHEME_CURRENT : meSCHEME_NORMAL);
#if MEOPT_OSD
	...
	for (cno = 0; cno < noColChng; cno++) {
	    ...
	    /*
	     * Output the character in the specified colour. Maintain the
	     * frame store 
	     */
	...
	if (meStyleCmpBColor
	    (meSchemeGetStyle(vp1->eolScheme), meSchemeGetStyle(scheme))) {
	    ii = ncol;
	...
	if (meSystemCfg & meSYSTEM_FONTFIX) {
		...
		spFlag = 0;
		/*
		 * Maintain the frame store and copy the string into the
		 * frame store with the colour information copy a space in 
		 * place of special chars, they are drawn separately after 
		 * the XDraw, the spaces are replaced with the correct
		 * chars 
		 */
		while (--len >= 0) {
		    *fssp++ = scheme;
		...
		meFrameXTermDrawString(frameCur, colToClient(scol + ccol),
				       row, (char *) sfstp + ccol, ii);
		while (--spFlag >= 0) {
		    while (((cc = s1[ccol]) & 0xe0) != 0)
			ccol++;
		    sfstp[ccol] = cc;
		    meFrameXTermDrawSpecialChar(frameCur,
						colToClient(scol + ccol),
						row - mecm.ascent, cc);
		    ccol++;
		}
	    } while (++cno < noColChng);
		...
		ii = blkp->column;
		len = ii - col;
		meFrameXTermDrawString(frameCur, colToClient(scol + col),
				       row, (char *) s1 + col, len);
		blkp++;
	...
	if (ii > col) {
		...
		*fssp++ = scheme;	/* Frame store colour fill */
	    ...
	    while (--cno);
	    meFrameXTermDrawString(frameCur, colToClient(scol + col), row,
				   (char *) buff, ii);
	}
#if DEBUGGING
	else
	    ii = 0;
	meFrameXTermDrawString(frameCur, colToClient(scol + col + ii - 1),
			       row, &drawno, 1);
#endif
#endif				/* _XTERM */
    }
	...
	do {
	    ...
	    /*
	     * Update the screen. Maintain the frame store and copy the
	     * sting into the frame store with the colour information 
	     */
	    len = blkp->column;
	    for (; ii < len; ii++) {
		*fssp++ = scheme;
	...
	vp1->endp = ii;
	/*
	 * Update the screen for end of the line. Maintain the frame store 
	 * copy. 
	 */
	for (; ii < len; ii++) {
	    *fssp++ = scheme;
	...
	do {
	     ...
	     */
	    offset = blkp->column - len;	/* Get width of coloured
						 * block */
	    scheme = blkp->scheme;
...
updateWindow(meWindow * wp)
{
    ...
    register int    row,
                    nrows;	/* physical screen line to update */
	...
	if (wp->updateFlags & WFLOOKBK) {
	    if (meHilightGetFlags(hilights[bp->hilight]) &
		(HFLOOKBLN | HFLOOKBSCH))
		hilightLookBack(wp);
	    wp->updateFlags &= ~WFLOOKBK;
	}
    }
...
#endif
#endif
	} else if ((lp->flag & meLINE_CHANGED) || (vptr->line != lp)) {
	    vptr->line = lp;
	    update = 1;
	}
	if (update)
	...
	meLineFlag      flag;
	/*
	 * store the baseLine flag and set to 0 as it may have a
	 * $line-scheme 
	 */
	flag = lp->flag;
	lp->flag = 0;
	for (; (--nrows >= 0); row++, vptr++) {
	    vptr->line = lp;
	    if (force || (vptr->endp > 1)
		|| (vptr->flag & (VFCHNGD | VFCURRL))
...
assessModeLine(meUByte * ml)
{
	...
	if (cc == '%') {
	    ...
	    case 'c':
		/*
		 * The column often changes with the line, so add that too 
		 */
...
updateModeLine(meWindow * wp)
{
	...
	if (cc == '%') {
	    ...
	    case 's':
		/*
		 * Horizontal split line character. Only displayed if
		 * split is enabled in the scroll bars 
		 */
...
reframe(meWindow * wp)
{
	...
	if ((ap != NULL) &&
	    (ap->line == wp->dotLine) && (ap->offs == wp->dotOffset)) {
	    meIPipe        *ipipe;
     ...
     */
    if (ii > 0) {		/* only one screen worth of lines max */
	if (--ii >= wp->textDepth)
	    ii = wp->textDepth - 1;
    } else if (ii < 0) {	/* negative update???? */
	ii += wp->textDepth;
	if (ii < 0)
	    ii = 0;
    } else
...
updateScrollBar(meWindow * wp)
{
    ...
    scheme =
	sbarScheme +
	((frameCur->windowCur == wp) ? meSCHEME_CURRENT : meSCHEME_NORMAL);
    ...
    /*
     * Flipbox is provided where we have not got a good reverse video
     * character for the scroll box (UNIX !!). When WMRVBOX is defined we
     * may use a <SPACE> character for the scroll box and paint in reverse 
     * video, this then makes a better scroll box as required. Simply get 
     * the status and record the offset of the box, we will flip fcol and
     * bcol when we get there. If it is not defined then set to the end of
     * scroll bar + 1. 
     */
	...
	for (; row < endrow; row++) {
		...
		if (meSystemCfg & meSYSTEM_FONTFIX) {
		    ...
		    meFrameXTermDrawString(frameCur, colToClient(col),
					   rowToClient(row), fstp, len);
		    if (!((cc = *wbase) & 0xe0)) {
			meFrameXTermDrawSpecialChar(frameCur,
						    colToClient(col),
						    rowToClientTop(row),
...
pokeUpdate(void)
{
	...
	/*
	 * Got here - therefore overlaps the window. Determine vertical
	 * starting position on the frame. ii initially contains the
	 * startOffset 
	 */
	...
	/*
	 * Determine the end position on the frame and work out the number 
	 * of lines that need to be updated. ii will contain the
	 * (endOffset - startOffset) i.e the number of video rows to be
	 * updated 
	 */
	...
	/*
	 * Iterate down the video frame until we have exhausted our row
	 * count. Note that we only mark the underlying video structure as 
	 * changed and NOT the window contents. This ensures that the
 ...
 */
/*
 * screenUpdate: user routine to force a screen update always finishes
 * complete update 
 */
...
screenUpdate(int f, int n)
{
	...
	if (frameCur->windowCur->bufferLast != frameCur->bufferCur) {
		...
		WFMODE | WFREDRAW | WFMOVEL | WFSBOX;
	}
	/*
	 * if top of window is the last line and there's more than one,
	 * force refame and draw 
	 */
	...
	if (frameCur->windowCur->
	    updateFlags & (WFREDRAW | WFRESIZE | WFSELHIL | WFSELDRAWN))
	    shilightWindow(frameCur->windowCur);	/* Update
							 * selection
							 * hilight */
	...
	updateline(frameCur->depth,
		   frameCur->video.lineArray + frameCur->depth, NULL);
	frameCur->video.lineArray[frameCur->depth].flag &= ~VFCHNGD;
    }
#if MEOPT_OSD
    /*
...
int
update(int flag)
{				/* force=meTRUE update past type ahead? */
#if MEOPT_CALLBACK
    register int    index;
    meUInt          arg;
#endif
	...
	execFuncHidden(ME_SPECIAL | SKEY_redraw, index,
		       0x80000002 - sgarbf);
    else
#endif
 ...
 */
/*
 * poking can draw onto of the cursor and erase it, this is often an
 * unwanted side effect (particularly with the matching fence drawing).
...
pokeScreen(int flags, int row, int col, meUByte * scheme, meUByte * str)
{
    ...
    drawCursor = ((cursorState >= 0) && blinkState &&
		  ...
		  (col <= frameCur->cursorColumn)
		  && ((col + len) > frameCur->cursorColumn));
...
#endif				/* _ME_WINDOW */
	{
	    ...
	    while (len--) {
		schm = *scheme++;
		if ((schm == meCHAR_LEADER)
		    && ((schm = *scheme++) == meCHAR_TRAIL_NULL))
		    schm = 0;
		else
...
#ifdef _ME_WINDOW
	{
		...
		while (len--) {
		    ...
		    *fssp++ = schm;
			...
			meFrameXTermDrawSpecialChar(frameCur, col,
						    row - mecm.ascent, cc);
		    }
		    col += mecm.fwidth;
		}
...
#ifdef _ME_WINDOW
	{
		...
		meFrameXTermDrawString(frameCur, colToClient(col), row,
				       sfstp, ii);
		ii = 0;
		while (--spFlag >= 0) {
...
screenPoke(int f, int n)
{
	...
	|| ((row = meAtoi(fbuf)),
	    (meGetString((meUByte *) "col", 0, 0, fbuf, meBUF_SIZE_MAX) <=
	     0))
	|| ((col = meAtoi(fbuf)),
	    (meGetString((meUByte *) "scheme", 0, 0, fbuf, meBUF_SIZE_MAX)
	     <= 0))
	|| (meGetString((meUByte *) "string", 0, 0, sbuf, meBUF_SIZE_MAX)
	    <= 0))
	return meFALSE;
    if ((n & POKE_COLORS) == 0)
...
#endif
{
    ...
    meUByte         mlw[meBUF_SIZE_MAX];	/* what we want to be
						 * there */
    meUByte        *mlwant;	/* what we want to be there */
    meUByte        *s1;

    /*
     * If this is a termination request then exit immediately as the
     * output terminal window may have been destroyed so there is no
     * canvas onto which the message may be written. 
     */
	...
	frameCur->mlStatus =
	    (frameCur->mlStatus & ~MLSTATUS_KEEP) | MLSTATUS_RESTORE;
    } else
	...
	*mlwant = '\0';
     ...
     */
    /*
     * SWP - changed the mlwrite when the string is to long for the ml
     * line so that it always displays the last part. This is for two
     * reasons: 1) If this is an MWSPEC then it must not touch the string, 
     * so can't null terminate. 2) The latter part is usually the more
     * important, i.e. where the prompt is 
     */
    if (offset > frameCur->width) {
     ...
     * bottom line is not reset properly. SWP - I've commented it out
     * again - this only alleviates another bug which should be/should
     * have been fixed. 
     */
    /*
    ...
    if (flags & MWPAUSE) {
	/*
	 * Change the value of frameCur->mlStatus to MLSTATUS_KEEP cos we
	 * want to keep the string for the length of the sleep 
	 */
	...
	/*
	 * Change the value of frameCur->mlStatus to MLSTATUS_KEEP cos we
	 * want to keep the string till we get a key 
	 */
	meUByte         scheme = (globScheme / meSCHEME_STYLES),
	    oldMlStatus = frameCur->mlStatus,
	    oldkbdmode = kbdmode;
	pokeScreen(POKE_NOMARK + 0x10, frameCur->depth,
		   frameCur->width - 9, &scheme, (meUByte *) "<ANY KEY>");
	vp1->endp = frameCur->width;
	frameCur->mlStatus = MLSTATUS_KEEP;
	/*
...
mlerase(int flag)
{
	...
	mlwrite(((frameCur->
		  mlStatus & MLSTATUS_POSML) ? MWSPEC | MWUSEMLCOL |
		 MWCURSOR : MWSPEC), frameCur->mlLineStore);
	frameCur->mlStatus =
	    (frameCur->mlStatus & ~MLSTATUS_RESTORE) | MLSTATUS_KEEP;
...
addColor(int f, int n)
{
    ...
    if ((meGetString((meUByte *) "Index", 0, 0, buff, meBUF_SIZE_MAX) ==
	 meABORT) || ((index = (meColor) meAtoi(buff)) == meCOLOR_INVALID)
	|| (meGetString((meUByte *) "Red", 0, 0, buff, meBUF_SIZE_MAX) ==
	    meABORT)
	|| ((r = (meUByte) meAtoi(buff)),
	    (meGetString((meUByte *) "Green", 0, 0, buff, meBUF_SIZE_MAX)
	     == meABORT))
	|| ((g = (meUByte) meAtoi(buff)),
	    (meGetString((meUByte *) "Blue", 0, 0, buff, meBUF_SIZE_MAX) ==
	     meABORT)))
	return meFALSE;
...
addColorScheme(int f, int n)
{
     ...
     */
    static meUByte *schmPromptM[4] =
	{ (meUByte *) "Normal", (meUByte *) "Current",
(meUByte *) "Select", (meUByte *) "Cur-Sel" };
    static meUByte *schmPromptP[4] =
	{ (meUByte *) " fcol", (meUByte *) " bcol", (meUByte *) " nfont",
(meUByte *) "rfont" };
    meStyle        *hcolors;
    meStyle        *dcolors;
    meColor         scheme[4][4];
     ...
     */
    if ((meGetString
	 ((meUByte *) "Color scheme index", 0, 0, buf,
	  meBUF_SIZE_MAX) <= 0) || ((index = meAtoi(buf)) < 0))
	return mlwrite(MWABORT | MWPAUSE,
		       (meUByte *) "Invalid scheme index number");
	...
	return mlwrite(MWABORT | MWPAUSE,
		       (meUByte *) "Cannot duplicate undefined scheme %d.",
		       n);
    else
	 ...
	 */
	if ((styleTable =
	     meRealloc(styleTable,
		       (index + 1) * meSCHEME_STYLES * sizeof(meStyle))) ==
	    NULL)
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "Cannot allocate scheme table");
	for (ii = styleTableSize; ii <= index; ii++)
	    memcpy(styleTable + (ii * meSCHEME_STYLES), defaultScheme,
		   meSCHEME_STYLES * sizeof(meStyle));
	...
	for (jj = 2; jj < 4; jj++) {
	    meStrcpy(prompt, schmPromptM[ii]);
	    meStrcat(prompt, schmPromptP[jj]);
	    if (meGetString(prompt, MLEXECNOUSER, 0, buf, meBUF_SIZE_MAX)
		<= 0)
		break;
	...
	if (!(meSystemCfg & meSYSTEM_FONTS))
	    ...
	    font = 0;
	/*
	 * if color is used then reverse font is done using color so
	 * remove this font. If not using color then set the color to the
	 * defaults 
	 */
	if (meSystemCfg &
	    (meSYSTEM_RGBCOLOR | meSYSTEM_ANSICOLOR | meSYSTEM_XANSICOLOR))
	    font &= ~meFONT_REVERSE;
	else {
...
meVideoDetach(meWindow * wp)
{
    ...
    /*
     * Clean up the Virtual video frame if it is empty. Note that vvptr == 
     * frameCur->video is handled correctly and is not deleted. 
     */
...
meVideoAttach(meVideo * vvptr, meWindow * wp)
{
	...
	if (((vvptr = (meVideo *) meMalloc(sizeof(meVideo))) == NULL) ||
	    ((vs =
	      (meVideoLine *) meMalloc(frameCur->depthMax *
				       sizeof(meVideoLine))) == NULL)) {
	    meNullFree(vvptr);
	    return (meFALSE);
	}

	/*
	 * Reset the strucure and chain to the existing frameCur->video
	 * list 
	 */
	...
	vvptr->next = frameCur->video.next;	/* Chain into
						 * frameCur->vvideo list */
