/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com estruct.h - Structures
 * and their defines. Originally written by Dave G. Conroy for MicroEmacs
...
#define meTOKENBUF_SIZE_MAX   meBUF_SIZE_MAX+4	/* meBUF_SIZE_MAX + an
						 * overrun safe area */
#define meMLDISP_SIZE_MAX     meBUF_SIZE_MAX+20	/* meBUF_SIZE_MAX +
						 * completion label */
...
#define meCHAR_TRAIL_LEADER   meCHAR_LEADER	/* trail of a leader char
						 * (must be same char) */
...
/*
 * An meScheme is simply an index into the meStyle table. Each scheme
 * created by add-color-scheme consists of 8 meSTYLEs, as there are upto
 * 256 schemes there can be 8*256 styles hence its a meUShort. note that
 * users enter the base scheme number, i.e. 0, 1, 2, ... which is then
 * multiplied by 8 and stored, i.e. 0, 8, 16, ... 
 */
...
#define meSCHEME_RCURSEL  7	/* Reverse current selected foreground
				 * color */
...
#define meMEMORY_BOUND          8	/* the size memory is rounded up
					 * to */
...
/*
 * The box chars BC=Box Chars, N=North, E=East, S=South, W=West, So BCNS = 
 * Box Char which has lines from North & South to centre, i.e.  the
 * vertical line | BCNESW = the + etc. 
 */
#define BCNS         0
#define BCES         1
#define BCSW         2
...
/*
 * SWP 2002-01-09 - Windows file timestamps use 2 longs, a dwHigh and a
 * dwLow to count the 100 nanosecs since 1601-01-01 (why???) So create an
 * meFiletime type to handle this which is then used everywhere else. 
 */
#ifdef _WIN32
...
typedef struct meVariable {
    struct meVariable *next;	/* Next pointer, MUST BE FIRST as with
				 * meVarList */
...
#define meMACRO_EXEC  meLINE_ANCHOR_NARROW	/* Buffer is being
						 * executed */
 ...
 * linked list of amark structures, with the head of the list being pointed 
 * to by anchorList in the buffer structure. 
 */
...
/*
 * A position, stores the current window, buffer, line etc which can be
 * restore later, used by push-position and pop-position 
 */
typedef struct mePosition {
    ...
    meUShort        winMaxRow;	/* window on a goto - this greatly
				 * improves */
    ...
    meUShort        name;	/* position name, (letter associated with
				 * it) */
} mePosition;
...
typedef struct meNarrow {
    ...
    meInt           name;	/* anchor name */
    meInt           markupCmd;	/* Command called to create markup line
				 * text */
...
#define BIFFILE    0x10		/* The buffer is a file - used at creation 
				 * only */
...
/*
 * The editor holds deleted text chunks in the meKillNode buffer. The kill 
 * buffer is logically a stream of ascii characters, however due to its
 * unpredicatable size, it gets implemented as a linked list of chunks.
 * (The d_ prefix is for "deleted" text, as k_ was taken up by the keycode 
 * structure 
 */
 ...
 */
#define HICASE     0x01
#define HILOOKBSCH 0x02		/* this indent has a lookback indent
				 * scheme */
...
/*
 * Virtual Video Structure. Provides the meVideoLine structure management
 * for horizontal split Windows. A single meVideo exists for each
 * meVideoLine structure. 
 */
...
/*
 * Frame Store The frame store holds the physical screen structure. This
 * comprises the text information and the colour information of each
 * character on the screen. The frame store is required primarily on
 * windowing platforms such as MS-Windows and X-Windows and allows the
 * canvas to be regenerated without recomputing the contents of the
 * window when the windowing sub-system requires the window (or parts of
 * the window) to be refreshed. Hence rather than EMACS generating new
 * lines every time we render from the Frame buffer which contains a
 * representation of the screen. (Color + ASCII text information). Under
 * MS-Windows we receive a WM_PAINT message. This message may be received
...
typedef struct {
    ...
    meScheme       *scheme;	/* index to the Style (fore + back + font) 
				 * of each cell */
...
typedef struct meFrame {
...
#ifdef _WINDOW
    meUByte        *titleBufferName;	/* Name of buffer used when last
    ...
    meUByte         pokeFlag;	/* Boolean meTRUE/meFALSE flag. meTRUE
				 ...
				 * performed. */
    meUByte         mlStatus;	/* ml status 0=not using it, 1=using it.
				 * 2=using it & its been broken so next
				 * time mlerease is used, it will restore */
} meFrame;
...
#if MEOPT_NARROW
/*
 * The following structure is used for narrow undos, the next and type
 * members must be in the same position as in the main meUndoNode, see
 * asserts in main.c 
 */
typedef struct meUndoNarrow {
    ...
    union {
	meInt           dotp;
	void           *dummy;	/* Maintain structure alignment with ptr
				 * union */
    } udata;
    meInt           count;
    meScheme        scheme;
...
/*
 * The variable register (#0 - #9) uses a linked structure This is so they 
 * can easily be pushed and poped 
 */
...
/*
 * Note that the first part of meDirList structure must remain the same as 
 * meNamesList as it is used for $file-names variable 
 */
typedef struct {
    int             exact;
    int             size;
...
#ifdef _WIN32
    /*
     * In MS-Windows then the recieving window is raised and has focus so
     * we do not need to send the frame as this is always the current
     * frame (where we have multiple frames) 
     */
