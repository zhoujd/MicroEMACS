/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com esearch.h - Searching
 * definitions. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
extern meRegex  meRegexStrCmp;	/* meRegex used by meRegexStrCmp if main
				 * is not used */
...
/*
 * iscanner - external front end to buffer searching returns meTRUE (1) if 
 * the search was successful meFALSE (0) if the search failed -1 if the
 * regex apat failed to compile (error) -2 if the regex apat failed to
 * compile (incomplete?) 
 */
extern int      iscanner(meUByte * apat, int n, int flags,
			 SCANNERPOS * sp);

/*
 * search arrays and variables, defined in search.c, but exported here so
...
#define ISCANNER_EOBQUIT 0x20	/* Quit if the end or beginning of buffer
				 * is reached */
