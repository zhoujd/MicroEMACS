/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com file.c - File handling.
 * Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This program is free
...
getFileStats(meUByte * file, int flag, meStat * stats, meUByte * lname)
{
	...
	*lname = '\0';
...
#else
	{
	    union REGS      reg;	/* cpu register for use of DOS
					 * calls */
	    reg.x.ax = 0x4300;
	...
	if (stats != NULL) {
		...
		stats->stmtime =
		    (((meUInt) fblk.ff_ftime) & 0x0ffff) | (((meUInt) fblk.
							     ff_fdate) <<
							    16);
	...
	if (stats != NULL) {
	    ...
	    stats->stmtime.dwHighDateTime =
		fd.ftLastWriteTime.dwHighDateTime;
	    stats->stmtime.dwLowDateTime =
	 ...
	 * or directory. Applications use this flag to mark files for
	 * backup or removal. FILE_ATTRIBUTE_HIDDEN The file or directory 
	 * is hidden. It is not included in an ordinary directory listing.
	 ...
	 * FILE_ATTRIBUTE_SYSTEM The file or directory is part of, or is
	 * used exclusively by, the operating system. FILE_ATTRIBUTE_NORMAL
	 * The file or directory has no other attributes set. This attribute
	...
	if ((lname == NULL) && (stats == NULL)) {
		...
		if ((jj =
		     readlink((char *) lbuf, (char *) buf,
			      meBUF_SIZE_MAX)) <= 0) {
		    if (flag & 1)
			mlwrite(MWABORT | MWPAUSE,
				(meUByte *) "[%s symbolic link problems]",
				file);
		    return meFILETYPE_NASTY;
		}
		buf[jj] = '\0';
		if ((buf[0] == DIR_CHAR)
		    || ((ss = meStrrchr(lbuf, DIR_CHAR)) == NULL))
		    meStrcpy(lbuf, buf);
		else {
		    ss++;
		    ii = (size_t) (ss - lbuf);
	    ...
	    } while (((jj = lstat((char *) lbuf, &statbuf)) == 0)
		     && (--maxi > 0) && S_ISLNK(statbuf.st_mode));
		...
		if (flag & 1)
		    mlwrite(MWABORT | MWPAUSE,
			    (meUByte *) "[%s dangling symbolic link]",
			    lbuf);
		if (stats != NULL)
	...
	if (S_ISDIR(statbuf.st_mode)) {
	    if (flag & 2)
		mlwrite(MWABORT | MWPAUSE, (meUByte *) "[%s directory]",
			file);
	    return meFILETYPE_DIRECTORY;
	}
	if (flag & 1) {
		...
		mlwrite(MWABORT | MWPAUSE, (meUByte *) "[%s is a FIFO]",
			file);
#endif
#ifdef S_IFCHR
	    else if (S_ISCHR(statbuf.st_mode))
		mlwrite(MWABORT | MWPAUSE,
			(meUByte *) "[%s is character special]", file);
#endif
#ifdef S_IFBLK
	    else if (S_ISBLK(statbuf.st_mode))
		mlwrite(MWABORT | MWPAUSE,
			(meUByte *) "[%s is block special]", file);
#endif
	    else
		mlwrite(MWABORT | MWPAUSE,
			(meUByte *) "[%s not a regular file]", file);
	}
	return meFILETYPE_NASTY;
    }
#else
...
/*
 * Search the directory and subdirectories for MicroEmacs macro
 * directories 
 */
int
mePathAddSearchPath(int index, meUByte * path_name, meUByte * path_base,
		    int *gotUserPath)
{
    /*
	...
	if (meUserName != NULL) {
	    ...
	    if (getFileStats(base_name, 0, NULL, NULL) ==
		meFILETYPE_DIRECTORY) {
		/*
	 ...
	 * * path. We do not add any directories to the search path that do
	 * not * exist. 
	 */
	for (ii = 0; (ss = subdirs[ii]) != NULL; ii++) {
	    ...
	    /*
	     * Test the directory for existance, if it does not exist then 
	     * do not add it as we do not want to search any directory
	     * unecessarily. 
	     */
	    if (getFileStats(base_name, 0, NULL, NULL) ==
		meFILETYPE_DIRECTORY) {
		...
		/*
		 * if the root dir does not exist there's no point
		 * continuing 
		 */
		break;
	}
...
int
fileLookup(meUByte * fname, meUByte * ext, meUByte flags,
	   meUByte * outName)
{
	 ...
	 * concatinate it, otherwise we do !!. Compare the complete
	 * extension with the end of the string, allowing double
	 * extensions to be handled properly. 
	 */
	...
	if (flags & meFL_EXEC) {
		...
		(getFileStats(outName, 0, NULL, NULL) ==
		 meFILETYPE_REGULAR))
		return 1;
	} else
#endif
...
executableLookup(meUByte * fname, meUByte * outName)
{
	...
	while (--ii >= 0)
	    ...
	    {
		if (fileLookup
		    (fname, NULL, meFL_CHECKDOT | meFL_USEPATH, outName))
		    return ii + 1;
		return 0;
	    }
    }
    ...
    while (--ii >= 0)
	if (fileLookup
	    (fname, execExtensions[ii], meFL_CHECKDOT | meFL_USEPATH,
	     outName))
	    return ii + 1;
#endif
#ifdef _UNIX
    ...
    flags =
	(meStrchr(fname, DIR_CHAR) !=
	 NULL) ? meFL_CHECKDOT | meFL_EXEC : meFL_USEPATH | meFL_EXEC;
...
bufferOutOfDate(meBuffer * bp)
{
    meStat          stats;
    if ((bp->fileName != NULL)
	&& getFileStats(bp->fileName, 0, &stats, NULL)
...
gwd(meUByte drive)
{
...
#ifdef _WIN32
    /*
     * Change drive to the destination drive. remeber so that we can get
     * back. Note we do no test for fails on the drive. We will pick up
     * the current working directory if it fails. 
     */
    if (drive != 0) {
	int             curDrive;
    ...
    /*
     * change the drive back - dont care if it fails cos theres nothing we 
     * can do! 
     */
    if (drive != 0)
	_chdrive(drive);
...
inputFileName(meUByte * prompt, meUByte * fn, int corFlag)
{
    ...
    s = meGetString(prompt, (MLFILECASE | MLNORESET | MLMACNORT), 0, buf,
		    meBUF_SIZE_MAX);
    if (s > 0) {
...
#if MEOPT_CRYPT
int
resetkey(meBuffer * bp)
{				/* reset the encryption key if needed */
    /*
     * if we are in crypt mode 
     */
    if (meModeTest(bp->mode, MDCRYPT)) {
	if ((bp->cryptKey == NULL) && (setBufferCryptKey(bp, NULL) <= 0))
	    return meFALSE;

	/*
	 * check crypt is still enabled (user may have removed the key)
...
{
    meUByte         bfname[meBUF_SIZE_MAX];	/* Working file name
						 * buffer */
    meUByte        *fn;
    FILENODE       *curFile;
    FILENODE       *cfh = NULL;
	...
	while ((dp = readdir(dirp)) != NULL) {
	    if (((noFiles & 0x0f) == 0) &&
		((curHead =
		  (FILENODE *) meRealloc(curHead,
					 sizeof(FILENODE) * (noFiles +
							     16))) ==
		 NULL))
		return NULL;
		...
		meUByte         link[1024];
		if ((llen =
		     readlink((char *) bfname, (char *) link, 1024)) > 0) {
		    link[llen] = '\0';
	...
	findfirst(bfname, &fblk,
		  FA_RDONLY | FA_HIDDEN | FA_SYSTEM | FA_DIREC);
    while (!done) {
	if (((noFiles & 0x0f) == 0) &&
	    ((curHead =
	      (FILENODE *) meRealloc(curHead,
				     sizeof(FILENODE) * (noFiles + 16))) ==
	     NULL))
	    return NULL;
	curFile = &(curHead[noFiles++]);
	...
	do {
	    ...
	    curFile->fname = ff;
	    curFile->lname = NULL;
	    curFile->mtime.dwLowDateTime =
		fd.ftLastWriteTime.dwLowDateTime;
	    curFile->mtime.dwHighDateTime =
		fd.ftLastWriteTime.dwHighDateTime;
	    /*
	     * construct attribute string 
	     */
...
readDirectory(meUByte * fname, meBuffer * bp, meLine * blp, meUInt flags)
{
	...
	if (fnode->sizeHigh > 0) {
	    ...
	    len +=
		sprintf((char *) buf + len, "%7dK ", fnode->sizeLow >> 10);
	else
	...
	if (FileTimeToLocalFileTime(&fnode->mtime, &ftmp) &&
	    ...
	    len += sprintf(buf + len, "%4d/%02d/%02d %02d:%02d:%02d ",
			   tmp.wYear, tmp.wMonth, tmp.wDay, tmp.wHour,
			   tmp.wMinute, tmp.wSecond);
	else
#endif
 ...
 */
/*
 * Note for Dynamic file names readin sets the buffer file name to point
 * to fname (freeing old if not NULL and bp->fileName != fname) so fname
 * should be dynamically allocated or the file name reset after readin. 
 */
/*
...
readin(register meBuffer * bp, meUByte * fname)
{
	...
	int             ft;
	if ((ft =
	     getFileStats(fn, 1, &(bp->stats), lfn)) == meFILETYPE_HTTP)
	    meModeSet(bp->mode, MDVIEW);
	    ...
	    if (lfn[0] != '\0')
		/*
		 * something was found, don't want to do RCS on this, the
		 * link may be dangling which returns 3 
		 */
		fn = lfn;
#if MEOPT_RCS
	    ...
	    if (ft == meFILETYPE_NOTEXIST) {	/* File not found.  */
		...
		if ((getFileStats(dirbuf, 0, NULL, NULL) !=
		     meFILETYPE_DIRECTORY)
#ifdef _UNIX
		    || meTestWrite(dirbuf)
#endif
		     ...
		     * READ ONLY DIR 
		     */
		    mlwrite(MWPAUSE, (meUByte *) "%s: %s", dirbuf,
			    strerror(errno));
		    /*
		     * Zap the filename - it is invalid. We only want a
		     * buffer 
		     */
		    mlwrite(0, (meUByte *) "[New buffer %s]",
			    getFileBaseName(fname));
		    meNullFree(bp->fileName);
		    bp->fileName = NULL;
		    ss = meABORT;
		} else {
		...
		mlwrite(MWABORT, (meUByte *) "[%s: %s]", fn,
			strerror(errno));
#else
		...
		&&
		((ui =
		  (bp->stats.stsizeHigh << 12) | (bp->stats.
						  stsizeLow >> 20)) >
		 fileSizePrompt)) {
		meUByte         prompt[200];
		meUByte        *tt;
		tt = getFileBaseName(fn);
		sprintf((char *) prompt, "File %s is %dMb, continue", tt,
			ui);
		if (mlyesno(prompt) <= 0)
		    goto error_end;
	    }
		 ...
		 */
		meModeSet(bp->mode, MDVIEW);
		mlwrite(MWCURSOR | MWCLEXEC, "[Reading %s (system file)]",
			fn);
	    } else
#endif
		/*
		 * Depending on whether we have write access set the
		 * buffer into view mode. Again root gets the privelidge
		 * of being able to write if possible. 
		 */
		if ((meTestWrite(fn)) || (
#ifdef _UNIX
					     (meUid != 0) &&
     ...
     */
    if (ss != meABORT) {
	mlwrite(MWCLEXEC, (meUByte *) "[Read %d line%s]", bp->lineCount,
		(bp->lineCount == 1) ? "" : "s");
	if (fn == afn)
	    /*
		   ...
		   meUInt uoffset, meUInt loffset, meInt length)
{
...
#endif
	ss = ffReadFile(fname, flags | meRWFLAG_INSERT, bp, lp, uoffset,
			loffset, length);
	...
	ss = meTRUE;
	if (!(flags & meRWFLAG_SILENT))
	    mlwrite(MWCLEXEC, (meUByte *) "[inserted %d line%s]", nline,
		    (nline == 1) ? "" : "s");
    }
    meFrameLoopBegin();
...
insertFile(int f, int n)
{
     ...
     */
    if (((s =
	  getFileStats(fname, meFINDFILESINGLE_GFS_OPT, &stats,
		       NULL)) != meFILETYPE_REGULAR) && ((n & 4)
							 ||
							 ((s !=
							   meFILETYPE_HTTP)
							  && (s !=
	...
	return meABORT;
#if MEOPT_EXTENDED
    if ((fileSizePrompt > 0) && ((n & 4) == 0)
	&& ((uoffset = (stats.stsizeHigh << 12) | (stats.stsizeLow >> 20))
	    > fileSizePrompt)) {
	meUByte         prompt[200];
	meUByte        *tt;
    ...
    frameCur->bufferCur->dotLineNo = frameCur->windowCur->dotLineNo;	/* must 
									 * have 
									 * the 
									 * line 
									 * no. 
	...
	if (meGetString((meUByte *) "UOffest", 0, 0, arg, meSBUF_SIZE_MAX)
	    <= 0)
	    return meABORT;
	uoffset = meAtoi(arg);
	if (meGetString((meUByte *) "LOffest", 0, 0, arg, meSBUF_SIZE_MAX)
	    <= 0)
	    return meABORT;
	loffset = meAtoi(arg);
	if ((meGetString((meUByte *) "Length", 0, 0, arg, meSBUF_SIZE_MAX)
	     <= 0) || ((length = meAtoi(arg)) == 0))
	    return meABORT;
    }

    if (((s =
	  meBufferInsertFile(frameCur->bufferCur, fname, flags, uoffset,
			     loffset, length)) > 0) && (n & 2))
	getFileStats(fname, meFINDFILESINGLE_GFS_OPT,
		     &(frameCur->bufferCur->stats), NULL);

    /*
     * move the mark down 1 line into the correct place 
     */
    frameCur->windowCur->markLine =
	meLineGetNext(frameCur->windowCur->markLine);
...
findFileSingle(meUByte * fname, int bflag, meInt lineno, meUShort colno)
{
	...
	if ((bp->fileName != NULL) && (bp->name != NULL)
	    && (bp->name[0] != '*')) {
	    if (
#ifdef _UNIX
...
#if MEOPT_SOCKET
	    /*
	     * at this point the type of an ftp file (i.e. reg of dir) is
	     * unknown, the filename will be changed,but the comparison of 
	     * ftp: file names must allow for this 
	     */
	    if (fnlen && !meStrncmp(bp->fileName, fname, fnlen) &&
		(bp->fileName[fnlen] == DIR_CHAR)
		&& (bp->fileName[fnlen + 1] == '\0'))
		break;
#endif
	}
    }
...
findFileList(meUByte * fname, int bflag, meInt lineno, meUShort colno)
{
	...
	&& meTestRead(fileName)) {
	/*
	 * if the base name has a wild card letter (i.e. *, ? '[') and a
	 * file with that exact name does not exist then load any files
...
findSwapFileList(meUByte * fname, int bflag, meInt lineno, meUShort colno)
{
	...
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[Failed to find file %s]", fname);
...
findFile(int f, int n)
{
    ...
    *ss = '\0';
	...
	return meABORT;
    n = (n &
	 (BFND_CREAT | BFND_BINARY | BFND_CRYPT | BFND_RBIN | BFND_NOHOOK))
	| BFND_MKNAM;
...
{
    meUByte         fname[meBUF_SIZE_MAX];	/* file user wishes to
						 * find */
...
{				/* visit a file in VIEW mode */
	...
	| BFND_MKNAM;
    /*
     * Add view mode globally so any new buffers will be created with view 
     * mode 
     */
...
writeCheck(meUByte * pathname, int flags, meStat * statp)
{
	...
	return mlwrite(MWABORT, (meUByte *) "Cannot write to: %s",
		       pathname);
	...
	return mlwrite(MWABORT, (meUByte *) "Read Only Directory: %s",
		       dirbuf);
#endif
    /*
...
static meUByte *
writeFileChecks(meUByte * dfname, meUByte * sfname, meUByte * lfname,
		int flags)
{
    register int    s;
    meStat          stats;
	...
	(((s =
	 ...
	 || ((s == meFILETYPE_FTP)
	     && (dfname[meStrlen(dfname) - 1] == DIR_CHAR)))) {
	s = meStrlen(dfname);
    ...
    if (((s =
	  getFileStats(dfname, 3, &stats, lfname)) != meFILETYPE_REGULAR)
	&& (s != meFILETYPE_NOTEXIST)
#if MEOPT_SOCKET
	&& (s != meFILETYPE_FTP)
#endif
	)
	...
	if (flags & meWRITECHECK_CHECK) {
		...
		{
		    sprintf((char *) prompt,
			    "Buffer %s is the same file, overwrite",
			    bp->name);
		    if (mlyesno(prompt) <= 0) {
			ctrlg(meFALSE, 1);
			return NULL;
...
fileOp(int f, int n)
{
    meUByte         sfname[meBUF_SIZE_MAX],
                    dfname[meBUF_SIZE_MAX],
                    lfname[meBUF_SIZE_MAX],
                   *fn = NULL;
    int             rr = meTRUE,
	dFlags = 0,
	fileMask = -1;
    ...
    if ((n &
	 (meFILEOP_FTPCLOSE | meFILEOP_DELETE | meFILEOP_MOVE |
	  meFILEOP_COPY | meFILEOP_MKDIR | meFILEOP_CHMOD |
	  meFILEOP_TOUCH)) == 0)
	...
	if (inputFileName((meUByte *) "Delete file", sfname, 1) <= 0)
	    ...
	    rr = mlwrite(MWABORT, (meUByte *) "[Cannot delete %s]",
			 sfname);
	else if ((n & meFILEOP_CHECK) && !isFtpLink(sfname)) {
	    if (meTestExist(sfname)) {
		mlwrite(MWABORT | MWCLEXEC,
			(meUByte *) "[%s: No such file]", sfname);
		rr = -2;
	    } else if (meTestWrite(sfname)) {
		sprintf((char *) lfname, "%s is read only, delete",
			sfname);
	...
	if ((inputFileName(prompt, sfname, 1) <= 0) ||
	    ...
	    rr = 0;
		  ...
		  writeFileChecks(dfname, sfname, lfname,
				  (n & meFILEOP_WC_MASK))) == NULL)
	    rr = -6;
	else
	 ...
	 */
		   ...
		   getFileStats(sfname, 0, NULL,
				NULL)) != meFILETYPE_NOTEXIST)
#if MEOPT_SOCKET
		 && (s != meFILETYPE_FTP)
#endif
	    ) {
	    mlwrite(MWABORT | MWCLEXEC, (meUByte *) "[%s already exists]",
		    sfname);
	    rr = -7;
	} else
	...
	int             s;
	if ((inputFileName
	     ((meUByte *) ((n & meFILEOP_CHMOD) ? "Chmod" : "Touch"),
	      sfname, 1) <= 0)
	    || (meGetString((meUByte *) "To", 0, 0, dfname, meBUF_SIZE_MAX)
		<= 0))
	    rr = 0;
	/*
	 * check that nothing of that name currently exists 
	 */
		   ...
		   getFileStats(sfname, 0, NULL,
				NULL)) != meFILETYPE_REGULAR)
		 && (s != meFILETYPE_DIRECTORY)) {
	    mlwrite(MWABORT | MWCLEXEC,
		    (meUByte *) "[%s not a file or directory]", sfname);
	    rr = -6;
	    ...
	    fileTimes.actime = fileTimes.modtime =
		time(NULL) + meAtoi(dfname);
	    ...
	    SYSTEMTIME      st;
	    if ((fp =
		 CreateFile(sfname, GENERIC_READ | GENERIC_WRITE,
			    FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
			    OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
			    NULL)) == INVALID_HANDLE_VALUE) {
		mlwrite(MWABORT | MWCLEXEC,
			(meUByte *) "[%s: Cannot access file]", sfname);
		rr = -6;
	    } else {
		GetSystemTime(&st);
...
autowriteout(register meBuffer * bp)
{
	...
	if (!createBackupName(fn, ff, '#', 1) &&
	    (ffWriteFileOpen(fn, meRWFLAG_WRITE | meRWFLAG_AUTOSAVE, bp) >
	     0)) {
	    meLine         *lp;
	    ...
	    while ((lp != bp->baseLine) &&
		   (ffWriteFileWrite
		    (meLineGetLength(lp), meLineGetText(lp),
	    ...
	    if (ffWriteFileClose
		(fn, meRWFLAG_WRITE | meRWFLAG_AUTOSAVE, bp) > 0) {
		mlerase(MWCLEXEC);
		return;
	    }
	}
    }
    mlwrite(0, (meUByte *) "[Auto-writeout failure for buffer %s]",
	    bp->name);
}
...
writeOut(register meBuffer * bp, meUInt flags, meUByte * fn)
{
	 ...
	 * file. This should really be done using a $system flag - but one 
	 * assumes that this is the default action for the super user. 
	 */
	if (meUid == 0) {
	 ...
	 */
	else if ((bp->stats.stgid != meGid) && meGidSize
		 && meGidInGidList(bp->stats.stgid))
	...
	/*
	 * Change the view status back to the file's permissions. For root 
	 * we let the super user do as they wish so no read protection is
	 * added. 
	 */
	if (
#if (defined _UNIX)
...
static meUByte  fileHasBinary[] =
    "File had binary characters which will be lost, write";
static meUByte  fileHasInLnEn[] =
    "File had inconsistent line endings which will be corrected, write";
/*
...
writeout(register meBuffer * bp, int flags)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "[No file name set for buffer %s]",
		       bp->name);
    else {
	meStat          stats;
	/*
	...
	if (flags & 1) {
		...
		sprintf((char *) prompt, "%s modified, write",
			bp->fileName);
		if (mlyesno(prompt) <= 0)
		    return ctrlg(meFALSE, 1);
	    }
		...
		if (mlyesno
		    ((bp->
		      fileFlag & meBFFLAG_BINARY) ? fileHasBinary :
		     fileHasInLnEn) <= 0)
		    return ctrlg(meFALSE, 1);
...
writeBuffer(int f, int n)
{
    ...
    if ((n & 0x01)
	&& (frameCur->bufferCur->
	    fileFlag & (meBFFLAG_BINARY | meBFFLAG_LTDIFF))
	&&
	(mlyesno
	 ((frameCur->bufferCur->
	   fileFlag & meBFFLAG_BINARY) ? fileHasBinary : fileHasInLnEn) <=
	 0))
	return ctrlg(meFALSE, 1);
    if ((fn =
	 writeFileChecks(fname, fn, lname,
			 (n & 0x01) | meWRITECHECK_BUFFER)) == NULL)
	return meABORT;

    if (!writeOut
	(frameCur->bufferCur, ((n & 0x02) ? meRWFLAG_IGNRNRRW : 0), fn))
	return meFALSE;
...
saveBuffer(int f, int n)
{
     ...
     * NULL and still be saved, e.g. the buffer name is *stdin* - so be
     * careful 
     */
    if ((n & 0x01) && (frameCur->bufferCur->fileName != NULL) &&	/* Are 
									 * we 
									 * Checking 
									 * ?? 
									 */
	(meTestExist(frameCur->bufferCur->fileName) == 0) &&	/* Does
								 * file
								 * actually 
								 * exist ? 
								 */
	!meModeTest(frameCur->bufferCur->mode, MDEDIT))	/* Have we edited
							 * buffer ? */
	/*
	 * Return, no changes.  
	 */
...
saveSomeBuffers(int f, int n)
{
	...
	if (bufferNeedSaving(bp)) {
		...
		if (bp->fileName != NULL)
		    sprintf((char *) prompt,
			    "%s: Save file (?/y/n/a/o/g) ? ",
			    bp->fileName);
		else
		    sprintf((char *) prompt,
			    "%s: Save buffer (?/y/n/a/o/g) ? ", bp->name);
		if ((status =
		     mlCharReply(prompt, mlCR_LOWER_CASE,
				 (meUByte *) "ynaog",
				 (meUByte *)
				 "(Y)es, (N)o, Yes to (a)ll, N(o) to all, (G)oto, (C-g)Abort ? "))
		    < 0)
		    return ctrlg(meFALSE, 1);
		else if (status == 'g') {
		    swbuffer(frameCur->windowCur, bp);
		    /*
		     * return abort to halt any calling macro (e.g.
		     * compile) 
		     */
		    return meABORT;
		} else if (status == 'o')
...
appendBuffer(int f, int n)
{
    ...
    if (((ss = getFileStats(fname, 3, NULL, lname)) != meFILETYPE_REGULAR)
	&& (ss != meFILETYPE_NOTEXIST))
	return meABORT;
...
changeFileName(int f, int n)
{
    ...
    if ((s =
	 inputFileName((meUByte *) "New file name", fname, 1)) == meABORT)
	return s;

    if (((ft = getFileStats(fname, 3, NULL, lname)) != meFILETYPE_REGULAR)
	&& (ft != meFILETYPE_NOTEXIST)
#if MEOPT_SOCKET
	&& (ft != meFILETYPE_FTP)
#endif
	)
	...
	meModeClear(frameCur->bufferCur->mode, MDVIEW);	/* no longer read
							 * only mode */
#endif
    frameAddModeToWindows(WFMODE);	/* and update ALL mode lines */
...
void
pathNameCorrect(meUByte * oldName, int nameType, meUByte * newName,
		meUByte ** baseName)
{
    register meUByte *p,
                   *p1;
    ...
    p = p1 = oldName;
    /*
     * search for 1) set to root, xxxx/http:// -> http:// (for urls) 2)
     * set to root, xxxx/ftp:// -> ftp:// (for urls) 2) set to root,
     * xxxx/file:yy -> yy (for urls) 3) set to root, xxxx///yyyyy ->
     * //yyyyy (for network drives) 4) set to root, xxxx//yyyyy -> /yyyyy
     * 5) set to Drive, xxxx/z:yyyyy -> z:yyyyy 6) set to home,
     * xxxx/~yyyyy -> ~yyyyy 
     */
    for (;;) {
	if (isHttpLink(p1)) {
	    ...
	    p = p1;
	    /*
	     * loop here as if at the start of the file name, this is so
	     * it handles "file:ftp://..." correctly 
	     */
	    continue;
	} else if (flag != 2) {
	    /*
	     * note that ftp://... names are still processed,
	     * ftp://.../xxx//yyy -> ftp://.../yyy etc 
	     */
	    if (p1[0] == DIR_CHAR) {
#ifndef _UNIX
		if (p1[1] == DIR_CHAR) {
		    /*
		     * Got a xxxx///yyyyy -> //yyyyy must not optimise
		     * further 
		     */
		    flag = 1;
		    while (p1[2] == DIR_CHAR)
		...
		/*
		 * Got a home, xxx/~/yyy -> ~/yyy, xxx/~yyy -> ~yyy,
		 * ftp://.../xxx/~/yyy -> ftp://.../~/yyy 
		 */
		if (flag == 3) {
		    if (p1[1] == '/') {
		...
		/*
		 * got a Drive, xxxx/z:yyyyy -> z:yyyyy - remove ftp:// or 
		 * //yyyy/... 
		 */
		flag = 0;
		p = p1;
	...
	if (p[0] == '\0') {
	    ...
	    *p1 = '\0';
		...
		if ((nameType == PATHNAME_PARTIAL)
		    && (meStrchr(p, DIR_CHAR) == NULL)) {
		    /*
		     * special case when user is entering a file name and
		     * uses complete with 'xxxx/~yy' 
		     */
		    *p1++ = '~';
		    meStrcpy(p1, p);
		    if (baseName != NULL)
		...
		if ((p[0] != '\0') && (p[0] != DIR_CHAR)
		    ...
		    ((reg =
		      regFind(NULL,
			      (meUByte *) "history/alias-path")) != NULL)
		    ...
		    while ((reg != NULL)
			   && (((int) meStrlen(reg->name) != ll)
			       || meStrncmp(p, reg->name, ll)))
			reg = regGetNext(reg);
		}
		if (reg != NULL) {
		    if (reg->value != NULL) {
...
fileNameCorrect(meUByte * oldName, meUByte * newName, meUByte ** baseName)
{
	...
	*baseName = bn;
    ...
    if (!fileNameWild(bn)) {
	/*
	 * with windows naff file name case insensitivity we must get the
	 * correct a single name letter case by using FindFirstFile
	 */
	HANDLE         *handle;
	WIN32_FIND_DATA fd;
...
getDirectoryList(meUByte * pathName, meDirList * dirList)
{
	...
	if (pathName[0] == 'f') {
	    ...
	    hlp.next = &hlp;
	    hlp.prev = &hlp;
	    if (ffReadFile(pathName, meRWFLAG_SILENT, NULL, &hlp, 0, 0, 0)
		>= 0) {
		noFiles = 0;
		lp = hlp.next;
		while (lp != &hlp) {
		...
		if (noFiles &&
		    ...
		    (meRegexComp
		     (&meRegexStrCmp,
		      (meUByte *)
		      "\\([-\\a]+ +\\d+ \\S+\\( +\\S+\\)?\\( +\\d+,\\)? +\\d+\\( +0[xX]\\h+\\)? +\\a\\a\\a +\\d\\d? +\\(\\d\\d\\d\\d\\|\\d\\d?:\\d\\d\\)\\|\\d\\d-\\d\\d-\\d\\d +\\d\\d?\\:\\d\\d[aApP][mM] +\\(\\d+\\|<DIR>\\)\\) +\\(.*\\)",
		      0) == meREGEX_OKAY)) {
		    noFiles = 0;
		    lp = hlp.next;
		    while (lp != &hlp) {
			if (meRegexMatch
			    (&meRegexStrCmp, lp->text, meLineGetLength(lp),
			     0, meLineGetLength(lp), meRSTRCMP_WHOLE)) {
			    ii = meRegexStrCmp.group[7].end -
				meRegexStrCmp.group[7].start;
			    if ((fls[noFiles] = meMalloc(ii + 2)) == NULL)
				break;
			    meStrncpy(fls[noFiles],
				      lp->text +
				      meRegexStrCmp.group[7].start, ii);
			    if (lp->text[0] == 'd')
				fls[noFiles][ii++] = '/';
			    else if ((lp->text[0] == 'l')
				     &&
				     ((ss =
				       meStrstr(fls[noFiles],
						" -> ")) != NULL))
				ii = ((size_t) ss) -
				    ((size_t) fls[noFiles]);
			    else if (isDigit(lp->text[0])
				     && (lp->
					 text[meRegexStrCmp.group[6].
					      start + 1] == 'D'))
				fls[noFiles][ii++] = '/';
			    fls[noFiles++][ii] = '\0';
			}
	...
	if (((len = strlen(pathName)) > 0)
	    ...
	    pathName[len - 1] = '\0';
	    if ((handle =
		 FindFirstFile(pathName, &fd)) != INVALID_HANDLE_VALUE) {
	...
	/*
	 * append the *.* - Note that this function assumes the pathName
	 * has a '/' and its an array with 3 extra char larger than the
	 * string size 
	 */
	...
	for (ii = 1, dwMask = 1; ii <= 26; ii++, dwMask <<= 1) {
	    if ((ii == 1) || (dwAvailableDrives & dwMask)) {	/* Drive
								 * exists
								 * ?? */
		/*
	 ...
	 */
	if (((reg =
	      regFind(NULL, (meUByte *) "history/alias-path")) != NULL)
	    && ((reg = regGetChild(reg)) != NULL)) {
	    do {
		if (((noFiles & 0x0f) == 0) &&
		    ((fls =
