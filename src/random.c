/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com random.c - Random
 * Routines. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
meMalloc(size_t s)
{
	...
	mlwrite(MWCURSOR | MWABORT | MWWAIT,
		(meUByte *) "Warning! Malloc failure");
    return r;
}
...
/*
 * case insensitive str compare done by simply turning all letters to
 * lower case 
 */
int
...
/*
 * case insensitive str compare/diff This is slightly different to the
 * above as if the strings are the same when ignore the case, it then
 * returns the case sensitive result. 
 */
int
...
/*
 * sort the given list of strings out on the nth plus character. Can be
 * used on lines by adding (sizeof(meLine)-1) to the offset. 
 */
#ifdef _NOQSORT
void
...
sortStringsCmp(const void *v1, const void *v2)
{
    ...
    ii = sortStringsCmpFunc(*((const meUByte **) v1) + sortStringsOffset,
			    *((const meUByte **) v2) + sortStringsOffset);
...
sortLines(int f, int n)
{
	...
	return meFALSE;
    for (ii = 0, l = meLineGetNext(sL); ii < noL;
	 l = meLineGetNext(l), ii++) {
	list[ii] = l;
#if MEOPT_UNDO
...
/*
 * buffer-info. Display the current position of the cursor, in origin 1
 * X-Y coordinates, the character that is under the cursor (in hex), and
 * the fraction of the text that is before the cursor. The displayed
 * column is not the current column, but the column that would be used on
 * an infinite width display. Normally bound to C-x = 
 */
int
bufferInfo(int f, int n)
{
	...
	meLineGetLength(frameCur->windowCur->dotLine);
    ecol = getccol();
    ...
    if (n == 0) {
	/*
	 * macro call - put info into $result only and in a more usable
	 * form 
	 */
	sprintf((char *) resultStr,
		"|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|0x%02x|", predlines + 1,
		numlines + 1, savepos,
		meLineGetLength(frameCur->windowCur->dotLine), predchars,
		numchars, ratio,
		(int) (frameCur->windowCur->dotLineNo -
		       frameCur->windowCur->vertScroll),
		frameCur->windowCur->textDepth - 1, col, ecol, curchar);
	return meTRUE;
    }
    /*
     ...
     */
    sprintf((char *) resultStr,
	    "Line %d/%d Col %d/%d Char %d/%d (%d%%) Win Line %d/%d ACol %d/%d char %d (0x%02x)",
	    predlines + 1, numlines + 1, savepos,
	    meLineGetLength(frameCur->windowCur->dotLine), predchars,
	    numchars, ratio,
	    (int) (frameCur->windowCur->dotLineNo -
		   frameCur->windowCur->vertScroll),
...
/*
 * transpose-character. Twiddle the two characters on either side of dot.
 * If dot is at the end of the line transChars the two characters before
 * it. Return with an error if dot is at the beginning of line; it seems
 * to be a bit pointless to make this work. This fixes up a very common
 * typo with a single stroke. Normally bound to C-t. 
 */
int
...
/*
 * transpose-lines. Swap the line before with dot and move down (unless at 
 * the end where there is no advance. If dot is the top the move down 1 to 
 * start. Normally bound C-x C-t. 
 */
int
transLines(int f, int n)
{
    ...
    for (i = 0, j = abs(n);;) {
	if (((ln1 =
	      frameCur->windowCur->dotLine) ==
	     frameCur->bufferCur->baseLine)
	    || ((ln2 = meLineGetNext(ln1)) ==
		frameCur->bufferCur->baseLine))
	    break;
...
/*
 * quote-char. Quote the next character, and insert it into the buffer.
 * All the characters are taken literally, with the exception of the C-j,
 * which always has its line splitting meaning. The character is always
 * read, even if it is inserted 0 times. Normally bound to C-q 
 */
int
quote(int f, int n)
{
	...
	return (meTRUE);
    if ((c =
	 mlCharReply((meUByte *) "Quote: ", mlCR_QUOTE_CHAR, NULL,
		     NULL)) < 0)
...
meTab(int f, int n)
{
	...
	((meSystemCfg & meSYSTEM_TABINDANY) ||
	 ((meSystemCfg & meSYSTEM_TABINDFST)
	  && (frameCur->windowCur->dotOffset == 0))))
...
#if MEOPT_WORDPRO
    /*
     * If a newline was typed, fill column is defined, the argument is
     * non- negative, wrap mode is enabled, and we are now past fill
     * column, and we are not read-only, perform word wrap. 
     */
	...
	int             bufIndentWidth =
	    (int) (frameCur->bufferCur->indentWidth);
	int             ss =
	    (bufIndentWidth * (n - 1)) + (bufIndentWidth -
					  (getccol() % bufIndentWidth)) -
	    n;
	/*
	 * insert the required number of TABs as spaces first - this
	 * handles over mode The extra spaces required are inserted next 
	 */
...
#if MEOPT_EXTENDED
/*
 * backward-delete-tab. If the previous character was a <TAB> then delete. 
 * If the pseudo tabs are in operation then delete spaces to the next tab
 * stop position. Normally bound to S-tab 
 */
int
meBacktab(int f, int n)
{
    ...
    if ((meLineGetLength(frameCur->windowCur->dotLine) == 0) ||	/* Zero
								 * line
								 * length
								 * ?? */
	((doto = frameCur->windowCur->dotOffset - 1) < 0))	/* At
								 * start
								 * of line 
								 * ?? */
	...
	if (meLineGetChar(frameCur->windowCur->dotLine, doto) == meCHAR_TAB) {	/* Tab 
										 * char 
										 * ?? 
										 */
...
/*
 * delete-blank-lines. What this command does depends if dot is sitting on 
 * a blank line. If dot is sitting on a blank line, this command deletes
 * all the blank lines above and below the current line. If it is sitting
 * on a non blank line then it deletes all of the blank lines after the
 * line. Normally this command is bound to C-x C-o. 
 */
int
windowDeleteBlankLines(int f, int n)
{
    ...
    while (meLineGetLength(lp1) == 0
	   && (lp2 =
	       meLineGetPrev(lp1)) != frameCur->bufferCur->baseLine) {
	lp1 = lp2;
	lld++;
    }
    lp2 = lp1;
    nld = 0;
    while ((lp2 = meLineGetNext(lp2)) != frameCur->bufferCur->baseLine
	   && meLineGetLength(lp2) == 0)
	++nld;
    if (nld == 0)
	return (meTRUE);
    if (bufferSetEdit() <= 0)	/* Check we can change the buffer */
...
int
winsert(void)
{				/* insert a newline and indentation for
				 * Wrap indent mode */
    ...
    register int    i;
    meUByte         ichar[meSBUF_SIZE_MAX];	/* buffer to hold indent
						 * of last line */
...
indentInsert(void)
{
	...
	&&
	((comCont =
	  meIndentGetCommentContinue(indents[frameCur->bufferCur->indent]))
	 != NULL)) {
	doto = frameCur->windowCur->dotOffset;
...
meNewline(int f, int n)
{
	...
	((frameCur->bufferCur->fillmode == 'c')
	 || (frameCur->bufferCur->fillmode == 'r')))
	f = 1;
    else
	f = 0;
#endif
...
/*
 * forward-delete-char. This is real easy, because the basic delete
 * routine does all of the work. Watches for negative arguments, and does
 * the right thing. If any argument is present, it kills rather than
 * deletes, to prevent loss of text if typed with a big argument. Normally 
 * bound to C-d. 
 */
int
forwDelChar(int f, int n)
{
    ...
    /*
     * Always make ldelete save the deleted stuff in a kill buffer unless
     * only one character and not in letter kill mode. 
     */
...
killLine(int f, int n)
{
    ...
    } else if (frameCur->windowCur->dotLine ==
	       frameCur->bufferCur->baseLine)
	return meErrorEob();
    else if (f == meFALSE) {
	chunk =
	...
	if ((chunk = frameCur->windowCur->dotOffset) == 0)
	    chunk = meLineGetLength(frameCur->windowCur->dotLine) + 1;
		  ...
		  meLineGetLength(frameCur->windowCur->dotLine) - chunk) ==
		 0)
	    chunk = 1;
...
/*
 * ml-write. This function writes a string on the message line mainly for
 * macro usage 
 */
int
mlWrite(int f, int n)
{
    register int    status;
    meUByte         buf[meBUF_SIZE_MAX];	/* buffer to recieve
						 * message into */

    if (n == 0)
	mlerase(0);
    else if ((status =
	      meGetString((meUByte *) "Message", 0, 0, buf,
			  meBUF_SIZE_MAX)) <= 0)
	return status;
    else {
	status = MWSPEC;
...
/*
 * move forward of backward the the next non-white char skipping '#' lines 
 * and comments Flags passed in are 0x01 - dont skip # lines 0x02 -
 * currently in a #define with '\'s 0x04 - in a C comment, so dont skip
 * other comments 
 */
...
moveToNonWhite(meUByte forwFlag, meUByte * flags, meInt depth)
{
	...
	if (forwFlag) {
		...
		if (frameCur->windowCur->dotOffset >=
		    meLineGetLength(frameCur->windowCur->dotLine)) {
		    /*
		     * Go back lines until we find a line which 1) Doesn't 
		     * start with a '#' 2) Has some text on it Move cursor 
		     * to end or if we find a c++ "//" comment then to the 
		     * char before that 
		     */
		    if (!((*flags) & MTNW_KNOWN))
			*flags |= MTNW_KNOWN | MTNW_NOTINHASH;
		    if ((lp =
			 meLineGetNext(frameCur->windowCur->dotLine)) ==
			frameCur->bufferCur->baseLine)
			return 0;
			...
			if ((ss[0] == 'e') && (ss[1] == 'l')) {
			    /*
			     * Moving forwards, we want to skip #else &
			     * #elifs, but findfence moves to the next #e? 
			     * if it was an #elif so we must check it is
			     * an endif 
			     */
			    frameCur->windowCur->dotOffset =
				((size_t) ss - (size_t) lp->text) - 1;
			    do {
			...
			/*
			 * skip the hash line itself, and if line has a \
			 * then next etc 
			 */
			inq = 0;
			lc = ' ';
			do {
				...
				/*
				 * Exit if we're in a hash define and
				 * we're at a '\' 
				 */
				else if ((ch == '\\') && (lc != '\\')
					 && (lc != '\'')) {
				    if ((lp =
					 meLineGetNext(frameCur->
						       windowCur->
						       dotLine)) ==
					frameCur->bufferCur->baseLine)
					return 0;
		    ...
		    frameCur->windowCur->dotOffset =
			((size_t) ss - (size_t) lp->text) - 1;
		    ch = meLineGetChar(frameCur->windowCur->dotLine,
				       frameCur->windowCur->dotOffset);
		    break;
		} else {
		    frameCur->windowCur->dotOffset++;
		...
		for (;;) {
		  ...
		  newline_skip:
		     ...
		     */
		    if ((lp =
			 meLineGetPrev(frameCur->windowCur->dotLine)) ==
			frameCur->bufferCur->baseLine)
			return 0;
		    frameCur->windowCur->dotLine = lp;
		    frameCur->windowCur->dotLineNo--;
			...
			if (((*flags) & MTNW_NOTINHASH) && (ch == '#')
			    && !((*flags) & MTNW_SKIPHASH)) {
			  hash_skip:
			    /*
			     * Moving backwards, we want to skip #else &
			     * #elifs find fence does this nicely for us 
			     */
			    if ((ss[0] == 'e') && (ss[1] == 'l')) {
				frameCur->windowCur->dotOffset =
				    ((size_t) ss - (size_t) lp->text) - 1;
			...
			do {
				...
				/*
				 * change ch from '*' to another bogus
				 * char, this stops / * / / / being
				 * misinterpreted 
				 */
				comCont++;
				ch = 0x01;
				...
				/*
				 * change ch from '/' to another bogus
				 * char, this stops the double '/' created 
				 * at a stop start c comment being
				 * interpreted as a c++ comment 
				 */
				comCont--;
				ch = 0x01;
				...
				if ((ch == '\\') && (lc != '\\')
				    && (lc != '\'')) {
				    /*
				     * must do a quick check to see if
			...
			*flags |= MTNW_KNOWN | MTNW_NOTINHASH;
		    ...
		    if ((*flags == (MTNW_NOTINHASH | MTNW_KNOWN))
			&& (comCont >= 0)) {

			/*
			 * If not in a hash and the current line has no
			 * mismatched close comment, we must check the
			 * previous line to see if it has a trailing '\'.
			 * If so then this line is part of a hash define,
			 * so we must skip to the line before the # 
			 */
			meUByte         tflags;
			long            lineNo =
			    frameCur->windowCur->dotLineNo;
			...
			if (((ch =
			      moveToNonWhite(forwFlag, &tflags,
					     depth)) != 0)
	...
	if (!isSpace(ch)) {
		...
		if (forwFlag) {
		    ...
		    if (c2 == '*') {	/* c comment, go to the end of it */
			if (findfence('*', forwFlag, depth) <= 0)
			    return 0;
			frameCur->windowCur->dotOffset++;
			continue;
		    } else if (c2 == '/') {	/* c++ comment, go to the
						 * end of the line */
			frameCur->windowCur->dotOffset =
			    meLineGetLength(frameCur->windowCur->dotLine);
			continue;
			...
			if (findfence('/', forwFlag, depth) <= 0)
			    return 0;
			/*
			 * ZZZZ - note that at this point we should check
			 * for a # at the start of the line and do the
			 * right stuff 
			 */
			continue;
		    }
...
findQuoteFence(meUByte qtype, meUByte flags, meUByte forwFlag)
{
    ...
    for (;;) {
	/*
	 * windowBackwardChar & windowForwardChar goes 1 character past
	 * the end of the line, therfore want to omit this character in
	 * the search. Check for end of line, and go back another
	 * character if true. 
	 */
	if (forwFlag) {
	    if (frameCur->windowCur->dotOffset <
		...
		if (comStt > 0)
		    /*
		     * if we have found an open comment it is more likely
		     * that the quote is part of a comment than the open
		     * comment being part of a multi- line string, quit 
		     */
		    break;
		lp = frameCur->windowCur->dotLine;
		do {
	...
	c = meLineGetChar(frameCur->windowCur->dotLine,
			  frameCur->windowCur->dotOffset);
	if (c == qtype) {
	    /*
	     * must count the '\'s before the character to check whether
	     * its the close or just quoted in a string, e.g. "\\\"" etc. 
	     */
	    register meInt  ii = frameCur->windowCur->dotOffset;
	    while ((--ii > 0)
		   && (meLineGetChar(frameCur->windowCur->dotLine, ii) ==
		       '\\'));
	    /*
	     * if its an odd number then its a quote (note that ii is over 
	     * decr) 
	     */
...
/*
 * find the matching bracket, brace or comment: Has 4 end points, leave
 * cursor at end point and returns 1 of the following -1 There is no
 * matching bracket as this is inside quotes 0 Failed. There is no match,
 * error.  1 Success.  2 There is no matching bracket as this is inside
 * comments 
 */
/*
...
findfence(meUByte ch, meUByte forwFlag, meInt depth)
{
    ...
    /*
     * safe-guard ME crash caused by too many fences creating recursion
     * nightmare 
     */
    if (++depth > 255)
	...
	if (forwFlag) {
	    for (;;) {
		if ((lp =
		     meLineGetNext(lp)) == frameCur->bufferCur->baseLine)
		    return meFALSE;
		ii++;
		ss = lp->text;
		while (((cc = *ss++) != '\0') &&
		...
		if (cc == '#') {
			...
			do {
				...
				return meFALSE;
			    /*
			     * findfence can only succeed with a line
			     * containing #e[nl] 
			     */
	    ...
	    for (;;) {
		if ((lp =
		     meLineGetPrev(lp)) == frameCur->bufferCur->baseLine)
		    return meFALSE;
		ii++;
		ss = lp->text;
		while (((cc = *ss++) != '\0') &&
...
#if MEOPT_HILIGHT
    else if (frameCur->bufferCur->indent
	     && (meHilightGetFlags(indents[frameCur->bufferCur->indent]) &
		 HICMODE))
	inCom = 0;
#endif
    else
	...
	if ((cc = moveToNonWhite(forwFlag, &inCom, depth)) == 0)
	    break;
	if (cc == '/') {
	    cc = meLineGetChar(frameCur->windowCur->dotLine,
			       frameCur->windowCur->dotOffset + 1);
	    if (cc != '*')
		continue;
	    if (ch == '/')
		...
		return meTRUE;
	    /*
	     * one possible gotcha here is a / * / / / / * / comment -
	     * check for it 
	     */
	    if ((ch == '/') && frameCur->windowCur->dotOffset
		&&
		(meLineGetChar
		 (frameCur->windowCur->dotLine,
		  frameCur->windowCur->dotOffset - 1) == '/'))
		...
		if (findQuoteFence(cc, inCom, forwFlag) == meFALSE) {
		     ...
		     * end quote on hte same line then it is more likely
		     * this was a single quote, i.e it's like that. Handle
		...
		if ((ss =
		     meStrchr(fenceString + meFENCE_BRACKET_OFFSET,
			      cc)) != NULL) {
		int             ii = ss - fenceString;

		/*
		 * check for an oposite direction bracket - i.e. a ')'
		 * when going forward If so return a failure 
		 */
		if ((ii & 1) != forwFlag)
		    return 0;
		/*
		 * This is a same direction bracket - i.e. a '(' when
		 * going forward If so find the other side of the bracket
		 * (using findfence) 
		 */
		if ((ii =
		     findfence(fenceString[ii ^ 1], forwFlag,
			       depth)) <= 0) {
		    if (!inAps)
			inAps = ii;
		    break;
...
gotoFence(int f, int n)
{
	...
	if (ch == '*') {
	    /*
	     * if its a comment fence, default is forward, but check for
	     * reverse 
	     */
		...
		/*
		 * must move back into the comment as well, or we will end 
	    ...
	    /*
	     * this is a #if, #elif, #else, #endif fence workout which and 
	     * the direction 
	     */
	    ...
	    /*
	     * this is a simple bracket fence, i.e. (,),{,},[,] the
	     * direction and close ch can be calculated 
	     */
	...
	if ((ret = findfence(ch, forwFlag, 0)) > 0) {
		...
		return meTRUE;
	    /*
	     * We just want to show the location so, update, delay then
	     * move back 
	     */
	    update(meFALSE);
		...
		/*
		 * the redraw has changed the top line - must do a major
		 * update 
		 */
...
prevCToken(meUByte * token, int size)
{
	...
	((offset > 0)
	 && !isSpace(frameCur->windowCur->dotLine->text[offset - 1])))
	return 0;
    return !meStrncmp(frameCur->windowCur->dotLine->text + offset, token,
		      size);
}
...
getCoffset(meHilight * indentDef, int onBrace, int *inComment)
{
    ...
    *inComment = 0;
	...
	case '}':
	  ...
	  find_bracket_fence:
		...
		if (cc == '{') {
		    ret = getccol();
		    if ((ret == 0)
			|| (ret < meIndentGetBraceIndent(indentDef))) {
			if (brakCont < 0)
			    return -brakCont;
			return 0;
		    }
	...
	case '{':
		...
		if ((cc = moveToNonWhite(0, &mtnwFlag, 0)) != 0) {
			...
			mlp = NULL;
			       ...
			       && (frameCur->windowCur->dotOffset >= 3)
			       && !meStrncmp(frameCur->windowCur->dotLine->
					     text +
					     frameCur->windowCur->
					     dotOffset - 3, "enum", 4)) {
			indent = 3;
			...
			if ((cc = gotoFrstNonWhite()) == 'e') {
			    if (!meStrncmp
				(frameCur->windowCur->dotLine->text +
				 frameCur->windowCur->dotOffset,
				 "extern \"C\"", 10))
				indent = 2;
			} else if ((cc == 't') &&
				   !meStrncmp(frameCur->windowCur->
					      dotLine->text +
					      frameCur->windowCur->
					      dotOffset, "typedef", 7)
				   &&
		...
		if (!brakCont) {
			 ...
			 * indent lines by statement unless we are
			 * indenting the closing '}' in which case return
			 * 0 
			 */
			if (ii != 0)
			    brakCont =
				ii +
				meIndentGetBraceStatementIndent(indentDef);
			else if (onBrace > 0)
			    brakCont = 0;
			else
			    brakCont =
				meIndentGetStatementIndent(indentDef);
	...
	case '[':
		...
		while (((ch =
			 meLineGetChar(frameCur->windowCur->dotLine,
		       ...
		       || (ch == '\t'));
		brakCont = getccol();
		if ((meIndentGetContinueMax(indentDef) > 0)
		    && (brakCont > meIndentGetContinueMax(indentDef))) {
		    meUShort        off;
		    int             ii;
	...
	case '"':
	    /*
	     * only return if findQuoteFence returns meFALSE, returns -1
	     * if we could be in a comment 
	     */
	...
	default:
		...
		if (cc == 'e') {
		    if (((frameCur->windowCur->dotOffset == 0)
			 ||
			 isSpace(meLineGetChar
				 (frameCur->windowCur->dotLine,
				  frameCur->windowCur->dotOffset - 1)))
			&& !meStrncmp(frameCur->windowCur->dotLine->text +
				      frameCur->windowCur->dotOffset,
				      "else", 4)
			&&
			(isSpace
			 (meLineGetChar
			  (frameCur->windowCur->dotLine,
			   frameCur->windowCur->dotOffset + 4))
			 ||
			 (meLineGetChar
			  (frameCur->windowCur->dotLine,
			   frameCur->windowCur->dotOffset + 2) == '{')))
		    ...
		    if (((frameCur->windowCur->dotOffset == 0)
			 ...
			 isSpace(meLineGetChar
				 (frameCur->windowCur->dotLine,
				  frameCur->windowCur->dotOffset - 1)))
			&& !meStrncmp(frameCur->windowCur->dotLine->text +
				      frameCur->windowCur->dotOffset, "if",
				      2)
			&&
			(isSpace
			 (meLineGetChar
	...
	if (moveToNonWhite(0, &mtnwFlag, 0) == ')') {
		...
		|| prevCToken((meUByte *) "for", 3)
		|| prevCToken((meUByte *) "while", 5)
		|| prevCToken((meUByte *) "switch", 6)) {
		frameCur->windowCur->dotOffset = 0;
		gotoFrstNonWhite();
		brakCont =
		    getccol() + meIndentGetStatementIndent(indentDef);
	    } else {
		if (onBrace < 0)
	...
	} else if (!prevCToken((meUByte *) "do", 2)
		   && !prevCToken((meUByte *) "else", 4)) {
	    if (onBrace < 0)
		brakCont += meIndentGetStatementIndent(indentDef);
	    else if (brakCont)
		brakCont += meIndentGetContinueIndent(indentDef);
	...
	/*
	 * this is a new context brase (i.e. one not following an if or
	 * else etc. used to define a new variable in mid function. The
	 * next line should be indented by a Statement not (Brace +
	 * BraceStatement) 
	 */
	brakCont += (meIndentGetStatementIndent(indentDef) << 1) -
	    meIndentGetBraceIndent(indentDef) -
	    meIndentGetBraceStatementIndent(indentDef);
    return brakCont;
}
...
/*
 * do C indent, returns -ve if failed (god knows how) the indent on
 * success. 
 */
int
doCindent(meHilight * indentDef, int *inComment)
{
	...
	if ((cc == 'c') &&
	    !meStrncmp(frameCur->windowCur->dotLine->text +
		       frameCur->windowCur->dotOffset, "case", 4)
	    && !isAlpha(frameCur->windowCur->dotLine->
			text[frameCur->windowCur->dotOffset + 4]))
	    addInd += meIndentGetCaseIndent(indentDef);
	else if ((cc == 'e') &&
		 !meStrncmp(frameCur->windowCur->dotLine->text +
			    frameCur->windowCur->dotOffset, "else", 4)
		 &&
		 (!isAlpha
		  (frameCur->windowCur->dotLine->
		   text[frameCur->windowCur->dotOffset + 4])
	    ...
	    onBrace = 2;
		 ...
		 !meStrncmp(frameCur->windowCur->dotLine->text +
			    frameCur->windowCur->dotOffset, "default", 7)
		 && !isWord(frameCur->windowCur->dotLine->
			    text[frameCur->windowCur->dotOffset + 7]))
	    addInd += meIndentGetCaseIndent(indentDef);
	else if ((cc == 'p') &&
		 ((ii =
		   7,
		   !meStrncmp(frameCur->windowCur->dotLine->text +
			      frameCur->windowCur->dotOffset, "private",
			      ii))
		  || (ii =
		      6,
		      !meStrncmp(frameCur->windowCur->dotLine->text +
				 frameCur->windowCur->dotOffset, "public",
				 ii))
		  || (ii =
		      9,
		      !meStrncmp(frameCur->windowCur->dotLine->text +
				 ...
				 "protected", ii)))
		 && !isAlpha(frameCur->windowCur->dotLine->
			     text[frameCur->windowCur->dotOffset + ii])) {
	    ii += frameCur->windowCur->dotOffset;
	    while (((cc =
		     meLineGetChar(frameCur->windowCur->dotLine,
				   ii++)) == ' ') || (cc == '\t'));
	...
	*inComment = 0;
     ...
     * statement line. i.e. > statement; // blah blah blah blah > // more
     * blah blah blah We want to preserve the alignment of the C++ comment
     * if we find them stacked like this. If this lines start position
     * exactly matches the previous line then we say that the comment
     * position is correct. If the comment is in a different column then we
     * assume it is supposed to be aligned with the statement. i.e >
     * statement; // blah blah blah blah > // more blah blah blah > //
     * Statement aligned comment. So restyling the above should not change
	 ...
	 * comments is aligned with the previous line, if it is then we
	 * must keep the same alignment. 
	 */
	frameCur->windowCur->dotLine =
	    meLineGetPrev(frameCur->windowCur->dotLine);
	frameCur->windowCur->dotOffset = 0;
	if (setccol(curInd)) {	/* Move to position on prev line */
		...
		/*
		 * Matched our position with the current position, use
		 * this as the indent. 
		 */
		addInd =
		    curInd - getCoffset(indentDef, onBrace, inComment);
		if (addInd < 0)
	...
	if (*inComment)
	    ind += comInd;
	else if (((onBrace < 0) && (ind == 0)) || ((ind += addInd) < 0))
	    ind = 0;
    }
...
meAnchorGet(meBuffer * bp, meInt name)
{
	...
	if (p->name == name) {
	  ...
	  try_again:
	    ...
	    /*
	     * Okay, we failed to find the mark so only chance now is the
	     * mark is in a narrow 
	     */
#if MEOPT_NARROW
	    {
		...
		while (nrrw != NULL) {
			...
			if (lp == bp->dotLine) {
			    /*
			     * We've found the mark in this narrow, remove 
			     * the narrow and try again 
			     */
	    ...
	    meLine * lp, meUShort off, int silent)
{
	...
	if (p->line != lp) {
	    ...
	    while (q != NULL) {
		if ((q != p) && (q->line == p->line)
		    && (meAnchorGetLineFlag(q) == flag))
		    break;
		q = q->next;
	    }
	    if (q == NULL)
...
setAlphaMark(int f, int n)
{
    ...
    if ((cc =
	 mlCharReply((meUByte *) "Place mark: ", mlCR_QUIT_ON_USER, NULL,
		     NULL)) == -2)
	cc = mlCharReply((meUByte *) "Place mark: ", mlCR_ALPHANUM_CHAR,
			 NULL, NULL);

    if (cc < 0)
	return ctrlg(meFALSE, 1);

    return meAnchorSet(frameCur->bufferCur, cc,
		       frameCur->windowCur->dotLine,
		       frameCur->windowCur->dotOffset, 0);
}
...
gotoAlphaMark(int f, int n)
{
	...
	*s = '\0';
    ...
    if ((cc =
	 mlCharReply((meUByte *) "Goto mark: ", mlCR_QUIT_ON_USER, NULL,
		     NULL)) == -2)
	cc = mlCharReply((meUByte *) "Goto mark: ", mlCR_ALPHANUM_CHAR,
			 NULL, NULL);
	...
	if (ii == 0)
	    return mlwrite(MWABORT | MWCLEXEC,
			   (meUByte *) "[No marks in buffer]");
	allmarks[ii] = '\0';
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[Marks in buffer:%s]", allmarks);
    }
...
cmpBuffers(int f, int n)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "[Not enough windows to compare]");
	...
	for (;;) {
	    ...
	    /*
	     * Back to the start of the buffers. Advance to the next
	     * character in the buffer. if the current character is a
	     * space then advance to the next non-white character in the
	     * buffer. 
	     */
	    wp = swp;
	    if (((moreData = meWindowForwardChar(wp, 1)) > 0)
		&& (cc == ' ')) {
		do {
	    ...
	    while ((wp = wp->next) != NULL) {
		if ((wp->flags & meWINDOW_NO_CMP) == 0) {
		    if (((winData = meWindowForwardChar(wp, 1)) > 0)
			&& (cc == ' ')) {
			do {
			    tmpc = meWindowGetChar(wp);
			    if (!isSpace(tmpc))
...
createCallback(int f, int n)
{
	...
	mac->callback =
	    ((tp.tv_sec - startTime) * 1000) + (tp.tv_usec / 1000) + n;
...
callBackHandler(void)
{
	...
	if ((mac->callback >= 0) && (mac->callback < tim)) {
	     ...
	     * receive the key "callback", this must be bound to this
	     * macro, otherwise the input handler will not know what macro
	     * to call 
	     */
	    if (frameCur->bufferCur->inputFunc >= 0) {
		/*
		 * binary chop through the key table looking for the
		 * character code. ** If found then return the index into
		 * the names table. 
		 */
		...
		do {
		    ...
		    if ((status =
			 (ME_SPECIAL | SKEY_callback) - ktp[mid].code) ==
			0) {
			/*
			 * Found - return index 
			 */
...
setCursorToMouse(int f, int n)
{
	...
	if (wp->vertScrollBarMode & WMSCROL)	/* Scroll bar enabled ?? */
	    mouse_pos |= MISBSPLIT + ii;	/* Yes - report scroll bar 
						 * component */
    ...
    /*
     * Work out the new postion of the cursor in the buffer. Save the old
     * position so that we can later check if we have moved. 
     */
    odoto = wp->dotOffset;
    odotp = wp->dotLine;
     ...
     */
    if (wp->dotLine != ln)
	wp->horzScroll = wp->horzScrollRest;	/* Set the line scroll to
						 * the window offset */
     ...
     * changed cursor position. Check the line first as WFMOVEL does the
     * column as well 
     */
    if (wp->dotLine != odotp)
     ...
     */
    if ((frameCur->bufferCur == wp->buffer)
