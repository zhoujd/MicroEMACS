/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com bind.c - Binding
 * functions. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
/*
 * meGetKey get a key sequence from either the current macro execute
 * line, a keyboard macro or the keyboard flag - meGETKEY flags 
 */
meUShort
meGetKey(int flag)
{
	...
	if ((tok[0] != '@') || (tok[1] != 'm') || (tok[2] != 'n')) {
	    ...
	    /*
	     * check there are no superfluous chars in the string, fail if 
	     * found as this is probably a bad bind string 
	     */
	    if (*tp != '\0')
		return 0;
	    return cc;
	...
	if (tok[3] == 'a')
	    execstr = tp;
	/*
	 * Force an update of the screen to to ensure that the user can
	 * see the information in the correct location 
	 */
	update(meTRUE);
    }
    /*
...
meGetStringFromKey(meUShort cc, register meUByte * seq)
{
    ...
    if (cc & ME_PREFIX_MASK) {
	seq +=
	    meGetStringFromChar(prefixc
				[(cc & ME_PREFIX_MASK) >> ME_PREFIX_BIT],
				seq);
	*seq++ = ' ';
    }
...
{				/* describe the command for a certain key */
     ...
     */
    if ((cc = meGetKey(meGETKEY_SILENT)) == 0)	/* get a silent command
						 * sequence */
	return ctrlg(0, 1);
     ...
     */
    mlwrite(MWCURSOR, (meUByte *) "%s\"%s\" %s%s", dskyPrompt, outseq,
	    argStr, ptr);
...
charMaskTblInit(void)
{
	...
	charMaskTbl2[ii] &=
	    ~(CHRMSK_LOWER | CHRMSK_UPPER | CHRMSK_HEXDIGIT |
	      CHRMSK_SPLLEXT);
	charCaseTbl[ii] = ii;
    }
    for (; ii < 256; ii++) {
	...
	*ss = '\0';
...
#if MEOPT_MAGIC
	/*
	 * reset the last magic search string to force a recompile as char 
	 * groups may change 
	 */
	mereRegexInvalidate();
#endif
	if (flags & 0x1A003)
	    return mlwrite(MWABORT,
			   (meUByte *)
			   "[Cannot set flags u, l, A, L or U]");
	meStrcpy(tnkyPrompt + 14, "chars");
	if (meGetString(tnkyPrompt, MLFFZERO, 0, buf1, meBUF_SIZE_MAX) <=
	    0)
	    return meFALSE;
	if (flags & 0x20000) {
	    ss = buf1;
		...
		while ((c1 = *ss++) != '\0') {
		    if ((c1 == meCHAR_LEADER)
			&& ((c1 = *ss++) == meCHAR_TRAIL_NULL))
			c1 = 0;
		    c1 = toUserFont(c1);
		    if ((c2 = *ss++) == '\0')
			break;
		    if ((c2 == meCHAR_LEADER)
			&& ((c2 = *ss++) == meCHAR_TRAIL_NULL))
			c2 = 0;
	...
	/*
	 * convert the string to user font and take out any LEADER
	 * padding, 0's are now 0's so need int count 
	 */
	ss = buf1;
	dd = buf1;
	ii = 0;
...
/*
 * match fname to a function in the names table and return index into
 * table or -1 if none fname; name to attempt to match 
 */
int
...
int
bindkey(meUByte * prom, int f, int n, meUShort * lclNoBinds,
	meBind ** lclBinds)
{
    register int    ss;
    register meUShort cc;	/* command key to bind */
    register meIFuncII kfunc;	/* ptr to the requexted function to bind
				 * to */
	...
	if ((n < 1) || (n > ME_PREFIX_NUM))
	    return mlwrite(MWABORT,
			   (meUByte *) "[Prefix number is out of range]");
	ssc = prefixc[n];
	...
	return mlwrite(MWABORT, (meUByte *) "Can not locally bind [%s]",
		       buf);
#endif
    mlwrite(MWCURSOR | MWCLEXEC, (meUByte *) "%s [%s] to: ", prom, buf);
    if ((cc =
	 meGetKey((ss) ? meGETKEY_SILENT | meGETKEY_SINGLE :
		  meGETKEY_SILENT)) == 0)
	return ctrlg(0, 1);
...
{
    return bindkey((meUByte *) "local bind", f, n,
		   &(frameCur->bufferCur->bindCount),
		   &(frameCur->bufferCur->bindList));
}
int
mlBind(int f, int n)
{
...
unbindkey(meUByte * prom, int n, meUShort * lclNoBinds, meBind ** lclBinds)
{
    ...
    if ((cc =
	 meGetKey((n ==
		   0) ? meGETKEY_SILENT | meGETKEY_SINGLE :
		  meGETKEY_SILENT)) == 0)
	return ctrlg(0, 1);

    /*
     * If interactive then there is no really need to print the string
     * because you will probably not see it !! The TTputc() is dangerous
     * and squirts characters over the screen if the previous mlwrite()
     * determined that we were in a macro and has not moved the cursor to
     * the message line. Hence, safer bet is to re-do the mlwrite(); my
     * preference would be to remove the keyboard echo !! - Jon Green
     * 13/03/97 
     */
    meGetStringFromKey(cc, outseq);	/* change to something printable */
    mlwrite(MWCURSOR | MWCLEXEC, (meUByte *) "%s unbind: %s", prom,
	    outseq);
#if MEOPT_LOCALBIND
    if (lclNoBinds != NULL) {
	register meBind *ktp;
	...
	if ((rr = delete_key(cc)) > 0) {
		...
		if (mask) {
			...
			ss = mlNoBinds;
			while (--ss >= 0)
			    if ((mlBinds[ss].code & ME_PREFIX_MASK) ==
				mask)
	...
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[%s Key \"%s\" not bound]", prom,
		       outseq);
    return meTRUE;
}
...
{
    return unbindkey((meUByte *) "Local", n,
		     &(frameCur->bufferCur->bindCount),
		     &(frameCur->bufferCur->bindList));
}
int
...
buildlist(int n, meUByte * mstring)
{
    ...
    meUByte         _outseq[meBUF_SIZE_MAX];	/* output buffer for
						 * keystroke sequence */
    meUByte        *outseq = _outseq + 4;	/* output buffer for
						 * keystroke sequence */
     ...
     */
    if ((wp =
	 meWindowPopup(BcommandsN, (BFND_CREAT | BFND_CLEAR | WPOP_USESTR),
		       NULL)) == NULL)
	...
	/*
	 * Skip command if its a hidden macro (and bit 1 of arg n is set)
	 * or we are executing an apropos command and current string
	 * doesn't include the search string 
	 */
	if (((n & 0x01) && (cmd->id >= CK_MAX)
	     && (((meMacro *) cmd)->hlp->flag & meMACRO_HIDE))
	    || ((mstring != NULL)
		&& !regexStrCmp(cmd->name, mstring, meRSTRCMP_WHOLE)))
...
/*
 * list-functions bring up a fake buffer and list the key bindings into it 
 * with view mode 
 */
int
...
int
commandApropos(int f, int n)
{				/* Apropos (List functions that match a
				 * substring) */
    meUByte         mstring[meSBUF_SIZE_MAX];	/* string to match cmd
						 * names to */
...
descBindings(int f, int n)
{
    ...
    if ((wp =
	 meWindowPopup(BbindingsN, (BFND_CREAT | BFND_CLEAR | WPOP_USESTR),
		       NULL)) == NULL)
	return meFALSE;
    bp = wp->buffer;
