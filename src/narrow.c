/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com narrow.c - Narrow out
 * regions of a buffer. Copyright (C) 1999-2001 Steven Phillips Copyright 
...
meNarrow       *
meBufferCreateNarrow(meBuffer * bp, meLine * slp, meLine * elp, meInt sln,
		     meInt eln, meInt name, meScheme scheme,
		     meUByte * markupLine, meInt markupCmd, meInt undoCall)
{
    meWindow       *wp;
    meNarrow       *nrrw,
                   *cn,
	...
	bp->dotLineNo = sln;
	if (markupLine == NULL) {
	    execBufferFunc(bp, markupCmd,
			   (meEBF_ARG_GIVEN | meEBF_USE_B_DOT |
			    meEBF_HIDDEN), meNarrowNameGetMarkupArg(name));
    ...
    if (meAnchorSet
	(bp, name, elp,
	 (meUShort) (((name & meNARROW_TYPE_MASK) ==
		      meNARROW_TYPE_TO_BOTTOM) ? 1 : 0), 1) <= 0) {
	meFree(nrrw);
	return NULL;
    }
	...
	elp->flag =
	    (elp->
	     flag & ~meLINE_SCHEME_MASK) | (ii << meLINE_SCHEME_SHIFT) |
	    meLINE_CHANGED;
	nrrw->scheme = ii;
    } else
	nrrw->scheme = 0;
...
static void
meBufferExpandNarrow(meBuffer * bp, register meNarrow * nrrw,
		     meUByte * firstLine, meInt undoCall)
{
    ...
    /*
     * Note that bp->dotLineNo is used outside this function to setup the
     * undo 
     */
     ...
     */
    markup = (nrrw->markupCmd) && (lp2 != bp->baseLine) && (undoCall
							    || (lp2->
								flag &
								meLINE_MARKUP));
    if (markup) {
	...
	if (wp->buffer == bp) {
		...
		if (markup && (wp->dotLineNo == bp->dotLineNo)) {
		    /*
		     * the markup line has been removed, move to the next
		     * line 
		     */
		    wp->dotLine = lp2;
		    wp->dotOffset = 0;
	...
	if (firstLine != NULL) {
		...
		/*
		 * existing line was to short, so created a replacement
		 * line. must now remove the old line. Anchors are updated 
		 * later and no window can be using this line as a dot or
		 * mark 
		 */
		...
		meLineResetAnchors(meLINEANCHOR_ALWAYS |
				   meLINEANCHOR_FIXED, bp, lp1, lp2, 0, 0);
	    if (lp1 != lp2)
		meFree(lp1);
	}
    }
		     ...
		     meUByte * firstLine, meInt undoCall)
{
	...
	meUndoAddUnnarrow(bp->dotLineNo, bp->dotLineNo + nrrw->noLines,
			  nrrw->name, scheme, nrrw->markupCmd,
			  nrrw->markupLine);
    }
#endif
    if (nrrw->next != NULL)
	...
	if (nrrw->markupLine->flag & meLINE_ANCHOR)
	    /*
	     * move any anchors on the markup line to the start of the
	     * first narrowed line 
	     */
	    meLineResetAnchors(meLINEANCHOR_ALWAYS | meLINEANCHOR_FIXED,
			       bp, nrrw->markupLine,
			       meLineGetNext(meLineGetPrev
					     (nrrw->markupLine)), 0, 0);
	meFree(nrrw->markupLine);
    } else if (nrrw->scheme &&
	       (((lp =
		  bp->dotLine)->flag & meLINE_SCHEME_MASK) ==
		(nrrw->scheme << meLINE_SCHEME_SHIFT))) {
	/*
	...
	if (lp->flag & meLINE_ANCHOR_NARROW) {
	    ...
	    while (aa != NULL) {
		if ((meAnchorGetLine(aa) == lp)
		    && (meAnchorGetType(aa) == meANCHOR_NARROW)) {
		    /*
    ...
    if (!undoCall
	&& ((nrrw->name & meNARROW_TYPE_MASK) != meNARROW_TYPE_OUT)) {
	nn = frameCur->bufferCur->narrow;
	while (nn != NULL) {
	    if ((nn->name & meNARROW_NUMBER_MASK) ==
		(nrrw->name & meNARROW_NUMBER_MASK)) {
...
meBufferCollapseNarrowAll(meBuffer * bp)
{
    ...
    /*
     * redoing the narrows must be done in the right order, i.e. last in
     * the list first! So go to the last and then use the prev pointers to 
     * move back. 
     */
	...
	if (nrrw->expanded) {
	    noLines = nrrw->noLines;
	    if (nrrw->markupLine != NULL) {
		meAssert(meLineGetPrev(nrrw->markupLine) ==
			 nrrw->slp->prev);
		meAssert(meLineGetNext(nrrw->markupLine) ==
			 nrrw->elp->next);
		meLineGetNext(meLineGetPrev(nrrw->markupLine)) =
		    nrrw->markupLine;
		meLineGetPrev(meLineGetNext(nrrw->markupLine)) =
		    nrrw->markupLine;
		noLines--;
	    } else {
...
meInt
meBufferRegionExpandNarrow(meBuffer * bp, meLine ** startLine,
			   meUShort soffset, meLine * endLine,
			   meUShort eoffset, meInt remove)
{
    /*
     * the kill probably includes one or more narrows, find them, expand
     * then and add them to the size of the kill 
     */
    meLine         *slp,
                   *plp,
                   *clp,
    ...
    /*
     * if at the start of the line and this is a narrow markup line then
     * this narrow should be included 
     */
    if ((soffset == 0) && (clp != bp->baseLine) &&
	((meLineGetFlag(clp) &
	  (meLINE_ANCHOR_NARROW | meLINE_MARKUP | meLINE_PROTECT)) ==
	 (meLINE_ANCHOR_NARROW | meLINE_MARKUP | meLINE_PROTECT))
	...
	if ((meLineGetFlag(clp) & meLINE_ANCHOR_NARROW) &&
	    ...
	    if (elp != NULL)
		/*
		 * currently trundling down a narrowed out section add to
		 * adjLen 
		 */
		adjLen += len + 1;
	    plp = clp;
...
narrowBuffer(int f, int n)
{
	...
	if (frameCur->bufferCur->narrow == NULL)
	    return mlwrite(MWABORT | MWCLEXEC,
			   (meUByte *) "[Current buffer not narrowed]");

	while (frameCur->bufferCur->narrow != NULL)
	    meBufferRemoveNarrow(frameCur->bufferCur,
				 frameCur->bufferCur->narrow, NULL, 0);
	return meTRUE;
    } else if (n == 2) {
	if ((frameCur->windowCur->dotLine->flag & meLINE_ANCHOR_NARROW) &&
	    ((nrrw =
	      meLineGetNarrow(frameCur->bufferCur,
			      frameCur->windowCur->dotLine)) != NULL)) {
	    meBufferRemoveNarrow(frameCur->bufferCur, nrrw, NULL, 0);
	    return meTRUE;
	}
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[No narrow on current line]");
    } else if ((n != 3) && (n != 4))
    ...
    if (f & 0x10) {
	if (meGetString
	    ((meUByte *) "Line Scheme", 0, 0, buf, meBUF_SIZE_MAX) <= 0)
	    return meABORT;
	scheme =
	    convertUserScheme(meAtoi(buf), frameCur->bufferCur->scheme);
    } else
	scheme = meSCHEME_INVALID;

    if (f & 0x20) {
	if (meGetString
	    ((meUByte *) "Markup Line", 0, 0, ml1b, meBUF_SIZE_MAX) <= 0)
	    return meABORT;
	if ((n == 3) &&
	    (meGetString
	     ((meUByte *) "Markup Line", 0, 0, ml2b, meBUF_SIZE_MAX) <= 0))
	    return meABORT;
	ml1 = ml1b;
	ml2 = ml2b;
	markupCmd = -1;
    ...
    if (f & 0x40) {
	if ((meGetString
	     ((meUByte *) "Markup Command", MLCOMMAND, 2, buf,
	      meBUF_SIZE_MAX) <= 0)
    ...
    if (n == 3) {
	/*
	 * meNARROW_TYPE_TO_TOP must be greater than
	 * meNARROW_TYPE_TO_BOTTOM to ensure the top narrow is first in
	 * the list en therefore expanded first if all lines in between a
	 * delete. This ensures the top stays at the top 
	 */
	n = meNARROW_TYPE_TO_TOP;
	if (elp != frameCur->bufferCur->baseLine) {
	    if ((nrrw =
		 meBufferCreateNarrow(frameCur->bufferCur, elp,
				      frameCur->bufferCur->baseLine, eln,
				      frameCur->bufferCur->lineCount,
				      meNARROW_TYPE_TO_BOTTOM, scheme, ml2,
				      markupCmd, 0)) == NULL)
		return meFALSE;
	    n = meANCHOR_NARROW | meNARROW_TYPE_TO_TOP | (nrrw->
							  name &
							  meNARROW_NUMBER_MASK);
	}
	if (sln != 0)
	    nrrw =
		meBufferCreateNarrow(frameCur->bufferCur,
				     meLineGetNext(frameCur->bufferCur->
						   baseLine), slp, 0, sln,
				     n, scheme, ml1, markupCmd, 0);
    } else
	nrrw =
	    meBufferCreateNarrow(frameCur->bufferCur, slp, elp, sln, eln,
				 meNARROW_TYPE_OUT, scheme, ml1, markupCmd,
				 0);
