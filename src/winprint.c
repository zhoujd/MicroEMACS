/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com winprint.c - Windows
 * Print handler. Copyright (c) 1998-2001 Jon Green Copyright (C)
...
#define PAGSIZ_EXACT             1	/* Require exact page size (if
					 * possible) */
...
/*
 * Set this flag to use the new way that we should enumerate fonts. This
 * only operates for NT 4.0 and Win 95. We typeically use the existing
 * Font enumeration that is good for win32s 
 */
#define USE_EnumFontFamiliesEx    0	/* 1 = use Ex rather then old
					 * function */
...
printMakeDevNames(char *driver, char *printer, char *port, int defaultp)
{
	 ...
	 */
	if ((hDevNames =
	     GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, jj)) != NULL) {
...
getPrinterInfo(int index)
{
	...
	if (EnumPrinters(PRINTER_ENUM_FAVORITE | PRINTER_ENUM_LOCAL,
			 NULL, 2,
			 (LPBYTE) pi2, dwNeeded, &dwNeeded, &dwReturned)) {
	    int             ii;	/* Loop counter */
		...
		/*
		 * Find the named printer from our parameters this is
		 * always the best. 
		 */
		if ((index == -1) && (	/* ((printer.param
					 * [mePI_WINDRIVER].p != NULL) &&
					 * (pi2[ii].pDriverName != NULL)
					 * && (strcmp (printer.param
					 * [mePI_WINDRIVER].p,
					 * pi2[ii].pDriverName) == 0)) && */
					 ((printer.param[mePI_WINDEVICE].
					   p != NULL)
					  && (pi2[ii].pPrinterName != NULL)
					  &&
					  (strcmp
					   (printer.param[mePI_WINDEVICE].
					    p, pi2[ii].pPrinterName) == 0))
					 &&
		 ...
		 */
		if (((index == -1)
		     && (pi2[ii].Attributes & PRINTER_ATTRIBUTE_DEFAULT))
		    || (index == ii)) {
		    DEVNAMES       *hDev;
		    char           *dicp;
		    int             jj;
		    ...
		    hDev->wDriverOffset = jj;
		    jj +=
			sprintf(&dicp[jj], "%s", pi2[ii].pDriverName) + 1;
		    hDev->wDeviceOffset = jj;
		    jj +=
			sprintf(&dicp[jj], "%s", pi2[ii].pPrinterName) + 1;
		    hDev->wOutputOffset = jj;
...
printGetDC(void)
{
    ...
    printDlg.hDC = CreateDC(&((char *) (hDev))[hDev->wDriverOffset], &((char *) (hDev))[hDev->wDeviceOffset], NULL,	/* &((char 
															 *)(hDev))[hDev->wOutputOffset]*/
			    NULL);
 ...
 */
static int
constructFontTable(HDC hDC, HFONT * fontTable, int charx, int chary,
		   char *fontName)
{
	...
	if (ii == 0) {
	    ...
	    /*
	     * Get the width and height of the font. This is a mono font
	     * so all fonts should be the same size 
	     */
...
AbortProc(HDC hdc, int nCode)
{
     ...
     */
    return			/* (printStatus & PRINT_SPOOLING) ? meTRUE 
				 * : meFALSE; */ bPrint;
}
 ...
 */
static int
pcdComputeFont(PAGEDESC * pd, int pagex, int selx, int pagey, int sely,
	       int pointy)
{
    HFONT           hFont;
    LOGFONT         logfont;
    ...
    /*
     * Match horizontally. Choose a low number for the vertical scale -
     * will correct later. 
     */
    else
	...
	/*
	 * A Font has been created, now determine what the size of the
	 * page is. 
	 */
	...
	if (sely != PAGSIZ_DONTCARE) {
	    ...
	    /*
	     * Font is too wide, try to reduce the font to fit
	     * horizontally. 
	     */
    ...
    /*
     * Back compute the settings according to the user request Sort out
     * the horizontal sizing first. 
     */
    if ((selx == PAGSIZ_EXACT) && (paperx <= pd->paperx))
	pd->upagex = pagex;
    else
	/*
	 * Compute the new active page size from the logical paper size 
	 */
...
int             CALLBACK
pcdEnumFontFamiliesCallback(ENUMLOGFONTEX * lpelf, NEWTEXTMETRIC * lpntm,
			    int fontType, LPARAM lParam)
#else
int             CALLBACK
pcdEnumFontFamiliesCallback(ENUMLOGFONT * lpelf, NEWTEXTMETRIC * lpntm,
			    int fontType, LPARAM lParam)
#endif
{
    /*
	...
	if ((index >= 0) && (printer.param[mePI_FONTFACE].p != NULL)
	    &&
	    (strcmp
...
pcdChange(HWND hWnd)
{
    ...
    int             masks[] =
	{ PFLAG_ENBLHEADER, PFLAG_ENBLFOOTER, PFLAG_ENBLLINENO };
    int             combos[] =
	{ IDC_HEIGHT, IDC_WIDTH, IDC_COLUMNS, IDC_ROWS };
    int             cindicies[] =
	{ mePI_PAGEY, mePI_PAGEX, mePI_COLS, mePI_ROWS };
    int             ecombos[] = { IDC_WIDTH_EXACTLY, IDC_HEIGHT_EXACTLY };
    int             ecindicies[] =
	{ mePI_SPECX, mePI_SPECY, mePI_FONTFACE };
    int             ii;
    char            buf[500];
    HWND            dWnd;
    DWORD           value;
	...
	char           *spos,
	               *epos;	/* Start and end text positions */
     ...
     */
    printer.param[mePI_FONTFACE].p =
	regGetString(regSet(regPrint, printNames[mePI_FONTFACE], buf),
		     (meUByte *) "Courier New");
    ...
    pcdComputeFont(&pd,
		   ...
		   printer.param[mePI_SPECX].l,
		   printer.param[mePI_PAGEY].l,
		   printer.param[mePI_SPECY].l, 10);
...
pcdDeviceName(HWND hWnd)
{
	 ...
	 */
	regSet(regPrint, printNames[mePI_WINDRIVER],
	       &((char *) (hDev))[hDev->wDriverOffset]);
	regSet(regPrint, printNames[mePI_WINDEVICE],
	       &((char *) (hDev))[hDev->wDeviceOffset]);
	regSet(regPrint, printNames[mePI_WINPORT],
	       &((char *) (hDev))[hDev->wOutputOffset]);
	regSet(regPrint, printNames[mePI_WINDEFAULT],
	       (hDev->wDefault == 0) ? "0" : "1");
	GlobalUnlock(printDlg.hDevNames);
    } else
...
pcdDialogue(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	for (ii = 0; ii < 3; ii++)
	    SendMessage(dWnd, CB_INSERTSTRING, ii,
			(LPARAM) pcdExactlyStrings[ii]);
	/*
	 * Re-fill the dialogue with a default value 
	 */
	SendMessage(dWnd, CB_SETCURSEL,
		    (WPARAM) printer.param[mePI_SPECX].l, 0);
	...
	for (ii = 0; ii < 3; ii++)
	    SendMessage(dWnd, CB_INSERTSTRING, ii,
			(LPARAM) pcdExactlyStrings[ii]);
	SendMessage(dWnd, CB_SETCURSEL,
		    (WPARAM) printer.param[mePI_SPECY].l, 0);
	...
	for (ii = 0; ii < 6; ii++)
	    SendMessage(dWnd, CB_INSERTSTRING, ii,
			(LPARAM) meItoa(ii + 1));
	SendMessage(dWnd, WM_SETTEXT, 0,
		    (LPARAM) meItoa(printer.param[mePI_ROWS].l));
	...
	for (ii = 0; ii < 6; ii++)
	    SendMessage(dWnd, CB_INSERTSTRING, ii,
			(LPARAM) meItoa(ii + 1));
	SendMessage(dWnd, WM_SETTEXT, 0,
		    (LPARAM) meItoa(printer.param[mePI_COLS].l));
	...
	for (ii = 0; ii <= 3; ii++)
	    SendMessage(dWnd, CB_INSERTSTRING, ii,
			(LPARAM) meItoa(ii * 80));
	SendMessage(dWnd, WM_SETTEXT, 0,
		    (LPARAM) meItoa(printer.param[mePI_PAGEX].l));
	...
	if ((printStatus & PRINT_DIALOGUE) == 0) {
	    ...
	    printDlg.hDevNames =
		printMakeDevNames(printer.param[mePI_WINDRIVER].p,
				  printer.param[mePI_WINDEVICE].p,
				  printer.param[mePI_WINPORT].p,
				  printer.param[mePI_WINDEFAULT].l);
	    printDlg.Flags =
		(PD_DISABLEPRINTTOFILE | PD_HIDEPRINTTOFILE | PD_NOPAGENUMS
		 | PD_PRINTSETUP | PD_RETURNDC | PD_RETURNDEFAULT);
	 ...
	 */
	{
	    ...
	    EnumFontFamiliesEx(printGetDC(), &logfont,
			       (FONTENUMPROC) pcdEnumFontFamiliesCallback,
			       (LPARAM) dWnd, 0);
	}
#else
	EnumFontFamilies(printGetDC(), NULL,
			 (FONTENUMPROC) pcdEnumFontFamiliesCallback,
			 (LPARAM) dWnd);
#endif
	/*
	 ...
	 */
	SetTimer(hWnd, PRINT_TIMER_ID, PRINT_TIMER_RESPONSE,
		 (TIMERPROC) pcdTimerCallback);
	return meTRUE;
	...
	case IDC_PC_PRINT:
		...
		pcdChange(hWnd);
		/*
		 * Save all of the settings back to the registry so that
		 * we use them next time.  First copy the computed values
		 * out of the printer descriptor. 
		 */
		...
		for (ii = 0; ii < PRINT_MAX; ii++) {
			...
			regSet(regPrint, printNames[ii],
			       meItoa(printer.param[ii].l));
		    else
			regSet(regPrint, printNames[ii],
			       printer.param[ii].p);
		}
...
WinPrintSetColor(HDC hDC, int colNo, int setBgCol)
{
	...
	if (GetDeviceCaps(hDC, RASTERCAPS) & RC_PALETTE) {
	    ...
	    /*
	     * Create a new entry for the colour Check the current palette 
	     * for the colour. If we have it already we do not need to
...
WinPrintThread(LPVOID wParam)
{
     ...
     */
    /*
     * if (((printStatus & PRINT_DIALOGUE) == 0) || (printDlg.hDC ==
     * NULL))
     */
    if (printDlg.hDC == NULL) {
	MessageBox(curHwnd, "Device context not configured", title,
     ...
     */
    paperArea.left = GetDeviceCaps(printDlg.hDC, LOGPIXELSX);	/* 1/4
								 * inch */
    paperArea.right = GetDeviceCaps(printDlg.hDC, HORZRES);
    paperArea.bottom = GetDeviceCaps(printDlg.hDC, VERTRES);	/* Vertical 
								 * resolution 
								 */
    paperArea.top = GetDeviceCaps(printDlg.hDC, LOGPIXELSY);	/* 1/4
								 * inch */

    /*
     * Set up the font size 
     */
    if (constructFontTable
	(printDlg.hDC, fontTab, pd.cell.sizeX, pd.cell.sizeY,
	 pd.fontName) == meFALSE)
	goto dlg_exit;
...
do { \
	...
	for (;;) {
		...
		if ((cc == 'c') || (cc == 'f')) {
			...
			if ((charIdx + ll) > ihead->length) {
			    mlwrite(0,
				    "[Illegal printer character Esc-%02x]",
				    cc);
			    break;
			}
			if (cc == 'c') {
			    ...
			    ll |= hexToNum(cc);
			    cc = ihead->text[charIdx++];
			    WinPrintSetColor(printDlg.hDC, ll,
					     (cc == 'B'));
			} else {
			    /*
			     * change the font 
	...
	/*
	 * The last command that we performed should have been an end of
	 * page. Simply close the document. 
	 */
  ...
  error_exit:
    MessageBox(hDlgCancel, "Printer Job Unsucessful", title,
	       MB_ICONEXCLAMATION | MB_OK);
...
WinPrint(meUByte * docName, meLine * ihead)
{
	...
	return mlwrite(MWABORT,
		       "Print spooler currently processing a job in the background");
...
printSetup(int n)
{
    ...
    if ((n == -2) || (n == 0)) {
	/*
	 * Set up the dialogue box and return the status. Note that we
	 * explicitly test the return status for meTRUE which is returned
	 * if the dialogue 'Print' button is pressed. Any other value that 
	 * is returned indicates that the 'Cancel' button was pressed or
	...
	if (DialogBox(ttInstance,
		      ...
		      curHwnd, (DLGPROC) pcdDialogue) != meTRUE)
	    return meFALSE;
    } else if (n > 0) {
	/*
