/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com exec.c - Command
 * execution. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
int
biChopFindString(register meUByte * ss, register int len,
...
/*
 * token: chop a token off a string return a pointer past the token src - 
 * source string, tok - destination token string (must be
 * meTOKENBUF_SIZE_MAX in size, returning a string no bigger than
 * meBUF_SIZE_MAX with the \0) 
 */
meUByte        *
token(meUByte * src, meUByte * tok)
{
    ...
    dd = tok;
    /*
     * note tokEnd is set to tok+meBUF_SIZE_MAX-1 to leave room for the
     * terminating \0 
     */
	...
	*dd++ = '"';
	while ((cc = *ss++) != '"') {
		...
		case 'P':
		    /*
		     * Go to set position, defined by \p - replace with
		     * \CXAP 
		     */
		    *dd++ = meCHAR_LEADER;
...
/*
 * meGetString Get a string input, either from the macro line or from the
 * user prompt - prompt to use if we must be interactive option - options
 * for getstring defnum - Default history number buffer - buffer to put
 * token into size - size of the buffer 
 */
int
meGetString(meUByte * prompt, int option, int defnum, meUByte * buffer,
	    int size)
{
    /*
...
{				/* get a macro line argument */
    ...
    status =
	meGetString(NULL, MLNOHIST | MLFFZERO, 0, tok, meBUF_SIZE_MAX);
...
fnctest(void)
{
	...
	if (cmd == NULL) {
	    ...
	    mlwrite(MWWAIT,
		    (meUByte *)
		    "cmdHash Error: [%s] should be in position %d",
	...
	if (meStrcmp(cmd->anext->name, cmd->name) < 0) {
	    ...
	    mlwrite(MWWAIT,
		    (meUByte *)
		    "cmdHead Error: [%s] should be before [%s]",
		    cmd->anext->name, cmd->name);
	}
	cmd = cmd->anext;
    }
    /*
     * Scan through the key table looking for the character code.  If
     * found then return the index into the names table.  
     */
 ...
 * executed as a command. This function will be used by execute-line and
 * by all source and startup files.
 * 
 ...
 * 
 * !endm End a macro !if (cond) conditional execution !else !endif !return 
 * Return (terminating current macro) !goto <label> Jump to a label in the 
 * current macro
 * 
...
docmd(meUByte * cline, register meUByte * tkn)
{
  ...
  try_again:
	 ...
	 * derivative up in a name table, but after profiling found this
	 * to be the second biggest consumer of cpu and the function was
	 * 3rd (token was the worst at 42%, the call to look-up the
	 * derivative was second taking over 9% and docmd took 7%). By
	 * changing this to an optimised set of if statements embedded in
	 * docmd the time usage in my tests dropped from 4.52 sec to 2.73, 
	 * i.e. 40% time saving. So yes this does look horrid but its
	 * fast! 
	 */
	status = -1;
	...
	if (status < 0)
	    return mlwrite(MWABORT | MWWAIT,
			   (meUByte *) "[Unknown directive %s]", tkn);
	...
	if (execlevel == 0) {
	  elif_jump:
		...
		if (meGetString
		    (NULL, MLNOHIST | MLFFZERO | MLEXECNOUSER, 0, tkn,
		     meBUF_SIZE_MAX) <= 0) {
	    ...
	    case DRRETURN:
		/*
		 * Stop the debugger kicking in on a !return, a macro
		 * doing !return 0 is okay 
		 */
		meRegCurr->force = 1;
	...
	if (cmdstatus)
		...
		/*
		 * A double !force - macro says \CG is okay, remove all
		 * input (must remove the \CG) and rest flag 
		 */
		TTinflush();
		TTbreakFlag = 0;
		status = meTRUE;
...
macroPrintError(meInt type, meLine * hlp, meUByte * macName)
{
    ...
    mlwrite(MWABORT | MWWAIT,
	    (meUByte *) "[%s:: %s executing %s, line %d]", errorLine->text,
	    (type) ? "Halt" : "Error", macName, ln + 1);
    return ln;
}
...
{
#if MEOPT_DEBUGM
    static meUByte  __dobufStr1[] =
	"(C)ontinue, (S)tep, (V)ariable, <any>=next, (^G)Abort, (^L)redraw ? ";
    meByte          debug = 0;
#endif
...
#if MEOPT_DEBUGM
	/*
	 * if $debug == meTRUE, every line to execute gets echoed and a
	 * key needs to be pressed to continue ^G will abort the command 
	 */

	if ((macbug & 0x02)
	    && (!execlevel || (macbug & 0x04)
		|| ((execlevel == 1) && !meStrncmp(tline, "!el", 3)))) {
	    meUByte         dd,
	                    outline[meBUF_SIZE_MAX];	/* string to hold
							 * debug line text 
							 */
	    meLine         *tlp = hlp;
	    meUShort        cc;
	    ...
	    dd = macbug & ~0x80;
	    /*
	     * force debugging off while we are getting input from the
	     * user, if we don't and any macro is executed (idle-pick
	     * macro) then ME will spin and crash!! 
	     */
	    macbug = 0;
	    ...
	    sprintf((char *) outline, "%s:%d:%d [%s] ? ",
		    meRegCurr->commandName, lno, execlevel, tline);
	  loop_round2:
	    mlwrite(MWSPEC, outline);	/* Write out the debug line */
	    /*
	     * Cannot do update as if this calls a macro then we will end
	     * up in an infinite loop, the best we can do is call the
		...
		if (((cc =
		      meGetKeyFromUser(meFALSE, 0,
				       meGETKEY_SILENT | meGETKEY_SINGLE))
		     & 0xff00) == 0)
		    cc = toLower(cc);
		switch (cc) {
		case '?':
		    mlwrite(MWSPEC, __dobufStr1);	/* Write out the
							 * debug line */
		    goto loop_round;
		case 'L' - '@':
		    goto loop_round2;
	...
	if (TTbreakTest(0)) {
	    ...
	    case DRTGOTO:
		{
		    register int    linlen;	/* length of line to
						 * execute */
    ...
    if (mcStore)
	mlwrite(MWABORT | MWWAIT,
		(meUByte *) "[Missing !emacro termination]");
    else
	status = meTRUE;
...
static int
donbuf(meLine * hlp, meVarList * varList, meUByte * commandName, int f,
       int n)
{
    meRegister      rp;
    meUByte         oldcle;
	...
	/*
	 * macro recursion gone too deep, bail out. attempt to force the
	 * break out by pretending C-g was pressed 
	 */
	TTbreakFlag = 1;
	return mlwrite(MWABORT | MWWAIT,
		       (meUByte *) "[Macro recursion gone too deep]");
    }
    /*
     * save the arguments 
     */
...
execFunc(int index, int f, int n)
{
	...
	if (selhilight.uFlags) {
		...
		if (isComSelKill(index)) {
			...
			selhilight.bp = frameCur->bufferCur;	/* Select
								 * the
								 * current 
								 * buffer */
			...
			if (!(selhilight.flags & SELHIL_KEEP)
			    && isComSelDelFix(index))
			...
			if (isComSelSetMark(index) &&
			    ...
			    selhilight.markOffset = frameCur->windowCur->markOffset;	/* Current 
											 * mark 
											 * offset 
											 */
			    selhilight.markLineNo = frameCur->windowCur->markLineNo;	/* Current 
											 * mark 
											 * line 
											 * number 
											 */
			...
			if (selhilight.flags & SELHIL_CHANGED)
			    meBufferAddModeToWindows(selhilight.bp,
						     WFSELHIL);
		    }
		}
	    } else if (!meRegCurr->force
		       &&
		       ((selhilight.
			 flags & (SELHIL_ACTIVE | SELHIL_KEEP)) ==
			SELHIL_ACTIVE))
		selhilight.flags &= ~SELHIL_ACTIVE;
	}
    } else {
...
/*
 * Execute the given command, but in a hidden way, i.e. the 'key' is not
 * recorded 
 */
void
...
execBufferFunc(meBuffer * bp, int index, int flags, int n)
{
	...
	if (bp == frameCur->bufferCur) {
	     ...
	     */
	    meAnchorSet(frameCur->bufferCur, meANCHOR_EXEC_BUFFER,
	...
	/*
	 * force a check on update, just incase the Func has done
	...
	if (sh.flags != 0) {
		...
		bp = bp->next;
	    if ((bp != NULL) && (sh.dotLineNo < bp->lineCount)
		&& (sh.markLineNo < bp->lineCount))
...
executeBuffer(int f, int n)
{
    ...
    meUByte         bufn[meBUF_SIZE_MAX];	/* name of buffer to
						 * execute */
    meInt           ln;
...
/*
 * execFile: yank a file into a buffer and execute it if there are no
 * errors, delete the buffer on exit 
 */
/*
 * Note for Dynamic file names This ensures that nothing is done to fname 
 */
int
execFile(meUByte * fname, int f, int n)
{
...
#ifdef _NANOEMACS
	/*
	 * don't print-out the '[Failed to load file ne.emf]' error for ne 
	 * - who cares 
	 */
	status = 1;
#endif
     ...
     */
    if (!fileLookup
	(fname, (meUByte *) ".emf", meFL_CHECKDOT | meFL_USESRCHPATH, fn)
	|| (ffReadFile(fn, meRWFLAG_SILENT, NULL, &hlp, 0, 0, 0) ==
	    meABORT)) {
	if (status)
	    return meABORT;
	return meFALSE;
...
executeNamedCommand(int f, int n)
{
    ...
    meUByte         buf[meBUF_SIZE_MAX];	/* buffer to hold
...
lineExec(int f, int n, meUByte * cmdstr)
{
    ...
    /*
     * store the execute-line's !force count on the prev registry if
     * greater, the docmd resets the force count so a !force !force
     * execute-line "..." effectively loses the !force's during the
...
{
    register int    status;	/* status return */
    meUByte         cmdstr[meBUF_SIZE_MAX];	/* string holding command
						 * to execute */
     ...
     */
    if ((status =
	 meGetString((meUByte *) ":", 0, 0, cmdstr, meBUF_SIZE_MAX)) <= 0)
	return status;
