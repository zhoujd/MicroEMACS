/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com line.c - Line handling
 * operations. Copyright (C) 1988-2009 JASSPA (www.jasspa.com) This
...
meLineMalloc(int length, int editLine)
{
	...
	lp->text[length] = '\0';
	/*
	 * Always flag the line as changed, this may seem redundant but
	 * some macros can blow aware a buffer and reconstruct an almost
	 * identical buffer which use the same lines (eg *buffer-info*
	 * toolbar tool) and unless the lines are flagged as changed the
		   ...
		   meUShort offset, meInt adjust)
{
    ...
    while (p != NULL) {
	if (p->line == lp) {
	    if (((flags & (meLINEANCHOR_IF_LESS | meLINEANCHOR_IF_GREATER))
		 == 0) || ((flags & meLINEANCHOR_IF_GREATER)
			   && (p->offs > offset))
		|| ((flags & meLINEANCHOR_IF_LESS)
		    && (p->offs <= offset))) {
		p->line = nlp;
	    ...
	    } else if ((flags & meLINEANCHOR_COMPRESS)
		       && (p->offs > (offset + adjust))) {
		p->line = nlp;
		nlp->flag |= meAnchorGetLineFlag(p);
...
bufferSetEdit(void)
{
	...
	return rdonly();
    if (!meModeTest(frameCur->bufferCur->mode, MDEDIT)) {	/* First
								 * change, 
								 * so */
	if ((frameCur->bufferCur->name[0] != '*') &&
	    (bufferOutOfDate(frameCur->bufferCur)) &&
	    (mlyesno((meUByte *) "File changed on disk, really edit") <=
	     0))
	...
	frameCur->bufferCur->autoTime =
	    ((tp.tv_sec - startTime + autoTime) * 1000) +
	    (tp.tv_usec / 1000);
	if (!isTimerExpired(AUTOS_TIMER_ID)
	    && (!isTimerSet(AUTOS_TIMER_ID)
		|| (meTimerTime[AUTOS_TIMER_ID] >
		    frameCur->bufferCur->autoTime)))
	    timerSet(AUTOS_TIMER_ID, frameCur->bufferCur->autoTime,
		     ((long) autoTime) * 1000L);
    }
    return meTRUE;
}
...
lineSetChanged(register int flag)
{
    ...
    if (frameCur->bufferCur->windowCount != 1) {	/* If more than 1
							 * window, update
							 * rest */
	...
	frameCur->windowCur->updateFlags |= flag;	/* update current
							 * window */
}
...
lineMakeSpace(int n)
{
    ...
    if (lp_old == frameCur->bufferCur->baseLine) {	/* At the end:
							 * special */

/*---	Allocate a new line */
	if ((lp_new = meLineMalloc(n, 1)) == NULL)	/* Allocate new
							 * line */
	    return NULL;
	...
	if (lp_old->flag & meLINE_ANCHOR)
	     ...
	     */
	    meLineResetAnchors(meLINEANCHOR_IF_LESS | meLINEANCHOR_RETAIN,
			       frameCur->bufferCur, lp_old, lp_new, 0, 0);
	...
	if (n > (int) lp_old->unused) {	/* Hard: reallocate */
		...
		*cp2++ = *cp1++;
	     ...
	     */
	    lp_new->flag =
		(lp_old->flag & ~meLINE_ANCHOR) | meLINE_CHANGED;
	    lp_old->prev->next = lp_new;
	    lp_old->next->prev = lp_new;
	    if (lp_old->flag & meLINE_ANCHOR) {
		/*
		 * update the position of any anchors - important for
		 * narrows 
		 */
		meLineResetAnchors(meLINEANCHOR_IF_GREATER |
				   meLINEANCHOR_RELATIVE,
				   frameCur->bufferCur, lp_old, lp_new,
				   doto, n);
		meLineResetAnchors(meLINEANCHOR_ALWAYS |
				   meLINEANCHOR_RETAIN,
				   frameCur->bufferCur, lp_old, lp_new, 0,
				   0);
	    }
	    meFree(lp_old);
...
lineInsertChar(int n, int c)
{
    ...
    if ((cp = lineMakeSpace(n)) == NULL)	/* Make space for the
						 * characters */
...
lineInsertString(register int n, register meUByte * cp)
{
     ...
     */
    if ((n == 0) && ((n = meStrlen(cp)) == 0))
	return meTRUE;

    lineSetChanged(WFMAIN);

    if ((lp = lineMakeSpace(n)) == NULL)	/* Make space for the
						 * characters */
	return meFALSE;		/* Error !!! */
    while (n-- > 0)		/* Copy in the characters */
	*lp++ = *cp++;
...
lineInsertNewline(meInt flags)
{
	...
	/*
	 * the length of the rest of the line is greater than doto+32 so
	 * use the current line for the next line and create a new this
	 * line 
	 */
	lp2 = lp1;
...
#if MEOPT_EXTENDED
	/*
	 * special case: when undoing the PROTECTed flag is overridden,
	 * but if the offest is 0 the anchors must remain on the original
	 * line 
	 */
...
#endif
	{
		 ...
		 */
		meLineResetAnchors(meLINEANCHOR_IF_LESS |
				   meLINEANCHOR_RETAIN,
				   frameCur->bufferCur, lp2, lp1, doto, 0);
		/*
		 * adjust offset of remaining ones on the second line
		 * (lp2) 
		 */
		if (doto && (lp2->flag & meLINE_ANCHOR))
		    meLineResetAnchors(meLINEANCHOR_IF_GREATER |
				       meLINEANCHOR_RELATIVE,
				       frameCur->bufferCur, lp2, lp2, doto,
				       0 - ((meInt) doto));
	    }
	}
    } else {
	...
	*cp1 = '\0';
	...
	if (lp1->flag & meLINE_ANCHOR)
	    /*
	     * move to the second line anchors with an offset greater than 
	     * the split offset 
	     */
	    meLineResetAnchors(meLINEANCHOR_IF_GREATER |
			       meLINEANCHOR_RELATIVE, frameCur->bufferCur,
...
#if MEOPT_EXTENDED
    /*
     * scheme & user set flags should remain on the top line even if
     * offset was 0 - important for narrows etc 
     */
    if (doto || !(flags & meBUFINSFLAG_UNDOCALL)) {
	lp1->flag |=
	    lflag & (meLINE_MARKUP | meLINE_PROTECT | meLINE_SET_MASK |
		     meLINE_SCHEME_MASK);
	lp2->flag &=
	    ~(meLINE_MARKUP | meLINE_PROTECT | meLINE_SET_MASK |
	      meLINE_SCHEME_MASK);
    }
#endif
...
/*
 * insert the given text string into the current buffer at dot Note - this 
 * function avoids any meLINE_PROTECT issues, it assumes that the legality 
 * of the action is already checked 
 */
int
bufferInsertText(meUByte * str, meInt flags)
{
	...
	if (frameCur->windowCur->dotOffset) {
	    ...
	    *ss = meCHAR_NL;
		...
		/*
		 * Update the position of any anchors at the start of the
		 * line, these are left behind 
		 */
		if (frameCur->windowCur->dotLine->flag & meLINE_ANCHOR)
		    meLineResetAnchors(meLINEANCHOR_IF_LESS |
				       ...
				       meLineGetNext(line), 0, 0);
#if MEOPT_EXTENDED
		/*
		 * scheme & user set flags should remain on the top line
		 * as well - important for narrows etc 
		 */
		meLineGetNext(line)->flag |=
		    frameCur->windowCur->dotLine->
		    flag & (meLINE_MARKUP | meLINE_PROTECT |
			    meLINE_SET_MASK | meLINE_SCHEME_MASK);
...
int
bufferInsertTab(int f, int n)
{				/* insert tabs forward into text */
    register int    s;
...
/*
 * ask for and insert a string into the current buffer at the current
 * point 
 */
int
bufferInsertString(int f, int n)
{
     ...
     */
    if ((status =
	 meGetString((meUByte *) "String", 0, 0, tstring,
		     meBUF_SIZE_MAX)) <= 0)
	return status;
#if MEOPT_EXTENDED
	...
	return status;
#endif
    if ((status = bufferSetEdit()) <= 0)	/* Check we can change the 
						 * buffer */
	return status;
...
killSave(void)
{
    meShort         count = 0;
    meKill         *thiskl,
                   *lastkl;	/* pointer to last klist element */
...
/*
 * Add a new kill block of size count to the current kill. returns NULL on 
 * failure, pointer to the 1st byte of the block otherwise. 
 */
meUByte        *
killAddNode(meInt count)
{
    ...
    if ((nbl =
	 (meKillNode *) meMalloc(sizeof(meKillNode) + count)) == NULL)
	return NULL;
    nbl->next = NULL;
    nbl->data[count] = '\0';
    *currkill = nbl;
...
mldelete(meInt noChrs, meUByte * kstring)
{
    ...
    if (nn <= ii) {
	/*
	 * this can be treated as a special quick case because it is
	 * simply removing a few chars from the middle of the line -
	 * removal as window position updates are special case and very
	 * easy 
	 */
	meUByte        *s1,
	               *s2;
	...
	if (slp->flag & meLINE_ANCHOR)
	    meLineResetAnchors(meLINEANCHOR_IF_GREATER |
			       meLINEANCHOR_COMPRESS |
			       meLINEANCHOR_RELATIVE, frameCur->bufferCur,
			       slp, slp, (meUShort) (soff + nn), -nn);
	...
	*ks = '\0';
	...
	/*
	 * construct the new joined start and end line, 3 posibilities a.
	 * use the start line b. use the end line c. create a new line
	 * NOTE There is a 4th possibility, the two joined lines could be
	 * too long (>0xfff0) for a single line. This is tested for in
	 * mldelete line and avoid so if this function is used else where, 
	 * beware! 
	 */
	int             newl;
	...
	if (meLineGetMaxLength(slp) >= newl) {
	    /*
	     * here we have got to test for one special case, if the elp
	     * is the buffer baseLine then eoff must be 0, we can't remove 
	     * this line so can't remove last '\n' and increment nn or but 
	     * don't consider this an error? Not sure yet 
	     */
	    if (elp == frameCur->bufferCur->baseLine) {
		slp->text[soff] = '\0';
		felp = elp;
		/*
		 * increment the no-lines cause we're only pretending
		 * we've removed the last line 
		 */
		frameCur->bufferCur->lineCount++;
	    } else {
		meUByte        *s1,
	    ...
	    if (slp->flag & meLINE_ANCHOR)
		meLineResetAnchors(meLINEANCHOR_IF_GREATER |
				   meLINEANCHOR_FIXED, frameCur->bufferCur,
				   slp, slp, (meUShort) soff, soff);
	    if (elp->flag & meLINE_ANCHOR)
		meLineResetAnchors(meLINEANCHOR_IF_GREATER |
				   meLINEANCHOR_RELATIVE,
				   frameCur->bufferCur, elp, slp,
				   (meUShort) eoff, soff - eoff);
	    slp->flag =
		(slp->flag & ~meLINE_NOEOL) | (elp->flag & meLINE_NOEOL);
		...
		*s1++ = *s2++;
		...
		meLineResetAnchors(meLINEANCHOR_ALWAYS |
				   meLINEANCHOR_RELATIVE,
				   frameCur->bufferCur, elp, elp,
				   (meUShort) 0, soff - eoff);
	    }
	    if (slp->flag & meLINE_ANCHOR)
		meLineResetAnchors(meLINEANCHOR_IF_LESS |
				   meLINEANCHOR_RETAIN,
				   frameCur->bufferCur, slp, elp,
				   (meUShort) soff, 0);
#if MEOPT_EXTENDED
	    elp->flag =
		(elp->
		 flag & ~(meLINE_MARKUP | meLINE_SCHEME_MASK)) | (slp->
								  flag &
								  (meLINE_MARKUP
								   |
								   meLINE_SCHEME_MASK));
		...
		*s1++ = *s2++;
	    ...
	    slp->flag |=
		(fslp->
		 flag & (meLINE_MARKUP | meLINE_SCHEME_MASK)) | (elp->
								 flag &
								 meLINE_NOEOL);
#endif
	    felp = slp;
    ...
    /*
     * due to the last '\n' & long line special cases above, it is
     * possible that we aren't actually removing any lines, check this 
     */
    if (fslp != felp) {
	/*
	 * link out the unwanted section fslp -> meLineGetPrev(felp) and
	 * free 
	 */
	felp->prev->next = NULL;
	fslp->prev->next = felp;
	...
	if (wp->buffer == frameCur->bufferCur) {
		...
		if (wp->dotLineNo > elno)
			...
			wp->dotOffset =
			    (meUShort) (soff + wp->dotOffset - eoff);
		    else if ((wp->dotLineNo != slno)
			     || (wp->dotOffset > soff))
			wp->dotOffset = (meUShort) soff;
		    wp->dotLine = slp;
		    wp->dotLineNo = slno;
		...
		if (wp->markLineNo > elno)
			...
			&& (wp->markOffset > eoff))
			wp->markOffset =
			    (meUShort) (soff + wp->markOffset - eoff);
		    else if ((wp->markLineNo != slno)
			     || (wp->markOffset > soff))
			wp->markOffset = (meUShort) soff;
		    wp->markLine = slp;
		    wp->markLineNo = slno;
...
ldelete(meInt nn, int kflag)
{
     ...
     */
    if ((len =
	 nn + frameCur->windowCur->dotOffset - meLineGetLength(ll)) > 0) {
	do {
	    lflag |= meLineGetFlag(ll);
	    ll = meLineGetNext(ll);
	    if (ll == frameCur->bufferCur->baseLine) {
		/*
		 * The last but ones line can only be removed if the
		 * current offset is 0 
		 */
	 ...
	 * count has been adjusted appropriately, but neither should we
	 * generate an error. So only generate an error if the number of
	 * chars we want to delete but can't is greater that 1 
	 */
	if (len > 1)
	    ret = meFALSE;
	else
	 ...
	 * the offset on the last line, i.e. if G is a wanted char and D
	 * is a char that should be deleted, we start with:
	 * GGGGGGDDDDDDDDDD DDDDDDDDDDDDDDDD DDDDDDDDDDDDDDDD
	 * DDDDDDDGGGGGGGG We will try to get it to GGGGGG
	 * DDDDDDDGGGGGGGG All we have to do is reduce the no chars
	 * appropriately. Note: if there is a narrow on the last line we
	 * should that that too so set len to 1. 
	 */
	...
	/*
	 * the kill probably includes one or more narrows, find them,
	 * expand then and add them to the size of the kill 
	 */
	meLine         *slp,
	               *dlp;
	meInt           dln;
	...
	slp = frameCur->windowCur->dotLine;
	nn +=
	    meBufferRegionExpandNarrow(frameCur->bufferCur, &slp,
				       frameCur->windowCur->dotOffset, ll,
				       (meUShort) len, 1);
...
yank(int f, int n)
{
    ...
    commandFlag[CK_YANK] =
	(comSelStart | comSelSetDot | comSelSetMark | comSelSetFix);
    if (n == 0) {
	/*
...
reyank(int f, int n)
{
	...
	while ((--n >= 0)
	       && ((reyankLastYank = reyankLastYank->next) != NULL));
	if (reyankLastYank == NULL)
	    return mlwrite(MWABORT,
			   (meUByte *) "[not enough in the kill-ring]");
    } else {
	if (lastflag == meCFYANK)
	    /*
	     * Last command was yank. Set reyankLastYank to the most
	     * recent delete, so we reyank the second most recent 
	     */
	    reyankLastYank = klhead;
	else if (lastflag != meCFRYANK)
	    /*
	     * Last command was not a reyank or yank, set reyankLastYank
	     * to NULL so we bitch 
	     */
	    reyankLastYank = NULL;
	...
	if (reyankLastYank == NULL)
	    ...
	    return mlwrite(MWABORT,
			   (meUByte *)
			   "[reyank must IMMEDIATELY follow a yank or reyank]");
