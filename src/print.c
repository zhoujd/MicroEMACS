/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com print.c - Buffer printing 
 * routines. Copyright (C) 1996-2001 Jon Green & Steven Phillips
 ...
 */
static meUByte  hilchars[mePRINTSTYLE_SIZE] =
    { 'b', 'i', 'l', 'r', 'u', 'F', 'B' };
...
doTranslation(meUByte * dest, meUByte * str, int n)
{
    ...
    while (((cc = *str++) != '\0') && (--n >= 0)) {
	/*
	 * Convert special characters i.e. box chars to an ASCII
	 * equivelent 
	 */
...
printColor(int f, int n)
{
	...
	printer.color =
	    meRealloc(printer.color, sizeof(meUInt) * (colNo + 1));
...
printScheme(int f, int n)
{
	...
	(meGetString((meUByte *) "Fore-col", 0, 0, buf, meBUF_SIZE_MAX) ==
	 meABORT)
	|| ((fc = (meUByte) meAtoi(buf)),
	    (meGetString((meUByte *) "Back-col", 0, 0, buf, meBUF_SIZE_MAX)
	     == meABORT))
	|| ((bc = (meUByte) meAtoi(buf)),
	    (meGetString((meUByte *) "Font", 0, 0, buf, meBUF_SIZE_MAX) ==
	     meABORT)))
	return meFALSE;
	...
	printer.scheme =
	    meRealloc(printer.scheme, sizeof(meStyle) * (schmNo + 1));
...
printGetParams(void)
{
...
#endif
    /*
     * silent printing may be required, don't know yet - so don't mlwrite
     * yet 
     */
    /*
    ...
    /*
     * Get the registry directory & the name of the driver out of the
     * registry 
     */
	...
	return mlwrite(MWABORT | MWPAUSE,
		       (meUByte *) "[Printer driver not setup]");
	...
	for (jj = 0; (jj < mePRINTSTYLE_SIZE) && (order[jj] != '\0'); jj++) {
		...
		if (hilchars[kk] == order[jj]) {
			...
			subkey[1] = '\0';
			if ((regData =
			     vregFind(regPrint, (meUByte *) "%s/%s",
				      fontPaths[ii], subkey)) != NULL) {
			    fontStrings[ii][jj] =
				regGetString(regData, NULL);
				...
				fontStrlen[ii][jj] =
				    (meUShort)
				    meStrlen(fontStrings[ii][jj]);
				fontMasks[ii][jj] = 1 << (kk + 16);
			    }
#if TESTPRINT
			    fprintf(fp,
				    "Scheme %s - %d set to %c %d [%s]\n",
				    fontPaths[ii], jj, order[jj],
				    fontStrlen[ii][jj],
				    fontStrings[ii][jj]);
#endif
			}
...
printComputePageSetup(int f)
{
    ...
    xtraWidth +=
	(printer.param[mePI_COLS].l - 1) * printer.param[mePI_WSEP].l;
    if (printer.pLineNumDigits > 0)
	xtraWidth += printer.param[mePI_COLS].l * printer.pLineNumDigits;

    /*
     * allow for the row dividers, top/bottom margins and header/footer
     * depths 
     */
    xtraDepth = (printer.param[mePI_ROWS].l - 1);
    xtraDepth +=
	printer.param[mePI_MTOP].l + printer.param[mePI_MBOTTOM].l;
    xtraDepth += printer.pNoHeaderLines + printer.pNoFooterLines;

    /*
     * Automatically fill some of the entries. 
     */
    if ((printer.param[mePI_PAGEX].l == 0)
	&& (printer.param[mePI_PAGEY].l == 0)) {
	/*
	...
	printer.param[mePI_PAGEX].l =
	    (printer.param[mePI_PAPERX].l -
	     xtraWidth) / printer.param[mePI_COLS].l;
	printer.param[mePI_PAGEY].l =
	    (printer.param[mePI_PAPERY].l -
	     xtraDepth) / printer.param[mePI_ROWS].l;
    } else if ((printer.param[mePI_PAPERX].l == 0)
	       && (printer.param[mePI_PAPERY].l == 0)) {
	/*
	 * The paper size is undefined, determine the paper size 
	 */
	printer.param[mePI_PAPERX].l =
	    printer.param[mePI_PAGEX].l * printer.param[mePI_COLS].l +
	    xtraWidth;
	printer.param[mePI_PAPERY].l =
	    printer.param[mePI_PAGEY].l * printer.param[mePI_ROWS].l +
	    xtraDepth;
    }
     ...
     */
    if ((printer.param[mePI_PAPERX].l <
	 (printer.param[mePI_PAGEX].l * printer.param[mePI_COLS].l +
	  xtraWidth))
	|| (printer.param[mePI_PAPERY].l <
	    (printer.param[mePI_PAGEY].l * printer.param[mePI_ROWS].l +
	...
	return mlwrite(MWABORT | MWPAUSE,
		       (meUByte *)
		       "Invalid paper size %d x %d. Page requirements %d x %d",
		       printer.param[mePI_PAPERX].l,
		       printer.param[mePI_PAPERY].l,
		       printer.param[mePI_PAGEX].l *
		       printer.param[mePI_COLS].l + xtraWidth,
		       (printer.param[mePI_PAGEY].l *
			printer.param[mePI_ROWS].l + xtraDepth));
    else if (!f && ((printer.param[mePI_FLAGS].l & PFLAG_SILENT) == 0))
	mlwrite(0,
		(meUByte *)
		"Valid paper size %d x %d. Page requirements %d x %d of %d x %d",
		printer.param[mePI_PAPERX].l, printer.param[mePI_PAPERY].l,
		printer.param[mePI_PAGEX].l, printer.param[mePI_PAGEY].l,
		printer.param[mePI_COLS], printer.param[mePI_ROWS]);
    return meTRUE;
}
...
printInit(int f, int n)
{
    ...
    /*
     * if the line numbers aren't required then set printer.pLineNumDigits 
     * to zero so we only have to check that value 
     */
...
#ifdef _WIN32
    /*
     * On windows call printSetup to handle the windows printer
     * initialization and the setup dialog if required. 
     */
	...
	regSet(regPrint, printNames[mePI_PAPERX],
	       meItoa(printer.param[mePI_PAPERX].l));
	regSet(regPrint, printNames[mePI_PAPERY],
	       meItoa(printer.param[mePI_PAPERY].l));
	regSet(regPrint, printNames[mePI_PAGEX],
	       meItoa(printer.param[mePI_PAGEX].l));
	regSet(regPrint, printNames[mePI_PAGEY],
	       meItoa(printer.param[mePI_PAGEY].l));
    }
    return meTRUE;
}
...
expandText(meUByte * buf, meUByte * strp, int level)
{
	...
	if (cc == '%') {
	    switch (cc = *strp) {
	    case 'c':
		if (((cc = *++strp) == meCHAR_LEADER)
		    && ((cc = *++strp) == meCHAR_TRAIL_NULL))
		    *buf++ = '\0';
		else
		    *buf++ = cc;
		break;
	    case 'h':
		buf +=
		    sprintf((char *) buf, "%02d", printer.ptime->tm_hour);
		break;
	    case 'm':
		buf +=
		    sprintf((char *) buf, "%02d", printer.ptime->tm_min);
		break;
	    case 's':
		buf +=
		    sprintf((char *) buf, "%02d", printer.ptime->tm_sec);
		break;
	    case 'D':
		buf +=
		    sprintf((char *) buf, "%02d", printer.ptime->tm_mday);
		break;
	    case 'M':
		buf +=
		    sprintf((char *) buf, "%02d",
			    printer.ptime->tm_mon + 1);
		break;
	    case 'Y':
		buf +=
		    sprintf((char *) buf, "%02d",
			    printer.ptime->tm_year % 100);
		break;
	    case 'y':
		buf +=
		    sprintf((char *) buf, "%d",
			    printer.ptime->tm_year + 1900);
		break;
	    case 'f':
...
printSetScheme(meScheme col, meUByte * buff)
{
    ...
    /*
     * If the scheme is not registered then there is no colour change to
     * perform 
     */
	...
	for (fstDiff = 0; fstDiff < fontCount; fstDiff++) {
	    if (((ss & fontMasks[0][fstDiff]) !=
		 (cs & fontMasks[0][fstDiff]))
		|| ((ss & fontMasks[1][fstDiff]) !=
		    (cs & fontMasks[1][fstDiff])))
		break;
	}
	sore = 1;
    } else {
	...
	for (;;) {
	    /*
	     * Always want to set the fore and back-ground colors (col 0
	     * may not be black!) 
	     */
		...
		str =
		    meTParm((char *) fontStrings[sore][ii], cno, rr, gg,
			    bb);
		if (buff != NULL)
...
composePage(int f)
{
	...
	if (f < 0) {
		...
		bb = mePrintColorGetBlue(cc);
		str =
		    meTParm((char *) printer.param[mePS_BGCOL].p, cno, rr,
			    gg, bb);
		meStrcpy(p, str);
		p += strlen(str);
	    }
	...
	addFormatedLine(&head, &tail, buf, printer.param[mePI_MTOP].l,
			NULL, 0);
    if (printer.pNoHeaderLines)
	addFormatedLine(&head, &tail, buf, printer.pNoHeaderLines,
			printer.param[mePS_HEADER].p, 1);
	...
	for (ll = 0; ll < printer.param[mePI_PAGEY].l; ll++) {
		 ...
		 */
		ii = ((yy * printer.param[mePI_COLS].l +
		       xx) * printer.param[mePI_PAGEY].l) + ll;
		...
		if (lp != NULL) {
		    /*
		     * Add the continuation marker - we have already done
		     * the line numbers 
		     */
		    if (((lp->flag & meLINE_CHANGED) == 0)
			&& (printer.pLineNumDigits > 0)) {
			for (ii = printer.pLineNumDigits - 2; --ii >= 0;
			     *p++ = ' ')
			    /*
			     * NULL 
			     */ ;
	...
	if (yy + 1 < printer.param[mePI_ROWS].l) {
		...
		/*
		 * If there is another column then add the vertical
		 * crossover separator. 
		 */
		if (xx > 0)
    ...
    /*
     * Render the footer and the separator between the footer and the
     * textual page 
     */
    if (printer.pNoFooterLines)
	addFormatedLine(&head, &tail, buf, printer.pNoFooterLines,
			printer.param[mePS_FOOTER].p, 1);
...
dumpToBuffer(meBuffer * bp, meLine * lp)
{
	...
	while (jj != tlp->length) {
	    ...
	    /*
	     * Construct a new line for the buffer and translate out any
	     * 0's 
	     */
...
#endif
{
	 ...
	 * all of the colour changes to the end of the line, add the
	 * length of storage of each to our line total (ll) 
	 */
	 ...
	 * markers until later (i.e. a line that continues on the next
	 * line) This basically saves some space and is easy to deal with
	 * later 
	 */
	...
	if ((printer.param[mePI_FLAGS].l & PFLAG_ENBLTRUNCC))
	     ...
	     */
	    ll +=
		expandText(nlp->text + ll, printer.param[mePS_ECONT].p, 1);
	nlp->text[ll] = '\0';
    }
    printer.pNoLines += ii;
    return (meTRUE);
...
static int
printSection(meWindow * wp, long sLineNo, long numLines, meLine * sLine,
	     meLine * eLine, int nn)
{
     ...
     */
    for (numLines += sLineNo, printer.pLineNumDigits = 1; numLines > 0;
	 printer.pLineNumDigits++)
	numLines /= 10;
	...
	if (meStrcmp(wp->buffer->name, printer.param[mePS_BUFFER].p) == 0)
	    ...
	    return mlwrite(MWABORT,
			   (meUByte *) "[Cannot print \"%s\" buffer]",
			   printer.param[mePS_BUFFER].p);
	if (((dbp =
	      bfind(printer.param[mePS_BUFFER].p,
		    BFND_CREAT | BFND_CLEAR)) == meFALSE)
	    || (((printer.param[mePI_FLAGS].l & PFLAG_SILENT) == 0)
		&& (meWindowPopup(dbp->name, WPOP_USESTR, NULL)) == NULL))
	    return mlwrite(MWABORT,
			   (meUByte *) "[Failed to create print buffer]");
	break;
    case PDEST_COMLINE:
    case PDEST_FILE:
	if ((printer.param[mePS_FILE].p == NULL)
	    || (*printer.param[mePS_FILE].p == '\0'))
	...
	if ((fp = fopen((char *) fname, "wb")) == NULL)
	    return mlwrite(MWABORT,
			   (meUByte *)
			   "Unable to open printer spool file %s", fname);
	break;
    }
    ...
    for (ii = 0; ii < 256; ii++)
	charMaskTbl1[ii] =
	    (charMaskTbl1[ii] & ~(CHRMSK_DISPLAYABLE | CHRMSK_PRINTABLE)) |
	    ((charMaskTbl1[ii] & CHRMSK_DISPLAYABLE) ? CHRMSK_PRINTABLE :
	     0) | ((charMaskTbl1[ii] & CHRMSK_PRINTABLE) ?
		   CHRMSK_DISPLAYABLE : 0);
    /*
     * Initialise the printer 
     */
    ...
    /*
     * Set up the buffer modes correctly so that we save the codes
     * correctly 
     */
...
#if MEOPT_HILIGHT
    /*
     * Clear the 2nd entry just in case the hilighting is not defined. The 
     * 2nd entry is used as a continuation when hilighting is active. 
     */
	 ...
	 */
	while ((sLine != eLine)
	       && (printer.pNoLines < printer.pLinesPerPage)) {
#if MEOPT_HILIGHT
	    vps[0].line = sLine;
		...
		status =
		    mlwrite(MWABORT,
			    (meUByte *)
			    "Internal error: Cannot add new prin line");
		goto quitEarly;
	    }
	    sLine = meLineGetNext(sLine);
#if MEOPT_HILIGHT
	    /*
	     * Propogate the next line information back into the current
	     * line 
	     */
	...
	if ((clp = composePage(0)) == NULL) {
		...
		mlwrite(MWABORT,
			(meUByte *)
			"Internal error: Cannot compose printer page");
	    goto quitEarly;
	}
    ...
    case PDEST_COMLINE:
	{
	    ...
	    q = cmdLine;
	    if ((p == NULL) || (*p == '\0')) {
		mlwrite(MWPAUSE | MWABORT,
			(meUByte *)
			"No command line specified. Print file %s", fname);
		goto quitEarly;
	    }
	    ...
	    *q = '\0';		/* Terminate the string */
	    ...
	    /*
	     * Call me's system to print the file. Me's version is used
	     * cause its return value is correctly evaluated and on unix a 
	     * " </dev/null" safety arg is added to the cmdLine 
	     */
	    if ((status = doShellCommand(cmdLine, 0)) > 0)
		status = (resultStr[0] == '0') ? meTRUE : meFALSE;
	    if (status <= 0)
		mlwrite(MWABORT, (meUByte *) "[Failed to print file %s]",
			fname);
	    break;
...
#ifdef _WIN32
	    status =
		WinPrint((frameCur->bufferCur->fileName !=
			  NULL) ? frameCur->bufferCur->
			 fileName : (frameCur->bufferCur->name !=
				     NULL) ? frameCur->bufferCur->
			 name : (meUByte *) "UNKNOWN source", ihead);
  ...
  quitEarly:
    ...
    if ((status > 0) &&
	((printer.param[mePI_FLAGS].l & PFLAG_SILENT) == 0))
	mlwrite(0, (meUByte *) "Printing - Done. %d page(s).",
		printer.pPageNo);
    return status;
...
printRegion(int f, int n)
{
    ...
    return printSection(frameCur->windowCur, startLineNo, numLines,
			startLine, endLine, n);
}
...
{
    return printSection(frameCur->windowCur, 0,
			frameCur->bufferCur->lineCount,
			meLineGetNext(frameCur->bufferCur->baseLine),
			frameCur->bufferCur->baseLine, n);
}
