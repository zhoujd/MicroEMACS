/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com dirlist.c - System
 * directory tree display routines. Copyright (c) 1996-2001 Jon Green &
 * Steven Phillips Copyright (C) 2002-2009 JASSPA (www.jasspa.com) This
...
dirConstructNode(meUByte * name, int mask)
{
    ...
    if ((dnode =
	 (DIRNODE *) meMalloc(sizeof(DIRNODE) + meStrlen(name))) != NULL) {
	dnode->mask = mask;
	dnode->next = NULL;
	dnode->lhead = NULL;
	dnode->lnext = NULL;
...
dirLinkNode(DIRNODE * root, DIRNODE * dnode)
{
	 ...
	 * Add the new node 
	 */
	for (dnext = dprev->next; dnext != NULL;
	     dprev = dnext, dnext = dnext->next)
...
/*
 * Give the path of:- dos c:/xxxx/yyyyy unix /xxxx/yyyyy it adds the
 * path to the tree, as soon as a dir is encountered which doesn't exist
 * it returns NULL, else returns a pointer to the last node. NOTE: on
 * unix symbolic links are coped with and the path also added, so function 
 * can be recursive. 
 */
static DIRNODE *addLinkPath(DIRNODE * dnode, meUByte * pathname);
...
addPath(meUByte * pathname, int mask)
{
...
#endif
	dirlist =
	    dirConstructNode((meUByte *) "Desktop ",
			     DIR_UNREAL | DIR_UNKNOWN);
    }
	...
	*q = '\0';
	...
	if ((ndp = dirFindNode(dp, p)) == NULL) {
	    if (getFileStats(pathname, 0, NULL, NULL) !=
		...
		*q = cc;
		...
		q[-1] = '\0';
		if ((ii =
		     readlink((char *) pathname, (char *) lbuf,
			      1024)) > 0) {
#if DEBUG
...
#ifdef _UNIX
	    /*
	     * Under unix we just have to add the root '/' to the tree if
	     * its not already there 
	     */
	    if ((dn = dirFindNode(dnode, (meUByte *) "/")) == NULL)
		dirLinkNode(dnode,
			    dirConstructNode((meUByte *) "/",
					     DIR_UNKNOWN));
	    else
		dn->mask &= ~DIR_GONE;
#endif
#ifdef _WIN32
	    /*
	     * Under Windows get the list of drives and add to the
	     * directory list. 
	     */
	    meUByte         drvname[] = "X:/";
	    int             curDrive;
...
#ifdef _UNIX
			    /*
			     * Check if its a symbolic link - readlink
			     * does not like the trailing '/' so don't
			     * copy it 
			     */
 ...
 */
static void
dirDrawDirectory(meBuffer * bp, DIRNODE * dnode, int iLen, meUByte * iStr,
		 meUByte * fname)
{
    DIRNODE        *dn,
                   *ndn;
    meUByte         buf[meBUF_SIZE_MAX],
                   *fn;		/* Working line buffer */
    int             len,
                    flen;	/* Length buffer under construction */
	...
	if ((dn->child != NULL) && ((dnode->mask & DIR_HIDDEN) == 0)) {
	    /*
	     * Add a bar for the level depending on whether there is a
	     * next present 
	     */
...
dirDrawDir(meUByte * fname, int n)
{
     ...
     */
    if ((wp =
	 meWindowPopup(dirBufName, (BFND_CREAT | BFND_CLEAR | WPOP_USESTR),
		       NULL)) == NULL)
	return;
...
directoryTree(int f, int n)
{
	...
	if ((dnode = addPath(buf, 0)) == NULL)
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "[%s not a directory]", buf);
    } else {
	if (meStrcmp(frameCur->bufferCur->name, dirBufName))
	    return mlwrite(MWABORT,
			   (meUByte *) "[buffer is not *directory*]");
	if ((dnode =
	     findDirLine(frameCur->windowCur->dotLineNo, buf)) == NULL)
	    return meABORT;
    }
    dn = dnode;
    while ((dn->lname != NULL) && (dn->child != NULL))
    ...
    /*
     * If the users has asked for a toggle then work out whether this is
     * an open or close 
     */
    if (n & LDO_TOGGLE) {
