/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com basic.c - Basic Movement
...
/*
 * move to a particular line. argument (n) must be a positive integer for
...
windowGotoLine(int f, int n)
{
    ...
    if ((f == meFALSE) || (n == 0)) {
	if ((status =
	     meGetString((meUByte *) "Goto line", 0, 0, arg,
			 meSBUF_SIZE_MAX)) <= 0)
	    return (status);

	/*---	Skip white space */

	for (n = 0; (arg[n] == '\t' || arg[n] == ' ') && arg[n] != '\0';
	     n++)
	    /*
	     * NULL 
	     */ ;
	...
	/*
	 * SWP moved these two lines into this bracket as if arguement is
	 * given the default must be absolution (previously undefined as
	 * status was left uninitialised. 
	 */
	...
	frameCur->windowCur->dotLineNo = 0;
	frameCur->windowCur->updateFlags |= WFMOVEL;
    } else if ((frameCur->bufferCur->lineCount - nlno) <
	       (nlno - frameCur->windowCur->dotLineNo)) {
	/*
    ...
    nlno -= frameCur->windowCur->dotLineNo;
    /*
     * force the $scroll variable to no smooth so if the line is off the
     * screen the reframe will center the line 
     */
    {
	meUByte         sscrollFlag;
...
/*
 * windowGotoAbsLine Goto absolute line is similar to windowGotoLine
 * except it considers narrowed out text as well 
 */
int
windowGotoAbsLine(meInt line)
{
    ...
    if (frameCur->bufferCur->narrow != NULL) {
	/*
	 * To ensure we get back to the right line (could be currently
	 * narrowed out), drop an alpha mark. 
	 */
	if (rr > 0)
	    rr = meAnchorSet(frameCur->bufferCur, meANCHOR_ABS_LINE,
			     frameCur->windowCur->dotLine, 0, 1);
	meBufferCollapseNarrowAll(frameCur->bufferCur);
	if (rr > 0) {
	    if ((rr =
...
windowBackwardParagraph(int f, int n)
{
	...
	do {
		...
		break;		/* Exit */
	    if ((line_len =
		 meLineGetLength(frameCur->windowCur->dotLine)) == 0)
		break;		/* Exit */
	    for (i = 0; i < line_len; i++) {
		c = meLineGetChar(frameCur->windowCur->dotLine, i);	/* Get 
									 * character 
									 */
		if (c != meCHAR_TAB && c != ' ') {	/* Character on
							 * line ?? */
    ...
    frameCur->windowCur->updateFlags |= WFMOVEL;	/* force screen
							 * update */
    return meTRUE;
}

/*
 * go forword to the end of the current paragraph here we look for a
 * <NL><NL> or <NL><TAB> or <NL><SPACE> combination to delimit the
 * beginning of a paragraph 
 */
int
...
{
    register int    suc = meTRUE;	/* success of last
					 * windowBackwardChar */
    int             i;		/* Local counter. */
    int             line_len;	/* Length of the line */
    char            c;		/* Temporary character */
