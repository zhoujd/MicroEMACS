/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com search.c - Search
...
meUByte        *srchLastMatch = NULL;	/* pointer to the last match
					 * string */
...
mere_compile_pattern(meUByte * apat)
{
    ...
    if (((ii = meRegexComp(&mereRegex, apat,
			   (exactFlag) ? meREGEX_ICASE : 0)) !=
	 meREGEX_OKAY) && reportErrors)
	mlwrite(MWABORT, (meUByte *) "[Regex Error: %s]",
		meRegexCompErrors[ii]);
    return ii;
}
...
{
    register meLine *lp = frameCur->windowCur->dotLine,
	*nlp;
    register meInt  lnno = frameCur->windowCur->dotLineNo,
	nlnno;
    register int    ii = frameCur->windowCur->dotOffset,
	jj,
	kk,
	count = *n,
	flags;
	 ...
	 * main problems are caused by search string with no length, such
	 * as ^, or [ ]*$ on a line with no end spaces. In these cases is
	 * the user is trying to replace it then they couldn't interupt as
	 * this immediately returned success, hence the move! 
	 */
	if (TTbreakTest(1)) {
	    ctrlg(meFALSE, 1);
	    return meFALSE;
	...
	flags =
	    (lp == frameCur->bufferCur->baseLine) ? meREGEX_ENDBUFF : 0;
	if (lnno == 0)
	...
	if (direct == meFORWARD) {
		...
		/*
		 * never match $ or \n on the last line going forwards,
		 * only \' 
		 */
		return meFALSE;
	    kk = jj;
	    ...
	    /*
	     * when searching backward, we must not match the current
	     * position forwrds, always the char before and onwards. if ii 
	     * == 0 then its now -1! 
	     */
	    kk = ii - 1;
	    ii = 0;
	...
	if ((kk < 0)
	    ||
	    ((ii =
	      meRegexMatch(&mereRegex, meLineGetText(lp), jj, ii, kk,
			   flags)) == 0)) {
	    if (direct == meFORWARD) {
		flags &= ~meREGEX_BEGBUFF;
		/*
		 * On entry, if count=0 then no line limit, therefore test 
		 * if (--count == 0) as count == -1 
		 */
		while ((--count != 0) && !(flags & meREGEX_ENDBUFF)) {
		    ...
		    if ((ii =
			 meRegexMatch(&mereRegex, meLineGetText(lp), ii, 0,
				      ii, flags)) != 0)
			break;
		}
	    } else {
	...
	do {
		...
		jj += kk;
		/*
		 * we don't want the last \n or move nlp to past the last
		 * line so drop out here 
		 */
		if (--ii == 0)
		    break;
	    ...
	    if (direct != meFORWARD)
		flags |= meREGEX_BACKWARD;
	    ii = meRegexMatch(&mereRegex, mereNewlBuf, jj, offs, offe,
			      flags);
	    if (ii)
		break;
	...
	SCANNERPOS * sp)
{
    register meUByte cc,
                    aa,
                    bb;		/* character at current position */
    ...
    for (;;) {
	/*
	 * Get the character resolving newlines, and test it against first 
	 * char in pattern. 
	 */
	if (direct == meFORWARD) {
	    if ((cc = *curptr++) == '\0') {	/* if at EOL */
		if (curline != frameCur->bufferCur->baseLine) {
		    curline = meLineGetNext(curline);	/* skip to next
							 * line */
		    curlnno++;
		    curptr = curline->text;
	...
	if (curoff == 0) {
	    if ((curline =
		 meLineGetPrev(curline)) == frameCur->bufferCur->baseLine)
		cc = '\0';
	    else {
		if ((*count > 0) && (--(*count) == 0))
		    break;
	...
	if (cEq(cc, aa)) {	/* if we find it.. */
		...
		if (direct == meFORWARD) {
			...
			if (matchline != frameCur->bufferCur->baseLine) {
			    matchline = meLineGetNext(matchline);	/* skip 
									 * to 
									 * next 
									 * line 
									 */
		...
		if (curptr == curline->text) {
		    ...
		    curoff =
			((size_t) curptr) - ((size_t) curline->text) - 1;
		matchoff =
		    ((size_t) matchptr) - ((size_t) matchline->text);
	    } else {		/* Reverse. */
	    ...
	    setShowRegion(frameCur->bufferCur, curlnno, curoff, matchlnno,
			  matchoff);
...
readpattern(meUByte * prompt, int defnum)
{
     ...
     */
    if ((status =
	 meGetString(prompt, MLSEARCH, defnum, srchPat,
		     meBUF_SIZE_MAX)) > 0) {
	exactFlag = (meModeTest(frameCur->bufferCur->mode, MDEXACT) == 0);
#if MEOPT_MAGIC
	/*
	 * Only make the meta-pattern if in magic mode, since the pattern
	 * in question might have an invalid meta combination. 
	 */
...
/*---	Local variable defintions */
    ...
    meUByte         tpat[meBUF_SIZE_MAX];	/* temporary to hold
						 * search pattern */
	...
	case SL_GETSEARCH:
	    ...
	    if ((status = readpattern
		 ((kind ==
		   meFALSE ? (meUByte *) "Replace" : (meUByte *)
		   "Query replace"), 1 + lastReplace)) <= 0)
		return status;	/* Aborted out - Exit */

	    slength = meStrlen(srchPat);	/* Get length of search
						 * string */
	...
	case SL_EDITREPLACE:	/* Edit replacement string */
	    ...
	    i = expandexp(-1, srchPat, meBUF_SIZE_MAX - 17, 9, tpat, -1,
			  NULL, 0);
	    meStrcpy(tpat + i, "] with");

	    if ((status =
		 meGetString(tpat, MLSEARCH, 0, rpat,
			     meBUF_SIZE_MAX)) <= 0) {
		lastReplace = 0;
		return status;
	    }			/* End of 'if' */
		...
		tpat[i + 6] = ' ';
		tpat[i + 7] = '[';
		i = expandexp(-1, rpat, meBUF_SIZE_MAX - i - 13, i + 8,
			      tpat, -1, NULL, 0);
		meStrcpy(tpat + i,
			 ((i + 30) <
			  frameCur->
	...
	case SL_QUERYINPUT:	/* Get input from user. */
...
#endif
	    /*
	     * SWP 2003-05-10 - Found nasty problem using toolbar with
	     * query-replace-all-strings, the command can lead to changing 
	     * buffer, the prompt can lead to the idle macro being
	     * executed with in this case was the toolbar, and as the
	     * buffer had changed it refreshed the tool buffers which use
	     * the search command, this changes the regex info as this is
	     * stored in a global. This leads to a crash! As a fix cash
	     * the info now and restore after mlCharReply returns, as this 
	     * is a query call this is not likely to be a macro so not
	     * really a performance issue, just make it work! 
	     */
	    {
		meUByte         srchPatStore[meBUF_SIZE_MAX];	/* replacement 
								 * pattern 
								 */
#if MEOPT_MAGIC
		...
		if ((srchLastMagicStore = srchLastMagic) != 0) {
		    ...
		    if (ii > 10)
			ii = 10;
		    memcpy(groupStore, mereRegex.group,
			   ii * sizeof(meRegexGroup));
		}
#endif
		meStrcpy(srchPatStore, srchPat);
		cc = mlCharReply(tpat,
				 mlCR_LOWER_CASE | mlCR_UPDATE_ON_USER |
				 mlCR_CURSOR_IN_MAIN,
				 (meUByte *) "y na!elu",
				 (meUByte *)
				 "(Y)es, (N)o, Yes to (a)ll, (E)dit replace, (L)ast, (U)ndo, (C-g)Abort ? ");
		meStrcpy(srchPat, srchPatStore);
#if MEOPT_MAGIC
		if ((srchLastMagic = srchLastMagicStore) != 0) {
		    mere_compile_pattern(srchPat);
		    memcpy(mereRegex.group, groupStore,
			   ii * sizeof(meRegexGroup));
		}
#endif
	    }
		...
		if (lastline == NULL) {	/* Restore old position. */
		...
		/*
		 * must research for this instance to setup the
		 * auto-repeat sections correctly 
		 */
	...
	case SL_SUBSTITUTE:	/* Substitute the string */
...
#if MEOPT_MAGIC
	    /*
	     * only set the new regex slength here as if the user is doing 
	     * a query replace and does an undo, the slength must be
	     * correct for the last insertion 
	     */
	    if (srchLastMagic)
		...
		if (srchLastMagic &&	/* Magic string ?? */
			...
			/*
			 * if start offset is < 0, it was a ? auto repeat
			 * which was not found, therefore replace str ==
			 * "" 
			 */
			...
			/*
			 * subtract one from insert length as one is added 
	    ...
	    if (kind) {		/* Save position for undo. */
	    ...
	    if (slength == 0) {
		meWindowForwardChar(frameCur->windowCur, 1);
		if ((ff > 0) && (frameCur->windowCur->dotOffset == 0)
		    && (--ff == 0))
		    state_mc = SL_EXIT;
	    }
	    break;
	...
	default:
	    return mlwrite(MWABORT,
			   (meUByte *) "[Bad state machine value %d]",
			   state_mc);
    ...
    sprintf((char *) resultStr, "%c%d%c%c%c%s%c", cc, numsub, cc,
	    ((!kind) ? 'a' : ((onemore) ? 'l' : 'q')), cc, rpat, cc);

    return mlwrite(((nn < 0) || (numsub == nn)) ? MWCLEXEC
		   : (MWABORT | MWCLEXEC), (meUByte *) "%d substitutions",
		   numsub);
}				/* End of 'replaces' */
...
searchForw(int f, int n)
{
    ...
    /*
     * Ask the user for the text of a pattern.  If the response is meTRUE
     * (responses other than meFALSE are possible), search for the
     * pattern. 
     */
...
searchBack(int f, int n)
{
     ...
     */
    if ((status =
	 readpattern((meUByte *) "Reverse search", 1 + lastReplace)) > 0) {
	lastReplace = 0;
	do {
#if MEOPT_MAGIC
	    if (srchLastMagic)
 ...
 * set to the last user search. system uses of iscanner (such as tags,
 * next, timestamp etc) mean that the srchRPat and meRegex may have been
 * trashed. 
 */
/*
...
huntForw(int f, int n)
{
     ...
     * mode until after we entered the pattern. 
     */
    if (srchPat[0] == '\0')
...
#if MEOPT_MAGIC
    /*
     * if magic the recompile the search string SWP 2003-10-06 - check if 
     * the last main search was magic, use of &xse will set the STRCMP bit 
     * but srcPat remains set to the last main search which could be non-
     * regex 
     */
    srchLastMagic &= meSEARCH_LAST_MAGIC_MAIN;
    if (srchLastMagic) {
...
#if MEOPT_MAGIC
    /*
     * if last was a magic search then recompile the search string SWP
     * 2003-10-06 - check if the last main search was magic, use of &xse
     * will set the STRCMP bit but srcPat remains set to the last main
     * search which could be non- regex 
     */
    srchLastMagic &= meSEARCH_LAST_MAGIC_MAIN;
    if (srchLastMagic) {
	if (mere_compile_pattern(srchPat) != meREGEX_OKAY)
...
iscanner(meUByte * apat, int n, int flags, SCANNERPOS * sp)
{
    ...
    return scanner(((direct == meFORWARD) ? apat : srchRPat), direct,
		   beg_or_end, &n, sp);
}
...
searchBuffer(int f, int n)
{
    ...
    if (((rr =
	  meGetString((meUByte *) "Search Flags", 0, 0, flagsStr, 20)) > 0)
	&&
	((rr =
	  meGetString((meUByte *) "Search String", MLSEARCH,
		      1 + lastReplace, srchPat, meBUF_SIZE_MAX)) > 0)) {
	flags = 0;
