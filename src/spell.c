/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com spell.c - Spell checking
 * routines. Copyright (C) 1994-2001 Steven Phillips Copyright (C)
...
typedef struct meSpellRule {
    ...
    meByte          changeLen;
    /*
     * rule is used by find-words for continuation & guessList to
     * eliminate suffixes 
     */
    meUByte         rule;
} meSpellRule;
...
meDictionaryAddWord(meDictionary * dict, meDictWord * wrd)
{
	...
	if ((dict->table =
	     (meDictAddr *) meRealloc(dict->table, dict->dSize)) == NULL)
	    return meFALSE;
    }
...
meDictionaryLoad(meDictionary * dict)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "Failed to open dictionary [%s]",
		       dict->fname);
    fseek(fp, 0, SEEK_END);
    dUsed = ftell(fp);
    fseek(fp, 0, SEEK_SET);
	...
	return mlwrite(MWABORT,
		       (meUByte *) "[%s does not have correct id string]",
		       dict->fname);
	...
	free(table);
	return mlwrite(MWABORT, (meUByte *) "failed to read dictionary %s",
		       dict->fname);
    }
    fclose(fp);
...
meSpellInitDictionaries(void)
{
    ...
    while (dict != NULL) {
	if (!(dict->flags & DTACTIVE) && (meDictionaryLoad(dict) <= 0)) {
	    if (ldict == NULL)
		dictHead = dict->next;
	    else
	...
	dSize = dUsed + TBLINCSIZE;
	if (((dictIgnr =
	      (meDictionary *) meMalloc(sizeof(meDictionary))) == NULL)
...
meDictionaryFind(int flag)
{
    ...
    if (meGetString
	((meUByte *) "Dictionary name", MLFILECASE, 0, tmp,
	 meBUF_SIZE_MAX) <= 0)
	return meFALSE;

    if (!fileLookup
	(tmp, (meUByte *) ".edf", meFL_CHECKDOT | meFL_USESRCHPATH,
	 fname)) {
	meStrcpy(fname, tmp);
	found = 0;
    } else
    ...
    if (!(flag & 1) || (!found && !(flag & 2))) {
	mlwrite(MWABORT, (meUByte *) "[Failed to find dictionary %s]",
		fname);
	return NULL;
    }
...
dictionaryAdd(int f, int n)
{
	...
	if ((meSpellInitDictionaries() <= 0) ||
	    ...
	    (lineInsertNewline(0) <= 0) || (lineInsertNewline(0) <= 0))
	    return meABORT;
...
spellRuleAdd(int f, int n)
{
	...
	if ((rule =
	     mlCharReply((meUByte *) "Rule flag: ", 0, NULL, NULL)) < 0)
	    return meABORT;
	if ((f == meFALSE) && (rule == '-')) {
	    ...
	    if (meGetString
		((meUByte *) "Guess score", MLNOSPACE, 0, buff,
		 meBUF_SIZE_MAX) <= 0)
		return meABORT;
	...
	if (rule == '*') {
	    ...
	    if ((rule != '*')
		&& ((rule < FRSTSPELLRULE) || (rule > LASTSPELLRULE)
		    || (rule == '_')))
		return mlwrite(MWABORT,
			       (meUByte *) "[Invalid spell rule flag]");

	    rule -= FRSTSPELLRULE;
	    if (((rr = meMalloc(sizeof(meSpellRule))) == NULL) ||
		(meGetString
		 ((meUByte *) "Ending", MLNOSPACE, 0, buff,
		  meBUF_SIZE_MAX) <= 0)
		|| ((rr->ending = (meUByte *) meStrdup(buff)) == NULL)
		||
		(meGetString
		 ((meUByte *) "Remove", MLNOSPACE, 0, buff,
		  meBUF_SIZE_MAX) <= 0)
		|| ((rr->remove = (meUByte *) meStrdup(buff)) == NULL)
		||
		(meGetString
		 ((meUByte *) "Append", MLNOSPACE, 0, buff,
		  meBUF_SIZE_MAX) <= 0)
		...
		return mlwrite(MWABORT,
			       (meUByte *)
			       "[Spell rule - ending has no closing ']']");
	    rr->endingLen = f;
	    if (n < 0) {
...
/*
 * Note the return value for this is: meABORT - there was a major failure
 * (i.e. couldn't open the file) meFALSE - user quit meTRUE - succeded
...
meDictionarySave(meDictionary * dict, int n)
{
	...
	((n & 0x01) &&
	 (((reg =
	    regFind(NULL,
		    (meUByte *) SP_REG_ROOT "/" SP_REGI_SAVE)) == NULL)
	  || (regGetLong(reg, 0) == 0)))) {
	...
	if (dict->flags & DTCREATE) {
	    meUByte         fname[meBUF_SIZE_MAX],
	                   ...
	                   *ss;
	    /*
	     * if the ctreated dictionary does not have a complete path
	     * then either use $user-path or get one 
	     */
		...
		ss = dict->fname;
		if (inputFileName
		    ((meUByte *) "Save to directory", fname, 1) <= 0)
		    return meFALSE;
    ...
    return mlwrite(MWABORT | MWPAUSE,
		   (meUByte *) "[Failed to write dictionary %s]",
		   dict->fname);
}
...
/*
 * dictionaryDelete Argument is a bitmask where: 0x01 prompt before
 * losing changes 0x02 delete ignore dictionary 0x04 delete all non-ignore 
 * dictionaries 
 */
int
...
wordTrySuffixRules(meUByte * word, int wlen, meUByte prefixRule)
{
    ...
    /*
     * Try all the suffix rules to see if we can derive the word from
     * another 
     */
	...
	if ((meRuleFlags[ii] & flags) == flags) {
		...
		if (wlen >= elen + clen) {
			...
			if ((elen == 0)
			    ||
			    (regexStrCmp
			     (word + nlen - elen, rr->ending,
			      meRSTRCMP_WHOLE) > 0)) {
			    dict = dictHead;
			    while (dict != NULL) {
				if ((wd =
				     meDictionaryLookupWord(dict, word,
							    nlen)) !=
				    NULL) {
				    /*
				     * check that the word found allows
				     * this rule 
				     */
				    meUByte        *flag,
				                   *sflag,
...
wordTryPrefixRules(meUByte * word, int wlen)
{
	...
	if (meRuleFlags[ii] & RULE_PREFIX) {
		...
		if ((wlen >= rr->appendLen) &&
			...
			cc = nw[elen];
			nw[elen] = '\0';
			if (regexStrCmp(nw, rr->ending, meRSTRCMP_WHOLE) <=
			    0) {
			    nw[elen] = cc;
		    ...
		    while (dict != NULL) {
			if ((wd =
			     meDictionaryLookupWord(dict, nw,
						    nlen)) != NULL) {
			    /*
...
wordTrySpecialRules(meUByte * word, int wlen)
{
	...
	/*
	 * Now try all the special rules - e.g. [0-9]*1st for 1st, 21st
	 * etc 
	 */
	cc = word[wlen];
	word[wlen] = '\0';
...
/*
 * wordCheckSearch Looks in all dictionaries for the given word, then
 * tries all the rules. Returns: meFALSE Word not found meTRUE found and
 * word is good meABORT found and word is erroneous 
 */
static int
...
/*
 * wordCheckBase Checks 1) The word is a word 2) Tries wordCheckSearch 3) 
 * If upper case then capitalises and tries wordCheckSearch 4) If
 * capitalised the makes lower case and tries wordCheckSearch If all the
 * above fail then it fails. Returns: meFALSE Word is okay meTRUE found
 * and word is good meABORT found and word is erroneous 
 */
static int
...
/*
 * wordCheck Check for the given word in all dictionaries, returns:
 * meFALSE Word not found meTRUE found and word is good meABORT found and
 * word is erroneous 
 */
static int
wordCheck(meUByte * word)
{
	...
	if (hyphen &&
	    (((len = ss - word) == 0)
	     || (wordCheckBase(word, (meUByte) len) > 0))) {
	    wordCurr = NULL;
	    return meTRUE;
	}
    }
...
wordApplySuffixRule(meUByte * wd, int wlen, meSpellRule * rr)
{
    ...
    if ((len = rr->endingLen) != 0) {
	if ((len > wlen)
	    || (regexStrCmp(ew - len, rr->ending, meRSTRCMP_WHOLE) <= 0))
	    return NULL;
    }
    ew -= rr->removeLen;
...
wordTestSuffixRule(meUByte * wd, int wlen, meSpellRule * rr)
{
    ...
    if (((len = rr->endingLen) != 0) &&
	((len > wlen)
	 || (regexStrCmp(wd + wlen - len, rr->ending, meRSTRCMP_WHOLE) <=
	     0)))
	return 0;
    return 1;
}
...
/*
 * wordGuessCalcScore Given the user source word (swd) and a dictionary
 * word (dwd) the 
 */
static int
wordGuessCalcScore(meUByte * swd, int slen, meUByte * dwd, int dlen,
		   int testFlag)
{
    int             scr = 0;
    meUByte         cc,
	...
	if ((dlen > 1) && (dwd[1] == cc)) {
		...
		if (ldd == dd)
		    ...
		    /*
		     * missed letter i.e. ltter -> letter, ignore missed ' 
		     * s etc 
		     */
		    scr += 22;
		swd++;
		...
		if (testFlag && (slen > 1))
		    /*
		     * if this is only a test on part of the word the
		     * adjust the other words length as well to keep the
		     * end point aligned this is because of base word
		     * tests. when testing the base of anomaly with
		     * anommoly the comparison is between [anommol] with
		     * [anomaly] the realignment caused by the double m
		     * means that it should be between [anommol] and
		     * [anomal] 
		     */
		    slen -= 2;
		else
		...
		/*
		 * extra letter i.e. lertter -> letter, ignore an extra '
		 * etc. 
		 */
		scr += 25;
	    swd += 2;
	    ...
	    if (testFlag && (dlen > 1))
		/*
		 * if this is only a test on part of the word the adjust
		 * the other words length as well to keep the end point
		 * aligned this is because of base word tests. when
		 * testing the base of anomaly with anommoly the
		 * comparison is between [anommol] with [anomaly] the
		 * realignment caused by the double m means that it should 
		 * be between [anommol] and [anomal] 
		 */
		dlen -= 2;
	    else
		dlen--;
			      ...
			      int bscore, int pflags)
{
	...
	if (((bscore + meRuleScore[ii]) < scores[meGUESS_MAX - 1]) &&
		...
		if (bscore + rr->rule < scores[meGUESS_MAX - 1]) {
		    ...
		    if ((curScr =
			 wordGuessCalcScore(sword, slen, bword, nlen,
					    0)) < maxScore)
...
wordGuessGetList(meUByte * word, int olen, meWORDBUF * words)
{
	...
	for (wlen = 1; wlen < olen; wlen++) {
	    ...
	    wbuff[wlen] = '\0';
	    if (((wlen == 1) && isSpllExt(cc)) || (wordCheck(wbuff) > 0)) {
	...
	for (ii = 0; ii < dict->tableSize; ii++) {
		...
		ent = (meDictWord *) mePtrOffset(dict->table, off);
		/*
		 * Check this word has not been erased or is an error
		 * word, if not then do guess 
		 */
		if (!meWordGetErrorFlag(ent)
		    && ((ww = meWordGetWord(ent))[0] != '\0')) {
		    int             noflags;
			...
			if (curScr < scores[meGUESS_MAX - 1]) {
			     ...
			     * wordGuessAddSuffixRulesToList returns true
			     * if the user aborted 
			     */
			    if (((curScr + bsufScr) <
				 scores[meGUESS_MAX - 1])
				&& wordGuessAddSuffixRulesToList(word,
								 olen, wb,
								 wlen,
								 flags,
								 noflags,
				...
				return -1;
			    if ((curScr =
				 wordGuessCalcScore(word, olen, wb, wlen,
						    0)) <
				scores[meGUESS_MAX - 1])
				wordGuessAddToList(wb, curScr, words,
						   scores);
			    if (TTbreakTest(0))
				return -1;
			}
			...
			while (--ff >= 0) {
			    if ((jj = flags[ff]) == '_')
				noflags = ff;
			    else if (meRuleFlags[(jj -= FRSTSPELLRULE)] &
				     RULE_PREFIX) {
				rr = meRuleTable[jj];
				while (rr != NULL) {
				    if ((ww =
					 wordApplyPrefixRule(wb,
							     rr)) !=
					NULL) {
					nlen = wlen + rr->changeLen;
					if ((jj =
					     nlen - longestSuffixRemove) <
					    mlen)
					    jj = (nlen <
						  mlen) ? nlen : mlen;
					else if (jj > olen)
					    jj = olen;
					curScr =
					    wordGuessCalcScore(word, jj,
							       ww, jj, 1);
					if (curScr <
					    scores[meGUESS_MAX - 1]) {
					    /*
					     * try all suffixes on the
					     * word 
					     */
					    if (((curScr + bsufScr) <
						 scores[meGUESS_MAX - 1])
						...
						for (f1 = f2 = 0; f2 < ff;)
						    if (flags[f2++] == '_')
							f1 = f2;
						if (wordGuessAddSuffixRulesToList(word, olen, ww, nlen, flags + f1, noflags - f1, words, scores, curScr, RULE_SUFFIX | RULE_MIXABLE))
						    return -1;
					    }
					    if ((curScr =
						 wordGuessCalcScore(word,
								    olen,
								    ww,
								    nlen,
								    0)) <
						scores[meGUESS_MAX - 1])
						wordGuessAddToList(ww,
								   curScr,
								   words,
								   scores);
					    if (TTbreakTest(0))
			...
			if ((curScr =
			     wordGuessCalcScore(word, olen, ww, wlen,
						0)) <
			    scores[meGUESS_MAX - 1]) {
			memcpy(wb, ww, wlen);
			wb[wlen] = '\0';
	...
	if (isSpllExt(cc)) {
	    /*
	     * if the last letter is a '.' etc then loop through the
	     * guesses adding one 
	     */
...
spellWordToUserFont(meUByte * dd, meUByte * ss)
{
    ...
    cc = toUserFont(cc);
    if ((caseFlags & SPELL_CASE_FUPPER)
	|| (caseFlags == (SPELL_CASE_FLOWER | SPELL_CASE_CUPPER)))
	cc = toUpper(cc);
    *dd++ = cc;
    while ((cc = *ss++) != '\0') {
	cc = toUserFont(cc);
...
spellWord(int f, int n)
{
    ...
    if (n & SPELLWORD_GET) {
	if (meGetString
	    ((meUByte *) "Enter word", MLNOSPACE, 0, resultStr + 1,
	     meWORD_SIZE_MAX) <= 0)
	    return meABORT;
	...
	for (;;) {
		...
		setShowRegion(frameCur->bufferCur,
			      frameCur->windowCur->dotLineNo, soff,
			      frameCur->windowCur->dotLineNo, eoff);
	...
	setShowRegion(frameCur->bufferCur, frameCur->windowCur->dotLineNo,
		      soff, frameCur->windowCur->dotLineNo, eoff);
	frameCur->windowCur->updateFlags |= WFMOVEL | WFSELHIL;
    } else {
	meUByte         cc;
	meUShort        off,
	...
	while (soff > 0) {
		...
		((soff == 0)
		 ||
		 !isAlphaNum(meLineGetChar
			     (frameCur->windowCur->dotLine, soff - 1))))
		break;
	}
	/*
	 ...
	 */
	if (((cc =
	      meLineGetChar(frameCur->windowCur->dotLine, soff)) != '.')
	    && !isAlphaNum(cc))
	    soff++;
	...
	if (meGetString
	    ((meUByte *) "Enter flags", MLNOSPACE, 0, word,
	     meWORD_SIZE_MAX) <= 0)
	    return meABORT;
	len = meStrlen(word);
	...
	if (n & SPELLWORD_INFO) {
		...
		while (rr != NULL) {
			...
			nwd = wordApplyPrefixRule(wd, rr);
			      ...
			      wordApplySuffixRule(wd, len, rr)) != NULL)
			nwd = wd;
		    else
			nwd = NULL;
		    if (nwd != NULL) {
			...
			if (wordCheck(word) > 0) {
				...
				for (jj = 0; jj < NOSPELLRULES; jj++) {
				    if (meRuleFlags[jj] ==
					(RULE_SUFFIX | RULE_MIXABLE)) {
					sr = meRuleTable[jj];
					while (sr != NULL) {
					    if ((swd =
						 wordApplySuffixRule(nwd,
								     ll,
								     sr))
						!= NULL) {
						meStrcpy(word, nwd);
	...
	if (((f =
	      mlCharReply((meUByte *) "Rule flag: ", 0, NULL,
			  NULL)) < FRSTSPELLRULE) || (f > LASTSPELLRULE))
	    return meABORT;
	f -= FRSTSPELLRULE;
	wd = buff + longestPrefixChange;
	meStrcpy(wd, word);
...
findWordsNext(void)
{
	...
	for (sfwCurIndx = 0; sfwCurIndx < sfwCurDict->tableSize;
	     sfwCurIndx++) {
	    off = meEntryGetAddr(sfwCurDict->table[sfwCurIndx]);
	    while (off != 0) {
		sfwCurWord =
		    (meDictWord *) mePtrOffset(sfwCurDict->table, off);
		if (!meWordGetErrorFlag(sfwCurWord)
		  ...
		  sfwJumpNotPreNotSuf:
			...
			if (sfwFlags || (wp[0] == sfwCurMask[0])) {
				...
				if (meRuleFlags
				    [flags[sufRule] -
				     FRSTSPELLRULE] & RULE_SUFFIX) {
				    sfwSufRule =
					meRuleTable[flags[sufRule] -
						    FRSTSPELLRULE];
				    while (sfwSufRule != NULL) {
					if ((swp =
					     wordApplySuffixRule(wp, len,
								 sfwSufRule))
					    != NULL) {
					    if (regexStrCmp
						(wp, sfwCurMask,
						...
						spellWordToUserFont(wp,
								    wp);
						return wp;
					    }
					    wordSuffixRuleRemove(swp,
								 sfwSufRule);
					}
				      sfwJumpNotPreGotSuf:
			...
			for (preRule = 0; preRule < flen; preRule++) {
			    if (meRuleFlags[flags[preRule] - FRSTSPELLRULE]
				& RULE_PREFIX) {
				sfwPreRule =
				    meRuleTable[flags[preRule] -
						FRSTSPELLRULE];
