/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com osd.c - On-Screen Display 
 * routines. Copyright (C) 1997-2001 Jon Green & Steven Phillips
...
#define MF_DISPTYPE 0x00000002	/* d - Disable display item type (i.e. the 
				 * .., > etc) */
...
#define MF_NOFILL   0x00004000	/* f - Do not fill the item to the full
				 * length */
...
#define RF_TSCHEME  0x00000080	/* H - title drawn color scHeme (or
				 * Hilight) */
...
#define RF_CONTROL  0x00001000	/* C - The dialog has a contorlling
				 * command */
...
#define RF_CONFIG   0x08000000	/* The display is currently being
				 * configured */
...
typedef struct osdDISPLAY {
    ...
    meShort         nbpContext;	/* Context number of the current notebook
				 * page */
    ...
    meUByte         parPosition;	/* Location of this dialog
					 * relative to parent */
...
dialogResetDisplays(osdDIALOG * rp, int type)
{
	...
	while (md != NULL) {
		...
		if (type == 2) {
		    ...
		    if ((md->curContext >= 0)
			&& (md->context[md->curContext].menu->
			    flags & (MF_DISABLE | MF_SEP)))
			md->curContext = -1;
		    if ((md->newContext >= 0)
			&& (md->context[md->newContext].menu->
			    flags & (MF_DISABLE | MF_SEP)))
			md->newContext = -1;
		    if ((md->defContext >= 0)
			&& (md->context[md->defContext].menu->
			    flags & (MF_DISABLE | MF_SEP)))
			md->defContext = -1;
		}
...
displayDestruct(osdDISPLAY * md)
{
	...
	if (osdCurChild != NULL) {
	    while ((osdCurChild->curContext >= 0) &&
		   (osdCurChild->context[osdCurChild->curContext].menu->
		    flags & MF_CHILD))
		osdCurChild =
		    osdCurChild->context[osdCurChild->curContext].child->
		    display;
	}
    }
    if (md == osdNewMd) {
	...
	if (osdNewChild != NULL) {
	    while ((osdNewChild->newContext >= 0) &&
		   (osdNewChild->context[osdNewChild->newContext].menu->
		    flags & MF_CHILD))
		osdNewChild =
		    osdNewChild->context[osdNewChild->newContext].child->
		    display;
	}
    }
...
itemFind(osdDIALOG * rp, int item, meUByte * name)
{
    ...
    /*
     * search for item in reverse order as 99% of times they're added in
     * order 
     */
...
{
    meUByte        *p,
                   *q;		/* Local character pointers */
    int             cc,
                    f,
                    mpflags;	/* Immediate character */
    ...
    /*
     * NOTE - we must cache the mp->flags as the item may not exist after
     * execution. 
     */
    if ((mpflags = mp->flags) & MF_NBPAGE) {
	/*
	 * This can only get executed when the user has selected it, so we 
	 * can use the osdCurChild variable 
	 */
	if (!(mp->iflags & MFI_ARG) || (osdCurChild->nbpContext == -1))
	    return meABORT;
	if (osdCurChild->context[osdCurChild->nbpContext].menu->argc !=
	    mp->argc) {
	    if (osdCurChild->context[osdCurChild->nbpContext].child !=
		NULL) {
		displayDestructChild(osdCurChild->
				     context[osdCurChild->nbpContext].
				     child);
		osdCurChild->context[osdCurChild->nbpContext].child = NULL;
	    }
	    osdCurChild->context[osdCurChild->nbpContext].menu->argc =
		mp->argc;
	}
	return meTRUE;
    }
    ...
    if (mpflags & MF_ENTRY) {
	/*
	 * If entry then must set frameCur->mlStatus to 0x08 bit to get
	 * entries at the osd entry field, must back up previous value.
	...
	/*
	 * Return the menu name to the caller. Strip out any Hotkey
	 * delimiters from the string prior to return. 
	 */
	...
	if (mp->iflags & MFI_NOTEXT)
	    *q = '\0';
		...
		if ((cc = *p++) == meCHAR_LEADER) {
			...
			(cc != meCHAR_TRAIL_HILSTART)
			&& (cc != meCHAR_TRAIL_HILSTOP)) {
			*q++ = meCHAR_LEADER;
			*q++ = cc;
		    }
		} else
	...
	if ((mpflags & MF_CHECK) && (n == 1))
	    /*
	     * set a force value for the execution as the macro is allowed 
	     * to fail to clear the checkbox and we don't want to kick in
	     * the macro debugging 
	     */
	    meRegHead->force = 1;
    }
...
osdDisplaySnapshotCreate(osdDISPLAY * md)
{
    ...
    if ((md->storeSchm =
	 meMalloc(area * 2 * (sizeof(meUByte) + sizeof(meUShort)))) ==
	NULL)
	return 0;
...
static void
osdDisplaySnapshotDraw(osdDISPLAY * md, int sx, int sy, int nx, int ny,
		       int drawToScreen)
{
    meFrameLine    *flp;
    meScheme       *colp;
...
menuSetItemLength(osdDIALOG * rp, osdITEM * mp)
{
	...
	if ((cc == meCHAR_NL) && !(rp->flags & RF_NEWLINE)) {
	    ...
	    /*
	     * the text processing means this is either a LEADER char
	     * (0xff) or a hotkey/hilight stop/start 
	     */
	    cc = *p++;
	...
	len = clen;
    /*
     * Determine if this is a special-menu & if so whether to display the 
     * type. If so then add the appropriate value for an auto or manual
     * open sub-menu 
     */
    if (mp->flags & MF_CHECK) {
	if (mp->strData[0] != '\0')
	...
	if (mp->strData[5] != '\0')
	    len++;
    } else if (!(mp->flags & MF_DISPTYPE)
	       && (mp->flags & (MF_BUTTON | MF_SUBMNU)))
	/*
	 * Add 2 for '[xxxx]', '..' or ' >' 
	 */
	len += 2;
...
fillScrollBar(register meSCROLLBAR * sb, int redraw)
{
	...
	if (sb->totLen > sb->barLen) {
	    int             shaftMovement;	/* Permitted movement on
						 * shaft */
		...
		boxTop =
		    (sb->curPos * shaftMovement) / (sb->totLen -
						    sb->wndLen);
	...
	if (!redraw && (sb->boxTop == boxTop) && (sb->boxLen == boxLen))
	    return 0;
	sb->boxTop = boxTop;
	sb->boxLen = boxLen;
    } else if (!redraw)
...
initScrollBar(meSCROLLBAR * sb, int sbarLen, int flags)
{
	...
	if ((sb =
	     meRealloc(sb, sizeof(meSCROLLBAR) + (sbarLen * 2))) == NULL)
	    return NULL;
    ...
    /*
     * Must always calc the shaft length as if this is greater than zero
     * even without a scroll bar this must be filled 
     */
    len = 0;
    if (flags & WMSPLIT)
	len++;
...
#define osdRENDITEM_FORCE   0x20	/* Force redraw of whole item eg
					 * scroll bars */

static void
menuRenderChildScrollBar(meSCROLLBAR * sb, osdDISPLAY * md,
			 osdCONTEXT * mcp)
{
    meUByte        *txtp1,
                   *txtp2;
...
menuRenderChildMain(osdDISPLAY * md, osdCONTEXT * mcp, int flags)
{
    ...
    for (; depth;
	 depth--, txtp += mdw, colp += mdw, ctxtp += cmdw, ccolp += cmdw) {
	memcpy(colp, ccolp, width * sizeof(meUShort));
	memcpy(txtp, ctxtp, width * sizeof(meUByte));
    }
    /*
     * Draw the text region to screen if required - draw the mcp width and 
     * height so the scroll bars are also rendered 
     */
    if (flags & osdRENDITEM_DRAW)
	osdDisplaySnapshotDraw(md, mcp->x, mcp->y, mcp->width, mcp->depth,
			       1);
}
...
menuRenderChildBoarder(osdDISPLAY * md, osdCONTEXT * mcp)
{
    ...
    *txtp1 = boxChars[BCNE];
    ...
    /*
     * If theres no vert scroll bar, render the right line (must allow for 
     * double width scroll bar flag) 
     */
    ...
    *txtp2 = boxChars[BCNW];
    ...
    /*
     * now check for a wide bottom scroll bar, if wide then render the
     * corners and the bar if there is no scroll bar 
     */
...
{
    meUByte         expbuf[meBUF_SIZE_MAX + 1],
                   *s1;
    int             start,
                    col;
...
{
    if (((focalMax + curPos) < 0) || ((focalMin + curPos) >= windSize)) {
	/*
...
menuRenderItem(osdDISPLAY * md, int offset, int flags)
{
	...
	if (mp->flags & MF_SCRLBOX) {
		...
		if (flags & osdRENDITEM_REFOCUS)
		    cmd->x =
			menuRenderChildRefocus(cmd->x, cmd->width,
					       child->wndWidth,
					       cmd->focalX[0],
					       cmd->focalX[1]);
		...
		if (flags & osdRENDITEM_REFOCUS)
		    cmd->y =
			menuRenderChildRefocus(cmd->y, cmd->depth,
					       child->wndDepth,
					       cmd->focalY[0],
					       cmd->focalY[1]);
		if (flags & osdRENDITEM_FORCE) {
	...
	/*
	 * Copy in the default colpair right across, only the hilight char 
	 * need to be changed 
	 */
	{
	    meUShort       *cp = colp;
	...
	if (mp->flags & MF_CHECK) {
	    ...
	    *txtp++ =
		windowChars[WCOSDBTTOPN + (offset == md->defContext)];
	    curx++;
	    maxx--;
	}
	/*
	...
	if (mp->flags & (MF_ENTRY | MF_COMBO)) {
		...
		osdRenderEntry(txtp, resultStr, md->flags, 0,
			       (int) md->width, mcp->awidth, mcp->adepth,
			       NULL);
		txtp += mcp->awidth;
		curx += mcp->awidth;
	    }
		...
		if ((cc == meCHAR_NL) && !(md->flags & RF_NEWLINE)) {
		    ...
		    if ((cc == meCHAR_LEADER)
			&& ((cc = *p++) != meCHAR_TRAIL_LEADER)) {
			/*
			 * the text processing means this is either a
			 * LEADER char (0xff) or a hotkey/hilight
			 * stop/start 
			 */
	...
	if (!(mp->flags & MF_DISPTYPE) && (mp->flags & MF_BUTTON)) {
	    *txtp++ =
		windowChars[WCOSDBTTCLS + (offset == md->defContext)];
	    curx++;
	    maxx++;
	}
...
osdDisp(meUByte * buf, meUByte * cont, int cpos)
{
    ...
    /*
     * The following code is used to try to solve a particularly nasty
     * bug. If the screen is updated from scratch then all the currently
     * displayed dialogs are reconfigured and redrawn. The problem with
     * this is that the dialog initialisation command may destroy the
     * dialog and rebuild it, thus losing the current context and if the
     * user is currently typing texting into an osd entry then BANG as
     * theres no current context. To reproduce create a callback macro
     * which updates the screen, e.g. the mail-checker, Open the organizer 
     * and start typing in any entry. When the callback macro executes
     * everything appears okay except the current entry is nolonger
     * hilighted as current. Now type a key and without the following code 
     * - BANG. The following attempts to solve this particular and
     * specific problem by restoring the current context 
     */
    static int      curChildItem = 0;
    if (md->curContext < 0) {
	/*
	 * we've lost the current context, try to restore, if we can't
	 * fail 
	 */
	int             ii;
	for (ii = 0; ii < md->numContexts; ii++)
    ...
    col =
	osdRenderEntry(txtp, buf, md->flags, cpos, (int) md->width, ww, dd,
		       &row);
	...
	menuRenderItem(md, md->curContext,
		       osdRENDITEM_DRAW | osdRENDITEM_REFOCUS);
    }
...
menuRenderItemGrid(osdDISPLAY * md, int x, int y, int width, int depth)
{
	...
	*tt++ = boxChars[BCNES];
	     ...
	     (cc == boxChars[BCESW]) || (cc == boxChars[BCSW]))
	*tt++ = boxChars[BCESW];
    else
	*tt++ = boxChars[BCNESW];
...
menuRender(osdDISPLAY * md)
{
	...
	/*
	 * Increment xstart so that its the start of the actual item the
	 * menuRenderLine allows for this Also reduce the number of rows
	 * be one to leave room for the bottom boarder 
	 */
	xstart++;
	noRows--;
    }
	...
	/*
	 * Note that the lengths of every item is the full colWidth
	 * regardless of whether the next is to be horizontally packed.
	 * The length is re-adjusted below 
	 */
	...
	for (; row < noRows;) {
		...
		if (drawGrid)
		    spare--;
		/*
		 * If this is a split then use all the spare to push this 
		 * line to the right 
		 */
		...
		do {
		    ...
		    /*
		     * restore the depth from being the row to just this
		     * item 
		     */
		    ...
		    /*
		     * is this the current notebook context? if so store
		     * it as we must correct the rendering later 
		     */
		    if (!(mp->flags & MF_CHILD) &&
			(md->nbpContext != -1) &&
			(mcp[itemNo].menu->argc ==
			 mcp[md->nbpContext].menu->argc))
			nbmcp = mcp + itemNo;
		    /*
		    ...
		    /*
		     * if this is a filled item then the spare is added to 
		     * the length else we must position it correctly 
		     */
			...
			xpos++;
			if (mp->flags & MF_GRID)
			    menuRenderItemGrid(md, xpos, row,
					       mcp[itemNo].width, depth);
		    }
		    /*
			...
			if (menuExecute(mp, md->flags, 1) > 0)
			    mcp[itemNo].mcflags |= CF_SET;	/* State
								 * is set */
			else
			    mcp[itemNo].mcflags &= ~CF_SET;	/* State
								 * is not
		    ...
		    if ((mp->flags & (MF_SCRLBOX | MF_CHILD)) ==
			(MF_SCRLBOX | MF_CHILD))
			menuRenderChildBoarder(md, &mcp[itemNo]);
		    menuRenderItem(md, itemNo,
				   osdRENDITEM_FORCE |
				   osdRENDITEM_REFOCUS);
		    xpos += ll + ss;
	...
	if (md->flags & RF_RESIZE)
	    md->drawnText[(row * md->width) + xstart - 1] =
		windowChars[WCOSDRESIZE];
...
osdDisplaySetDefaultContext(osdDISPLAY * tmd)
{
    ...
    md = tmd;
    while ((md->curContext >= 0)
	   && (md->context[md->curContext].menu->flags & MF_CHILD))
	md = md->context[md->curContext].child->display;
    curIsButton = ((md->curContext >= 0)
		   &&
		   ((md->context[md->curContext].menu->
		     flags & (MF_DISABLE | MF_DISPTYPE | MF_BUTTON)) ==
		    MF_BUTTON));
...
menuConfigure(osdDIALOG * rp, osdDISPLAY * md, int child)
{
    ...
    osdITEM        *mp;		/* Menu pointer */
    meShort         curItem = -1,
	newItem = -1;		/* Display's curContext & newContext item
				 * # */
    int             minWidth,
                    maxWidth;	/* The min/max allowed width */
    int             minDepth,
                    maxDepth;	/* The min/max allowed depth */
    int             ii,
                    jj;		/* Local loop counter */

    /*
     * Get the length of the dialog & Determine if there is any work to
     * perform 
     */
	...
	if (!(mp->flags & MF_DISABLE))
	    ii++;
    if (ii == 0) {
	mlwrite(MWABORT | MWPAUSE,
		(meUByte *) "[No active items in osd dialog %d]", rp->id);
	return NULL;
    }
    if (md == NULL) {
	if ((md =
	     meMalloc(sizeof(osdDISPLAY) +
		      (sizeof(osdCONTEXT) * (ii - 1)))) == NULL)
	    return NULL;
	memset(md, 0,
	       sizeof(osdDISPLAY) + (sizeof(osdCONTEXT) * (ii - 1)));
	md->dialog = rp;
	...
	if (ii > md->numContexts) {
	    ...
	    if ((nmd =
		 meMalloc(sizeof(osdDISPLAY) +
			  (sizeof(osdCONTEXT) * (ii - 1)))) == NULL)
		return NULL;
	    jj = sizeof(osdDISPLAY) +
		(sizeof(osdCONTEXT) * (md->numContexts - 1));
	    memcpy(nmd, md, jj);
	    memset(((char *) nmd) + jj, 0,
		   (sizeof(osdCONTEXT) * (ii - md->numContexts)));
	    /*
	     * link in the new and free off the old 
	     */
	    jj = md->numContexts;
	    while (--jj >= 0)
		if ((md->context[jj].child != NULL)
		    && (md->context[jj].child->display != NULL))
		...
		jj = tmd->numContexts;
		while (--jj >= 0)
		    if ((tmd->context[jj].child != NULL)
			&& (tmd->context[jj].child->display == md))
    ...
    /*
     * Setup the min/maximum allowable width & depth allowing for the
     * boarder title and grid 
     */
	 ...
	 */
	for (mcp = md->context, mp = rp->itemHead; mp != NULL;
	     mp = mp->next) {
	    int             flags;
	    /*
		...
		/*
		 * the item is a child type (dialog in a dialog).
		 * Construct the child 
		 */
		/*
		...
		if ((child->display != NULL) &&
		    ...
		    /*
		     * the current child is displaying a different dialog, 
		     * we must burn this one and start again 
		     */
		    displayDestruct(child->display);
		    child->display = NULL;
		}
		...
		if (mp->flags & MF_SCRLBOX) {
		    ...
		    crp->depth[0] = mnd;
		    flags =
			gsbarmode & (WMVWIDE | WMUP | WMDOWN | WMSCROL |
				     WMRVBOX | WMHWIDE);
		    width += 2;
		    depth += 2;
		    if (flags & WMHWIDE)
		...
		if (mp->flags & MF_SCRLBOX) {
		    ...
		    if (cmd->width > width) {
			if ((sb =
			     initScrollBar(child->horzSBar, width,
					   flags | WMHORIZ)) == NULL)
			    return NULL;
			sb->totLen = cmd->width;
			sb->curPos = -1;
		    ...
		    if (cmd->depth > depth) {
			if ((sb =
			     initScrollBar(child->vertSBar, depth,
					   flags)) == NULL)
			    return NULL;
			sb->totLen = cmd->depth;
			sb->curPos = -1;
	...
	meShort         nbpContext = -1;	/* Current Notebook page
						 * Menu context pointer */
	...
	for (ii = 0; ii < md->numContexts; ii++) {
	    /*
	     * If a display was given with a curContext then this finds
	     * the new context number 
	     */
	    if (rp->flags & RF_NOTEBOOK) {
		if (mcp[ii].child == NULL)
		    mcp[ii].mcflags |=
			(nbpContext == -1) ? CF_NBPTOP : CF_NBPBOT;
		else
		    nbpContext = ii;
	    }
	...
	/*
	 * Reset the width now cos if the width was set then thats been
	 * done and if the depth was fixed as well then we may need
	 * multi-columns 
	 */
	maxWidth = frameCur->width;
	...
	for (ii = jj = 0; ii < md->numContexts; ii++) {
		 ...
		 * setting the number of contexts to the number we can fit 
		 * in 
		 */
		...
		md->depth = maxDepth;	/* Depth of column (allow for
					 * border) */
	...
	/*
	 * any items that we have prepared but can't be rendered must be
	 * freed 
	 */
...
menuPosition(osdDISPLAY * md, int redraw)
{
	...
	if (md->flags & RF_ABSPOS) {
	    /*
	     * If this is an absolute position then set the requested
	     * position Not on a redraw, on redraws the correct position
	     * is already set 
	     */
	    md->x = md->dialog->x;
	    md->y = md->dialog->y;
	    ...
	    /*
	     * Position relative to the menu. Cases for positioning a
	     * sub-menu are:- Main menu as parent -------------------
	     * ============XXX============================ +----+ | | | |
	     * +----+ SubMenu as parent, right below +----+ | +----+ |
	     * >| | | | | +----| | +----+ Submenu as parent, right above
	     * Submenu as parent, left below Submenu as parent, left above 
	     */
	     ...
	     */
	    if (((pmd = md->prev) != NULL) && !(pmd->flags & RF_DISABLE)) {
		md->x = 0;
		md->y = 0;
		...
		/*
		 * From the previous menu item work out where the menu
		 * should be placed 
		 */
		if (flags & MF_SOUTH) {
		    /*
		     * calc the bottom if positioned below and the top if
		     * above and see which is best, favour below 
		     */
		    int             ii;

		    ii = (frameCur->depth + 1) - (md->y + mcp->depth +
						  md->depth);
		    ...
		    /*
		     * calc the right most column if positioned right and
		     * the left if to the left and see which is best,
		     * favour right 
		     */
		    int             ii;
		    ii = (frameCur->width) - (md->x + mcp->width +
					      md->width);
		...
		/*
		 * If there's an offset position add this to the calc
		 * position 
		 */
		md->x += md->dialog->x;
		md->y += md->dialog->y;
...
#define meOSD_KEY_MOVE     64	/* Current event was a key to change
				 * current item */
...
osdDisplayPush(int id, int flags)
{
    ...
    osdDISPLAY     *lmd,
                   *nmd;	/* Last/next menu display item */
    osdDIALOG      *rp;		/* Pointer to the root menu */
    int             initItem;

    initItem = (id & 0xffff0000) >> 16;
    id &= 0x0ffff;
    if ((rp = dialogFind(id)) == NULL) {	/* Find the root of the
						 * menu */
	mlwrite(MWABORT | MWPAUSE,
		(meUByte *) "[Cannot find osd object identity %d]", id);
	return NULL;
    }
    /*
	...
	if ((initItem) || (rp->flags & RF_INITITEM)) {
		...
		if ((osdCurChild->context[ii].menu->item == initItem) &&
		    ...
		    /*
		     * if the default is not a child we have the default
		     * item 
		     */
		    if ((osdCurChild->context[ii].menu->
			 flags & MF_CHILD) == 0) {
			/*
			 * if this is an entry force entry by loading a
			 * space key event 
			 */
			if (osdCurChild->context[ii].menu->
			    flags & MF_ENTRY)
			...
			/*
			 * the child has no initial item, select the first 
			 * item and finish 
			 */
			for (ii = 0; ii < osdCurChild->numContexts; ii++)
			    if (!
				(osdCurChild->context[ii].menu->
				 flags & MF_SEP)) {
				osdCurChild->curContext = ii;
				osdCurChild->context[ii].mcflags |=
				    CF_UPDATE;
				break;
			    }
			break;
...
osdRestoreAll(int garb)
{
	...
	if (garb) {
	    /*
	     * reconstruct the menu - if this fails the user is in big
	     * trouble, I recon this will core-dump! the good news is that 
...
osdDisplayEvaluate(void)
{
    ...
    for (mcp = osdCurChild->context, ii = 0; ii < osdCurChild->numContexts;
	 ii++, mcp++) {
	/*
	 * Ignore any non-state entries 
	 */
...
osdDisplaySetNewFocus(int flags)
{
	...
	if (osdCurChild->curContext >= 0)
	    osdDisplaySub(osdCurChild, flags);
	       ...
	       && ((osdCurMd->next == NULL)
		   || (flags & (meOSD_ENTER_MENU | meOSD_FOCUS_MENU))))
	osdDisplaySub(osdCurChild, flags);
}
...
osdDisplayMove(osdDISPLAY * md, int dx, int dy)
{
    ...
    /*
     * two different paths, one - the simple case where this is the last
     * dialog and we can optimise the refresh for this. two where there
     * are children where we just have to grit our teeth and bare it 
     */
    if ((tmd = md->next) == NULL) {
	/*
	...
	/*
	 * Draw an area to screen which covers both the old and new
	 * position 
	 */
	if (dx < 0) {
	    xx = md->x;
...
osdDisplayMouseMove(osdDISPLAY * md)
{
    ...
    /*
     * Enable all mouse movements - important if we've come from
     * meGetStringFromUser 
     */
    TTallKeys = 1;
    mmx = mouse_X;
    ...
    for (;
	 (cc =
	  meGetKeyFromUser(meFALSE, 0,
			   meGETKEY_SILENT)) !=
	 (ME_SPECIAL | SKEY_mouse_drop_1);) {
...
osdDisplayMouseResize(void)
{
	...
	if (cc == (ME_SPECIAL | SKEY_mouse_move_1)) {
	    if ((mmx != mouse_X) || (mmy != mouse_Y)) {
		osdDisplayResize(mouse_X - osdCurMd->x + 1,
				 mouse_Y - osdCurMd->y + 1);
		mmx = mouse_X;
		mmy = mouse_Y;
		TTflush();
	    }
...
{
    menuRenderChildScrollBar(sb, osdNewChild,
			     &osdNewChild->context[osdNewChild->
						   newContext]);
}
...
boxDragScrollBarEvent(meSCROLLBAR * sb)
{
     ...
     */
    /*
     * If the buffer is wholly contained in the window then there is
     * nothing to do - quit now. Make sure that the top line is showing 
     */
	...
	startPos = sb->curPos;
	/*
	 * We normalise the scroll bar if there is over-hanging text by
	 * supplying a scroll at the start of the scroll 
	 */
	if (maxPos < startPos)
	    startPos = maxPos;

	/*
	 * Remeber the starting mouse position. Keep the precision up by
	 * ustilising the fractional mouse components if available. Store
	 * as a fixed point 8 bit fractional integer 
	 */
	if (sb->flags & WMHORIZ)
	...
	/*
	 * Build a ratio factor of number of lines to the length of the
	 * scroll. Again keep the precision by storing as a 8-bit
	 * fractional integer. 
	 */
	if (scrollLength <= 0)
	    ...
	    mouseRatio =
		((long) (sb->totLen - sb->wndLen) << 8) / scrollLength;
    }
    curPos = startPos;
    /*
	...
	if (mouseRatio > 0) {
	    /*
	     * Compute the current mouse position. Use the fractional
	     * mouse information if it is available. 
	     */
	    if (sb->flags & WMHORIZ)
	    ...
	    /*
	     * Compute the required current position in the buffer. Take
	     * care with precision here. If we are dealing with a very
	     * large buffer then reduce the accuracy of the calculation to 
	     * ensure that the buffer does not overflow. Biggest we can
	     * cater for here is 2^23 lines - this should be sufficient !!
	     * Simple compute by multiplying the delta mouse position by the 
	     * ratio and add to our reference start line. 
	     */
	    if (mouseRatio >= (1 << 16))
 ...
 */
static int
osdDisplayGetMousePosition(osdDISPLAY * md, meShort xx, meShort yy,
			   int leftPick)
{
    osdCONTEXT     *mcp;
    int             ii;
	...
	if (yy == 0) {
	    ...
	    /*
	     * On the rest of the title bar only a left pick button does
	     * anything Handle the moving of a display 
	     */
	    if (leftPick == 1)
	...
	if ((yy >= mcp->y) && (yy < (mcp->y + mcp->depth)) &&
		...
		/*
		 * if it has an argument then the argument is the item
		 * number to execute 
		 */
		if ((leftPick == 1) && (mcp->menu->iflags & MFI_ARG)
		    && !(mcp->menu->flags & MF_NBPAGE)) {
		    osdITEM        *mp;
		    int             itm;
		    itm = mcp->menu->argc;
		    ii = md->numContexts;
		...
		if (mcp->menu->flags & MF_SCRLBOX) {
			...
			/*
			 * only playing with the scroll bar if 1) Its a
			 * leftPick event 2) we're not past the end of
			 * scroll-bar 3) There is one 
			 */
			if ((leftPick == 1) && (child->vertSBar != NULL)
			    && (yy < child->wndDepth)) {
			    /*
			     * calculate the scroll bar's x & y position.
			     * easier to do here as this could be inside
			     * another scrolled dialog etc. 
			     */
			    child->vertSBar->xPos =
				mouse_X - xx + child->wndWidth;
			...
			/*
			 * Always return 0 as any event has now been
			 * handled 
			 */
			return 0;
		    }
...
osdDisplayMouseLocate(int leftPick)
{
    ...
    /*
     * Iterate backwards over the menu display blocks to locate the
	...
	if ((mouse_Y >= md->y) && (mouse_Y < (md->y + md->depth)) &&
	    ...
	    osdNewMd = md;
	    rr = osdDisplayGetMousePosition(md, mouse_X, mouse_Y,
					    leftPick);
	    if ((rr == 0) && closeSub)
		rr = -1;
	    return rr;
...
osdDisplayTabMove(osdDISPLAY * md, int dir, int mustSel)
{
  ...
  try_again:
    /*
     * loop through the contexts find the closest match (going backwards
     * just for ease) 
     */
    for (ii = md->numContexts, btt = 0, bii = -1; ii--;) {
	/*
	 * only allow tab to select the current tab-page item, ignore the
	 * other pages 
	 */
	if ((md->context[ii].menu->flags & MF_TAB) &&
	    ((tt = md->context[ii].menu->item) != tab) &&
	    (((md->context[ii].menu->flags & MF_NBPAGE) == 0) ||
	     (md->context[ii].menu->argc ==
	      md->context[md->nbpContext].menu->argc))) {
	    if (dir)
		tt -= tab;
	    else
		tt = tab - tt;
	...
	if (bii < 0) {
	     ...
	     * select the file. This is for things like file list widgets. 
	     */
	    if (mustSel)
		dir = 1;
	...
	if (md->context[bii].menu->flags & MF_CHILD) {
	    if (!osdDisplayTabMove
		(md->context[bii].child->display, dir, 1)) {
		if (loopItem < 0)
		    loopItem = bii;
		      ...
		      int posCur[2], int posBest[2])
{
	...
	if (!(md->context[ii].menu->flags & MF_SEP) &&
		 ...
		 */
		if (osdDisplayFindClosest
		    (dir, md->context[ii].child->display, pos, posCur,
		     posBest) > 0) {
		    md->newContext = ii;
		    ret = 1;
		}
...
{
    /*
     * go through the whole dialog looking for the closest item in the
     * right direction 
     */
    osdDISPLAY     *mdTop,
                   *md = osdCurChild;
    ...
    if (dir == osdMOVE_TOP)
	/*
	 * make the posCur a fictitious very top point and move DOWN to
	 * the first real item. As y is a short -65536 must be higher than 
	 * any y 
	 */
	posCur[1] = -65536;
	 ...
	 */
	if ((mdTop != md)
	    && (md->context[md->curContext].menu->flags & MF_SCRLBOX))
	    posCur[1] = 0;
    ...
    if ((posBest[1] > 16384)
	&& ((dir == osdMOVE_PAGE_UP) || (dir == osdMOVE_PAGE_DOWN)))
	return osdDisplayKeyMove((dir == osdMOVE_PAGE_UP) ? osdMOVE_TOP :
				 osdMOVE_BOTTOM);
	...
	if (((dir & (osdMOVE_DOWN | osdMOVE_RIGHT))
	     || ((osdCurMd->dialog->id == osdMainMenuId)
		 && (dir & osdMOVE_UP))) && ((osdCurChild->curContext < 0)
					     || (osdCurChild->
						 context[osdCurChild->
							 curContext].menu->
						 flags & MF_SUBMNU))) {
		...
		return osdDisplayFindClosest(dir, osdNewMd, NULL, posCur,
					     posBest);
	    }
	}
	/*
	 ...
	 */
	    ...
	    /*
	     * special case mainly triggered with the main menu, if in a
	     * main sub-menu item which does not have a sub-menu then move 
	     * up and across 
	     */
	    if ((dir & (osdMOVE_LEFT | osdMOVE_RIGHT)) && (posBest[1] > 0)
		&& (osdCurMd->prev->context[osdCurMd->prev->curContext].
		    menu->flags & MF_SOUTH)) {
		osdNewMd = osdNewChild = osdCurMd->prev;
		osdNewMd->newContext = osdNewMd->curContext;
		posBest[1] = -1;
		if (osdDisplayFindClosest
		    (dir, osdNewMd, NULL, posCur, posBest) < 0)
		    osdNewChild = osdNewMd;
		return 1;
	    } else if ((dir & osdMOVE_LEFT) ||
		       ((dir & osdMOVE_UP)
			&& (osdCurMd->prev->
			    context[osdCurMd->prev->curContext].menu->
			    flags & MF_SOUTH))) {
		/*
		...
		while ((osdNewChild->curContext >= 0) &&
		       (osdNewChild->context[osdNewChild->curContext].
			menu->flags & MF_CHILD)) {
		    osdNewChild =
			osdNewChild->context[osdNewChild->curContext].
			child->display;
...
osdDisplayRedraw(void)
{
	 ...
	 * reconstruct the menu - if this fails the user is in big
	 * trouble, I recon this will core-dump! the good news is that
	 * this really shouldn't fail! SWP 30/12/98 - This does core-dump, 
	 * unfortunately this does happen, particulary on NoteBooks where
	 * an error in the child creation of a new page can lead to this
	 * type of failure. return -1 and hope the calling function can
	 * cope. 
	 */
	if ((tmd = menuConfigure(tmd->dialog, tmd, 0)) == NULL) {
	    sgarbf = meTRUE;
	...
	/*
	 * Now draw to screen, render an area big enough to cover both the 
	 * old and the new 
	 */
	if (tmd->x < mnx)
	    mnx = tmd->x;
    ...
    *retState = meFALSE;
	...
	if (meSystemCfg & meSYSTEM_OSDCURSOR) {
		...
		while (md != osdCurMd) {
		    ...
		    /*
		     * If this is a scroll box then correct the cursor
		     * position. The scroll box x,y child positions can go 
		     * out side of the window if the currently selected
		     * item is scrolled up or down, in which case the
			...
			if ((xx < 0) || (xx >= mp->len) ||
			    (yy < 0) || (yy >= mp->height)) {
			    /*
			     * the current item is not visible in the
			     * window, hide the cursor 
			     */
			    ii = -1;
			    break;
			}
			/*
			 * Special correction for scroll box to account
			 * for the borders 
			 */
			xx++;
			yy++;
		...
		/*
		 * only move the cursor here if the item is not scrolled
		 * off and this part of the dialog is in a visible part of 
		 * the window 
		 */
		if ((ii == 0) &&
	...
	case ME_SPECIAL | SKEY_mouse_pick_1:
		...
		&& (osdNewChild->context[osdNewChild->newContext].menu->
		    flags & MF_REPEAT)) {
		state =
		    meOSD_MOUSE_MENU | meOSD_EXECUTE_MENU |
		    meOSD_OPEN_MENU;
		/*
		 * Enable all mouse timed events 
		 */
	...
	case ME_SPECIAL | SKEY_mouse_drop_3:
	    if (!(osdCurMd->flags & RF_RISLBUT)) {
		/*
		 * Close the dialog - drop back to the parent of last
		 * manually opened sub-menu or close all 
		 */
		osdDISPLAY     *md = osdCurMd;
		while (md->next != NULL)
		    md = md->next;
		while (((md = md->prev) != NULL) &&
		       ((osdDialogGetCurContextFlags(md) &
			 (MF_SUBMNU | MF_MANUAL)) !=
			(MF_SUBMNU | MF_MANUAL)));
		if (md != NULL) {
	...
	case ME_SPECIAL | SKEY_mouse_drop_1:
	    ...
	    if (nit == -1) {
		/*
		 * complete miss, if this is part of a manually open
		 * sub-menu then drop back to its parent dialog, otherwise 
		 * abort 
		 */
		osdDISPLAY     *md = osdCurMd;
		while (md->next != NULL)
		    md = md->next;
		while (((md = md->prev) != NULL) &&
		       ((osdDialogGetCurContextFlags(md) & MF_MANUAL) ==
			0));
		if (md != NULL) {
		    osdNewChild = osdNewMd = md;
		    osdDisplayPop(md->next);
	...
	case ME_SPECIAL | SKEY_mouse_drop_2:
	    /*
	     * middle buttons execute the current item - particularly
	     * useful for wheel mice 
	     */
	    if (osdCurChild->curContext >= 0) {
		...
		for (;;) {
			...
			break;
		    osdNewChild =
			osdNewChild->context[osdNewChild->newContext].
			child->display;
		}
		nit = 1;
		state =
		    meOSD_EXECUTE_MENU | meOSD_OPEN_MENU | meOSD_FOCUS_MENU
		    | meOSD_ENTER_MENU;
	    }
	    break;
...
#else
	{
		...
		if (f == 0) {
		    /*
		     * if scrolling by a page then assume its a page-up
		     * and we want to scroll a child dialog 
		     */
		    nit = osdDisplayKeyMove(osdMOVE_PAGE_UP);
		    state = meOSD_KEY_MOVE;
		} else if (osdCurMd->flags & (RF_TITLE | RF_BOARDER))
		    /*
		     * Allow the menu to move if it has a title bar or a
		     * boarder This is different to the mouse as for the
		     * mouse to be able to move the menu it must have a
		     * title bar 
		     */
		...
		/*
		 * Close current sub-menu's up to the first real dialog
		 * (one with a title bar) if one is open, otherwise follow 
		 * through and close whole dialog 
		 */
		if ((osdCurChild->curContext >= 0) &&
		    (osdCurChild->context[osdCurChild->curContext].menu->
		     flags & MF_MANUAL) && (osdCurMd->next != NULL)) {
		    osdDisplayPop(osdCurMd->next);
		} else {
		    osdNewMd = osdCurMd;
		    ...
		    if ((osdNewMd == NULL)
			|| (osdNewMd->flags & RF_DISABLE))
			state = meOSD_QUIT_MENU;
		    else {
			osdNewChild = osdNewMd;
			while (((osdNewChild->newContext =
				 osdNewChild->curContext) >= 0)
			       && (osdNewChild->
				   context[osdNewChild->newContext].menu->
				   flags & MF_CHILD))
			    osdNewChild =
				osdNewChild->context[osdNewChild->
						     newContext].child->
		...
		if ((osdCurMd->defContext >= 0) &&
			...
			nit = 1;
			state =
			    meOSD_EXECUTE_MENU | meOSD_OPEN_MENU |
			    meOSD_FOCUS_MENU | meOSD_ENTER_MENU;
		    }
		    break;
		}
	      execute_item:
		...
		if (cc == ' ')
			...
			nit = osdDisplaySelectFirstLetter(cc, osdCurChild);
				 ...
				 && ((nit = osdDisplaySelect(cc, osdCurMd))
				     != 0)))
			state =
			    meOSD_EXECUTE_MENU | meOSD_OPEN_MENU |
			    meOSD_FOCUS_MENU | meOSD_ENTER_MENU;
	...
	if ((state & meOSD_EXECUTE_MENU)
	    ...
	    && (mp->cmdIndex < 0)) {
	    /*
	     * Hot keys can be used to reference other items to execute
	     * cause they themselves are separaters so get the real thing
	     * to execute 
	     */

	    /*
	     * if it has an argument then the argument is the item number
	     * to execute 
	     */
	    ...
	    if (nit < 0) {
		mlwrite(MWABORT | MWPAUSE,
			(meUByte *) "[Nothing to execute for item]");
		continue;
	    }
	    osdNewChild->newContext = nit;
	}
	if (!(mp->flags & (MF_SUBMNU | MF_COMBO)) &&
	    ((state & meOSD_EXECUTE_MENU)
	     || ((state & meOSD_KEY_MOVE)
		 && (mp->flags & (MF_ENTRY | MF_NBPAGE))))) {
	    osdDISPLAY     *nem,
	                   *nec;
	    osdCONTEXT     *nep;
	    int             necflags;
		...
		/*
		 * if no non-exit was found & this is not an entry or
		 * combo box field then we can quit before execute - store 
		 * the md's flags so the item can be properly executed 
		 */
		*retState = osdNewChild->flags;
	    ...
	    if (osdNewChild == NULL) {
		/*
		 * This must have been an entry with no non-exit - quit
		 * but flag as success 
		 */
		*retState = meTRUE;
		break;
...
osd(int f, int n)
{
    ...
    osdDISPLAY     *md,
                   *pmd;	/* Pointer to the osdDisplayHd */
    meUByte         oldClexec;	/* command line execution flag */
    meUByte         buf[meBUF_SIZE_MAX];	/* Reply buffer */
    int             noDis,
                    ii,
                    jj;		/* Status of the invocation */

    /*
     * If no arguments are defined then a menu is being defined. 
     */
    if ((f == meFALSE) || (n < 0)) {
	 ...
	 */
	if (((ii =
	      meGetString((meUByte *) "Identity", 0, 0, buf, 16)) <= 0)
	    || (buf[0] == '\0') || (buf[0] == 'E'))
	    /*
	     * The 'E' test is to catch the string "ERROR", returned when
	     * the user gives an undefined variable, e.g. %undefined. This 
	...
	if ((id = meAtoi(buf)) < 0) {
	    ...
	    flags = meAtoi(buf);
	    if (flags == 0)
		return frameSetupMenuLine(0);	/* hide the menu return
						 * whether it existed */
	    if (flags < 0) {
		/*
	...
	if (id > 32767)
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "Invalid identity [%s]", buf);
	...
	if ((meGetString((meUByte *) "Item", 0, 0, buf, 16) <= 0) ||
	    ...
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "Invalid item identity [%s]", buf);
	...
	if (item == 0) {
		...
		return meABORT;
	    if (meGetString((meUByte *) "Flags", 0, 0, buf, meBUF_SIZE_MAX)
		<= 0)
		return meABORT;
	    ...
	    if (flags & RF_MSCHEME) {
		if (meGetString
		    ((meUByte *) "Scheme", 0, 0, buf, meBUF_SIZE_MAX) <= 0)
		    return meABORT;
		...
		if ((meGetString((meUByte *) "X-pos", 0, 0, buf, 16) <= 0)
			...
			(meGetString((meUByte *) "Y-pos", 0, 0, buf, 16) <=
			 0)))
		    return meABORT;
		rp->y = (meShort) meAtoi(buf);
	    }
	    /*
	     * If theres a size given, get it. Note the following values
	     * mean: 0 - Use minimum space required, max is screen-size
	     * -1 - Set the size to the screen-size -2 - Use minimum space 
	     * required but no restriction on the max size 
	     */
	    if (rp->flags & RF_SIZE) {
		for (; item < 2; item++) {
		    if ((meGetString((meUByte *) "Width", 0, 0, buf, 16) <=
			 0)
			|| ((rp->width[item] = (meShort) meAtoi(buf)),
			    (meGetString
			     ((meUByte *) "Depth", 0, 0, buf, 16) <= 0)))
			return meABORT;
		...
		if (flags & RF_TSCHEME) {
		    if (meGetString
			((meUByte *) "Scheme", 0, 0, buf,
			 meBUF_SIZE_MAX) <= 0)
			return meABORT;
		    rp->tScheme =
			convertUserScheme(meAtoi(buf), rp->mScheme);
		} else
		    rp->tScheme = rp->mScheme;
		if (meGetString
		    ((meUByte *) "Text", MLEXECNOUSER, 0, txtbuf,
		     meBUF_SIZE_MAX) > 0)
		...
		if (((ii =
		      meGetString((meUByte *) "Command", MLCOMMAND, 0, buf,
				  meBUF_SIZE_MAX)) <= 0)
		...
		if (((ii =
		      meGetString((meUByte *) "Control", MLCOMMAND, 0, buf,
				  meBUF_SIZE_MAX)) <= 0)
	     ...
	     */
	    if ((ii =
		 meGetString((meUByte *) "Command",
			     MLCOMMAND | MLEXECNOUSER, 0, buf,
			     meBUF_SIZE_MAX)) < 0)
		return meABORT;
	...
	if ((rp = dialogFind(id)) == NULL)
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "[Osd dialog %d undefined]", id);
	...
	if (rp->flags & RF_NOTEBOOK) {
		...
		return mlwrite(MWABORT,
			       (meUByte *) "[Illegal NoteBook item]");
	}
	...
	if (flags & MF_CHILD) {
		...
		return mlwrite(MWABORT,
			       (meUByte *)
			       "[Cannot have child item with an Alpha dialog]");
	    iflags |= MFI_NOTEXT;
	    if (flags & MF_SCRLBOX)
		flags |= MF_SIZE;
	    txtlen = 0;
	...
	if (!(flags & MF_CHILD)) {
	    flags &= ~MF_SCRLBOX;
	    /*
	     * Get the string field - not needed if deleting a non alpha
	     * item 
	     */
	    if ((ii =
		 meGetString((meUByte *) "Text", MLFFZERO | MLEXECNOUSER,
			     0, txtbuf, meBUF_SIZE_MAX)) == meABORT)
		return meABORT;
	    else if (ii == meFALSE) {
		if (rp->flags & RF_ALPHA)
		    return mlwrite(MWABORT,
				   (meUByte *)
				   "[Cannot have item with no text in an Alpha dialog]");
		iflags |= MFI_NOTEXT;
		txtlen = 0;
	    } else
	...
	/*
	 * Get the numeric argument. Check for 'f' which means false or a
	 * value which means true. 
	 */
	argc = 1;
	if ((ii =
	     meGetString((meUByte *) "Argument", MLEXECNOUSER, 0, buf,
			 16)) < 0)
	    return meABORT;
	if (ii > 0) {
	    if (toLower(buf[0]) != 'f') {
	 ...
	 */
	if ((ii =
	     meGetString((meUByte *) "Command", MLCOMMAND | MLEXECNOUSER,
			 0, buf, meBUF_SIZE_MAX)) < 0)
	    return meABORT;
	...
	if ((flags & MF_ENTRY) && ((mp->height < 1) || (mp->len < 3))) {
	    ...
	    mp->len = 3;
	    return mlwrite(MWABORT | MWPAUSE,
			   (meUByte *) "[Entry size is too small]");
	}
  ...
  do_control_inter:
    /*
     * we must set the clexec flag to false to ensure we execute as if we
     * are not currently running a macro. This is for 2 reasons 1) We are 
     * now waiting for user input. If this flag is set then the rest of
     * the system thinks the opposite so things like screen updates wont
     * happen. 2) If a command like find-file is executed (not as a
     * string) it would not get the argument of the user and fail if this
     * flag was set. 
     */
    oldClexec = clexec;
    clexec = meFALSE;
...
{
    if (force || (osdMainMenuMd == NULL)
	|| (osdMainMenuMd->flags & RF_REDRAW)) {
	osdDIALOG      *rp;
	...
	if (((rp = dialogFind(osdMainMenuId)) == NULL) ||
	    ((osdMainMenuMd =
	      menuConfigure(rp, osdMainMenuMd, 0)) == NULL))
	    return;
    ...
    /*
     * Optimisation here - if the top menu is currently active * don't do 
     * anything 
     */
    if ((osdDisplayHd != NULL)
	&& (osdDisplayHd->dialog->id == osdMainMenuId))
	return;

    osdDisplaySnapshotDraw(osdMainMenuMd, 0, 0, osdMainMenuMd->width, 1,
			   1);
...
osdMainMenuCheckKey(int cc)
{
	...
	for (mcp = osdMainMenuMd->context, ii = 0;
	     ii < osdMainMenuMd->numContexts; ii++, mcp++) {
	    mp = mcp->menu;
	    if (mp->key == cc) {
		best = ii;
		break;
	...
	if (best != -1)
	    return (osdMainMenuId |
		    (osdMainMenuMd->context[best].menu->item << 16));
    }
    return 0;
}
...
osdBindKey(int f, int n)
{
	...
	return mlwrite(MWABORT | MWPAUSE,
		       (meUByte *) "[Osd dialog %s undefined]", buf);
    return bindkey((meUByte *) "Osd bind", f, n, &(rp->nobinds),
		   &(rp->binds));
}
int
