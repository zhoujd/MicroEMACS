/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com time.c - File time
 * stamping routines Copyright (C) 1991-2001 Jon Green Copyright (C)
...
meUByte         time_stamp[meTIME_STAMP_SIZE_MAX] = "<%Y%M%D\\.%h%m>";	/* Time 
									 * stamp 
									 * string 
									 */
...
createTimeStampSrch(meUByte * buf, meUByte * pos)
{
	...
	if ((cc == '%') && ((cc = *ss++) != '\0') &&
	    ...
	    if (ii == 0) {
		/*
		 * for the year flags cater for an optional century (4
		 * digits) 
		 */
...
set_timestamp(meBuffer * bp)
{
    ...
    win.dotLine = meLineGetNext(frameCur->bufferCur->baseLine);	/* Save
								 * position 
								 */
    win.dotOffset = 0;
    win.dotLineNo = 0;
    ...
    if (iscanner
	(patt, 100, ISCANNER_PTBEG | ISCANNER_MAGIC | ISCANNER_EXACT,
	 NULL) == meTRUE) {
	...
	for (ii = 0; ii < TSNUMFIELD; ii++)
	    if ((jj = pos[ii]) != 0) {
		if (!ii
		    && ((mereRegexGroupEnd(jj) - mereRegexGroupStart(jj))
			== 4)) {
