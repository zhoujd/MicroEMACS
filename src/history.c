/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com history.c - histroy
 * saving and re-loading routines. Copyright (C) 1995-2001 Steven
 * Phillips Copyright (C) 2002-2009 JASSPA (www.jasspa.com) This program
...
initHistory(void)
{
     ...
     */
    if ((strHist =
	 (meUByte **) meMalloc(sizeof(meUByte *) * meHISTORY_COUNT *
			       meHISTORY_SIZE)) == NULL)
	meExit(1);
    /*
     * Initialise the array to NULLS so we know they don't point to a
     * history string 
     */
    memset(strHist, 0,
	   sizeof(meUByte *) * meHISTORY_COUNT * meHISTORY_SIZE);
