/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com window.c - Window
 * handling routines. Copyright (C) 1988-2009 JASSPA (www.jasspa.com)
...
/*
 * Window Tree Traversal Functions =============================== The
 * following Window Traversal Functions determine window adjacency on
 * window deletion, resizing etc. establishing adjacent windows that are
 * affected by the change in window status. Unfortunatly, the window
 * management is not particullary pleasant. It operates by using knowledge 
 * of the position of the window in the chain of windows structures
 * relying on the fact that whenever a window is split the windows are
 * always adjacent in the window list (take heed DO NOT re-order the
 * window list for any reason). Consider the following example of a
 * massively split window:- EXAMPLE SCREEN ==============
 * +---------+-------+------+---------+--+ | | | | 5 | | | | 2 |
 * +---------+ | | | | | 6 | | | +-------+ 4 +---------+ | | | | | 7 | | | 
 * | 3 | +---------+ | | | | | 8 | | | +-----+-+---+--+--+------+ | | | |
 * | | | | | | 9 | 10 | 11 | 12 | | | | | | | | | | | | | | | | |
 ...
 * reflect the linkage order and do not appear in the meWindow structure
 * itself. The key to sorting window adjacency is the fact that the
 * windows are consecutive within a column (or stripe) hence we use this
 * fact with the linkage information to determine if we are dealing with a 
 * vertically or horizontally stacked window. Note the case of 13/16. The
 * window order should tell you that a single vertical break exists for
 * 13+16. The bottom lines of 13 and 16 are disarticulated. Where as
 * 9,10,11 and 12 share a horizontal line. We also get information from
 * the Virtual Video structure meVideo. This contains a thread through the 
 * meWindow structure of all of the windows that are attached to it's
 * left. This makes it a lot easier to find left/right adjacency.. I had
 * trouble sorting out all of the cases so if you make changes ensure that 
 * you understand how the ordering is derived and interpreted. OPERATION
 * EFFECTS ================= Note that 20 was not present when this table 
 * was generated so work out values as if 1-19 were on screen.
 * +-----+------------------+------------------+------------------+ | | |
 * Grow Window | Grow Window | | | Delete Window | Vertically |
 * Horizontally |
 * +-----+------------------+------------------+------------------+ | 1 |
 * 2,3,9,13,14 | None | 2,3,9,13,14 | | 2 | 3 | 3 | 3,4 | | 3 | 2 |
 * 4,8,9,10,11,12 | 2,4 | | 4 | 2,3 | 3,8,9,10,11,12 | 5,6,7,8 | | 5 | 6 | 
 * 6 | 4,6,7,8 | | 6 | 5 | 7 | 4,5,7,8 | | 7 | 6 | 8 | 4,5,6,8 | | 8 | 7 | 
 * 3,4,9,10,11,12 | 4,5,6,7 | | 9 | 10 | 10,11,12,13,16 | 10 | | 10 | 9 |
 * 9,11,12,13,16 | 11 | | 11 | 10 | 9,10,12,13,16 | 12 | | 12 | 11 |
 * 9,10,11,13,16 | 11 | | 13 | 14,15 | 14,15 | 15,16,17 | | 14 | 15 |
 * 13,15 | 15 | | 15 | 14 | 13,14 | 13,16,17 | | 16 | 17,18,19 | 17,18,19
 * | 13,15,17 | | 17 | 18 | 16,18,19 | 18 | | 18 | 17 | 16,17,19 | 19 | |
 * 19 | 18 | 16,17,18 | 18 |
 * +-----+------------------+------------------+------------------+ Jon
 * Green 27th February 1997 
 */
...
meWindowGetAdjacentList(meWindow ** wlist, int direction, meWindow * wp)
{
	 ...
	 */
	    ...
	    /*
	     * Find the horizontal range of windows to left of current
	     * window 
	     */
	    for (twp = wp->video->window; twp != NULL;
		 twp = twp->videoNext) {
...
meWindowFixTextSize(meWindow * wp)
{
...
#if MEOPT_SCROLL
    /*
     * Determine and assign the scrolling mode. We need do not need a
    ...
    /*
     * SWP 18/8/98 - need to add WFSELDRAWN to the flag list as if an
     * another window which contains a hilighted region becomes part of
     * this window, this flag is the only way to get it re-evaluated. 
     */
...
#if MEOPT_FILEHOOK
    /*
     * Process the exit hook of the old buffer. Force mode line / scroll
     * bar update on old window 
     */
    if (frameCur->bufferCur->ehook >= 0)
	execBufferFunc(frameCur->bufferCur, frameCur->bufferCur->ehook, 0,
		       1);
#endif
    frameCur->windowCur->updateFlags |= WFMODE | WFSBAR;	/* Update
								 * scroll
								 * and
								 * mode
								 * lines */
...
#if MEOPT_FILEHOOK
    /*
     * Process the entry hook of the new buffer. Force mode line / scroll
     * bar update on new window 
     */
    if (frameCur->bufferCur->bhook >= 0)
	execBufferFunc(frameCur->bufferCur, frameCur->bufferCur->bhook, 0,
		       1);
#endif
...
windowGotoNext(int f, int n)
{
	...
	if (n & 0x04) {
	    wp = frameCur->windowList;
	    /*
	     * switch off bit 1 so it won't loop and bit 4 so we don't
	     * re-init 
	     */
	    n &= ~0x05;
...
windowGotoPrevious(int f, int n)
{
    ...
    for (;;) {
	if (n & 0x04) {
	    for (wp = frameCur->windowList; wp->next != NULL;
		 wp = wp->next)
		/*
		 * NULL 
		 */ ;
...
windowScrollUp(int f, int n)
{
    ...
    ii = frameCur->windowCur->dotLineNo - frameCur->windowCur->vertScroll -
	frameCur->windowCur->textDepth + 1;
    if (ii > 0)
	windowBackwardLine(meTRUE, ii);
    frameCur->windowCur->updateFlags |= (WFREDRAW | WFSBOX);	/* Mode
								 * line is 
								 * OK. */
    return meTRUE;
}
...
windowScrollDown(int f, int n)
{
     ...
     */
    if (frameCur->windowCur->vertScroll >=
	frameCur->bufferCur->lineCount - 1)
	return meErrorEob();

    if ((frameCur->windowCur->vertScroll +=
	 n) >= frameCur->bufferCur->lineCount)
...
windowScrollRight(int f, int n)
{
	...
	n = frameCur->windowCur->textWidth - 2;	/* Scroll a whole screen
						 * width */
	...
	if ((scrollFlag & 0x0f) == 3) {
		...
		frameCur->windowCur->updateFlags |= WFREDRAW;	/* Force
								 * complete 
								 * screen
								 * refresh 
								 */
	    }
	} else if ((scrollFlag & 0x0f)
		   && (frameCur->windowCur->horzScrollRest !=
		       frameCur->windowCur->horzScroll)) {
	    frameCur->windowCur->horzScrollRest =
		frameCur->windowCur->horzScroll;
	    frameCur->windowCur->updateFlags |= WFREDRAW;	/* Force
								 * complete 
...
windowSetScrollWithMouse(int f, int n)
{
	 ...
	 * nothing to do - quit now. Make sure that the top line is
	 * showing 
	 */
	if (frameCur->windowCur->textDepth >= bufferLength) {
	    mouseRatio = -1;
	    windowScrollUp(meTRUE,
			   bufferLength + frameCur->windowCur->textDepth);
	    return meTRUE;
	}

	/*
	 * Get the line number of the top row of the buffer. This is used
	 * as * the reference point for all of out scroll work 
	 */
	 ...
	 * ustilising * the fractional mouse components if available.
	 * Store as a fixed point * 8 bit fractional integer 
	 */
	startMouse = ((long) (mouse_Y) << 8) + (long) (mouse_dY);

	/*
	 * Determine how long the movable scroll region is. This is a
	 * simple calculation of the (scroll bar length - scroll box
	 * length) 
	 */
	scrollLength =
	    (long) ((frameCur->windowCur->
		     vertScrollBarPos[WCVSBDSHAFT - WCVSBSPLIT] -
		     frameCur->windowCur->vertScrollBarPos[WCVSBUP -
							   WCVSBSPLIT]) -
		    (frameCur->windowCur->
		     vertScrollBarPos[WCVSBBOX - WCVSBSPLIT] -
		     frameCur->windowCur->vertScrollBarPos[WCVSBUSHAFT -
							   WCVSBSPLIT]));
	...
	if (scrollLength <= 0)
	    ...
	    mouseRatio =
		((long) (bufferLength - frameCur->windowCur->textDepth) <<
		 8) / scrollLength;
    ...
    /*
     * Compute the current mouse position. Use the fractional mouse
    ...
    /*
     * Compute the required top row position in the buffer. Take care with 
     * precision here. If we are dealing with a very large buffer then
     * reduce the accuracy of the calculation to ensure that the buffer
     * does not overflow. Biggest we can cater for here is 2^23 lines -
     * this should be sufficient !! Simple compute by multiplying the
     * delta mouse position by the ratio and add to our reference start
     * line. 
     */
    if (mouseRatio >= (1 << 16))
    ...
    /*
     * Work out the top row of the buffer in the window. We use the line
     * number of the current line and the offset in the window of the
     * current line 
     */
    ...
    /*
     * Go and scroll - simple signed difference of the current top line
     * and the target top line - done when the scroll has finished -
     * simple !! 
     */
    return (windowScrollDown
	    (meTRUE, (int) (screenTopRow - currentTopRow)));
}
#endif
...
windowDeleteOthers(int f, int n)
{
	...
	wp = frameCur->windowList;
	while (wp != NULL) {
	    if ((wp->flags & (meWINDOW_NO_DELETE | meWINDOW_NO_OTHER_DEL))
		&& (wp != frameCur->windowCur))
		break;
	    wp = wp->next;
	}
	if (wp != NULL) {
		...
		nwp = wp->next;
		if (((wp->
		      flags & (meWINDOW_NO_DELETE | meWINDOW_NO_OTHER_DEL))
		     == 0) && (wp != cwp)) {
		    meWindowMakeCurrent(wp);
    ...
    meVideoAttach(&frameCur->video, frameCur->windowCur);	/* Attach
								 * to root 
								 * video
								 * bloc */
    ...
    frameCur->windowCur->depth =
	frameCur->depth - meFrameGetMenuDepth(frameCur);
...
windowDelete(int f, int n)
{
     ...
     * end column of the previous is greated then the current we delete
     * the next window Cases ----- pwp != NULL pwp == NULL |
     * +----------+ +---------+ OR +---------+---------+ |pwp | |curWin |
     * |curWin |nwp | | | | | | | | +-----+----+ +---------+
     * +---------+---------+ |curWn| |nwp | | | +-----+ 
     */
    if ((pwp == NULL) ||
	((pwp->frameColumn + pwp->width) >
	 (cwp->frameColumn + cwp->width))) {
	/*
     ...
     * check the next start column to determine if we merge the next
     * sibling or the previous child. Cases ----- nwp == NULL | pwp|
     * +----+---------+ OR +--------+ |pwp |curWin | |curWin | | | | | |
     * +----+---------+ +--------+ nwp != NULL +----+-------+ |pwp
     * |curWin | | | | +----+-------+ |nwp | | | +------------+ 
     */
    else if ((nwp == NULL) || (nwp->frameColumn < cwp->frameColumn)) {
	/*
	 * Determine if we merge the prev up or left 
	 */
	if ((pwp->frameColumn + pwp->width) ==
	    (cwp->frameColumn + cwp->width))
    ...
    /*
     * If the end columns are the same as the curent window and the
     * previous window then merge the current window into the previous
     * window Cases ----- | OR | +----------+ +----+------+ |pwp | | |
     * pwp | | | | | | +----------+ +----+------+ |curWin | |curWin | | |
     * | | +----------+ +-----------+ | | 
     */
    else if ((pwp->frameColumn + pwp->width) ==
	     (cwp->frameColumn + cwp->width)) {
#if MEOPT_EXTENDED
	/*
	...
	/*
	 * Set window flags for update - note we always set the mode line. 
	 * This is not always strictly necessary but it is not worth
	 * working out what should and should not be strictly set 
	 */
...
#endif
    /*
     * Must free AFTER meWindowMakeCurrent as this dicks with
     * frameCur->windowCur 
     */
...
windowSplitDepth(int f, int n)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "Cannot split a %d line window",
		       frameCur->windowCur->textDepth);
    if (frameCur->windowCount == meWINDOW_MAX)
	return mlwrite(MWABORT,
		       (meUByte *) "Cannot create more than %d windows",
		       frameCur->windowCount);
    if (((wp = (meWindow *) meMalloc(sizeof(meWindow))) == NULL)
	|| ((lp = meLineMalloc(frameCur->widthMax, 0)) == NULL)
	|| ((off = meLineMalloc(frameCur->widthMax, 0)) == NULL)) {
    ...
    meVideoAttach(frameCur->windowCur->video, wp);	/* Attach to
							 * frameCur->vvideo 
							 * block */
    ...
    ntrd =
	frameCur->windowCur->dotLineNo - frameCur->windowCur->vertScroll;
...
windowSplitWidth(int f, int n)
{
	...
	return mlwrite(MWABORT,
		       (meUByte *) "Cannot split a %d column window",
		       frameCur->windowCur->textWidth);
    if (frameCur->windowCount == meWINDOW_MAX)
	return mlwrite(MWABORT,
		       (meUByte *) "Cannot create more than %d windows",
		       frameCur->windowCount);
    ...
    /*
     * Compute the new size of the windows and reset the width counters
     * and the margins 
     */
    frameCur->windowCur->width /= 2;
    wp->width -= frameCur->windowCur->width;
    wp->frameColumn =
	frameCur->windowCur->frameColumn + frameCur->windowCur->width;
...
windowChangeDepth(int f, int n)
{
	...
	if (meGetString
	    ((meUByte *) "New depth", 0, 0, buff, meSBUF_SIZE_MAX) <= 0)
	    return meFALSE;
     ...
     * "frameCur->windowCur" that may undergo a change is size because of
     * the split 
     */
    if (meWindowGetAdjacentList(bwlist, WINDOW_NEXT, frameCur->windowCur)
	== NULL) {
	n = 0 - n;
	if (meWindowGetAdjacentList
	    (twlist, WINDOW_PREV, frameCur->windowCur) == NULL)
	    return mlwrite(MWCLEXEC | MWABORT,
			   (meUByte *) "[Only one vertical window]");
	meWindowGetAdjacentList(bwlist, WINDOW_NEXT, twlist[0]);
    } else {
	/*
	 * there's one gotcha, if the bottom windows have a depth lock and 
	 * the top ones dn't then we should use the top ones in preference 
	 */
	...
	if (ii) {
	    /*
	     * there is a lock on the next and none on the prev so use the 
	     * prev 
	     */
	    n = 0 - n;
	    meWindowGetAdjacentList(bwlist, WINDOW_NEXT, twlist[0]);
...
windowChangeWidth(int f, int n)
{
	...
	if (meGetString
	    ((meUByte *) "New width", 0, 0, buff, meSBUF_SIZE_MAX) <= 0)
	    return meFALSE;
     ...
     * the split 
     */
    if (meWindowGetAdjacentList(rwlist, WINDOW_RIGHT, frameCur->windowCur)
	== NULL) {
	n = 0 - n;
	if (meWindowGetAdjacentList
	    (lwlist, WINDOW_LEFT, frameCur->windowCur) == NULL)
	    return mlwrite(MWCLEXEC | MWABORT,
			   (meUByte *) "[Only one horizontal window]");
	meWindowGetAdjacentList(rwlist, WINDOW_RIGHT, lwlist[0]);
    } else {
	/*
	 * there's one gotcha, if the right windows have a width lock and
	 * the left ones don't then we should use the left in preference 
	 */
...
meWindowPopup(meUByte * name, int flags, meBuffer ** bufferReplaced)
{
	...
	*bufferReplaced = NULL;
	...
	if (bp == frameCur->bufferCur) {
	    /*
	     * It is already the current buffer so return the current
	     * window One complication is that this is often used with
	     * BFND_CLEAR so the current buffer is NACT and the fhook
	     * needs to be executed etc. The best way is to "swap" the
	     * buffer 
	     */
	...
	if (bp->windowCount > 0) {
	     ...
	     */
	    for (wp = frameCur->windowList;
		 (wp != NULL) && (wp->buffer != bp); wp = wp->next);
	}
...
static meFrameArea *
meFrameResizeWindowsCreateArea(meFrame * frame, meWindow * hwp,
			       meWindow * twp, meFrameArea ** areaNext)
{
    meFrameArea    *area;
	...
	do {
		...
		area->flags |= flag;
		cca =
		    meFrameResizeWindowsCreateArea(frame, hwp, pwp,
						   areaNext);
	...
	meAssert((area->
		  flags & (meFRAMEAREA_SPLIT_WIDTH |
			   meFRAMEAREA_SPLIT_DEPTH)) != 0);
	meAssert((area->
		  flags & (meFRAMEAREA_SPLIT_WIDTH |
			   meFRAMEAREA_SPLIT_DEPTH)) !=
		 (meFRAMEAREA_SPLIT_WIDTH | meFRAMEAREA_SPLIT_DEPTH));
    }
    return area;
}
...
meFrameResizeWindowsPrintArea(meFrameArea * area, int indent)
{
	...
	fprintf(tmpfp, "%*carea : 0x%02x %d %d %p\n", indent, ' ',
		area->flags, area->width, area->depth, area->window);
	fflush(tmpfp);
	if (area->childHead != NULL)
...
static void
meFrameResizeWindowsSetArea(meFrame * frame, meFrameArea * area, int scol,
			    int srow, int width, int depth, int flags)
{
    if (area->window != NULL) {
	/*
	 * fprintf(tmpfp,"area : 0x%02x %d %d
	...
	if (((area->flags & meFRAMEAREA_SPLIT_WIDTH)
	     && ((flags & meFRAMERESIZEWIN_WIDTH) == 0))
	    || ((area->flags & meFRAMEAREA_SPLIT_DEPTH)
		&& ((flags & meFRAMERESIZEWIN_DEPTH) == 0))) {
	/*
	 * not resizing in this direction, move on 
	 */
	area = area->childHead;
	while (area != NULL) {
	    meFrameResizeWindowsSetArea(frame, area, scol, srow, width,
					depth, flags);
	    area = area->next;
	}
    } else {
	...
	while ((min * count) > size) {
	     ...
	     * Don't delete one which holds the current window and try to
	     * preserve any locked ones and preferable a right or bottom
	     * one 
	     */
	    meFrameArea    *aa = NULL,
		*ba = NULL;
		...
		if ((ca->flags & meFRAMEAREA_CONTAINS_CUR) == 0) {
		    if (ca->
			flags & (meFRAMEAREA_LOCK_WIDTH |
				 meFRAMEAREA_LOCK_DEPTH |
				 meFRAMEAREA_MIN_WIDTH |
				 meFRAMEAREA_MIN_DEPTH))
			ba = ca;
		    else
			aa = ca;
		}
	...
	while (ca != NULL) {
	    --count;
	    nsz =
		(area->flags & meFRAMEAREA_SPLIT_WIDTH) ? ca->width : ca->
		depth;
	    if (nsz == 0) {
		nlcount++;
		nsz = min;
	    ...
	    if (ca->
		flags & ((area->flags & meFRAMEAREA_SPLIT_WIDTH) ?
			 meFRAMEAREA_MIN_WIDTH : meFRAMEAREA_MIN_DEPTH))
		mlcount++;
	...
	if (size) {
	    if (nlcount) {
		/*
		 * give the extra space to the unlocked areas and any min
		 * locked areas that are smaller than the average 
		 */
		size += nlcount * min;
		if (mlcount) {
		    ii = 0;
		...
		ca = area->childHead;
		while (ca != NULL) {
		    if (((area->flags & meFRAMEAREA_SPLIT_WIDTH) ? ca->
			 width : ca->depth) == 0) {
			nsz = size / nlcount;
			nszs[ii] = nsz;
			size -= nsz;
		...
		/*
		 * distribute the extra space to either all the minimum
		 * locked areas or if there are non of those to all the
		 * locked areas 
		 */
	...
	while (ca != NULL) {
		...
		depth = nszs[ii];
	    meFrameResizeWindowsSetArea(frame, ca, scol, srow, width,
					depth, flags);
	    if (area->flags & meFRAMEAREA_SPLIT_WIDTH)
		scol += width;
	    else
...
{
    meFrameArea     areaList[meWINDOW_MAX * 2],
                   *areaNext,
                   *areaHead;
    meWindow       *hwp,
                   *twp;
     ...
     */
    meFrameResizeWindowsSetArea(frame, areaHead, 0,
				meFrameGetMenuDepth(frame), frame->width,
				frame->depth - meFrameGetMenuDepth(frame),
				flags);
    /*
...
meFrameResizeWindows(meFrame * frame, int flags)
{
	 ...
	 */
		...
		twp = twp->prev;	/* Previous block */
		twid--;		/* Previous identity */
		if (col[twid] > ii)	/* This column greater than
					 * current */
	    ...
	    return mlwrite(MWABORT,
			   (meUByte *) "Cannot get into this state !!");
#endif
	/*
...
#if MEOPT_OSD
    rowdepth = (frame->depth - meFrameGetMenuDepth(frame)) / maxrow;	/* Unit 
									 * width 
									 * of 
									 * a 
									 * row 
	...
	/*
	 * Find the end colunm. Search forward until we find another
	 * window with the same top row. 
	 */
	...
	if (flags & meFRAMERESIZEWIN_DEPTH) {
	    jj = (jj ==
		  maxrow) ? frame->depth : (jj * rowdepth) +
		meFrameGetMenuDepth(frame);
	    wp->frameRow =
		(row[wid] * rowdepth) + meFrameGetMenuDepth(frame);
    ...
    /*
     * mlwrite (MWABORT, "There are %d rows and %d cols !!", maxrow,
     * maxcol);
     */
    return meTRUE;
...
{				/* save ptr to current window */
    ...
    if ((cc =
	 mlCharReply((meUByte *) "Set position: ", mlCR_QUIT_ON_USER, NULL,
		     NULL)) == -2)
	cc = mlCharReply((meUByte *) "Set position: ", mlCR_ALPHANUM_CHAR,
			 NULL, NULL);
	...
	pos->winMaxRow =
	    frameCur->windowCur->frameRow + frameCur->windowCur->width - 1;
	pos->winMaxCol =
	    frameCur->windowCur->frameColumn + frameCur->windowCur->depth -
	    1;
    }
    if (n & mePOS_BUFFER)
    ...
    if (n & mePOS_LINEMRK) {
	if (meAnchorSet
	    (frameCur->bufferCur, meANCHOR_POSITION_DOT | pos->anchor,
	     frameCur->windowCur->dotLine, frameCur->windowCur->dotOffset,
	     1) <= 0) {
	    pos->flags = 0;
	    return meABORT;
	}
    ...
    if (n & mePOS_MLINEMRK) {
	if (meAnchorSet
	    (frameCur->bufferCur, meANCHOR_POSITION_MARK | pos->anchor,
	     frameCur->windowCur->markLine,
	     frameCur->windowCur->markOffset, 1) <= 0) {
	    pos->flags = 0;
	    return meABORT;
	}
    }
...
{				/* restore the saved screen */
    ...
    if ((cc =
	 mlCharReply((meUByte *) "Goto position: ", mlCR_QUIT_ON_USER,
		     NULL, NULL)) == -2)
	cc = mlCharReply((meUByte *) "Goto position: ", mlCR_ALPHANUM_CHAR,
			 NULL, NULL);
    if (cc < 0)
	return ctrlg(meFALSE, 1);
	...
	if (ii == 0)
	    return mlwrite(MWABORT | MWCLEXEC,
			   (meUByte *) "[No positions set]");
	allpos[ii] = '\0';
	return mlwrite(MWABORT | MWCLEXEC,
		       (meUByte *) "[Valid positions:%s]", allpos);
    }
	...
	if (bwp == NULL) {
		 ...
		 * calculate the amount this window covers of the old
		 * window 
		 */
		ww = (pos->winMaxCol <
		      (wp->frameColumn + wp->width -
		       1)) ? pos->winMaxCol : (wp->frameColumn +
					       wp->width - 1);
		ww = ww -
		    ((pos->winMinCol >
		      wp->frameColumn) ? pos->winMinCol : wp->frameColumn);
		dd = (pos->winMaxRow <
		      (wp->frameRow + wp->depth -
		       1)) ? pos->winMaxRow : (wp->frameRow + wp->depth -
					       1);
		dd = dd -
		    ((pos->winMinRow >
		      wp->frameRow) ? pos->winMinRow : wp->frameRow);
		off = ww * dd;
		if ((ww < 0) && (dd < 0))
		    off = -off;
    ...
    if (n & mePOS_MLINEMRK) {
	if ((ret =
	     meAnchorGet(frameCur->bufferCur,
			 meANCHOR_POSITION_MARK | pos->anchor)) > 0) {
	    frameCur->windowCur->markLine = frameCur->bufferCur->dotLine;
	    frameCur->windowCur->markLineNo =
		frameCur->bufferCur->dotLineNo;
	    frameCur->windowCur->markOffset =
		frameCur->bufferCur->dotOffset;
	}
	...
	if (frameCur->windowCur->markLine == NULL)
	    ret = meFALSE;
	else if (pos->markOffset >
		 meLineGetLength(frameCur->windowCur->markLine)) {
	    frameCur->windowCur->markOffset =
		meLineGetLength(frameCur->windowCur->markLine);
	    ret = meFALSE;
	} else
	...
	if ((ret =
	     meAnchorGet(frameCur->bufferCur,
			 meANCHOR_POSITION_DOT | pos->anchor)) > 0) {
    ...
    } else if ((n & mePOS_LINENO)
	       && ((ret = windowGotoLine(1, pos->dotLineNo + 1)) <= 0))
	/*
	 * don't restore the offset or scrolls 
	 */
	n &= (mePOS_WINDOW | mePOS_BUFFER);
	...
	if (pos->dotOffset > meLineGetLength(frameCur->windowCur->dotLine)) {
	     ...
	     */
	    n &= (mePOS_WINDOW | mePOS_WINYSCRL | mePOS_BUFFER |
		  mePOS_LINEMRK | mePOS_LINENO);
	} else
    ...
    sflg = 0;
    if (n & mePOS_WINYSCRL) {
	if (frameCur->bufferCur->lineCount
	    && (pos->vertScroll >= frameCur->bufferCur->lineCount))
	...
	if (frameCur->windowCur->horzScroll != cc) {
	    ...
	    frameCur->windowCur->updateFlags |= WFREDRAW;	/* Force a 
								 * screen
								 * update */
	    sflg = 1;
	}
    ...
    if (!ret)
	return mlwrite(MWCLEXEC | MWABORT,
		       (meUByte *) "[Failed to restore %sposition]",
		       (n == 0) ? "" : "part of ");
    return meTRUE;
}
#endif
...
frameSetupMenuLine(int newTTsrow)
{
	 ...
	 */
	    ...
	    /*
	     * Try to insert the menu line by squashing the existing
	     * buffers down. 
	     */
	    for (wp = frameCur->windowList; wp != NULL; wp = wp->next) {
		if (wp->frameRow == 0) {
