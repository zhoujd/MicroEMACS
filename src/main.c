/*
 * -*- c -*- JASSPA MicroEmacs - www.jasspa.com main.c - Main entry
 * point. Originally written by Dave G. Conroy for MicroEmacs Copyright
 * (C) 1987 by Daniel M. Lawrence Copyright (C) 1988-2009 JASSPA
...
#define	maindef			/* Make main defintions - cannot define
				 * this at the top because all the main
				 * defs are needed to init edef's vars */

#include "efunc.h"		/* function declarations and name table */
#include "eskeys.h"		/* Special key names - Must include before 
				 * edef.h & ebind.h */
...
static char     meHelpPage[] =
    ...
    "Where options are:-\n"
    "  @<file> : Setup using <file>[.emf], default is " ME_SHORTNAME
    ".emf\n"
#if MEOPT_EXTENDED
...
#endif				/* _ME_WINDOW */
#if MEOPT_CLIENTSERVER
    "  -o      : One " ME_FULLNAME ", use " ME_SHORTNAME
    " server if available\n"
#endif
#if MEOPT_EXTENDED
...
meInit(meUByte * bname)
{
    ...
    /*
     * add 2 to hilBlockS to allow for a double trunc-scheme change Note:
     * ME is not yet 'initialised' so any meMalloc failure will lead to
     * exit in mlwrite so we don't need to check 
     */
    ...
    memcpy(styleTable, defaultScheme,
	   2 * meSCHEME_STYLES * sizeof(meStyle));
    /*
    ...
    /*
     * Create the frame - make sure that we do not access the screen
     * during the re-size 
     */
...
insertChar(register int c, register int n)
{
	...
	for (index = 0, n = 0;
		 ...
		 frameCur->windowCur->dotLine->length); ii--) {
	    if ((meLineGetChar
		 (frameCur->windowCur->dotLine,
		  frameCur->windowCur->dotOffset) != meCHAR_TAB)
		||
		(at_tab_pos
		 (getccol() + index + 1,
		  frameCur->bufferCur->tabWidth) == 0)) {
		lineSetChanged(WFMAIN);
#if MEOPT_UNDO
		meUndoAddRepChar();
#endif
		meLineSetChar(frameCur->windowCur->dotLine,
			      frameCur->windowCur->dotOffset++, c);
		index = 0;
	    } else {
		index++;
		n++;
...
execute(register int c, register int f, register int n)
{
    ...
    {
	/*
	 * If any of the modifiers have changed since the last key we
	 * executed then execute any commands bound to the M-pick or
	 * M-drop pseudo keys, e.g. the S-pick key will be generated
	 * between the following 2 keys: right S-right. These keys only
	 * get 'pressed' if a command is bound to them. The commands are
	 * executed in a hidden way, so they are not recorded in kbd
	 * macros or in the $recent-keys. This could led to a macro
	 * failing but alternatives have similar if not worse problems. 
	 */
    ...
    meRegCurr->n = n;
    /*
     * SWP - 27/2/99 - to enable input commands to not have to jump
     * through so many hoops I've changed the input support so that macros 
     * can fail in such a way as to indicate that they have not handled
     * this input, this allows them to pick and choose. The method chosen
     * is to check the command variable status if the macro aborted, if
     * .status is set to "0" then its not handled the input 
     */
#if MEOPT_EXTENDED
	...
	if (((cmdstatus = (execFunc(ii, f, n) > 0))) ||
	    ((ss =
	      getUsrLclCmdVar((meUByte *) "status",
	...
	meUByte         outseq[40];	/* output buffer for keystroke
					 * sequence */
	meGetStringFromKey((meUShort) c, outseq);	/* change to
							 * something
							 * printable */
	 ...
	 * special keys that are only added if they are bound and due to
	 * their frequence they can be in the input buffer before they're
	 * unbound leading to this error 
	 */
	c &= ~(ME_SHIFT | ME_CONTROL | ME_ALT);
	if (((c >= (ME_SPECIAL | SKEY_mouse_move))
	     && (c <= (ME_SPECIAL | SKEY_mouse_move_3)))
	    || ((c >= (ME_SPECIAL | SKEY_mouse_time))
		&& (c <= (ME_SPECIAL | SKEY_mouse_time_3))))
	    return meTRUE;
#endif
     ...
     */
    if (meModeTest(frameCur->bufferCur->mode, MDFENCE)
	&& ((c == '}') || (c == ')') || (c == ']'))) {
	frameCur->windowCur->dotOffset--;
	/*
...
static void
addModesList(meBuffer * bp, register meUByte * buf, meUByte * name,
	     meMode mode, int res)
{
    register int    ii,
                    nlen,
...
meUByte         meCopyright[] =
    "Copyright (C) 1988-" meCENTURY meYEAR " " ME_COMPANY_NAME " ("
    ME_COMPANY_SITE ")";
int
meAbout(int f, int n)
{
    ...
    if ((wp =
	 meWindowPopup(BaboutN, BFND_CREAT | BFND_CLEAR | WPOP_USESTR,
		       NULL)) == NULL)
	return meFALSE;
    bp = wp->buffer;
     ...
     */
    addLineToEob(bp,
		 (meUByte *) ME_FULLNAME " " meVERSION " - Date " meCENTURY
		 meDATE " - " meSYSTEM_NAME "\n\nGlobal Status:");
    tbp = bheadp;
    ii = 0;
    while (tbp != NULL) {
	ii++;
    ...
    sprintf((char *) buf, "  File name : %s",
	    (frameCur->bufferCur->fileName ==
	     NULL) ? (meUByte *) "" : frameCur->bufferCur->fileName);
    addLineToEob(bp, buf);

    getBufferInfo(&numlines, &predlines, &numchars, &predchars);
    sprintf((char *) buf,
	    "  Lines     : Total %6d, Current %6d\n  Characters: Total %6d, Current %6d",
...
exitEmacs(int f, int n)
{
    ...
    if (n & 0x04) {
	if (meGetString
	    ((meUByte *) "Exit code", 0, 0, (meUByte *) buff, 128) <= 0)
	    return meFALSE;
	ec = meAtoi(buff);
    } else
#endif
     ...
     */
    if ((n & 0x02) && ((saveSomeBuffers(f, (n & 0x01)) <= 0)
#if MEOPT_SPELL
...
#ifdef _CLIPBRD
	/*
	 * don't mess with the system clipboard from this point - could be 
	 * a problem for a shut-down macro 
	 */
	clipState |= CLIP_DISABLED;
#endif
	/*
	 * If bit 0x08 is set then create autosaves for any changed
	 * buffers, otherwise remove any autosaves 
	 */
	bp = bheadp;
	while (bp != NULL) {
...
#if MEOPT_CALLBACK
	{
	    ...
	    int             index;
	    if ((index =
		 decode_key(ME_SPECIAL | SKEY_shut_down, &arg)) >= 0)
		execFunc(index, 1, n);
	}
#endif
#if MEOPT_CLIENTSERVER
	/*
	 * the client-server is a psuedo ipipe but it needs to be stopped
	 * properly, therefore stop it first as the following ipipeRemove
	 * only does half a job and TTkillClientServer will be very
	 * confused if called after 
	 */
	TTkillClientServer();
#endif
	/*
	 * go round removing all the active ipipes, execute any dhooks set 
	 * on active buffers and remove all auto-write files 
	 */
#if MEOPT_IPIPES
	while (ipipes != NULL)
	    ipipeRemove(ipipes);
	...
	if (loopFrame->bufferCur->ehook >= 0)
	    execBufferFunc(loopFrame->bufferCur,
			   loopFrame->bufferCur->ehook, 0, 1);
	meFrameLoopEnd();
#endif
	/*
	...
	while (bp != NULL) {
		...
		execBufferFunc(bp, bp->dhook, 0, 1);	/* Execute the
							 * delete hook */
#endif
	    bp = nbp;
	}
...
#ifdef _ME_FREE_ALL_MEMORY
	{
	    ...
	    /*
	     * as we have deleted all frames the *scratch* is not show, so 
	     * we don't need a replacement so ztbuf does the lot! 
	     */
...
noMarkSet(void)
{
    return mlwrite(MWABORT | MWCLEXEC,
		   (meUByte *) "No mark set in this window");
}
...
{
    return mlwrite(MWABORT | MWCLEXEC,
		   (meUByte *) "[Command illegal in VIEW mode]");
}

/*
 * void function, does nothing but return false if an argument of zero
 * past, else return true. Used to get rid of unwanted unbounded-key bells 
 */
int
...
int
prefixFunc(int f, int n)
{				/* dummy prefix function */
    return meTRUE;
}
int
uniArgument(int f, int n)
{				/* dummy function for binding to
				 * universal-argument */
    return meTRUE;
}
...
#ifdef _UNIX
/*
 * sigchild; Handle a SIGCHILD action. Note for the older types of signal
 * it is not necessary to reset the signal handler because they "reappear" 
 * as soon as the signal handler is reset - hence we do it at the bottom 
 */
static void
sigchild(SIGNAL_PROTOTYPE)
{
    ...
    /*
     * Reload the signal handler. Note that the child is a special case
     * where the signal is reset at the end of the handler as opposed to
     * the head of the handler. For the POSIX or BSD signals this is not
     * required. 
     */
...
{
    /*
     * To get here we have received a signal and am about to die! Ensure
     * that we are in a DIE state to prevent any output on the display
     * which is being torn down. Our main purpose is to preserve the
...
doOneKey(void)
{
	...
	while ((c =
		meGetKeyFromUser(meTRUE, (n * mflag),
				 meGETKEY_COMMAND)) >= '0') {
	    if (c <= '9')
		n = n * 10 + (c - '0');
	    else if (c == '`')
	...
	if ((c >= '0' && c <= '9')) {
		 ...
		 */
		c = meGetKeyFromUser(meTRUE, (mflag * n),
				     meGETKEY_COMMAND);
		if (c >= '0' && c <= '9')
		    n = n * 10 + (c - '0');
		else
...
mesetup(int argc, char *argv[])
{
     ...
     * structure so an alternative definition is used, but elements within
     * it must remain the same 
     */
    assert(&(((meUndoNode *) NULL)->next) ==
	   &(((meUndoNarrow *) NULL)->next));
    assert(&(((meUndoNode *) NULL)->count) ==
	   &(((meUndoNarrow *) NULL)->count));
    assert(&(((meUndoNode *) NULL)->type) ==
	   &(((meUndoNarrow *) NULL)->type));
#endif
#ifdef _UNIX
    /*
    ...
    meXUmask = umask(0);
    umask(meXUmask);
    meXUmask = (meXUmask & (S_IROTH | S_IWOTH | S_IXOTH | S_IRGRP | S_IWGRP | S_IXGRP | S_IRUSR | S_IWUSR | S_IXUSR)) ^ (S_IROTH | S_IWOTH | S_IXOTH | S_IRGRP | S_IWGRP | S_IXGRP | S_IRUSR | S_IWUSR | S_IXUSR);	/* 00666 
																											 */
    meUmask =
	meXUmask & (S_IROTH | S_IWOTH | S_IRGRP | S_IWGRP | S_IRUSR |
		    S_IWUSR);
    meUid = getuid();
    meGid = getgid();
    /*
    ...
    /*
     * Init the registers - Make the head registers point back to
     * themselves so that accessing #p? gets #g? and not a core-dump 
     */
	...
	if (argv[carg][0] == '-') {
	    ...
	    case 'y':		/* -y rbin flag */
		/*
		 * don't want these options yet as they apply to the next
...
#if MEOPT_EXTENDED
	    case 'c':
		SetUsrLclCmdVar((meUByte *) "session-name",
				(meUByte *) (argv[carg] + 2), &usrVarList);
		break;
	    case 'f':
		{
		    meUByte         vname[16];
			...
			SetUsrLclCmdVar(vname, (meUByte *) argv[carg],
					&(__cmdArray[CK_ABOUT].varList));
		    }
		    sprintf((char *) vname, "%d", nn);
		    SetUsrLclCmdVar((meUByte *) "argc", vname,
				    &(__cmdArray[CK_ABOUT].varList));
		}
		break;
#endif
		...
		if (argv[carg][2] == 0) {
			...
			sprintf((char *) evalResult,
				"%s Error: Argument expected with option %s\nOption -h gives further help\n",
				argv[0], argv[carg]);
		...
		/*
		 * Reverse the video, simply exchange the forward and
		 * reverse schemes with each other. Note that this affects 
		 * the internal default scheme only and is over-ridden by
		 * any other schemes that are installed. 
		 */
	    case 'R':
		{
		    int             ii;
		    ...
		    cursorColor =
			(meColor)
			meStyleGetFColor(defaultScheme[meSCHEME_NORMAL]);
		}
		break;
		...
		sprintf((char *) evalResult, "%s %s - Date %s%s - %s\n",
			ME_FULLNAME, meVERSION, meCENTURY, meDATE,
			meSYSTEM_NAME);
		mePrintMessage(evalResult);
		meExit(0);
	    ...
	    case 'v':
		{
			...
			sprintf((char *) evalResult,
				"%s Error: Mal-formed -v option\n",
				argv[0]);
			mePrintMessage(evalResult);
			meExit(1);
		    }

		    if (((cc = getMacroTypeS(ss)) != TKREG)
			&& (cc != TKVAR) && (cc != TKCVR)
			&& (cc != TKENV)) {
			*tt = '\0';
			sprintf((char *) evalResult,
				"%s Error: Cannot set variable [%s] from the command-line\n",
				argv[0], ss);
			mePrintMessage(evalResult);
			meExit(1);
		    }
		    if ((tt = strchr(ss, '=')) != NULL) {
			*tt++ = '\0';
			if (setVar((meUByte *) ss, (meUByte *) tt, meRegCurr) <= 0) {	/* set 
											 * a 
											 * variable 
											 */
			    sprintf((char *) evalResult,
				    "%s Error: Unable to set variable [%s]\n",
				    argv[0], ss);
			    mePrintMessage(evalResult);
			    meExit(1);
			}
	...
	for (carg = 1; carg < rarg; carg++) {
		 ...
		 */
		findFileList((meUByte *) argv[carg],
			     (BFND_CREAT | BFND_MKNAM | binflag), gline,
			     gcol);
		gline = 0;
		gcol = 0;
		binflag = 0;
	...
	/*
	 * if only a -o (not -m) then pause before exiting to give the
	 * server me a chance to load the files, the file could be a temp
	 * which will be removed once this me exits 
	 */
	if (userClientServer == 2)
	    Sleep(250);
#endif
	...
	sprintf((char *) evalResult,
		"%s Error: Unable to connect to server\n", argv[0]);
	mePrintMessage(evalResult);
	meExit(1);
    }
#endif
    ...
    /*
     * initalize *scratch* colors & modes to global defaults & check for a 
     * hook 
     */
...
#endif
	/*
	 * make *scratch*'s history number very low so any other buffer is 
	 * preferable 
	 */
	mainbp->histNo = -1;
    }
	...
	for (carg = 1; carg < rarg; carg++) {
		...
		/*
		 * set up a buffer for this file - force the history so
		 * the first file on the command-line has the highest
		 * bufHistNo so is shown first 
		 */
		bufHistNo = obufHistNo + rarg - carg;
		noFiles +=
		    findFileList((meUByte *) argv[carg],
				 (BFND_CREAT | BFND_MKNAM | binflag),
				 gline, gcol);
		  ...
		  handle_stdin:
			...
			if (bp->histNo == bufHistNo) {
				...
				if (searchStr != NULL) {
				    ...
				    /*
				     * what can we do if we fail to find
				     * it? 
				     */
    ...
    /*
     * Set the screen for a redraw at this point cos its after the startup 
...
/*
 * _meAssert The receiving end of the macro meAssert(<boolean>). This
 * keeps microEMACS up allowing the debugger to be attached. This is
 * essential under Windows since Microsofts assert() is the pits and
 * invariably just exits the program without invoking the debugger. 
 */
...
_meAssert(char *file, int line)
{
	...
	if (cc == 'S') {
	    /*
	     * Try and perform an emergency save for the user - no
	     * guarantees here I am afraid - we may be totally screwed at
	     * this point in time. 
	     */
	    alarmState = meALARM_DIE;
	    exitEmacs(1, 0x28);
...
commandWait(int f, int n)
{
	...
	do {
	    /*
	     * Fix up the screen before waiting for input otherwise
	     * TTahead will be true and the screen will never be refreshed 
	     * - test with gdiff 
	     */
	    update(meFALSE);
	    TTsleep(-1, 1, varList);
	    if (((ss =
		  getUsrLclCmdVar((meUByte *) "wait", varList)) == errorm)
		|| !meAtoi(ss))
		break;
	    doOneKey();
	    if (TTbreakFlag) {
